...
- **Integration may materialize implied components as stubs** (user directive 2026-05-28). A **`stub`** is the thinnest maturity tier (`stub` → `rough-in` → (`partly-constructive`) → `firm`) — a *claim-free placeholder*: a real file (or dep-map row) with `status: stub` frontmatter, a one-or-two-line sketch, an **"Implied by"** provenance list, and a "Refinement pending" note. Purpose: cross-references resolve to a **live link** instead of a perpetually-deferred plain-text forward-ref (a live link to a missing file is a hard `linkcheck2` error). **Who/when:** `integrator-per-report` or `integrator-finalize` MAY create a stub for a *clearly-implied* component (**bar:** ≥2 converging references, or a rough-in row already standing for it — not merely speculative); stub-creation is the **preferred** resolution, plain-text-defer the fallback. **Critic handling:** a stub makes no claims, so the citation/surface/rotation/variant-axis checks no-op; the critic verifies only that it carries provenance and is wired into `SUMMARY.md`.
...
- **Unimplemented Palace components are NOT direct implementation targets** (user directive 2026-05-27). When Palace ships a stub (enum-only / JSON-only / aborting-branch config): **document it** as an L1>L0 obstruction theme with negative-anchor citations (documentation, not a lowering rule); **do not target it for filling in** (the goal is to match Palace's current feature set on the target L4 semantics, not extend Palace). The literature-anchored L1 form **may inform higher abstractions** — promote a speculative L1 operator to firm only when small AND when it simplifies the semantics of higher forms (e.g. `lanczos_step` factoring `krylov-step`'s symmetric case); a promotion that adds vocabulary without simplifying upstream is deferred. (Memory `project_unimplemented_palace_components`.)
