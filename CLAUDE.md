...
- **Two sub-kinds** (codified batch-22 meta-phase): a **leaf feature column** (a per-driver / output-product / boundary-mode chapter whose stage-(2) constituents are *vocabulary ops*) and a **meta-feature / spine-ROOT** (the lifecycle chapter whose stage-(2) constituents are *other feature columns* + driver-agnostic firm vocabulary). Both use the uniform `status: seed` token (no `(exemplar)` / `(composition-root)` qualifier — the prose names the sub-kind).
- **Column promotion off `seed` — the OWN-COMPOSITION rule (USER DIRECTIVE 2026-06-03; codified batch-26 meta-phase; memory `project_feature_column_promotion_rule`).** A column promotes off `seed` when its **OWN composition + its directly-owned constituents** are firm; **cross-linked SIBLING columns are treated as references, NOT blocking constituents.** A driver column promotes on its firm solve/assemble combinators + its own readout, independent of the output-product column it cross-links (and vice-versa); an output-product column promotes on its firm reduction verb + its own directly-owned constituents, independent of the producing driver column it cross-links. **This SUPERSEDES** the earlier emergent per-column rule ("promote past `seed` only once ALL constituents — including cross-linked sibling columns — are firm"), which combined with the output-product↔driver reciprocal cross-linking to create a **mutual-blocking deadlock** (`eigenmode` driver stays seed because it reduces into `eigenfrequency-qfactor`, which stays seed because its `eigenmode` constituent column is seed — each names the other as the non-firm blocker), making `seed` a permanent terminal state even after the verb between them firmed (`eigenfreq_qfactor_reduce` c082, `sparameter_reduce` c083). The columns are composition-roots over already-firm vocabulary; the drive on the *constituent vocabulary* is real and correct, but the inter-column rule was the over-constraint. The directive-3 cross-link wiring is a *reference relationship*, not a constituent-firmness dependency. The all-13-column re-evaluation under this rule (flip the `status:` token of every column whose own composition + directly-owned constituents are firm) is the batch-27 lead, dispatched to `layer-intro-author`.
...
- **Layout convention:** flat per-file `book/src/feature/<name>.{L4,L1,L0}.md` (one file per level), under a top-level `# Feature surfaces — entry points` Part placed AFTER `# Methodology`, BEFORE `# L4` (the spine reads top-down). Within a column the level ordering is **high→low (L4→L1→L0), DELIBERATELY NOT alphabetized** (the deliberate exception to the directive-3 alpha-within-cohort rule). **The directive-3 by-kind sub-chapter grouping DOES apply to the Feature Part** (user directive 2026-06-03; codified batch-23 meta-phase): the Part nests into **spine-ROOT (lifecycle)** / **driver-leaf** (5 drivers + boundary-mode) / **output-product** (capacitance / inductance / sparameters / eigenfrequency-qfactor / energy-fields) kind groupings, each with an intro page, matrix alpha-within-kind — **with the within-column high→low exception preserved**; the one-time reorg is a meta-phase-sequenced HIGH structural wave, new columns land in their kind grouping (not flat-appended).
...
- **Records/structs get a definition home — never described only by their USE (the record-definition obligation, user directive 2026-06-03; codified batch-23 meta-phase).** A record/struct named in a function signature (a config record like `ElectrostaticConfig` / `Config`/`IoData`; a per-solver state carrier; an L4 record type like `OpParams` / `Krylov` / `SimState` / `StepOutputs` / `PrevCarry`; a `{ field: type }` result) must have a home defining the record **in itself** — its fields, their types, their meaning, the construction-vs-run-time stratum of each field, and the L0 source home of the backing C++ struct / `IoData` surface it mirrors — NOT only how functions use it. Two homes by reuse: a record used by **only one** operator/feature chapter gets an in-chapter `## Record definition` section (the `harvester`/`layer-intro-author` authoring that chapter writes it); a record used across **≥2** chapters gets a cross-cutting `book/src/concepts/<record>.md` record-definition page (the `layer-intro-author` authors it — same ≥2-consumer bar as any concept page). The definition is the *data shape*; the operators define the *behavior over it* (do not restate operator algebra on the record page). Producer obligation: when your signature names an as-yet-undefined record, either define it (single-consumer) or flag `record-<name>-needs-definition-home` in Open questions (cross-cutting). Critic-side: the `surface-or-evidence` record-definition sub-check flags a signature-named record with no definition home anywhere. Codified into `harvester` / `layer-intro-author` / `critic` role-specs.
