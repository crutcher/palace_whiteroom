...
inputs:
  ...
  - book/src/L2/linear_combination.md (the firm L2 LHS anchor; created this cycle by the harvester)
  - book/src/L1/{scal,axpy,axpby,axpbypcz}.md (the four firm L1 fixed-arity leaves; RHS anchors)
  - book/src/L2-L1/chebyshev-iteration-fusion.md (the sibling L2>L1 lowering-chapter format precedent)
  - book/src/L2-L1/index.md (the L2>L1 theme-list dep-map)
  - book/src/SUMMARY.md (chapter registration under the L2>L1 Part)
  - palace/linalg/vector.cpp:702-758 (the AXPY / AXPBY / AXPBYPCZ free-function bodies — the pinned summation orders)
  - OQ linear-combination-harvester-formalization (this theme is the harvester report's §"Open questions" item 2 follow-up)
---

# CYCLE: L2>L1 theme sketch — linear-combination-fold-specialization

## Summary

This cycle's harvester firmed the L2 combinator `linear_combination :: [(Scalar,
Tensor[N])] -> Tensor[N]` (the variadic fold `foldl (\acc (a,t) -> acc + a·t) zeros
pairs`) and explicitly deferred two things to this L2>L1 theme (harvester report
§Dependencies + §"Open questions" item 2): (i) the **arity-dispatch lowering rule** —
which fixed-arity L1 leaf each fold-length lowers into — and (ii) the **pinned
summation order** each lowered L0 call evaluates in (the load-bearing content of the L2
entry's permutation IEEE-754 non-law, which the L2 algebra is order-agnostic about and
deferred here). This theme authors both. The lowering is a **fusion-selection rewrite**:
narrated forward (high→low), the L2 variadic fold lowers by reading the term-list length
and selecting the **maximal fused L1 BLAS-1 primitive** that matches the arity — `scal`
(arity 1), `axpy` (arity 2, unit second coeff), `axpby` (arity 2, general),
`axpbypcz` (arity 3) — falling back, beyond arity 3, to the iterated left-fold of
`axpbypcz`-into-output chains with the aliased coefficient pinned to 1 (the `γ=1`
accumulate-into shape, because Palace's fused-kernel surface ceilings at arity 3 —
there is no `AXPBYPCZPDW`). The theme is `firm`: the L2 LHS is firm, all four L1 RHS
leaves are firm, the dispatch rule IS the L2 entry's already-firm concatenation-
homomorphism law read as a lowering, and the summation orders are read straight off the
verified `vector.cpp` bodies. No speculative L1 operators (every RHS leaf is firm
already). Closes the harvester's deferred-to-L2>L1 follow-up.
...
Append this row to the §"Theme list" table in `book/src/L2-L1/index.md`, immediately
after the existing `chebyshev-iteration-fusion` row (currently `:13`).

```edit:book/src/SUMMARY.md
- [linear-combination-fold-specialization](./L2-L1/linear-combination-fold-specialization.md)
```

Insert this chapter entry under the **`# L2 > L1 — Lowering`** Part, as the last chapter
after the `chebyshev-iteration-fusion` entry (currently `book/src/SUMMARY.md:42`), before
the blank line separating the L2>L1 Part from the `# L1 — Mutation-Lifted Forms` Part.
Anchor:
...
- [linear-combination-fold-specialization](./L2-L1/linear-combination-fold-specialization.md)   <- new entry here
```

## Speculative operators proposed

**None.** This is firm-foundation work: the L2 LHS
([`book/src/L2/linear_combination.md`](../../book/src/L2/linear_combination.md), firm,
harvested this cycle) and all four L1 RHS leaves (`scal` / `axpy` / `axpby` /
`axpbypcz`, all firm) already exist. The theme is the lowering edge between firm
vocabulary on both sides; it proposes no new operators for harvester promotion.

## Supporting evidence

L0 ranges self-verified via codemap `read_range` this invocation (real-real path —
the path whose body is the pinned summation order; complex paths delegate to MFEM
member ops with the same operand order):

- `vector.cpp:702-712` — `AXPY(double,…)` `α==1.0` fast-path (`y += x`) else
  `y.Add(alpha, x)`. Verified.
- `vector.cpp:726-730` — `AXPBY(double,…)` → `add(alpha, x, beta, y, y)` (single fused
  pass). Verified.
- `vector.cpp:745-758` — real-real `AXPBYPCZ`: `if (gamma == 0.0) { add(alpha, x, beta,
  y, z); }` (`:749-751`, the arity-3→arity-2 collapse) `else { AXPBY(alpha, x, gamma, z);
  z.Add(beta, y); }` (`:753-756`, the two-pass split summation order). Verified —
  load-bearing for both the selection edge and the summation-order table.
- `vector.hpp:305-316` — `AXPY`/`AXPBY`/`AXPBYPCZ` free-function decls; ceiling at
  `AXPBYPCZ` (no arity-4 fused kernel). Verified.
- `nleps.cpp:343-344` — `AXPBYPCZ(…, 1.0, z.Real())` / `z.Imag()` (`z=0.0` seed at
  `:340`); the `γ=1` iterated-chain live witness. Verified.
- `romoperator.cpp:188-189` — `AXPBYPCZ(…, 1.0, u.Real())` / `u.Imag()`; the same
  `γ=1` iterated-chain shape. Verified.
- `timeoperator.cpp:217` — `AXPBYPCZ(1.0, RHS2, dt, k1, 0.0, k2)`; the `γ=0` fall-through
  live witness. Verified.

Artifact cross-references (read this invocation):
`reports/2026-05-28T231026Z-harvester-linear-combination-L2/CYCLE.md` (the firm L2 entry
+ its 7 laws + the deferral of the summation-order recording to this theme);
`book/src/L1/{scal,axpy,axpby,axpbypcz}.md` (the four firm leaf signatures, verified);
`book/src/L2-L1/chebyshev-iteration-fusion.md` (the lowering-chapter-format precedent);
`book/src/L2-L1/index.md` + `book/src/SUMMARY.md` (registration targets).

## Open questions / caveats

(Recorded inside the theme body's §"Open questions / caveats" — the lifting-note,
no-dedicated-test, arity-≥4-chunking, and `inner_product`-sibling items. Summarized for
the integrator:)

1. **Lifting note** kept in the theme's working-notes §, not the high→low body (per the
   layer-definition discipline). Value-faithful, not bit-faithful (loses pinned order).
2. **No dedicated test** — inherited from the L2 entry's empirical-match caveat; not a
   status reduction (source-transcription + live call sites + chebyshev-iteration
   firm-without-test precedent).
3. **Arity-≥4 chunking** is value-free / bit-pinned — the greedy-maximal left-fold is
   recorded as canonical because it matches Palace's `γ=1` iterated shape.
4. **`inner_product` sibling** is a separate future lowering theme (the parallel
   combinator-miner-on-inner-product track this cycle), not subsumed here.
```
