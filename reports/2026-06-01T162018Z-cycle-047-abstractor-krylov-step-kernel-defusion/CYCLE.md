...
inputs:
  ...
  - book/src/L1/apply_linop.md, axpy.md, axpby.md, axpbypcz.md, dot.md, nrm2.md, scal.md (firm L1 RHS leaves)
  - book/src/L2-L1/orthogonalize-composition-lowering.md (structural precedent — composition-lowering)
  - book/src/L2-L1/chebyshev-iteration-fusion.md (fusion-rotation sibling)
  - reference/palace/palace/linalg/iterative.cpp:427-464 (CG per-step kernel for-loop, L0 witness; self-verified this dispatch)
  - book/src/L1-L0/ksp-solve-mutation-rotation.md Sub-pattern B (the terminal L0 home for the CG body)
---

# CYCLE: L2>L1 theme sketch — krylov-step-kernel-defusion

## Summary

The L2 `krylov-step` entry is firm but carries **no §"Lowers from" section** and leaves variant-axis 6 (`book/src/L2/krylov-step.md:121`, the in-place-vs-out-of-place buffer axis) as a **dangling forward-reference** to an L2>L1 lowering file that does not exist. This dispatch authors that file: `book/src/L2-L1/krylov-step-kernel-defusion.md`. The theme is the **de-fusion of the per-step kernel** (one composition surface at L2) **into its seven firm L1 leaves** (`apply_linop`, `axpy`, `axpby`, `axpbypcz`, `dot`, `nrm2`, `scal`, the firm list at `book/src/L2/krylov-step.md:96`) under an outer fold, **plus the in-place→out-of-place buffer rotation** (the `:121` variant-axis-6 specialisation). Narrated forward (L2→L1, high→low): the named kernel composition **fans down** into its dataflow-forced primitive sequence, and the L2's uniformly-out-of-place buffer convention **specialises down** to the L0 in-place buffer reuse that `Vector::Add` / `w`-aliases-`V[j+1]` realise. The justification is **structural** (the de-fusion is the syntactic expansion of the L2 §Semantics five-primitive-group body into the L1 leaf sequence) with an **algebraic** flavour on the buffer rotation (the in-place/out-of-place forms are transparent-performance-equivalent per CLAUDE.md §Optimization tricks). No speculative L1 operators — all seven RHS leaves are firm post-cycle-004, and the LHS is firm. This resolves the dangling `:121` forward-ref, closes OQ `krylov-step-l2-l1-theme-gap`, and jointly with D3 (`ksp-solve-outer-driver-unfold`) closes `residual-l2-l1-gap-audit`.

## Proposed changes

```new:book/src/L2-L1/krylov-step-kernel-defusion.md
...
NOTE TO INTEGRATOR: append the following bullet to the §"Vocabulary cohort" **Firm at L2>L1** list (after the `incremental-least-squares-composition-lowering` bullet). This is THIS theme's own cohort bullet (dual-registration: row + own bullet are mine; the consolidated firm-count tally + cohort-growth-log are DEFERRED to D3 `ksp-solve-outer-driver-unfold`, the sole count-owner this cycle):
...
**None.** This theme is a lowering edge between firm vocabulary on both sides. The LHS `L2/krylov-step` is firm (cycle-004); all seven L1 RHS leaves (`apply_linop`, `axpy`, `axpby`, `axpbypcz`, `dot`, `nrm2`, `scal`) are firm post-cycle-004. No harvester hand-off required.

## Supporting evidence

- `reference/palace/palace/linalg/iterative.cpp:427-464` — the CG per-step kernel for-loop, the worked L0 specialisation of the de-fusion (self-verified this dispatch via codemap read_range + citecheck anchors `A->Mult`@:443, `AXPBY`@:440, `x.Add`@:448).
- `book/src/L2/krylov-step.md:96` — the firm seven-leaf L1 dependency list (the RHS family).
- `book/src/L2/krylov-step.md:121` — variant-axis-6, the in-place-vs-out-of-place buffer axis that explicitly deferred its specialisation "to the L2>L1 lowering" — the dangling forward-ref this theme resolves.
- `book/src/L2/krylov-step.md:129-132` — the L2-vs-L1 distinction; treated as CONTEXT (the conceptual gloss), NOT as the lowering. The actual forward L2→L1 rewrite is narrated fresh in §"The de-fusion rewrite".
- `book/src/L2-L1/orthogonalize-composition-lowering.md` — the structural precedent (one-L2-composition-fans-into-L1-vocabulary; the stop-at-the-L1-leaf delegation discipline for the in-place buffer mechanics).
- `book/src/L1-L0/ksp-solve-mutation-rotation.md` Sub-pattern B — the firm L0 terminal home for the CG body.

## Open questions

- (carried into the chapter §"Open questions / caveats") **Per-leaf in-place delegation boundary** for the lowering-verifier; **CG-as-worked-example, Arnoldi/Chebyshev stated-not-per-line-verified** (does not gate firm); **OQ `krylov-step-l2-l1-theme-gap` discharged**; **jointly-with-D3 closes `residual-l2-l1-gap-audit`**.
- **Index-tally deferral (coordination with D3).** Per the dispatch DUAL-REGISTRATION partition, I registered (1) my own index TABLE row and (2) my own §Vocabulary-cohort BULLET inline, and DEFERRED (3) the consolidated §Working-Notes firm-count tally + cohort-growth-log to D3 (`ksp-solve-outer-driver-unfold`, the SOLE count-owner this cycle, which writes the 19→21 tally accounting for both my landing and its own). I did NOT touch the tally. D3 is writing the same `L2-L1/index.md` (its own distinct row + the consolidated tally); the integrator's serial per-report re-read handles the two distinct-region index edits.
- **L1>L0 delegation-target on-disk check (done this dispatch).** The chapter live-links the per-leaf in-place-buffer delegation targets. I verified all on disk: `axpby-mutation-rotation.md` (the correct home for the `axpy` `x.Add`/`r.Add` overwrite — it lowers BOTH `axpy` and `axpby`; there is NO standalone `axpy-mutation-rotation.md`), `apply-linop-mutation-rotation.md` (HYPHEN, not underscore — corrected from an initial underscore draft), `orthogonalize-mutation-rotation.md`, `ksp-solve-mutation-rotation.md`, `chebyshev-iteration-fusion.md`, and `orthogonalize-composition-lowering.md` all EXIST on disk → live links are safe (no `linkcheck2` break). The only NOT-yet-on-disk live link is the D3 sibling `ksp-solve-outer-driver-unfold.md` (created in the same cycle by D3) — the integrator's serial per-report ordering should apply D3's `new:` block (or this theme should be applied after D3) so the link resolves; if D3 has not landed when this report is applied, defang `[ksp-solve-outer-driver-unfold](./ksp-solve-outer-driver-unfold.md)` to plain-text per the missing-anchor convention (two occurrences: §intro + §Verified-against).
