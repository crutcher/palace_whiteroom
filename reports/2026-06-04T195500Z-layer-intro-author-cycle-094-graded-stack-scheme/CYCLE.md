...
# CYCLE: graded-stack-scheme (P0-A)

## Summary

This report proposes the canonical machine-readable **graded-stack scheme** as a new
normative authoring-convention page, `book/src/methodology/graded-stack-scheme.md`, plus
the one-line `SUMMARY.md` wiring under `# Methodology`. The page is the authoritative
authoring spec that the two linters (D2, next wave) and all producers consume. It is
**non-authoritative relative to `METHODOLOGY-GRADED-STACK.md`** (same status convention as
`goal-flow.md`: if the page contradicts that doc, that doc wins). It specifies, in full:

1. the `rank:` token + the explicit mapping of every existing maturity-vocabulary value
   onto the ladder `roadmap_goal=0 < stub=1 < rough-in=2 < firm=3` (sub-rank 2.5 for
   `partly-constructive` / `rough-in (test-coverage-bounded)`; `obstruction` /
   `partial-obstruction` a separate rankable *kind*);
2. the typed-edge block grammar — `depends-on` (blocking) vs `reference` (free; an edge to
   a feature root is `reference`); optional ignored `kind:` annotation;
3. the feature-root marker `seed` as a parallel axis (NOT a ladder rung);
4. the **migration mapping** from the three current representations, with a concrete
   recommendation for where edge types live going forward (a single `edges:` frontmatter
   block superseding the ad-hoc `depends_on:`) AND the explicit P1-cost design fork flagged
   for the batch-30 meta-phase / human;
5. node-status for the 253/357 files with no maturity frontmatter, with the lowering-theme
   rule resolved cleanly (theme rank = min of its endpoints) and the index/concept-page
   node-status carved out as a P1 sub-task + an OQ (NOT forced in one pass).

The page is a methodology mirror **outside the subject DAG** (`METHODOLOGY-GRADED-STACK.md`
§2d), so it carries **no rank/edge frontmatter itself**. I own the `# Methodology` SUMMARY
ordering this cycle: `overview.md` → `goal-flow.md` → `resolution-ladder.md` (D3) →
`graded-stack-scheme.md` (D1, this report). I do NOT refresh `goal-flow.md`
(meta-phase-owned).

On-disk grounding (verified this cycle): 357 `.md` files under `book/src`; **104** carry a
`firmness:`/`status:` maturity line (the 10 `depends_on:`-frontmatter L1 entries + the 24
feature-column files `composes:`/`l0_ground_truth:` + the operator entries that carry
`firmness:`), leaving **253** with none (all concept pages, all L0 pages, all lowering
themes, the L2/L3 entries, the index pages). This corroborates the planner's count and is
the load-bearing fact the migration mapping (§4/§5) must address.

## Proposed changes

### Change 1 — new page `book/src/methodology/graded-stack-scheme.md`

```edit:book/src/methodology/graded-stack-scheme.md
[old]:
[new]:
...
### Change 2 — wire into `book/src/SUMMARY.md` under `# Methodology`

The `# Methodology` ordering this cycle is `overview.md` → `goal-flow.md` →
`resolution-ladder.md` (D3) → `graded-stack-scheme.md` (D1, this report). I insert the
`graded-stack-scheme.md` row LAST (after the `resolution-ladder.md` row D3 inserts). To keep
this report's edit independent of whether D3 has landed yet at integration time, I anchor on
the stable `goal-flow.md` row and add BOTH ordering-aware possibilities is avoided — instead I
anchor on the last existing methodology row and append. The integrator's per-report SUMMARY
merge resolves the final ordering (D3's `resolution-ladder.md` row sorts before this one per
the named ordering).
...
> **Integrator note (SUMMARY ordering):** D3 (`resolution-ladder.md`) inserts its row at the
> named position *between* `goal-flow.md` and `graded-stack-scheme.md`. If both reports stage,
> the final intended order is `overview → goal-flow → resolution-ladder → graded-stack-scheme`.
> This report's edit appends `graded-stack-scheme.md` after `goal-flow.md`; when D3's row is
> also applied, place `resolution-ladder.md` *above* `graded-stack-scheme.md`. Both are distinct
> new rows under the same section (the parallel-safe "distinct rows" case); the only coordination
> is the relative order of the two new rows, named here and in D3's scope.

## Supporting evidence

- **Authoritative spec read in full:** `METHODOLOGY-GRADED-STACK.md` — §1a/§1f (ladder + ranks +
  obstruction-as-separate-kind), §2a/§2b/§2c (root set + reachability + OWN-COMPOSITION falls
  out of the root marker), §2d (methodology pages outside the DAG), §3 (typed-edge minimal
  binary + ignored `kind:`), §4 (the two linters), §5 (adoption), §8 (role responsibilities,
  incl. the lowering-verifier "theme ≤ min(endpoints)" rule I used for §5), §9 (book methodology
  update instructions).
- **On-disk shapes grounding the migration mapping (§4/§5):**
  - The 10 `depends_on:`-frontmatter files (all L1): `book/src/L1/{bilinear-form, fe_assemble,
    participation_ratio, eliminate_rhs, essential_dofs, weak_form_term, eliminate_essential_bc,
    port_projection, eigenvalue-untransform, assemble_frequency_operator}.md`. Shape confirmed:
    `firmness:` + `depends_on:` (slug list) + `lowers_to:`/`lifts_from:` + `variant_axes:`.
  - Feature `composes:`/`l0_ground_truth:` frontmatter: `book/src/feature/eigenmode.L4.md`
    (`status: firm`, `composes:` of vocabulary ops), `book/src/feature/capacitance.L4.md`
    (`status: seed`, `composes:` mixing a sibling driver column AND a vocabulary reduce verb —
    the exact OWN-COMPOSITION case §3 splits into `reference` vs `depends-on`). 24 feature files
    carry `composes:`.
  - Prose dep-map table (NOT parseable): `book/src/L1/index.md:107-144` — the "Dependencies"
    cells mix slugs, prose qualifiers, and explicit non-dependencies ("sibling to `apply_linop`,
    NOT a dependency"), confirming option (b)/(c) are lossy.
  - Lowering-theme shape (no frontmatter; status in prose): `book/src/L4-L3/krylov-step-typed-wrapper-dissolution.md`.
  - Concept-page shape (no frontmatter): `book/src/concepts/dot.md` (narrative pointer — outside
    DAG) vs `book/src/concepts/config-record.md` (record-definition — a DAG node) — the §5
    two-sub-case split.
- **Counts (verified):** 357 `.md` under `book/src`; 104 with `firmness:`/`status:`; 253 without
  (corroborates planner). 54 files with `lowers_to:`/`lifts_from:` frontmatter (the §4(a)
  lowering-edge migration source).

## Open questions / caveats

- **`record-…-needs-definition-home` flagging:** none surfaced — this is a methodology page with
  no operator signatures naming an undefined record. (The record-definition obligation applies to
  layer/feature content, not to a scheme page.)
- **OQ `graded-stack-index-and-concept-node-status` (P1 sub-task — NOT forced this pass, per the
  planner's explicit allowance):** whether `L_n/index.md` pages and which `concepts/` pages are
  themselves DAG nodes (and with what `rank:`) is not cleanly resolvable in this single P0-A pass.
  Index pages are navigational overviews whose dep-map table is a derived view (§4(b)); concept
  pages split into outside-DAG narrative pointers vs in-DAG record-definition nodes (§5). Resolving
  this interacts with the table-vs-frontmatter fork (§4) and the group-intro pages. Recorded as a
  P1 sub-task: the P1 typing author makes the per-page node-vs-not-a-node call as part of the
  classification pass, against the §2d boundary. Flagged for batch-30 meta-phase intake.
- **OQ / DECISION POINT `graded-stack-edge-home-fork-p1-cost` (batch-30 meta-phase / human):** my
  recommendation is **option (a) per-chapter `edges:` frontmatter everywhere** (§4), which is the
  only deterministically-parseable, theme/concept-covering, single-source-of-truth option — but it
  **materially raises P1's cost** (hand-classify ~250 nodes vs parse ~15 index tables). The
  cheaper bounded compromise is **(a)-incremental** (type the feature-root `depends-on` closure +
  high-fan-out frontier first, lazy tail; linters warn-not-fail on untyped). This is the
  highest-leverage call in the campaign and sets the entire P1 cost — flagged here as a batch-30
  meta-phase / human decision point exactly as the planner requested.
- **OQ `graded-stack-feature-root-frontmatter-split`:** the scheme splits the historical feature
  `status: seed` into `feature_root: seed` (root marker) + `rank:` (composition-maturity, §3). The
  24 feature-column files currently encode both in one `status:` token. P1 must split them; until
  then D2's reachability GC should accept `status: seed` as the root marker (back-compat) AND read
  `status: firm`/etc. as the `rank:`. Recorded so D2 (linter) handles the transitional dual form.
  This is a transitional-period note, not a blocker.
- **`obstruction_resolution` sub-field is a scheme addition not spelled letter-for-letter in
  `METHODOLOGY-GRADED-STACK.md`** (the doc says "obstruction is a *kind* that is itself rankable",
  §1f, without fixing the frontmatter spelling). I chose `rank: obstruction` + `obstruction_kind:`
  + `obstruction_resolution:` as the concrete encoding. If the batch-30 meta-phase or D2 prefers a
  different spelling, this page is the place to fix it (it is the authoring contract). Flagged so
  D2's parser and this page stay in sync on the obstruction encoding.
- **`goal-flow.md` is meta-phase-owned and NOT touched here** (per scope). The §9 `goal-flow`
  GOAL/FLOW additions (the "two checkable health invariants" GOAL line + the typing-campaign FLOW
  paragraph + the `roadmap_goal` tier) and the `## Roadmap goals — unbuilt frontier` SUMMARY
  grouping + the per-`roadmap_goal`-chapter banner are flagged for the batch-30 meta-phase, which
  owns the goal-flow refresh. No `roadmap_goal` chapter exists on disk this cycle, so the SUMMARY
  grouping is not added yet (it lands when P2 mints the first one).
