---
agent: abstractor
invoked_at: 2026-05-29T18:02:02Z
...
inputs:
  ...
  - book/src/L1-L0/nrm2-mutation-rotation.md (reused Sub-pattern: Norml2 reduction)
  - book/src/L1-L0/scal-mutation-rotation.md (reused Sub-pattern A: in-place rescale)
  - book/src/L1-L0/matrix-weighted-norm-mutation-rotation.md (cycle-026 sub-pattern-reuse precedent)
  - palace/linalg/iterative.cpp:631-632 (GMRES Hessenberg consumer — load-bearing returned norm)
  - palace/linalg/operator.cpp:673,676 (power-iteration eigenvalue consumer — load-bearing returned norm)
  - palace/linalg/nleps.cpp:610-611,617 (NEP deflation consumer — load-bearing returned norm)
---

# CYCLE: L1>L0 theme sketch — normalize-mutation-rotation

## Summary

The firm L1 `normalize` operator (`normalize :: Tensor[N] -> (Scalar, Tensor[N])`, landed firm cycle-026, positive source `linalg::Normalize` at `palace/linalg/vector.hpp:262-270`) needs its L1>L0 mutation-rotation theme. This theme narrates forward how the pure-functional fused form `(β, û) = normalize(x)` lowers into Palace's in-place L0 free-function template `auto norm = Norml2(comm, x); MFEM_ASSERT(norm > 0.0, ...); x *= 1.0 / norm; return norm;`. The rewrite is **structural** and decomposes into three sub-patterns plus one guard, all of which **reuse** the existing firm BLAS-1 themes rather than re-derive: (A) the `Norml2` reduction inherits `nrm2-mutation-rotation` wholesale (no-buffer `sqrt∘abs∘Dot` chain); (B) the in-place `x *= 1.0/norm` rescale inherits `scal-mutation-rotation` Sub-pattern A (the real-path receiver overwrite); (C) the **load-bearing returned norm** — the value the fusion preserves that a bare `nrm2 ∘ scal` discards — is the whole reason `normalize` is a named primitive, evidenced by three distinct consumer shapes (GMRES Hessenberg sub-diagonal `iterative.cpp:631-632`, power-iteration eigenvalue estimate `palace/linalg/operator.cpp:673,676`, NEP deflation companion-scale `nleps.cpp:610-611,617`). The `MFEM_ASSERT(norm > 0.0)` guard is the **partiality witness** (the one semantic addition over the total `nrm2`/`scal` leaves). The B-weighted sibling `normalize_B` is recorded as a rough-in note (its `matrix-weighted-norm` constituent is rough-in, and there is no fused Palace `Normalize`-with-`B` site). This follows the cycle-026 `matrix-weighted-norm-mutation-rotation` precedent of reusing sibling sub-themes (apply-linop A + dot A) instead of restating them.

## Proposed changes

```new:book/src/L1-L0/normalize-mutation-rotation.md
...
**None.** This theme lowers the already-firm L1 `normalize` operator into existing firm L1 vocabulary (`nrm2` reduction + `scal` rescale). It proposes no new rough-in L1 operators and requires no harvester promotion.

The B-weighted sibling `normalize_B` is recorded as an in-chapter **rough-in note** (NOT a speculative operator of this theme) — it has no fused Palace site (the sole `linalg::Normalize` overload takes no `B`; the header comment is aspirational) and its norm constituent `matrix-weighted-norm` is itself `rough-in (test-coverage-bounded)`, so a fused `normalize_B` cannot be firmer than its constituent. It is tracked as a queued candidate inheriting the `matrix-weighted-norm` promotion gate, surfacing for promotion only if/when an inline B-weighted-normalise site (`scale = Norml2(comm, v, B, Bv); v *= 1.0/scale`, distinct from the unweighted `nleps.cpp:610-611`) appears.

## Supporting evidence

L0 source (self-verified via `tools/citecheck/citecheck.py --anchor` against on-disk `reference/palace/`, and cross-checked against the Read tool which reads on-disk directly):

- `palace/linalg/vector.hpp:262-270` — `linalg::Normalize` template, the positive source. The `Norml2` reduction binding is line 266, the `MFEM_ASSERT(norm > 0.0)` partiality guard line 267, the in-place rescale `x *= 1.0 / norm` line 268, the `return norm` line 269. citecheck reported the `Normalize` anchor at lines [262, 264] (262 = header comment, 264 = `inline auto Normalize` def line); both fall within the cited range 262-270, so the range citation is correct. **The on-disk file is authoritative; the dispatch noted codemap `read_range` can be +1 behind on brace boundaries — here the Read tool (on-disk) and citecheck (on-disk) agree on all anchors, so vector.hpp is NOT affected by the codemap drift.**
- Three load-bearing returned-norm consumer sites (all citecheck-`--anchor`-verified this invocation): GMRES Arnoldi Hessenberg `iterative.cpp:631-632`; power-iteration eigenvalue estimate + convergence test `palace/linalg/operator.cpp:673,676`; NEP deflation companion-scale `nleps.cpp:610-611,617`.

Reused sibling themes (cited, not re-derived):

- `book/src/L1-L0/nrm2-mutation-rotation.md` — Sub-pattern A (the `sqrt∘abs∘Dot(x,x)` no-buffer reduction chain + the load-bearing `std::abs` defensive-guard classification).
- `book/src/L1-L0/scal-mutation-rotation.md` — Sub-pattern A (the in-place `x *= 1.0/norm` receiver overwrite; it already names this very `Normalize` site at `:48-49,55-58`).
- `book/src/L1-L0/matrix-weighted-norm-mutation-rotation.md` (cycle-026) — the sub-pattern-reuse precedent (reuses apply-linop A + dot A) and the B-weighted boundary.

L1 anchor: `book/src/L1/normalize.md` (firm, cycle-026).

## Open questions / caveats

- **OQ `normalize-mutation-rotation-l1-l0-theme`** — RESOLVED by this dispatch (the theme is authored). The L1 entry `book/src/L1/normalize.md:104` referenced this theme as plain text, noting "The L1>L0 `normalize-mutation-rotation` theme is not authored by this dispatch — see Open questions; this entry references it as plain text, not a live link." With this theme landed, the integrator MAY upgrade that plain-text reference in `book/src/L1/normalize.md` to a live link `[normalize-mutation-rotation](../L1-L0/normalize-mutation-rotation.md)` per skill `upgrade-plain-text-ref-to-live-link-when-target-on-disk` (that cross-reference upgrade is the integrator's Phase 5 call, not this dispatch's — flagged here as a follow-up).
- **Returned-norm consumer-cohort completeness** — the three consumer shapes (Hessenberg sub-diagonal, eigenvalue estimate, deflation companion-scale) are the *recognised* load-bearing shapes; the cited sites are illustrative, not exhaustive. A `lowering-verifier` audit should confirm the `linalg::Normalize` template is the sole fused-normalise overload and the returned-norm consumer cohort is complete (no un-cited site discards-vs-consumes the norm in a fourth shape). Standard follow-up, not a status reduction.
- **`normalize_B` promotion gate** — the B-weighted sibling stays a rough-in note until (a) an inline B-weighted-normalise *rescale* site surfaces (the existing weighted `Norml2(comm, x, B, Bx)` callsites compute error-norms / eigenvector-norms but do NOT rescale), AND (b) its `matrix-weighted-norm` constituent promotes past `rough-in (test-coverage-bounded)`. Both gates are upstream of this theme. Tracked as a queued candidate.
- **Inherited single-rank / reduction-order caveats** — the MPI collective inside the inherited `Norml2 → Dot` reduction is a single-rank no-op (in scope per CLAUDE.md "Scope"); the reduction-tree non-associativity is inherited from `nrm2`/`dot` and not restated here. No new bit-determinism concern is introduced by the fusion.
