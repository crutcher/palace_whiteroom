...
scope: L2 operator: nrm2
status: pending
integrated_at: 2026-06-01T062913Z
integration_commit: c1f7ea3c651e65ed212aa8500c7c8572aaa2ec92
integration_notes: "Applied clean (staging row D2). book/src/L2/nrm2.md created firm (consumer-of-fold sqrt-abs-inner_product at y=x, NOT a fold member; std::abs load-bearing guard preserved). Repairer fixed edit:->new: directive. SUMMARY + L2/index dep-map row. Rides the design-fork (carried to batch-12 meta-phase). L2 firm 9->12 (cohort 3-of-13)."
inputs:
  - book/src/L1/nrm2.md (firm cycle-003; authoritative Palace-surface + L0 evidence)
  - book/src/L1-L0/nrm2-mutation-rotation.md (firm; the four-stage L0 chain + std::abs classification)
  - book/src/L3/nrm2.md (firm cycle-011; the L3 consumer — frontmatter + framing mirrored)
  - book/src/L2/index.md (fold-cohort boundary §; "nrm2 = √ ∘ inner_product at y=x, consumer NOT instance")
  - book/src/L3-L2/krylov-step-body-identity.md (the seven L1 primitives are L3-native/L2-native by signature shape)
  - palace/linalg/vector.hpp:255-260, 262-270 (L0 anchors; verified on-disk via citecheck --anchor)
  - directive l2-floor-under-l3-blas1-cohort (2026-05-31 foundation-first; D2 this cycle)
---

# CYCLE: Formalize nrm2 at L2

## Summary
`nrm2` is the BLAS-1 Euclidean-norm reduction `α = ‖x‖₂ = √⟨x, x⟩`. This dispatch builds the **L2 floor entry** so the firm L3 `nrm2` (cycle-011) rests on a *present* adjacent L2 parent (per the "Identity-lowerings still require both L levels" invariant). The entry is a **thin identity-in-form floor**: at the fusion-rotation layer there is no HPC/SIMD trick to unfold — Palace's `linalg::Norml2` is already the unfolded one-line composition `√ ∘ abs ∘ Dot` — so the L2 form is value-thread-isomorphic to the L1 form. The one genuinely-L2 content beyond identity is the **fusion-rotation framing**: `nrm2` is rendered as a **consumer of the `inner_product` fold** (`nrm2 = √ ∘ inner_product` at `y=x`), explicitly NOT a fold member (per the L2 §"Fold-cohort boundary" do-NOT-merge note), and the `std::abs` defensive guard is preserved as an explicit load-bearing numerical claim (L2 discipline: load-bearing numerical tricks survive as algebraic claims). The L2>L1 / L3>L2 rotations are narrated by the D5 themes this cycle; this entry defines `nrm2` in L2 vocabulary and cites those themes for the rotation work. Status lands `firm`.

## Proposed changes

```new:book/src/L2/nrm2.md
---
layer: L2
operator: nrm2
firmness: firm
...
- **L0 anchors verified on-disk** (`citecheck --anchor`, both `[ok]`):
  - `palace/linalg/vector.hpp:255-260` anchor `Norml2` → line 257 (template), body line 259 `return std::sqrt(std::abs(Dot(comm, x, x)));`.
  - `palace/linalg/vector.hpp:262-270` anchor `Normalize` → lines 262/264; `MFEM_ASSERT(norm > 0.0, ...)` at 267, `x *= 1.0 / norm` at 268.
- **`book/src/L3-L2/krylov-step-body-identity.md:97`** — confirmed the load-bearing statement: the seven L1 primitives `apply_linop, axpy, axpby, axpbypcz, dot, nrm2, scal` are L2-native / L3-native because each signature has no per-element loop visible. This is the structural justification for the identity-in-form floor on both adjacent edges (L2>L1 and L3>L2).
- **`book/src/L2/index.md:53,75`** — the `inner_product` dep-map row and §"Fold-cohort boundary" both name `nrm2` / `matrix-weighted-norm` as a **consumer** (`√ ∘ inner_product` at `y=x`), **NOT an instance** — the load-bearing framing this entry honors.
- **`book/src/L1/nrm2.md:11`** — the concept-page scaled-summation-correction note; the L1 entry is authoritative on the Palace surface; this L2 entry defers to it and does not re-derive.

## Open questions / caveats

- **No L2 `dot` floor exists** — the L3 `nrm2` entry lists `dot` as a same-layer L3 dependency (`book/src/L3/nrm2.md:95`), but at L2 the inner-product surface is the `inner_product` *fold* (firm cycle-019), not a `dot` leaf. This L2 entry correctly anchors its dependency on `inner_product` (the fold `nrm2` consumes), consistent with the L2 fold-cohort vocabulary. **No L2 `dot.md` floor entry exists or is needed** — `dot` is the L1 leaf the `inner_product` fold fuses up from; the L2 surface is the fold. This is intentional and not a coverage gap. (Flagging for the cycle-planner: if a future audit wants per-leaf L2 floors symmetric with the L3 `dot` floor, that is a separate plan item; the fold-cohort vocabulary makes it arguably redundant.)
- **L2 index intro / firm-count tally NOT touched** (per COUNT-OWNERSHIP directive — D7 owns the consolidated L2 firm-count this cycle). I appended only my dep-map row. The L2 index does not carry a single consolidated count line; the firm cohort is narrated inline in §"Working Notes" (currently "6→8" from batch-6). If D7's tally update wants a Working-Notes entry for the cycle-041 BLAS-1-floor cohort, that is D7's to author — I did not add one.
- **L2 index intro refresh** — the §Semantics motif list and §Vocabulary-cohort prose (`book/src/L2/index.md:19-44`) currently organize the firm cohort around named-compositions + fold-cohorts; the BLAS-1 identity-in-form *floor* entries (this `nrm2`, plus any sibling `axpy`/`scal`/`dot`/etc. floors landed by co-dispatched cohort-mates this cycle) are a *third* category (identity-in-form leaves consumed by the higher compositions). A future `layer-intro-author` pass may want to name this third category explicitly. Out of harvester scope — flagged for the intro author.
- **D5 L2-L1 + L3-L2 `nrm2` themes** — this entry cites the D5 lowering themes (`book/src/L2-L1/` and `book/src/L3-L2/` `nrm2` rotations) as forthcoming this cycle via plain-text / inline-code references where the target file does not yet exist on disk (no live links to absent files, per the `rough-in-forward-reference-must-be-plain-text-not-live-link` convention). If D5 does not land them this cycle, the references remain plain-text and resolve when the themes land. The existing firm [`L1-L0/nrm2-mutation-rotation`](../L1-L0/nrm2-mutation-rotation.md) and [`L3-L2/krylov-step-body-identity`](../L3-L2/krylov-step-body-identity.md) (both on-disk) are live links.
