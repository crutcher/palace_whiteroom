...
scope: L4>L3 theme sketch — fold-solve-time-step-dissolution
status: pending
inputs:
  - book/src/L4/fold_solve.md (cycle-058 D1 LEAD, firm; the LHS combinator this theme lowers — live-link forward-reference, lands at integration before the single finalize build)
  - book/src/L4-L3/solve-family-map-dissolution.md (cycle-055 D2; the MAP-sibling dissolution — structural-parallel precedent)
  - book/src/design/l4_calculus.md:150-184 (§3.7 iterate_while family + iterate_while_pure sugar — the shared parent) + :186-228 (§3.8 demand-pruning)
  - reports/2026-06-02T050136Z-harvester-fold-solve/CYCLE.md (D1; the entry this theme lowers)
  - L0 fold witness (transient fixed-schedule march): palace/drivers/transientsolver.cpp:33,35,36,77,89,93,98,99 + palace/models/timeoperator.cpp:312,410 (all citecheck --anchor verified on-disk this dispatch; codemap read_range was -2 drift on timeoperator)
  - L0 fold witness (driven-PROM SweepAdaptive state-generated greedy): palace/drivers/drivensolver.cpp:73,231,384,389
integrated_at: 2026-06-02T053505Z
integration_commit: PLACEHOLDER_SHA
integration_notes: |
  cycle-058 D2. Applied clean by integrator-per-report (staging row 2), housekept + committed by integrator-finalize.
  NEW book/src/L4-L3/fold-solve-time-step-dissolution.md firm L4>L3 theme (LHS = L4 fold_solve state-threaded fold shell,
  RHS = L3 explicit imperative time-sweep threading the field-state sol IN PLACE; substantive foldl-combinator →
  in-place-mutation translation; obstruction (opaque-library-ownership) per-step sub-leaf at ode->Step, negative anchor
  timeoperator.cpp:410; the fold-sibling of solve-family-map-dissolution c055, opposite carry axis) + L4-L3/index.md
  (theme-list row + Vocabulary-cohort Substantive-themes bullet + consolidated tally 7→8 / "8 firm", the tally edit
  deliberately superseding the old c055 4-shell solve_family composition sentence with the fold/map §3.7-children reframe) +
  SUMMARY.md chapter line. Build-relevant. L4>L3 firm 7→8 (0 rough-in). The same-cycle forward-reference to ../L4/fold_solve.md
  (D1) RESOLVED — D1 landed first per-report. cargo make book exit 0; the page renders, both cross-link directions live.
  0 new OQs (all caveat items already in the ledger). Gate hits: 0.
---

# CYCLE: L4>L3 theme sketch — fold-solve-time-step-dissolution

## Summary

This theme is the L4>L3 dissolution of the L4 [`fold_solve`](../L4/fold_solve.md) **state-threaded fold outer-driver combinator** (cycle-058 D1, firm) — the fold-sibling of [`solve_family`](../L4/solve_family.md)'s independent map. The LHS is the pure left fold `fold_solve op s0 schedule = foldl (\s t -> time_step_op op s t) s0 schedule` (capture `op` once, seed `s0` once, thread an immutable field-state carry forward through a schedule, each step's input the prior step's output). The RHS is the L3 **explicit imperative time-sweep that threads the field-state `sol` IN PLACE** — the `for (int step = 0; step < n_step; step++)` loop (`transientsolver.cpp:77`), the `TimeOperator` built once outside the loop (`:33`), the per-step body bottoming out in the **opaque MFEM `ODESolver` step** `ode->Step(sol, t, dt)` (`timeoperator.cpp:410`). This is a **genuine vocabulary translation, not an identity-in-named-terms rename**: the functional immutable carry becomes in-place mutation of `sol`; the abstract `schedule` becomes the concrete uniform `delta_t`/`t` march bounded by `n_step`; the opaque `time_step_op` quantified-over operator becomes the `ode->Step` **library boundary** — an `obstruction (opaque-library-ownership)` per-step sub-leaf the theme cites at Palace's CALL (NOT MFEM internals). The L3 image is the **sequential-obstruction outer sweep** (each step reads the prior `sol`), the load-bearing structural contrast with `solve_family`'s embarrassingly-parallel family map (its sibling theme's RHS carries NO obstruction; this theme's does). Status assigned: **`firm`** on the structural rotation — the in-place-threading loop, the operator-hoist placement, and the opaque per-step call are all read directly off positive Palace source (two structurally-parallel state-threaded sweeps).

## Proposed changes

```new:book/src/L4-L3/fold-solve-time-step-dissolution.md
...
The cycle-058 D1 harvester firmed [`fold_solve`](../L4/fold_solve.md) — the L4 **state-threaded fold outer-driver combinator**, one coordination shell *above* an opaque per-step solve. The combinator's own §"Lowers to" names the dissolution to L3 as **substantive** (not identity-in-form) and records the rotation *direction* in-line per the high→low discipline, but defers the theme itself to "a separate cycle-058 L4>L3 theme (`fold-solve-time-step-dissolution`), narrated forward from L4 to L3" (`fold_solve.md` §"Lowers to" / §Status). This chapter is that theme (canonical slug `fold-solve-time-step-dissolution`, cycle-058 dispatch D2).

`fold_solve` and [`solve_family`](../L4/solve_family.md) are the two children of the strawman §3.7 [`iterate-while`](../L4/iterate-while.md) family (RATIFIED batch-17 meta decision 2): a **map is the degenerate fold whose step ignores the accumulator**, `fold_solve` is the **non-degenerate** member that threads it. Their two L4>L3 dissolutions are correspondingly siblings, and this theme is **structurally parallel** to [`solve-family-map-dissolution`](./solve-family-map-dissolution.md) (cycle-055 D2) — but the load-bearing difference between the two RHS images IS the map/fold axis:

- [`solve-family-map-dissolution`](./solve-family-map-dissolution.md) (firm c055) — the L4 `map` over an independent RHS family → an L3 positional accumulating `for` that writes each member into its own collection slot. The family loop carries **NO `sequential-obstruction`** (the members are independent — an embarrassingly-parallel sweep written sequentially).
...
- **Reduction-chain** (secondary): the `foldl (\s t -> time_step_op op s t) s0 schedule` desugars to the explicit positional `for` with the carry threaded (the standard `foldl`-to-imperative-loop reduction); the non-degenerate member of the strawman §3.7 `iterate_while` family (the cap's alternate rendering) desugars to the same in-place-threading loop with the carry `{ field, remaining }` becoming the mutated `sol` + the loop counter (`book/src/design/l4_calculus.md:150-184`). The per-step opaque leaf does NOT participate in the reduction chain — it is the obstruction boundary.
...
None. This theme lowers an already-firm L4 combinator ([`fold_solve`](../L4/fold_solve.md), firm cycle-058 D1) assembled from the already-firm [`iterate-while`](../L4/iterate-while.md) §3.7 family. The per-step `time_step_op` and the `TimeState` carry are speculative rough-in sub-operators already named in the firm cap's §Dependencies (not introduced here); the per-step `time_step_op` is recorded as the opaque-library obstruction sub-leaf at L0 (`timeoperator.cpp:410`). No new speculative operator is introduced.

## Verified-against

L4 source (the LHS of this rewrite):

- `book/src/L4/fold_solve.md` (cycle-058 D1 LEAD, firm; **same-cycle sibling** — authored by D1, lands at integration before the single finalize build; the live link resolves once D1's create is applied) — the L4 state-threaded fold combinator: §Signature (the `fold_solve` / `foldl (\s t -> time_step_op op s t) s0 schedule` shape + the §3.7 carry-form rendering), §Semantics (the direct-fold form + the §3.7 non-degenerate-member rendering), §"Algebraic laws" (Law 1 fold-threading associativity / schedule-split, Law 2 operator-capture-once / construction-hoist, Law 3 seed left-identity, and the load-bearing NON-law: no commutativity / distribution — the map/fold distinction — the transported properties), §"Lowers to" (the in-line rotation-direction record this theme realizes), §Specializations (the two state-threaded sweeps — transient fixed-schedule + SweepAdaptive state-generated), §"The opaque per-step body" (the `obstruction (opaque-library-ownership)` deferral this theme realizes), §Status (the `firm` fold-spine basis).
- `book/src/L4/iterate-while.md` (firm cycle-007) — the §3.7 family whose non-degenerate carry-threading member the combinator IS (the alternate LHS rendering).
- `book/src/L4/solve_family.md` (firm cycle-055 D1) — the **map sibling** whose dissolution this theme is structurally parallel to (operator-capture hoist shared; the carry/no-carry axis the load-bearing difference).
...
- `book/src/L4-L3/solve-family-map-dissolution.md` (firm cycle-055 D2) — the **map-sibling dissolution** this theme is structurally parallel to; the load-bearing contrast is the RHS obstruction (the map's family loop carries NONE; this fold's loop carries a `sequential-obstruction` + an opaque per-step leaf).
- `book/src/L4-L3/iterate-while-dissolution.md` (firm cycle-047) — the inner-fold combinator dissolution the §3.7 family shares (referenced for the family-parent rendering).

L0 evidence (the fixed-schedule fold-sweep witness; self-verified exact against on-disk source this dispatch via `tools/citecheck/citecheck.py --anchor` — the codemap `read_range` was -2 drift on `timeoperator.cpp:410`, caught and corrected):
...
`firm` — on the **structural rotation**. The fold-shell dissolution (the once-captured `readonly` operator stratum → the `TimeOperator` + integrator construction hoisted outside the `for`; the immutable functional carry → the persistent `sol` field-state threaded IN PLACE; the abstract schedule + opaque per-step operator → the concrete `delta_t`/`n_step` march + the opaque `ode->Step` library call) is read **directly off positive Palace source** — every piece of the outer-sweep rotation shape is witnessed exactly by the transient sweep (`transientsolver.cpp:33-99` + `timeoperator.cpp:312,410`), with the driven-PROM SweepAdaptive sweep (`drivensolver.cpp:231-389`) the structurally-parallel second fold-spine witness. The operator-hoist placement (construction outside the loop), the in-place `sol` threading (`ode->Step(sol, ...)` mutating the persistent vector), and the opaque per-step call are all positive source facts, not reconstructions. The three coordinated rewrites are exhaustively cited against the firm cap's §Signature / §"Algebraic laws" / §Specializations and the L0 fold-sweep witness. Justification is `structural` + secondary `reduction-chain`. No speculative operator introduced. This theme is the **fold-sibling** of the firm [`solve-family-map-dissolution`](./solve-family-map-dissolution.md) (cycle-055 D2) — same operator-capture hoist, opposite carry axis (the map's RHS carries NO obstruction; this fold's RHS carries a `sequential-obstruction` + an opaque per-step leaf).

**On the per-step opaque-library sub-leaf (load-bearing scoping).** The per-step body `ode->Step(sol, t, dt)` (`timeoperator.cpp:410`) is an `obstruction (opaque-library-ownership)` sub-leaf — the MFEM `ODESolver` interior is library-owned, never exposed by Palace as a standalone callable, with NO conventional promotion route. This does **NOT** demote the whole theme to obstruction: the theme is `firm` on the *outer-sweep structural rotation* (the in-place-threading loop, the operator hoist, the concrete march — all read off positive source); only the *per-step body* is the opaque leaf, and the theme **records** that boundary (the negative anchor is Palace's CALL `:410`, NOT MFEM internals) rather than lowering it. This is the same scoping the cap's §"The opaque per-step body" establishes (the entry quantifies over `time_step_op`; the opacity is a lowering-layer concern recorded here). The distinction from the map sibling: the map's per-member `ksp.Mult` *does* lower (delegated to the firm [`ksp-solve-driver-dissolution`](./ksp-solve-driver-dissolution.md)); the fold's per-step `ode->Step` does NOT lower (opaque library leaf). The whole-theme `firm` is on the rotation shape, with the per-step opaque leaf explicitly recorded, not claimed-lowered.

**Scope (load-bearing)**: this theme covers the **fixed-schedule** fold dissolution, witnessed by the **transient** pipeline (the fixed `delta_t`/`n_step` march). The **driven-PROM SweepAdaptive** state-generated greedy march (`drivensolver.cpp:231-389`) shares the fold spine but generates its schedule from the carry (the next sample `omega_star = FindMaxError(state)` `:389`, the state-derived bound `:384`) — its dedicated dissolution (if warranted) is gated on the cap's OQ `fold-solve-greedy-schedule-source-generalization` (batch-18), NOT covered here. The other three pipelines: electrostatic + magnetostatic are the independent-**map** [`solve_family`](../L4/solve_family.md) (no carry — the map sibling, NOT folds); eigenmode's eigen-iteration is opaque-library-owned ([`eigsolve`](../L4/eigsolve.md), not probed as a fold). Do NOT claim cross-pipeline generality beyond the fixed-schedule transient fold + the SweepAdaptive fold-spine parallel.

This dispatch (cycle-058 D2) is the **fold-shell dissolution** for the `fold_solve` propagation, the L4>L3 half (the firm cap D1 authored this same cycle is the LHS). It realizes the in-line rotation direction the firm cap's §"Lowers to" records and is the authoritative home for the fold-shell stratum, structurally parallel to the per-family [`solve-family-map-dissolution`](./solve-family-map-dissolution.md) map sibling.
...
[append a row to the Theme list table, after the solve-family-map-dissolution row (line 21):]
...
```edit:book/src/L4-L3/index.md
[append a Vocabulary-cohort bullet under the "Substantive themes (firm):" sub-grouping, after the solve-family-map-dissolution bullet (line 42), and bump the consolidated tally — I am the sole index-touching dispatch into L4-L3/index.md this cycle, so I own the tally bump:]
...
[and replace the consolidated tally line (line 44) "(firm L4>L3 themes: 6 → 7 this cycle)" / "7 firm" to reflect the c058 D2 landing:]
...
[under the L4 > L3 Part, add a chapter entry after the solve-family-map-dissolution line:]
- [fold-solve-time-step-dissolution](./L4-L3/fold-solve-time-step-dissolution.md)
```

## Speculative operators proposed

None. This theme lowers the already-firm L4 [`fold_solve`](../L4/fold_solve.md) combinator (cycle-058 D1) assembled from the already-firm [`iterate-while`](../L4/iterate-while.md) §3.7 family. The per-step `time_step_op` and the `TimeState` carry are speculative rough-in sub-operators already named in the firm cap's §Dependencies (not introduced here); `time_step_op` bottoms out in the opaque-library obstruction sub-leaf at L0 (`timeoperator.cpp:410`). No new speculative operator is introduced.

## Supporting evidence

All L0 anchors codemap-localized and `tools/citecheck/citecheck.py --anchor`-verified against on-disk source this dispatch. The codemap `read_range` showed `ode->Step(sol, t, dt)` at line 408 but citecheck/on-disk confirms it is at **`timeoperator.cpp:410`** (codemap was -2 drift on the `TimeOperator::Step` body — the codemap-localization-vs-on-disk-source-of-truth discipline; citecheck/on-disk wins per the role-spec).

- Transient fixed-schedule fold witness (positive, the RHS default surface): `transientsolver.cpp:33` (operator built once), `:35` (uniform `delta_t`), `:36` (`n_step = GetNumSteps`), `:77` (`for (int step ...)` loop), `:89` (`time_op.Init()` seed), `:93` (`time_op.Step` per-step body), `:98`/`:99` (`GetE`/`GetB` readout) + `timeoperator.cpp:312` (integrator constructed once), `:410` (`ode->Step(sol, t, dt)` — the opaque MFEM step, the obstruction sub-leaf negative anchor).
- SweepAdaptive state-generated fold-spine witness (positive, the structural-parallel second witness, NOT covered by this theme's RHS): `drivensolver.cpp:73` (adaptive dispatch), `:231` (`SweepAdaptive` entry), `:384` (state-derived loop bound), `:389` (`FindMaxError` state-generated per-step input).

Motivating reports: D1 `reports/2026-06-02T050136Z-harvester-fold-solve/CYCLE.md` (the firm `fold_solve` entry this theme lowers); the cycle-055 D2 `solve-family-map-dissolution` (the structural-parallel map sibling); the L4 strawman `book/src/design/l4_calculus.md:150-184` (§3.7 the shared parent).

## Open questions / caveats

- **OQ `fold-solve-l3-entry-vs-dissolution-home`** (batch-18, inherited from the cap D1) — whether a standalone `L3/fold_solve` entry is warranted, or this dissolution theme is the authoritative L3-form home. Unlike the map sibling (whose family loop lifts cleanly → NO-ENTRY warrant, the dissolution-theme-is-home outcome), the fold's loop carries BOTH a carry-threading `sequential-obstruction` AND an opaque-library per-step body, so an L3 `partial-obstruction` entry (the [`chebyshev`](../L3/chebyshev.md) / `eigsolve` shape) is the likelier outcome. This theme's §"L3 form (RHS)" is the authoritative home until that future L3 dispatch decides; the theme records the rotation direction, not the L3-entry verdict.
- **OQ `fold-solve-greedy-schedule-source-generalization`** (batch-18, inherited from the cap D1) — this theme's RHS is the **fixed-schedule** form (transient). The SweepAdaptive state-generated greedy march (`drivensolver.cpp:231-389`) shares the fold spine but generates its schedule from the carry; whether its dissolution warrants a dedicated theme once a downstream PROM consumer pulls is a batch-18 size-judgment. The risk if mis-decided: forcing the greedy `FindMaxError`-generated schedule into the fixed-`n_step` RHS would assert a shape SweepAdaptive does not exhibit — avoided here by scoping the RHS to the fixed-schedule transient form and citing SweepAdaptive only as the parallel fold-spine witness.
- **Opaque per-step sub-leaf framing** (`time-step-op-opaque-mfem-integrator-boundary`): per scope, the per-step `obstruction (opaque-library-ownership)` is recorded at Palace's CALL `ode->Step(sol, t, dt)` (`timeoperator.cpp:410`), NOT MFEM internals — the theme documents the Palace/MFEM ownership boundary and does not (cannot) lower the MFEM `ODESolver` interior. This is the distinction from the map sibling's per-member solve (which *does* lower via `ksp-solve-driver-dissolution`): the fold's per-step body is library-owned, recorded not delegated.
- **Index registration partition**: per role-spec I authored (1) my own theme-list table row, (2) my own §Vocabulary-cohort bullet, AND (3) the consolidated tally bump (7 → 8) — I am the **sole index-touching dispatch into `L4-L3/index.md` this cycle** (no co-dispatched count-owner named in my scope), so per the index-dual-registration convention I own all three. SUMMARY.md registration of the new theme is likewise mine.
- **Forward-reference to the same-cycle D1 cap**: `book/src/L4/fold_solve.md` is authored by cycle-058 D1 (LEAD) THIS cycle; I forward-reference it as a live-link (`../L4/fold_solve.md`) per the planner-stated canonical slug — it lands at integration before the single finalize build, so the link resolves. The canonical theme slug `fold-solve-time-step-dissolution` matches the cap's §"Lowers to" forward-reference exactly (both planner-stated), so there is no cross-report slug divergence to reconcile.
