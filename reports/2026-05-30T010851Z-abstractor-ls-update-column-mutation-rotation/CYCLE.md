...
inputs:
  ...
  - book/src/L1-L0/back-solve-mutation-rotation.md (firm sibling theme, c029)
  - palace/linalg/iterative.cpp:634-640 (GMRES per-column running-QR update)
  - palace/linalg/iterative.cpp:813-819 (FGMRES per-column running-QR update; byte-identical to GMRES body)
  - palace/linalg/iterative.cpp:73-118 (GeneratePlaneRotation real + complex; unitarity contract :118)
  - palace/linalg/iterative.cpp:227-241 (ApplyPlaneRotation real + complex)
  - palace/linalg/iterative.cpp:629-632 (upstream orthogonalize + nrm2 producing h_new; NOT part of leaf)
  - palace/linalg/iterative.cpp:642 (residual exposure beta = std::abs(s[j+1]))
  - palace/linalg/iterative.cpp:612 (RHS init s[0]=beta at restart-cycle seed)
  - palace/linalg/iterative.hpp:193-194 (register split: ScalarType s,sn / RealType cs)
  - OQ ls-update-column-mutation-rotation-l1l0-theme-forthcoming-c029
---

# CYCLE: L1>L0 theme sketch — ls-update-column-mutation-rotation

## Summary

Authors the firm L1>L0 theme `ls-update-column-mutation-rotation` — the
lowering of the firm L1 leaf [`ls_update_column`](../L1/ls-update-column.md)
into the L0 per-column running-QR update loop at
`palace/linalg/iterative.cpp:634-640` (GMRES) and its byte-identical FGMRES
twin at `:813-819`. The rewrite is a structural four-element expansion of the
pure-functional L1 value bundle `{h_out, cs_j, sn_j, s_j, s_jp1, beta}` into
**four reference-update `*PlaneRotation` calls** that overwrite the caller's
flat Hessenberg column `Hj`, append into the rotation registers `cs[j] / sn[j]`,
and rotate the RHS register `s[j], s[j+1]`. The mutation rotation collapses
the L1 fresh value bundle into four in-place writes to four pre-allocated
registers; the destination *is* part of the input. The GMRES and FGMRES bodies
are **byte-identical** (the +5 line offset between `:634-640` and `:813-819` is
from PRECEDING differing FGMRES code, NOT from brace placement — explicitly
corrected vs the c030-audited error in the sibling `back-solve` theme).
Status `firm` (firm-on-positive-structure, exactly matching the L1 leaf and
the sibling `back-solve-mutation-rotation` theme). Completes the GMRES-restart
L1>L0 cohort: per-column producer (this theme) + restart-cycle terminal
consumer (`back-solve-mutation-rotation`, firm c029).

## Proposed changes

```new:book/src/L1-L0/ls-update-column-mutation-rotation.md
...
[append a new dep-map row IMMEDIATELY AFTER the `back-solve-mutation-rotation` row (line 33), preserving the existing pipe-table formatting; new row:]
...
[insert IMMEDIATELY AFTER the `back-solve-mutation-rotation` line (line 111), as a new SUMMARY entry under the L1>L0 Part:]
...
**None.** This theme proposes no new L1 operators. The L1 leaf
[`ls_update_column`](../L1/ls-update-column.md) is firm (cycle-029, audited);
the four L0 scalar Givens kernels have concept pages
([`givens_generate`](../concepts/givens_generate.md),
[`givens_apply`](../concepts/givens_apply.md)) and are NOT promoted to L1
operators per the L1 leaf's §Dependencies decision (scalar element-local
kernels, not L1 vocabulary). The upstream `orthogonalize` / `nrm2` and the
downstream `back-solve` are already firm L1 operators handled by their own
L1>L0 themes (boundary markers in this theme).

## Supporting evidence

L0 ranges (all self-verified via `tools/citecheck/citecheck.py --anchor` against
on-disk `reference/palace/` this invocation; full report in §Verified-against
above):

- `palace/linalg/iterative.cpp:634-640` — GMRES per-column running-QR update;
  the canonical Sub-pattern A site.
- `palace/linalg/iterative.cpp:813-819` — FGMRES twin; byte-identical body
  per direct line-by-line equivalence at `:634≡:813, :636≡:815, :638≡:817,
  :639≡:818, :640≡:819`.
- `palace/linalg/iterative.cpp:73-118` — GeneratePlaneRotation real
  (`:73-108` body with LAPACK scaling `:101-108`) + complex (`:112-118` body
  with `cs²+|sn|²=1` unitarity contract `:118`).
- `palace/linalg/iterative.cpp:227-235` — ApplyPlaneRotation real (`:227`)
  + complex (`:235`); the in-place 2-vector update kernel called four times
  per leaf invocation.
- `palace/linalg/iterative.cpp:629-632` — upstream `orthogonalize ▷ nrm2`
  producing `h_new` + `Hj` column-handle setup; boundary marker.
- `palace/linalg/iterative.cpp:642` — downstream convergence-test residual
  read `beta = std::abs(s[j+1])`; boundary marker (L1 leaf law-3
  byproduct).
- `palace/linalg/iterative.cpp:612` — RHS register seed `s[0] = beta` at
  restart-cycle start; boundary marker.
- `palace/linalg/iterative.hpp:192-194` — register declarations: flat `H`
  (column-major Hessenberg slab), `s, sn` (ScalarType), `cs` (RealType).
- `palace/linalg/iterative.hpp:222, 250, 256` — FGMRES inheritance from
  GMRES (same `H`, same `s, sn, cs`, plus FGMRES-specific `Z` register —
  NOT read by this leaf).

L1 / cross-theme anchors:

- `book/src/L1/ls-update-column.md` — the firm L1 leaf (c029).
- `book/src/L1-L0/back-solve-mutation-rotation.md` — firm sibling theme
  (c029); the structural template + the c030-audited brace-placement error
  to avoid.
- `book/src/L1-L0/orthogonalize-mutation-rotation.md`,
  `book/src/L1-L0/nrm2-mutation-rotation.md` — upstream sibling boundary
  themes.
- `book/src/L2/incremental-least-squares.md`,
  `book/src/L2-L1/incremental-least-squares-composition-lowering.md` —
  parent L2 named composition + L2>L1 theme; this leaf is the Face-1
  opaque-leaf the L2>L1 theme forward-references.
- `book/src/concepts/givens_generate.md`,
  `book/src/concepts/givens_apply.md`,
  `book/src/concepts/plane-rotation-stream.md` — the cross-cutting concept
  pages.

## Open questions / caveats

1. **Sibling `back-solve-mutation-rotation` Sub-pattern B narrative
   correction.** The sibling theme (firm c029) Sub-pattern B claims the
   GMRES/FGMRES line offset comes from "brace placement / line shift" — a
   c030-audited error per the dispatch directive. This theme explicitly
   avoids the error (the per-column-update body offset is from preceding
   differing FGMRES code, NOT brace placement; the bodies are byte-identical).
   A follow-up should correct the sibling's Sub-pattern B narrative to
   match (the back-solve bodies at `:652-660` ≡ `:831-840` ARE +1 line-
   shifted, but it MAY also be from preceding-code variation rather than
   brace placement — a check on the back-solve sibling specifically is
   warranted). Not in scope here; flagged for cycle-030+ lowering-verifier
   on `back-solve-mutation-rotation`.
2. **L3 sequential-obstruction follow-through.** The
   [`concepts/plane-rotation-stream`](../concepts/plane-rotation-stream.md)
   §"Sequential character" `:21-23` flags the replay chain as a
   `sequential-obstruction` candidate at L3. This L1>L0 theme does not
   address the L3 iteration rotation (out of scope); a future L3 entry on
   the running-QR stream / GMRES inner loop would inherit the structural
   sequential obstruction from the replay chain. Forward note only.
3. **Promotion follow-up at L2>L1.** The firm L2>L1 theme
   [`incremental-least-squares-composition-lowering`](../L2-L1/incremental-least-squares-composition-lowering.md)
   §Face-1 plain-text forward-references `ls_update_column` as
   speculative-pending-leaf (per the c028 plain-text-defer convention).
   With the L1 leaf now firm (c029) AND this L1>L0 theme now firm (this
   cycle), the L2>L1 theme's Face-1 plain-text reference can be upgraded
   to a live link in a subsequent dispatch via skill
   `upgrade-plain-text-ref-to-live-link-when-target-on-disk`. Not in scope
   here; flagged for cycle-030+ same-layer-cross-cutter on the L2>L1 theme.
4. **OQ closure.** This theme resolves OQ
   `ls-update-column-mutation-rotation-l1l0-theme-forthcoming-c029`
   (per dispatch directive). The integrator should mark the OQ entry
   closed when applying this report.
