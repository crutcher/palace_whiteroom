...
inputs:
  ...
  - book/src/spec/slices/arnoldi_step.md (references orthogonalize; reduction-blocked on this promotion)
  - book/src/concepts/orthogonalization.md (shared concept page; L1 contract)
  - book/src/concepts/sequential-obstruction.md (MGS-as-sequential-obstruction, §37-48)
  - book/src/L1/{dot,axpy,nrm2,scal}.md (firm L1 dependencies)
  - OQ l1-orthogonalize-promotion-from-arnoldi-step-and-orthog (cycle-010, HIGH)
  - L0 (verified via MCP codemap): palace/linalg/orthog.hpp:18-90, palace/linalg/iterative.cpp:308-325, test/unit/test-orthog.cpp:71-160
---

# CYCLE: Formalize orthogonalize at L1

## Summary

`orthogonalize` is the Gram-Schmidt orthogonalization primitive: given a stored basis
`V[0..m-1]` and a candidate vector `w`, it returns the component of `w` orthogonal to
`span(V)` together with the projection coefficients `H[0..m-1] = ⟨w, V[j]⟩`. It is the
field-side core of the GMRES/FGMRES Arnoldi inner loop (`iterative.cpp:630, 809`) and of
the ROM basis-extension path (`romoperator.cpp:224, 633`). It carries one runtime variant
axis — `gs_orthog ∈ {MGS, CGS, CGS2}` — that does not change the L1 contract but does
change the collective shape and finite-precision stability. The operator was previously
only present as the `concepts/orthogonalization.md` prose page and as retained L2/L3/L4
sections of the `orthog` slice; it had no firm L1 chapter. This dispatch promotes it to a
**firm** L1 entry (`book/src/L1/orthogonalize.md`): the L0 implementation is read in full
(header-only inline at `orthog.hpp:18-90`), all three variants carry dedicated parametric
test coverage (real / complex / B-weighted, plus the empty-basis edge case and a direct
substitutability assertion `⟨w', V[i]⟩ ≈ 0`), and the algebraic laws are standard
Gram-Schmidt facts modulo explicitly-recorded floating-point caveats. Firming this entry
unblocks further reduction of both `arnoldi_step.md` and `orthog.md` (the latter's L1 row
becomes a pointer to this entry; its plane-rotation-stream sub-slice is unaffected and stays
deferred to batch-3).

## Proposed changes

```edit:book/src/L1/orthogonalize.md
[create — full content in "Operator content" below]
```

```edit:book/src/L1/index.md
[append orthogonalize to "Firm" cohort list, append dep-map row, bump count 8→9; full diffs below]
```

```edit:book/src/SUMMARY.md
[add chapter entry under L1 Part: `- [orthogonalize](./L1/orthogonalize.md)` after the bilinear-form line]
```

### index.md edit 1 — Firm cohort header (count + entry)

Change `**Firm (8)**` to `**Firm (9)**` and append after the `ksp_solve` bullet:
...
### index.md edit 2 — dep-map row

Append after the `bilinear-form` row of the operator dep-map table:
...
The following is the full text of `book/src/L1/orthogonalize.md`.
