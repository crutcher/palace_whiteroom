...
inputs:
  ...
  - palace/linalg/nleps.cpp:505-537 (the positive deflated_solve block; read_range-verified this dispatch)
  - palace/linalg/nleps.cpp:329-347 (MatVecMult, the back-projection primitive; verified)
  - palace/linalg/nleps.cpp:354-362 (Jarlebring–Koskela–Mele 2018 / Effenberger 2013 literature anchors; verified)
  - palace/linalg/nleps.cpp:562-563, 664-667 (residual + Jacobian back-projection reuse sites; verified)
  - palace/linalg/nleps.cpp:606-619 (deflation-basis growth; non-orthonormal X; verified)
  - book/src/L1/lu_solve.md (firm cycle-022 L1 leaf)
  - book/src/L1/dot.md:43 (arg-1-conjugated convention)
  - book/src/L2/linear_combination.md (firm cycle-018; X·coords back-projection)
  - book/src/L2/gram.md (firm cycle-022; XᴴX builder)
  - book/src/L2-L1/index.md, book/src/SUMMARY.md (dep-map + chapter wiring; shared with parallel gram-fold-specialization abstractor)
---

# CYCLE: L2>L1 theme sketch — deflate-composition-lowering

## Summary

The L2 `deflate` named composition (`book/src/L2/deflate.md`, status `partly-constructive`) is
the oblique / Galerkin complementary projector `deflate(op, X, v) = v − X·(coords-solve)`. This
theme narrates **how that one L2 composition lowers into its L1 leaves** — forward, high→low:
the `coords ▷ schur-solve ▷ back-project` pipeline fans down into the firm L1
[`dot`](../L1/dot.md) coordinate-extraction fold, the firm L2 [`gram`](../L2/gram.md) Gram-matrix
build (itself an `inner_product`/`dot` fold), the firm L1 [`lu_solve`](../L1/lu_solve.md) small-dense
solve(s), and the firm L2 [`linear_combination`](../L2/linear_combination.md) back-projection,
landing on Palace's fused `deflated_solve` block (`palace/linalg/nleps.cpp:505-537`). The theme
**inherits `deflate`'s partly-constructive caveat verbatim**: the lowering of the **Galerkin core**
sub-part (`op.block = Galerkin`, `S = I`: a single `lu_solve(XᴴX, c)`) is **constructive** —
Palace never positively exhibits a bare-Gram `(XᴴX)⁻¹` deflation solve; it only ever exhibits the
**Schur-wrapped** triple-solve `SS = −S⁻¹(XᴴX)` ▷ `SS⁻¹·c` ▷ `S⁻¹·(·)` at
`palace/linalg/nleps.cpp:533-535`. This theme documents the lowering **as it is** (firm Schur-form
fan-down + constructive Galerkin-core fan-down) and **does NOT close** the promotion gate — that
gate (a positive bare-Gram-solve site outside `nleps.cpp`) stays open, identical to the L2 entry's.
Justification kind: **reduction-chain** (the L2 composition stages map onto a fixed small-step
sequence of L1 leaf calls in the source block) with the Schur sub-part **algebraic/structural** on
positive source and the Galerkin sub-part **constructive** on negative anchors + literature.

## Proposed changes

```new:book/src/L2-L1/deflate-composition-lowering.md
...
`gram-fold-specialization` *(forward-reference, plain-text; the Gram-build fold this theme consumes
whole — the chapter does not yet exist on disk, so this reference stays plain-text per the
`rough-in-rows-must-be-plain-text-when-anchor-missing` convention)*, and to
...
`gram-fold-specialization` theme's content *(forward-reference, plain-text — chapter not yet on
disk)*; this theme cites it and does not duplicate it.
...
None. This theme lowers `deflate` into **existing firm L1/L2 leaves only** — `dot` (L1, firm),
`gram` (L2, firm cycle-022), `lu_solve` (L1, firm cycle-022), `linear_combination` (L2, firm
cycle-018), `axpy` (L1, firm). No new rough-in operators are needed; the fan-down is entirely onto
vocabulary that already exists. (The Gram-build sub-fold's own L2>L1 lowering is the parallel-cycle
`gram-fold-specialization` theme — that is a sibling theme, not an operator this dispatch proposes.)

## Supporting evidence

The positive `deflated_solve` block and all reuse / literature / basis-growth anchors are
`read_range`-verified above (§ Verified-against). The decisive citations for the
partly-constructive split:

- **Firm Schur fan-down** — positive source `palace/linalg/nleps.cpp:532-536` (the `S` build, the
  three `fullPivLu().solve` calls, the `MatVecMult`, the `AXPY`), plus the source's own
  block-elimination comment naming the Schur complement (`:508-513`).
- **Constructive Galerkin-core fan-down** — NOT positively sourced; the negative anchor (no bare-Gram
  deflation solve anywhere in `palace/linalg/*.cpp`, recorded `book/src/L2/deflate.md:343-348`) +
  the deflation-scheme literature (`palace/linalg/nleps.cpp:354-362`). Evidence FOR a faithful
  `S = I` reduction; does NOT license a positive claim.

## Open questions / caveats

1. **(promote)** *Galerkin-core promotion gate — shared with the L2 `deflate` entry, NOT closed by
   this theme.* The bare-Gram single-`lu_solve` fan-down (Stage 3, `op.block = Galerkin`) is
   constructive: Palace exhibits only the Schur-wrapped triple-solve. Promotion to `firm` requires a
   **positive Palace source site** exhibiting `lu_solve(XᴴX, c)` directly (a future linear-EVP
   deflation, preconditioner deflation, or ROM-Galerkin projection without the `S = λI − H` Schur
   wrapping). When that site lands, BOTH the L2 entry and this theme promote together. → OQ ledger.

2. **(promote)** *`gram-fold-specialization` cross-reference is a forward-reference this cycle.* Stage
   2 AND the intro "Sibling to …" sentence cite the parallel-cycle `gram-fold-specialization` theme
   (the `gram` L2>L1 fan-down) by slug; that chapter is authored in the same cycle by a sibling
   abstractor I cannot see. **Repair-phase resolution (2026-05-29):** BOTH occurrences — the intro
   sentence (was at body :60–62) and the Stage-2 sentence (was at body :192) — have been **defanged to
   plain-text** (backtick slug, no `](./…)` link) per the
   `rough-in-rows-must-be-plain-text-when-anchor-missing` convention; the committed state is
   build-safe regardless of sibling ordering. The integrator must still confirm the slug matches the
   sibling's actual filename and that the dep-map / SUMMARY rows do not collide (this theme's rows are
   distinct: `deflate-composition-lowering`). **Integrator note:** if the `gram-fold-specialization`
   sibling theme lands *earlier* in this same integration pass (the file exists on disk at integration
   time), the integrator MAY re-link both plain-text references to a live
   `[`gram-fold-specialization`](./gram-fold-specialization.md)` reference — but the safe committed
   state is plain-text. → OQ ledger.

3. **(caveat)** *Source `SS` buffer name aliasing.* Palace reuses the `SS` buffer for both the Gram
   `XᴴX` (Stage 2, `:529`) and the Schur-modified Gram `−S⁻¹(XᴴX)` (Stage 3, `:533`). The fan-down
   correctly treats these as two distinct L2 values; the in-place overwrite is a transparent-perf
   detail. Noted so a future lowering-verifier does not read the `:533` self-assignment as a
   contradiction. Not gating.

4. **(caveat)** *No dedicated NLEPS unit test.* Inherited from the L2 entry
   (`book/src/L2/deflate.md:411-415`): NLEPS has zero deflation-projection unit-test coverage
   (`search_text` for `QuasiNewton|nleps|fullPivLu|deflat` over `test/unit/**` returns nothing). Per
   CLAUDE.md status guidance the absent test does not gate the syntactic-identity reduction-chain
   (the Schur fan-down is operator-algebra on positive source); the gate is the missing positive
   Galerkin site, not the missing test. Not separately gating.
