...
inputs:
  ...
  - scaffolding/open-questions.md `matrix-weighted-norm-and-bilinear-form-l1-rough-ins` (cycle-008 OQ)
  - book/src/L0/linalg-operator-file.md (L0 anchor for `Norml2(comm, x, B, Bx)`)
  - book/src/L1/dot.md (firm L1 dep)
  - book/src/L1/apply_linop.md (firm L1 dep)
  - book/src/L1/nrm2.md (structural template / boundary statement)
  - book/src/L1/index.md (dep-map; existing "queued" line for `nrm2_B`)
---

# CYCLE: Rough-in `matrix-weighted-norm` at L1

## Summary

Author the **rough-in** L1 entry for `matrix-weighted-norm`: the operator-weighted Euclidean norm `‖x‖_B = √(xᴴ B x)` for SPD `B`. This is the long-flagged boundary in the firm `nrm2` entry ("B-weighted overload ... is a separate L1 operator candidate (forthcoming) that depends on both `dot` and the operator-application primitive `apply_linop`") and the layer-intro-author's cycle-008 OQ `matrix-weighted-norm-and-bilinear-form-l1-rough-ins`. The operator is also referenced from `book/src/L1/index.md` "Queued" as `nrm2_B :: (x, B) → √(xᴴ B x)`.

L0 anchor: `palace::linalg::Norml2(MPI_Comm comm, const VecType &x, const Operator &B, VecType &Bx)` declared at `palace/linalg/operator.hpp:372-374`, implemented at `palace/linalg/operator.cpp:599-619` (two template specializations, real and complex). Call-site evidence is dense and consistent: all three eigensolver backends (ARPACK, SLEPc, NLEPS) use the operator-weighted norm as their `GetEigenvectorNorm` primitive when an opaque mass-matrix-like operator `*opB` is configured — i.e. this is the M-orthonormalisation norm for generalised eigenvalue problems.

Status: **rough-in**, not firm. Rationale (planner directive): test-coverage analysis has not been done; the only Palace test for the unweighted `Norml2` is `test/unit/test-vector.cpp:209-211` and there is no dedicated test exercising the SPD-weighted overload. Promotion to firm is gated on either (a) appearance of dedicated test coverage or (b) algebraic-law completeness verification on the SPD precondition (currently asserted by L0 source but not test-witnessed at this exact entry point).

Naming note (resolved during this dispatch): the L0 chapter `linalg-operator-file.md` (cycle-008) used the candidate slug `nrm2_weighted`; the cycle-008 OQ used `matrix-weighted-norm`; the L1 index's "Queued" line used `nrm2_B`. The planner directs **`matrix-weighted-norm`** as the canonical slug. The OQ resolution updates the index line to point at the new entry; the L0 chapter's prose reference will be reconciled by the integrator if needed (the L0 chapter is informational, not authoritative on L1 naming).
...
[1] Update "Queued (open questions)" section: remove the `nrm2_B` line (now harvested as `matrix-weighted-norm`).
[2] Add row to "Rough-in (test-coverage-bounded)" cohort list (after `eigsolve`).
[3] Add row to dep-map table after `eigsolve` row.
```

```edit:book/src/SUMMARY.md
[Add chapter entry under L1 Part, after `- [eigsolve](./L1/eigsolve.md)`:
   - [matrix-weighted-norm](./L1/matrix-weighted-norm.md)]
```

## Operator content

Content written into `book/src/L1/matrix-weighted-norm.md`:
...
8. **Self-bilinear identity**: `matrix_weighted_norm(x, B)² = xᴴ B x`. The defining identity, restated. Used directly by the L0 source (the implementation factors as `B.Mult(x, Bx); dot = Dot(comm, Bx, x); return std::sqrt(dot)` — `palace/linalg/operator.cpp:601-606`).
...
- L0 source pin: `palace/linalg/operator.hpp:372-374` (decl); `palace/linalg/operator.cpp:599-619` (impl, two specializations).
- L0 chapter anchor: `book/src/L0/linalg-operator-file.md` "linalg:: free functions" (lines 30-33), already explicitly identifies `Norml2(comm, x, B, Bx)` as the L0 anchor for a future L1 `nrm2_weighted` / `matrix-weighted-norm` operator.
- Sibling L1 entries: `book/src/L1/nrm2.md` (lines 13, 99-100) — explicit boundary statement; `book/src/L1/dot.md`, `book/src/L1/apply_linop.md` — firm dependencies.
- Callsite evidence: `palace/linalg/arpack.cpp:438, 470`; `palace/linalg/slepc.cpp:475, 505`; `palace/linalg/nleps.cpp:114, 146` — three-backend consistency, all in eigensolver `GetEigenvectorNorm` methods for M-orthonormalisation.
- OQ motivation: `scaffolding/open-questions.md` `matrix-weighted-norm-and-bilinear-form-l1-rough-ins` (cycle-008 by layer-intro-author).
- Priority motivation: `scaffolding/priorities.md` #17 (cycle-009 meta-phase, lower-layer-shared-vocabulary directive).

## Index update detail (full diff text)

For `book/src/L1/index.md`:

**Change 1** — "Queued (open questions)" section (around line 51): remove the `nrm2_B` line, replace with a forward-pointer that the rough-in has landed:
...
+ **Queued (open questions)** — small primitives that bottom-out remaining L0 patterns referenced by the firm cohort:
+
+ - `bilinear-form :: (x, A, y) → yᴴ A x` — operator-induced bilinear inner product; depends on `dot` and `apply_linop`. Sibling of `matrix-weighted-norm` (the energy norm is the diagonal `y = x` case plus outer `sqrt` plus SPD condition). Slug: `bilinear-form-l1-harvest`. The matrix-weighted-norm half of the cycle-008 OQ `matrix-weighted-norm-and-bilinear-form-l1-rough-ins` was landed cycle-010 as [`matrix-weighted-norm`](./matrix-weighted-norm.md) (rough-in).
```

**Change 2** — "Rough-in (test-coverage-bounded)" cohort list (after the `eigsolve` paragraph, around line 42): add a new bullet:
...
**Change 3** — dep-map table (after the `eigsolve` row, around line 65): add a new row:
...
**Change 4** (coordination note for the integrator) — sibling dispatches #4 (`bilinear-form`) and #5 (`nrm2_B-weighted` if dispatched under a distinct slug) will add adjacent rows. The integrator should serialise the three rows; if dispatch #5 is collapsed into this entry as a rename (i.e. the `nrm2_B-weighted` and `matrix-weighted-norm` slugs refer to the same operator), the dispatch #5 work product should be merged here. Naming-axis discussion is recorded in Open questions below.

## SUMMARY.md update detail

For `book/src/SUMMARY.md`, add a chapter entry after the `eigsolve` line (line 41) in the L1 Part:
...
If dispatch #4 (`bilinear-form`) is integrated in the same cycle, that entry would be added in the same block.

## Open questions / caveats

1. **Naming reconciliation across artifacts**. The L0 chapter `linalg-operator-file.md` (cycle-008) used candidate name `nrm2_weighted`; the cycle-008 OQ slug uses `matrix-weighted-norm`; the L1 `index.md` "Queued" line uses `nrm2_B`. The planner directs `matrix-weighted-norm` as canonical for this harvest. The L0 chapter prose is informational (not authoritative on L1 naming) and need not be updated immediately — but a future light editorial sweep could swap the `nrm2_weighted` candidate-name in `book/src/L0/linalg-operator-file.md:30-33, 73, 88` for `matrix-weighted-norm` to align. Recommend: leave for a future cross-layer cross-cutter sweep, not as integrator scope. **New OQ slug**: `matrix-weighted-norm-naming-sweep`.

2. **Complex-`x` with real-`B` element-type axis at L1**. The L0 complex specialization decomposes `B.Mult(x.Real(), Bx.Real()); B.Mult(x.Imag(), Bx.Imag())` because `B : Operator` (real). At L1 this is absorbed by `apply_linop`'s element-type variant axis — but the question of whether L1 admits a distinct "real-`B`-applied-to-complex-`x`" sub-variant remains. Cycle-008's `apply_linop` precedent says no (the L1 form collapses across operator element-type), but for `matrix-weighted-norm` specifically the SPD precondition interacts with `B`'s element-type. **New OQ slug**: `matrix-weighted-norm-mixed-element-type-variant`.

3. **L1>L0 lowering theme not yet authored**. The unfolded composition `√(dot(apply_linop(B, x), x))` belongs in a `matrix-weighted-norm-mutation-rotation` L1>L0 theme — not in this L1 entry. The theme would also formalise the `Bx`-as-caller-supplied-workspace pattern (a sub-case of `mutable-workspace-pattern` Category 2, but with caller-not-class ownership). Route to a future abstractor invocation. **New OQ slug**: `matrix-weighted-norm-mutation-rotation-l1-l0-theme`.

4. **Bilinear-form sibling harvest still pending**. The cycle-008 OQ pairs `matrix-weighted-norm` with `bilinear-form` (the non-diagonal, no-outer-sqrt, no-SPD variant). This dispatch (#3) covers only the first half; the second half is dispatch #4 in the planner's wave-1 setup. The OQ should be marked `partially-answered` by the integrator, with the bilinear-form half preserved as the residual.

5. **L0 chapter informational mention of `Normalize`**. The L0 source includes `linalg::Normalize(comm, x, B, Bx)` (`palace/linalg/operator.hpp:376-384`) which calls `Norml2(comm, x, B, Bx)` then scales `x *= 1.0 / norm`. This is a B-normalisation operator; at L1 it composes `matrix-weighted-norm` and `scal`, similar to how the plain `Normalize` composes `nrm2` and `scal`. Not in scope for this dispatch; recordable as a queued L1 operator candidate if not already. The unweighted `Normalize` does not yet have an L1 entry either — both belong to a future composite-utility harvest. **New OQ slug**: `normalize-and-normalize-b-weighted-l1-candidates` (pending check whether already in OQ ledger under a related slug).

6. **Test-coverage promotion gate alignment with `eigsolve` precedent**. The `eigsolve` rough-in (cycle-009) was status-promoted to firm only contingent on similar test / literature evidence. The same pattern applies here: the rough-in status is identical in spirit to `eigsolve`'s — well-anchored signature and algebraic laws, but no dedicated direct test. The integrator may want to consider whether a unified "test-coverage-bounded rough-in" status nomenclature should be canonicalised for cycle-010+ work. Recorded for meta-phase consideration in cycle-012.

7. **Single-rank scope and the `*opB` null-vs-non-null branch**. The Palace callsites all guard `if (opB) { ... linalg::Norml2(comm, x, *opB, Bx); } else { ... linalg::Norml2(comm, x); }` — i.e. when no `B` is configured the eigenproblem reduces to the standard form `A x = λ x` (no mass matrix) and the unweighted `nrm2` is used. At L1 these are two distinct operators (`matrix-weighted-norm` and `nrm2`); the dispatch is at the call site, not inside the operator. This is consistent with the algebraic law 9 (`matrix_weighted_norm(x, I) = nrm2(x)`) but the L0 dispatch is not by passing `I` — it's by selecting the unweighted overload. Recorded as a clarifying note in the L1 entry; no separate OQ needed.
