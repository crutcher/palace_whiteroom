...
inputs:
  ...
  - book/src/L4/ksp_solve.md (D1's cap — LHS; NOT yet on disk, forward-reference + integrator co-wiring note)
  - book/src/L3/ksp_solve.md (firm cycle-020 — RHS; the value-threaded outer-driver fold this cap lowers to)
  - book/src/L4-L3/krylov-step-typed-wrapper-dissolution.md §"What the L3 form for iterate_while looks like" (:158-200 — the kernel-half precedent)
  - book/src/L4-L3/iterate-while-dissolution.md (firm cycle-047 — the inner iterate_while fold dissolution this theme composes ABOVE)
  - book/src/concepts/solve-monad.md:1-68 (Solve = StateT SimState Identity; §Shape, §"Worked example — GMRES", §"Termination as a sum type")
  - book/src/design/l4_calculus.md §3.3-3.4 (:119-136 monad/state laws), §3.7 (:150-184 iterate_while), §3.8 (:186-228 demand-pruning)
  - L0 anchors (citecheck-verified this dispatch): iterative.cpp:417-418,427,484-485,563,703-704; iterative.hpp:52-55,98,101-108; ksp.cpp:296-310,301-307,308-309
  - cycle-046 L4-survey R2; OQ l4-ksp-solve-eigsolve-caps-gated-on-solve-monad-outer-driver-vocabulary
---

# CYCLE: L4>L3 theme sketch — ksp-solve-driver-dissolution

## Summary

D1 (wave 1) authored the firm **L4 `ksp_solve` cap** (`book/src/L4/ksp_solve.md`) — the `Solve`-monadic outer-driver `solve op inp = execState (solve_loop op inp) (initial_state inp)` that drives `krylov-step` to convergence and classifies termination once at the cycle boundary. This dispatch authors its **dedicated L4>L3 dissolution theme** `book/src/L4-L3/ksp-solve-driver-dissolution.md` (verified ABSENT — a genuine create), narrating forward (high→low) how that cap lowers into the firm L3 driver `book/src/L3/ksp_solve.md`.

The rotation is **substantive** (not identity-in-form): the `Solve = StateT SimState Identity` threading dissolves into explicit positional `(K, s)` value-threading + `iterate_while_L3` tail recursion; the `solve_loop` `do`/`unless` outer driver collapses into the L3 outer fold's predicate-read of `s.converged`; the `restart_cycle` per-cycle driver collapses into the L3 restart nesting + the once-per-cycle `fold_iterate` boundary write; and the `Outcome = Continue | Done Bool` sum collapses into the scattered L3 per-test termination branches + the soft-fail `Bool` `result.converged`. This is the **driver-half** L4>L3 theme — the analog the cycle-046 survey flagged warranted for the cap, parallel to how `krylov-step-typed-wrapper-dissolution` covers the kernel-half. It **composes strictly above** the inner `iterate-while-dissolution` (firm c047 — the inner `iterate_while` fold dissolution): this theme dissolves the *outer driver* and the *Outcome classification*; the inner-fold combinator dissolution is delegated to that firm theme.

## Registration (DUAL-REGISTRATION — stated explicitly)

The `book/src/L4-L3/index.md` index carries a single **§Theme list TABLE** (NOT the §Vocabulary-cohort-bullet + consolidated-firm-count structure the L4 *operator* index carries). VERIFIED this dispatch: the index has no consolidated firm-count tally, no §Vocabulary-cohort section, no growth-log / fork-flip prose — just the theme table (c047's D1/D2 "just appended rows since L4-L3/index has no consolidated tally" still holds). So:

- **(1) Own theme-table ROW** — AUTHORED below. Appended to `book/src/L4-L3/index.md` §Theme list after the `iterate-while-with-prev-dissolution` row. (This index's single registration artifact = the table row; there is no separate cohort-bullet artifact to author, and no consolidated tally to own or defer — this dispatch is the only L4>L3 landing this cycle.)
- **(2) SUMMARY.md** registration of the new chapter — AUTHORED below (own line, `summary-md-surgical-insert` shape), after the `iterate-while-with-prev-dissolution` line.

## Proposed changes

```new:book/src/L4-L3/ksp-solve-driver-dissolution.md
...
None. This theme lowers an already-firm L4 cap assembled from already-firm L4 vocabulary; no speculative L4 operator is introduced.

## Supporting evidence

- All four coordinated rewrites are cited against the firm cap (D1, `book/src/L4/ksp_solve.md` §Signature / §Semantics / §"Algebraic laws" Laws 1/2/5 / §"Lowers to") and the firm L3 entry (`book/src/L3/ksp_solve.md:38-104,160,169-173`), self-read this dispatch.
- L0 anchors citecheck-verified this dispatch (spot-checked `iterative.cpp:427` anchor `for`, `:563` anchor `restart`, `ksp.cpp:301-307` anchor `Warning` — all `[ok]`); the full anchor set (`iterative.cpp:417-418,427,463,484-485,563,703-704`; `iterative.hpp:52-55,98,101-108`; `ksp.cpp:296-310,308-309`) is transitive through the firm L3 driver, which already carries the citecheck-verified provenance.
- `book/src/concepts/solve-monad.md:1-68` read this dispatch: §Shape (`:5-19`), §"What stays out of the monad" (`:29-35`), §"Worked example — GMRES" (`:47-56`), §"Termination as a sum type" (`:58-68`).
- `book/src/design/l4_calculus.md` §3.3-3.4 (`:119-136`), §3.7 (`:150-184`), §3.8 (`:186-228`) read this dispatch.
- `book/src/L4-L3/iterate-while-dissolution.md` (firm c047) and `book/src/L4-L3/krylov-step-typed-wrapper-dissolution.md:158-200` (kernel-half precedent) read this dispatch.
- Deliverable absence confirmed: `ls book/src/L4-L3/ksp-solve-driver-dissolution.md` → No such file.

## Open questions / caveats

- **Slug reconciliation with D1's cap (same-cycle co-wiring).** D1's firm cap (`book/src/L4/ksp_solve.md` §"Lowers to" + §"Open questions") names the pending L4>L3 theme with the working slug `ksp-solve-outer-driver-dissolution` and explicitly flags: "if D3's dispatch lands a different slug, the integrator should reconcile the in-line reference." This dispatch lands the slug **`ksp-solve-driver-dissolution`** (parallel to the firm `krylov-step-typed-wrapper-dissolution` / `iterate-while-dissolution` naming; "driver" matches the firm L3 entry's "driver-half" framing). **Integrator co-wiring note**: when both D1's cap and this theme land, reconcile the cap's two in-line references (`§"Lowers to"` and the dep-map "Lowers to" cell, plus the §"Open questions" slug-mismatch note) from `ksp-solve-outer-driver-dissolution` to `ksp-solve-driver-dissolution`. The live link `[`ksp_solve`](../L4/ksp_solve.md)` in this theme is a **same-cycle forward-reference** to D1's create (lands before the single finalize build) — it resolves once D1's proposed-changes block is applied; no plain-text defang needed (both files land the same cycle, per the c047 D2→D1 forward-reference precedent).
- **`L4-L3/index.md` has no consolidated firm-count tally** (verified this dispatch — the index is a single §Theme list table, no §Vocabulary-cohort bullets, no growth-log / fork-flip prose). This dispatch appends only its own table row + the SUMMARY line; there is no consolidated tally to own or defer, and this is the only L4>L3 landing this cycle.
- **The `eigsolve` cap's L4>L3 dissolution is a sibling, not covered here.** The R3 `eigsolve` cap (if dispatched this cycle or later) consumes the same outer-driver vocabulary with a richer `Outcome` sum (a partial-success arm `0 < converged < requested`). Its L4>L3 dissolution is this theme's pattern *plus* the partial-success-arm scatter; it degenerates to this theme when the partial-success arm is absent (the `ksp_solve` case). It is a separate theme (the `eigsolve` driver), not re-derived here — noted for the forward planner.
- **The L3>L2 driver hop remains pending.** The firm L3 entry's §"Lowers to" (`book/src/L3/ksp_solve.md:169-173`) names the substantive L3>L2 driver consolidation (`L3-L2/ksp-solve-outer-driver`, pending the L2 `ksp_solve` promotion past `stub`). This theme covers only the L4>L3 hop; the L3>L2 hop is out of scope (a separate abstractor/lifter dispatch once L2 `ksp_solve` firms).
