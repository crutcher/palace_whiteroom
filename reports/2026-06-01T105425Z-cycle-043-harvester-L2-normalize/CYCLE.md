...
scope: L2 operator: normalize
status: pending
inputs:
  - book/src/L1/normalize.md (firm, cycle-027 — source of truth: signature, six laws, partiality, normalize_B rough-in note)
  - book/src/L3/normalize.md (firm, cycle-039 — iteration-rotation sibling; the §27/§131 "no interposed L2 entry" note goes stale once this floor lands)
  - book/src/L2/nrm2.md (firm, cycle-041 D2 — the norm constituent, a consumer-of inner_product)
  - book/src/L2/scal.md (firm, cycle-041 D3 — the rescale constituent, arity-1 member-of linear_combination)
  - book/src/L2/index.md (dep-map + cohort motifs; D2 owns the firm-count tally THIS cycle — deferred)
...
# CYCLE: Formalize normalize at L2

## Summary

This dispatch builds the L2 (fusion-rotation) floor entry for `normalize` — the fused vector-normalisation composite `normalize(x) = (β, x/β)` where `β = ‖x‖₂`, returning **both** the norm and the unit vector. It is the LAST genuine missing floor under the `l2-floor-under-l3-leaf-cohort` directive: the firm L3 [`normalize`](../L3/normalize.md) (cycle-039) currently rests directly on the firm L1 [`normalize`](../L1/normalize.md) (cycle-027) with no adjacent L2 parent, which the **Identity-lowerings still require both L levels** invariant forbids. The floor is **thin / identity-in-form**: the L2 form is value-thread-isomorphic to the L1 form, and there is **no genuine kernel fusion to un-fold** — Palace's `linalg::Normalize` (`vector.hpp:262-270`) is already the one-line composition `norm = Norml2(comm,x); x *= 1.0/norm; return norm`, with the norm reduction and the rescale already separate passes. The one L2-genuine content beyond floor-presence is the **fusion-naming**: `normalize` framed as the `nrm2 ∘ scal` composition in L2 vocabulary, citing the same-layer L2 floors [`nrm2`](./nrm2.md) + [`scal`](./scal.md) as its constituents. **Load-bearing framing preserved:** `normalize` is a fused composite, **NOT a fold leaf** — it is fork-INDEPENDENT on membership (like `nrm2` is a consumer-not-member), so it carries NO fold-parent and is design-final on the batch-12 leaf-vs-fold fork. The partiality non-law (`x = 0`) and the IEEE-754 reduction-tree caveats transport unchanged.

## Proposed changes

```new:book/src/L2/normalize.md
---
layer: L2
operator: normalize
firmness: firm
lowers_to:
  - book/src/L1/normalize.md (identity-in-form on the operator's signature; the L2>L1 normalize rotation is narrated by the D10 L2-L1 theme — see Lowers-to)
lifts_from:
...
> **Count-ownership note (cycle-043 D9):** this dispatch appends ONLY its own dep-map ROW (above) + chapter body + SUMMARY registration. It does **NOT** touch the L2/index consolidated firm-count running tally (currently "firm 12 → 17", dep-map "18 rows = 17 firm + 1 partly-constructive" at `book/src/L2/index.md:105`) — **D2 owns the tally this cycle** per the `parallel-blind-shared-index-count-divergence` convention. The integrator/owner reconciles the absolute count once post-cohort.
...
(The new `normalize` chapter is registered immediately after `nrm2` — adjacent to both its constituent floors `scal` (line 58) and `nrm2` (line 61). The `nrm2` line above is the unchanged anchor; the `normalize` line is the insertion.)

## Operator content

The full firm chapter body is authored inside the `new:book/src/L2/normalize.md` proposed-changes fence above (intro + Context + fused-composite-not-fold-member framing + Signature + Semantics + the six Algebraic-laws + non-laws + Dependencies + Variant-axes + Status + Lowers-to/Lifts-from + Evidence + L2-vs-L1). Summary of the load-bearing decisions:

- **Thin floor, not moderate.** `normalize` un-folds **no genuine kernel fusion** at L2 — Palace's `linalg::Normalize` (`vector.hpp:262-270`) is already the one-line composition `norm = Norml2(comm,x); x *= 1.0/norm; return norm`, with the norm reduction (line 266) and the rescale (line 268) already separate passes. There is no fused single-pass (blocked/SIMD/batched) norm-and-rescale kernel to decompose. This is the `nrm2`-floor situation (un-folds nothing), NOT the `divfree-projector`-floor situation (moderate, one genuine `AddMult` de-fusion). The one L2-genuine content is **fusion-naming**: framing `normalize` as the `nrm2 ∘ scal` composition over the two firm same-layer floors.
- **Fused composite, NOT a fold leaf — fork-INDEPENDENT (load-bearing per dispatch framing).** `normalize` is to the L2 `nrm2`/`scal` floors what `axpy` is to `scal`/vector-add: a recognised composite Palace ships as one symbol. It is a member of **neither** L2 fold (`inner_product` reduce-to-`Scalar` nor `linear_combination` reduce-to-`Tensor[N]`) — its codomain `(Scalar, Tensor[N])` is neither. So it carries **NO fold-parent** and is **design-final on the batch-12 leaf-vs-fold fork** — exactly as `nrm2` is fork-invariant on membership (consumer-not-member), `normalize` is fork-invariant by being a composite with no fold to fold into. Its constituents carry the fold relationships; the composite does not.
- **Constituent floors cited as `consumes`**, not as a fold of which `normalize` is a member: [`nrm2`](./nrm2.md) (the norm, consumer-of `inner_product`) + [`scal`](./scal.md) (the rescale, arity-1 member-of `linear_combination`). The factorisation `normalize(x) = (nrm2(x), scal(1/nrm2(x), x))` is law 6.
- **Partiality non-law preserved**: `normalize` is **partial** — undefined at `x = 0` (`MFEM_ASSERT(norm > 0.0)`, `vector.hpp:267`). Recorded as a precondition, the single semantic addition over the (total) `nrm2`/`scal` floors.
- **IEEE-754 reduction-tree caveats preserved**: reduction-tree non-associativity load-bearing (inherited from `nrm2`); reciprocal-vs-divide bit-difference an L1>L0 transparent-trick note. Both recorded as explicit non-laws.

## Supporting evidence

- **L0 source of truth** — `palace/linalg/vector.hpp:262-270` (`linalg::Normalize`), `:256-260` (`Norml2`). All anchors self-verified on-disk via `tools/citecheck/citecheck.py --anchor` 2026-06-01: `Normalize` → 262/264; `MFEM_ASSERT` → 267; `1.0 / norm` → 268; `return norm` → 269. Consumer sites verified: `palace/linalg/iterative.cpp:631` (`Hj[j + 1] = linalg::Norml2`), `:632` (`1.0 / Hj`); `palace/linalg/operator.cpp:673` (`l = Normalize`), `:676` (`res = std::abs(l - l0)`); `palace/linalg/nleps.cpp:610` (`Norml2(GetComm(), v)`), `:617` (`v2 / scale`). The `Norml2` body is at line **259** (not 255/258) — the comment is 255, template 256, signature 257, body 259; cited the definition span as `256-260`.
- **Firm sources of truth (per dispatch)** — `book/src/L1/normalize.md` (firm cycle-027, the authoritative surface entry: signature, six laws, partiality, `normalize_B` rough-in note, full L0 evidence chain) + `book/src/L3/normalize.md` (firm cycle-039, the iteration-rotation sibling).
- **Same-layer constituent floors** — `book/src/L2/nrm2.md` (firm cycle-041 D2) + `book/src/L2/scal.md` (firm cycle-041 D3). The `scal` floor's §Dependencies sibling-subsumption note (`book/src/L2/scal.md:223-228`) explicitly flagged `Normalize(x) = scal(1/nrm2(x), x)` paired with the returned norm as an OQ inherited from L1 ("Whether to harvest a fused `normalize` L2 primitive is an open question"); this entry **closes** that OQ — the fused composite now has its own L2 floor.
- **Directive / cohort context** — `book/src/L2/index.md` §"Identity-in-form floors" + §"Working Notes" (cycle-041 / cycle-042 floor cohorts + the `dot-l2-leaf-floor-vs-fold-only-design` fork note). `normalize` extends the floor cohort with a *composite* floor (the prior floors were leaves or gates); it is the **last genuine missing floor** of the `l2-floor-under-l3-leaf-cohort` directive.

## Open questions / caveats

- **L3 `normalize` §27/§131 "no interposed L2 entry" goes STALE once this floor lands** (route to c044 sweep, NOT corrected here). The firm L3 entry `book/src/L3/normalize.md` records at line 27 ("**Downward** to L1: `normalize` lowers to L1 [`normalize`](../L1/normalize.md) directly, with **no interposed L2 entry and no `L3-L2`/`L3-L1` theme file**") and line 131 ("The L2 layer hosts no standalone `normalize` entry … the L3>L1 hop is therefore direct") that there is no L2 `normalize`. This floor supersedes those notes — once landed, the L3 entry's downward rotation should re-anchor to a **direct L3>L2** hop onto this new floor (and the D11 L3-L2 `normalize-body-identity` theme, if co-dispatched), not a layer-skipping L3>L1 hop. Per one-operator-per-dispatch discipline this is **NOT corrected in this dispatch**; routed to the **c044 sweep** as flagged in the dispatch prompt. (The same staleness affects the L3 frontmatter `lowers_to: book/src/L1/normalize.md (… no L3-L2/L3-L1 theme)`.)
- **D10 L2-L1 `normalize` lowering theme + D11 L3-L2 `normalize-body-identity` theme** (co-dispatched this cycle) — the L2>L1 and L3>L2 rotations are narrated by those themes; this entry forward-references them as plain text / inline-code (not live links — the chapters do not yet exist at dispatch time, per the missing-anchor convention). If a co-dispatched theme uses a different slug than `normalize` (e.g. `normalize-leaf-identity` vs `normalize-fold-specialization`), the dispatch-cohort slug-naming-normalization OQ (`book/src/L2/index.md:108`) covers it — `normalize` is a *composite*, so the natural L2>L1 slug is `normalize-composite-identity` or `normalize-leaf-identity`; flag for the meta-phase naming pass if it diverges.
- **`L2/index` running-count tally is D2-owned this cycle** — this dispatch deliberately did NOT update the "firm 12 → 17" / "18 rows" tally at `book/src/L2/index.md:105` (it would become "firm → 18, dep-map 19 rows = 18 firm + 1 partly-constructive" with this row, but the owner reconciles the absolute count to avoid the parallel-blind divergence). Surfaced here so the integrator/owner knows the `normalize` row contributes +1 firm.
- **No new variant axis, no fold-membership** — confirmed `normalize` introduces no axis beyond the inherited element-type, and is fork-INDEPENDENT (no fold-parent). No open question on the leaf-vs-fold fork for this entry — it is design-final by construction.
