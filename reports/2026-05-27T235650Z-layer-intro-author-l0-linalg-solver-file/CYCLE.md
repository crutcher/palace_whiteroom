...
invoked_at: 2026-05-27T23:56:50Z
...
# CYCLE: L0 / linalg-solver-file

## Summary

This report proposes a new L0 file-overview chapter `book/src/L0/linalg-solver-file.md` covering `palace/linalg/solver.{hpp,cpp}` — the home of the **abstract `Solver<OperType>` base class** that the entire Palace solver hierarchy inherits from, plus its **only concrete sibling defined in the same file**, `MfemWrapperSolver<OperType>` (already covered in detail by [`mfem-wrapper-solver`](./mfem-wrapper-solver.md); this chapter is the file-level wrapping around it).

The chapter is **L0 bundle 6** (deferred from cycle-010 OQ `l0-bundle-6-candidates`). It rounds out the linalg/ file-overview cohort: `vector.{hpp,cpp}`, `operator.{hpp,cpp}`, `iterative.{hpp,cpp}`, `ksp.cpp` are already covered; `solver.{hpp,cpp}` was the last unanchored file in the linalg/ Krylov-and-preconditioner spine.

**Important scope correction surfaced in source inspection:** the dispatch prompt described `Solver<OperType>` as "the abstract base class for direct solvers like LU/Cholesky, exterior to the `BaseKspSolver<>` Krylov hierarchy." This framing is incorrect. `Solver<OperType>` is the type-axis root for **all** Palace solvers (preconditioners + iterative solvers + the MFEM wrapper); the eight concrete subclass families (`MfemWrapperSolver`, `IterativeSolver`+3, `JacobiSmoother`, `ChebyshevSmoother`+1, `DistRelaxationSmoother`, `GeometricMultigridSolver`, `BlockDiagonalPreconditioner`) all inherit from it. Direct solvers (MUMPS / SuperLU / Strumpack) are MFEM-side classes consumed through `MfemWrapperSolver`; they do not have a Palace-side `Solver<OperType>` subclass dedicated to them. The chapter below adopts the corrected framing. See Open questions §1.

Three proposed changes:
1. **New file**: `book/src/L0/linalg-solver-file.md` (the chapter).
2. **L0/index.md edit**: add row under "File overviews" cohort.
3. **SUMMARY.md edit**: add row under L0 Part.

## Proposed changes

```new:book/src/L0/linalg-solver-file.md
...
- **Existing L0 file-overview cohort precedent** — `linalg-vector-file.md`, `linalg-operator-file.md`, `ksp-factory-file.md`, `linalg-iterative-file.md`, `mpi-globalsum-and-collectives.md`. The proposed chapter follows the same shape: at-a-glance, region-by-region structure, notes-for-higher-layers, dependencies, evidence.
- **`Solver<OperType>` is the type-axis root** — confirmed by `mcp__palace-codemap__search_text` query `: public Solver<` returning eight distinct concrete-subclass declarations in seven sibling files (`jacobi.hpp`, `iterative.hpp`, `blockprecond.hpp`, `chebyshev.hpp` (×2), `gmg.hpp`, `distrelaxation.hpp`, plus `solver.hpp` itself for `MfemWrapperSolver`).
- **No other concrete subclass declared in `solver.{hpp,cpp}` itself** — the file pair is only 138+209 = 347 lines and the AST subtree shows two class declarations only (one for `Solver<OperType>`, one for `MfemWrapperSolver<OperType>`).
- **The `Mult2` / `MultTranspose2` virtuals are present in the base but `MFEM_ABORT`-defaulted** — confirmed by direct read of `solver.hpp:52-64`. This is a load-bearing observation: the with-preallocated-temporary form is an opt-in optimisation, not the canonical path.
- **`BaseKspSolver` is NOT a `Solver<OperType>`** — confirmed by reading `ksp.hpp:41-42` showing it composes `std::unique_ptr<IterativeSolver<OperType>> ksp` + `std::unique_ptr<Solver<OperType>> pc` rather than inheriting. This is the load-bearing distinction between the composition class and the type-axis root.

## Open questions / caveats

1. **Dispatch-prompt framing was inaccurate.** The dispatch said "the abstract `Solver<OperType>` base class for direct solvers like LU/Cholesky, exterior to the `BaseKspSolver<>` Krylov hierarchy." Source inspection shows `Solver<OperType>` is the type-axis root of ALL Palace solvers (preconditioners + iterative + MFEM-wrapped) — not specifically a direct-solver base, and not exterior to the Krylov hierarchy (iterative solvers including `CgSolver` inherit from it). The chapter adopts the corrected framing. **No action required** other than noting this for the cycle-planner's reference; the OQ `l0-bundle-6-candidates` description should be updated when next touched.

2. **Direct-solver wrappers do not have a Palace-side `Solver<OperType>` subclass.** MUMPS, SuperLU, Strumpack are MFEM-side `mfem::Solver` subclasses consumed through `MfemWrapperSolver`. There is no `MumpsSolver : public Solver<OperType>` or similar in Palace; the direct-vs-iterative classification happens consumer-side, not by class hierarchy. The chapter's "What's not here" section flags this. If a future L1 entry wants to distinguish direct vs iterative at the type level, the distinction would have to come from the `Solver<OperType>` value's construction (e.g., a variant axis on the L1 `Solver` type carrying "direct" / "iterative" / "preconditioner" classification).

3. **`Mult2` / `MultTranspose2` are uncited at the higher-layer level.** The with-preallocated-temporary virtuals are present in `solver.hpp:52-64` but no harvester or abstractor has touched them. They are an opt-in hot-path optimisation — when L1 `ksp_solve` is harvested (cycle-007+), the harvester should decide whether to lift them as a separate operator (`ksp_solve_with_workspace`) or fold them under [`mutable-workspace-pattern`](./mutable-workspace-pattern.md)'s "workspace-as-input-argument" form. Surfacing as a forward note rather than a new OQ — the harvester will make the decision in context.

4. **`SetInitialGuess` flag handling.** The `initial_guess` member is an L0 stateful flag affecting how `Mult`'s second argument is interpreted. At L1 this lifts as either an additional argument (`ksp_solve :: Solver -> Vector -> Maybe Vector -> Vector`) or a variant on the `Solver` value (`SolverConfig { initial_guess: bool, ... }`). The natural choice depends on how the rest of the configuration state is treated; the same question applies to `BaseKspSolver`'s `SetInitialGuess` (covered in [`kspsolver-base-class`](./kspsolver-base-class.md) §"Use and lifecycle"). Not new — already covered by the rough-in `L1/ksp_solve` planning. No new OQ needed.

5. **The chapter does not include a tests section** — there is no dedicated unit test for `Solver<OperType>` or `MfemWrapperSolver` in isolation (the wrapper is exercised through the preconditioner pipelines and the full-application regressions). This matches the precedent in [`linalg-iterative-file`](./linalg-iterative-file.md) and [`ksp-factory-file`](./ksp-factory-file.md), which also omit a "Test coverage" section. Confirmed by grep for `MfemWrapperSolver` and `Solver<OperType>` across `palace/test/unit/`: no direct hits.
