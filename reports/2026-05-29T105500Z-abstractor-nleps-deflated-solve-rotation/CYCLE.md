...
inputs:
  ...
  - book/src/L1-L0/lu-solve-mutation-rotation.md (firm sibling, cycle-023)
  - book/src/L1-L0/nleps-deflated-residual-mutation-rotation.md (firm sibling, cycle-023)
  - palace/linalg/nleps.cpp:504-537 (the deflated_solve lambda — verified verbatim this dispatch)
  - palace/linalg/nleps.cpp:329-347, :474, :495-502, :541-542, :681-682, :726, :732-735 (constituents/call/lag sites — verified)
  - OQ nleps-deflated-solve-l1-l0-lowering-theme (open-questions.md:784-787; the carry-forward this dispatch enacts)
---

# CYCLE: L1>L0 theme sketch — nleps-deflated-solve-mutation-rotation

## Summary

This completes the NLEPS L1>L0 lowering cohort. The firm L1 `nleps_deflated_solve` operator
(landed cycle-023, `book/src/L1/nleps_deflated_solve.md`) is the block (Schur-complement) solve
of the extended deflated `(n+k)` NEP system inside Palace's `QuasiNewtonSolver`. This theme
records how that pure L1 form lowers into its single positive L0 site — the `deflated_solve`
lambda at `palace/linalg/nleps.cpp:504-537`, verified verbatim this dispatch. The rewrite is
**structural**: the pure `{ x1, x2 } = nleps_deflated_solve(K, P, λ, b1, b2)` expands into the
L0 lambda with `(x1, x2)` as in-place destination buffers and an early `k == 0` return that is
the plain big-space solve. Three sub-patterns carry it: **(A)** the big-space block
`x1 = T(σ)⁻¹·b1` via `opInv->Mult(b1, x1)` (the `ksp_solve` leaf); **(B)** the coordinate block
— the Gram `XᴴX` build (`linalg::Dot` double-loop), the Schur complement `SS = −S⁻¹·(XᴴX)`, and
the coordinate solve `x2 = SS⁻¹·(b2 − Xᴴ·x1)`, all dense `k×k` `fullPivLu().solve` (the
`lu_solve` leaf, now firm/live-linkable per cycle-023); **(C)** the back-projection correction
`x1 ← x1 − X·(S⁻¹·x2)` via `MatVecMult(X, S.fullPivLu().solve(x2))` folded by
`linalg::AXPY(-1.0, …)` (the `linear_combination` + `axpy` leaves). No new vocabulary is
proposed — every constituent is already firm L1/L2. The theme is `firm`. The one non-trivial
recording is the **load-bearing block-elimination structure**: `S⁻¹` appears **twice** (once
inside `SS`, once in the final correction), the structural signature of the Schur elimination —
distinct from the bare-Galerkin `(XᴴX)⁻¹` projection (the cycle-021/022 over-unification guard,
and the deflate-promotion finding: the bare `(XᴴX)⁻¹` solve never appears here).

## Proposed changes

### (a) Create the theme chapter

The full firm theme body is enclosed in the `new:` fence below. Chapter code uses
4-space-indented blocks (no nested triple-backtick fences) so fence parity is trivially balanced
(the cycle-019 fence-truncation guard).

```new:book/src/L1-L0/nleps-deflated-solve-mutation-rotation.md
---
status: firm
layer: L1>L0
...
### (b) Append the dep-map row to `book/src/L1-L0/index.md`

Insert one new firm row immediately **after** the `lu-solve-mutation-rotation` row (currently
the last theme row, before the `minres-iteration` / `bicgstab-iteration` obstruction rows). The
exact line to insert after:

`| [lu-solve-mutation-rotation](./lu-solve-mutation-rotation.md) | ... | firm *(...)* |`
...
NOTE for integrator: this row is non-overlapping with the parallel `apply_nonlinear_pencil`
L1>L0 abstractor — that abstractor proposes a distinct `apply-nonlinear-pencil-mutation-rotation`
row. Place both rows distinctly (this one after `lu-solve-mutation-rotation`).

### (c) Add the SUMMARY.md chapter entry

Under the `# L1 > L0 — Lowering` Part (SUMMARY.md:80), append after the
`lu-solve-mutation-rotation` entry (currently SUMMARY.md:99, the last L1>L0 chapter):

```text
- [nleps-deflated-solve-mutation-rotation](./L1-L0/nleps-deflated-solve-mutation-rotation.md)
```

NOTE for integrator: shared file with the parallel `apply_nonlinear_pencil` L1>L0 abstractor —
that abstractor adds `[apply-nonlinear-pencil-mutation-rotation]`. Both entries are
non-overlapping insertions under the same Part; place both.

## Speculative operators proposed

**None.** This theme lowers the already-firm L1 `nleps_deflated_solve` operator (landed
cycle-023) and composes only firm L1/L2 leaves (`ksp_solve`, `lu_solve`, `dot`, `axpy`,
`linear_combination`). It proposes no new rough-in vocabulary, so there is nothing for the
harvester to promote. The L2 `gram` / `deflate` combinators it references are already on the L2
dep-map (cycle-022); this theme only marks the upward fan-out boundary and re-confirms the
deflate-promotion guard.

## Supporting evidence

All L0 ranges self-verified verbatim this dispatch via `palace-codemap` `read_range` /
`search_text` (see the chapter's §Verified-against for the per-line audit). Key reads:

- `palace/linalg/nleps.cpp:504-537` — the complete `deflated_solve` lambda. The lambda starts at
  `:504` (the `// Linear solve with the extended operator of the deflated problem.` comment),
  signature `:505-507`, block-system comment `:508-513`, body `:514-536`, close `:537`. The
  Schur-elimination statement `:512` reads verbatim:
  `// x2 = SS^-1 (b2 - A x1) where SS = (B - A T^-1 U) = - X^* X S^-1`.
- `palace/linalg/nleps.cpp:533-535` — the three `fullPivLu().solve` sites confirmed by
  `search_text fullPivLu().solve` → lines 533/534/535 (also 563/665/667 in sibling lambdas, out
  of this theme's scope). `:533` `SS = -S.fullPivLu().solve(SS);`, `:534`
  `x2 = SS.fullPivLu().solve(x2);`, `:535` `const ComplexVector XSx2 = MatVecMult(X,
  S.fullPivLu().solve(x2));`.
- `palace/linalg/nleps.cpp:329-347` — `MatVecMult(X, y)` body confirmed: `z.SetSize(n)` /
  `z = 0.0` then per-`j` two `linalg::AXPBYPCZ` on `z.Real()` / `z.Imag()`.
- Call sites confirmed by `search_text deflated_solve` → `:505` (def), `:542`, `:682`, `:735`
  (the three invocations). Tolerance sites `:541` / `:681` / `:734`; lag sites `:474` / `:726`.

Sibling-theme structure precedent followed: `book/src/L1-L0/nleps-deflated-residual-mutation-rotation.md`
(firm, cycle-023) and `book/src/L1-L0/lu-solve-mutation-rotation.md` (firm, cycle-023) — same
frontmatter block, §Status / §L1 form / §L0 form / §Rewrite-forward / per-sub-pattern citations
/ §Applicability / §Justification / §Speculative-operators / §Verified-against layout.

## Open questions / caveats

- **Promote OQ `nleps-deflated-solve-l1-l0-lowering-theme`** (`scaffolding/open-questions.md:784-787`)
  — this dispatch ENACTS it. The theme is now authored firm on disk (pending integration). The
  per-report integrator should append a closed-disposition record (the per-report integrator does
  NOT edit the Open entry in place; meta-phase owns the Closed-index migration). Answer-link:
  `book/src/L1-L0/nleps-deflated-solve-mutation-rotation.md` (`status: firm`). All the lowering
  content the OQ named (in-place `x1`/`x2` destination buffers, the `opInv->Mult` + `MatVecMult`
  + `linalg::AXPY` L0 build-form, the per-use `SetRelTol` `:541`/`:681`, the `eig_opInv` lag
  `:474`/`:532`) is recorded in the §Rewrite-forward "L0-only material the L1 signature drops"
  list. *Meta-phase action:* migrate the OQ to the Closed index with this answer-link.

- **NLEPS L1>L0 cohort completion.** This theme is the last NLEPS L1>L0 *deflation* lowering
  (with cycle-023's `lu-solve` + `nleps-deflated-residual`). The remaining un-themed NLEPS L1>L0
  item is `apply_nonlinear_pencil` (the bare-pencil leaf) — being authored by the **parallel
  `apply_nonlinear_pencil` L1>L0 abstractor this cycle**, not this dispatch. Per the cycle-023
  staging note (`reports/cycle-023-integrator-staging/STAGING.md:165`), after both land the
  NLEPS L1>L0 lowering cohort has no remaining open item.

- **Deflate-promotion gate stays OPEN — confirmed, not changed** (OQ
  `nleps-deflated-solve-firm-landed-deflate-promotion-gate-stays-open`, `:774`). The chapter's
  §The block-elimination structure re-states the finding from a *lowering* viewpoint: the Gram
  `XᴴX` is built positively (`:529`) but only ever solved Schur-wrapped (`SS = −S⁻¹·(XᴴX)` at
  `:533`); the bare `(XᴴX)⁻¹` Galerkin core is absent. This theme does **not** touch the L2
  `deflate` entry (out of scope) and does **not** change its `partly-constructive` status. *No
  meta-phase action beyond the existing OQ trigger* (a ROM / eigensolver-locking-scoped dispatch
  surfacing a bare `(XᴴX)⁻¹` solve outside `nleps.cpp`).

- **Source comment vs code discrepancy** (recorded, not a defect): the block-comment line
  `x1 = x1 - X S x2` (`:513`) writes `S` where the executed code applies `S⁻¹` (`.solve(x2)` at
  `:535`). The L1 operator entry already records this (`book/src/L1/nleps_deflated_solve.md:91`);
  the theme follows the code (`x1 − X·(S⁻¹·x2)`), not the comment. No action — flagged so a
  future reader of the L0 comment is not misled.

- **Shared-file coordination flag** (for the integrator, not an open question): `index.md` +
  `SUMMARY.md` are co-edited with the parallel `apply_nonlinear_pencil` L1>L0 abstractor. The
  proposed rows/entries above are distinct, non-overlapping insertions (this theme's slug is
  `nleps-deflated-solve-mutation-rotation`; the sibling's is
  `apply-nonlinear-pencil-mutation-rotation`). Apply both.
