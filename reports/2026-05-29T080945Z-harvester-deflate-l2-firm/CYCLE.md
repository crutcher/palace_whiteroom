...
inputs:
  ...
  - book/src/L1/nleps_deflated_residual.md (firm, cycle-022 — the over-unification guard, the other-direction precedent)
  - book/src/L2/orthogonalize.md (firm — the over-unification target; the named-composition structural sibling)
  - book/src/L2/linear_combination.md (firm — the X·coords reconstruction fold)
  - book/src/L2/inner_product.md (firm — the scalar fold gram lifts to a matrix; via gram)
  - book/src/L1/dot.md (firm — the pinned arg-1-conjugated convention)
  - book/src/L2/index.md (the L2 dep-map; deflate row currently rough-in :55)
  - palace/linalg/nleps.cpp:505-537,547-576,660-668,329-347,606-619,354-362 (positive source sites)
---

# CYCLE: Formalize `deflate` at L2

## Summary

`deflate` is the L2 named-composition oblique/Galerkin complementary projector that removes
the converged-eigenvector deflation subspace `span(X)` from a vector. The cycle-021
combinator-miner roughed it in (`book/src/L2/index.md:55`) as `I − X(XᴴX)⁻¹Xᴴ` with the
explicit over-unification guard against `orthogonalize`. This dispatch firms it — but the
**core authoring decision** (which the planner flagged as the crux) resolves *against* making
the bare Galerkin form `I − X(XᴴX)⁻¹Xᴴ` the firm form. Reading the actual Palace
`deflated_solve` lambda (`palace/linalg/nleps.cpp:505-537`) shows the projector Palace
positively exhibits is the **NLEPS Schur-complement-modified** oblique projection: the
source's own block-elimination comment at `palace/linalg/nleps.cpp:512` states the Schur
complement is `SS = (B − A T⁻¹ U) = −XᴴX·S⁻¹` with `S = λI − H` — the Gram `XᴴX` is wrapped in
`S⁻¹` factors on both sides, not used bare. The bare Galerkin form is the *literature
idealization* (Effenberger 2013, `palace/linalg/nleps.cpp:357`), the `S = I` degenerate case,
and is **not positively exhibited anywhere in Palace**. So `deflate` lands **`partly-constructive`**:
the structural decomposition (the `coords ▷ schur-solve ▷ back-project` pipeline) is firm and
exhaustively cited; the **generic-Galerkin-core sub-part** (`I − X(XᴴX)⁻¹Xᴴ`, the `S = I`
specialization) is materialized from literature / negative anchors with a stated promotion
condition. The over-unification guard against `orthogonalize` (do NOT collapse — `deflate`
carries the `(XᴴX)⁻¹` / Schur correction that Gram-Schmidt does not) is stated as a first-class
section.

## Proposed changes

```new:book/src/L2/deflate.md
# deflate
...
see § Status). It is built over the [`gram`](./gram.md)<!--rough-in--> Gram-matrix constructor,
...
L1 [`dot`](../L1/dot.md) (the coordinate extraction `Xᴴ v`), and the `gram`<!--rough-in-->
constructor (`XᴴX`, being firmed in a parallel wave-2 dispatch; plain-text forward-reference
until its chapter lands).
...
- `v` — `Tensor[N]` — read-only; the vector to project against the deflation subspace.
- result — `Tensor[N]` — the deflated vector `v − X·(coordinate solve)`, same length axis `N`.

The axis `N` is uniform across `X` and `v`. The deflation-cardinality axis `k` is the
...
| **Algorithm** | Sequential / batched **rank-1 subtraction** `w ← w − ⟨w,X[j]⟩X[j]` (the `project ▷ subtract` `dot ▷ axpy` chain, `book/src/L2/orthogonalize.md:114-142`). **No Gram matrix, no solve.** | Build the explicit **Gram matrix** `XᴴX` (via [`gram`](./gram.md)<!--rough-in-->, `palace/linalg/nleps.cpp:526-531`) and **`lu_solve`** it (the Schur-modified solve, `palace/linalg/nleps.cpp:533-535`). **The solve is the algorithm.** |
...
- `gram`<!--rough-in--> (L2, being firmed in parallel wave-2 dispatch; plain-text
  forward-reference) — the Gram-matrix constructor `XᴴX` (the all-pairs `inner_product` fold,
  `palace/linalg/nleps.cpp:526-531`). `deflate` consumes the Gram as the coefficient matrix of
  the coordinate solve. (Live link to be wired at integration once `gram.md` is on disk.)
...
- [`inner_product`](./inner_product.md) (L2, firm) — transitively, via `gram` (the Gram is the
  all-pairs lift of the scalar `inner_product` fold). Not a direct `deflate` dependency.
...
- **L0**: the fused `deflated_solve` block (`palace/linalg/nleps.cpp:505-537`) — the Gram
  ...
  reference.
- **L2**: the *named composition* `deflate op X v → Tensor[N]` — the `coords ▷ schur-solve ▷
...
- `book/src/L2/index.md:54-55` — the rough-in `gram` / `deflate` dep-map rows (this dispatch
  switches `:55` to firm + a live link to `gram` is deferred until `gram.md` lands).
