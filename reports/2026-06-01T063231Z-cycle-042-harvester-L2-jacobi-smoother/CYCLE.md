...
scope: L2 operator: jacobi-smoother
status: pending
inputs:
  - book/src/L1/jacobi-smoother.md (firm; the mutation-rotated source of truth — signature, six laws, three non-laws, variant axes, complete L0 evidence)
  - book/src/L3/jacobi-smoother.md (firm cycle-037; the iteration-rotation rendering — the L3 form this L2 floor sits beneath)
  - book/src/L2/scal.md (firm cycle-041 D3; the L2-floor-under-L3 structural template for an identity-in-form floor entry)
  - book/src/L2/chebyshev-iteration.md (firm cycle-012; the polynomial-sibling L2 entry — the de-fused recurrence form jacobi-smoother is the degree-zero member of)
  - book/src/L2/index.md (the Operator dep-map + Working-Notes; firm count 12 + 1 partly-constructive, OWNED by D11 this cycle — tally deferred)
...
# CYCLE: Formalize jacobi-smoother at L2

## Summary

Builds the L2 (fusion-rotation) floor entry for `jacobi-smoother` — the
constructed-operator-gate apply for the diagonal preconditioner, whose per-call
body is one elementwise product `op.dinv ⊙ x = (ω · D⁻¹) ⊙ x` of the damped
inverse diagonal against the input. This is a **standalone constructed-operator
gate, fork-INDEPENDENT, with NO fold-parent** — it is not a member of the
BLAS-1 leaf/fold cohort (`linear_combination` / `inner_product`), so the
cycle-041 `dot-l2-leaf-floor-vs-fold-only-design` leaf-vs-fold fork pending the
batch-12 meta-phase does NOT apply to it. The entry is a foundation-first
backfill under the directive `l2-floor-under-l3-jacobi-smoother`: the firm L3
[`jacobi-smoother`](../L3/jacobi-smoother.md) (cycle-037) currently rests on the
firm L1 [`jacobi-smoother`](../L1/jacobi-smoother.md) directly, skipping a layer
down with no present adjacent L2 parent; per **Identity-lowerings still require
both L levels** the L2 floor must exist so the L3 entry rests on a present
adjacent same-named L2 parent and a reader at L2 finds the gate defined in L2
vocabulary. The rotation L2 ↔ L1 is **identity-in-form**: the apply is a single
whole-tensor elementwise product with no kernel fusion to unfold (the genuine
fusion-rotation observation here is **negative** — unlike the
[`chebyshev-iteration`](./chebyshev-iteration.md) sibling, whose `ApplyOrder0` /
`ApplyOrderK` element-fused kernels DO de-fuse into a three-term recurrence,
the Jacobi apply has no fused multi-operation kernel to unfold). Status `firm`,
...
The full firm chapter body is authored inside the `edit:book/src/L2/jacobi-smoother.md`
proposed-changes fence above (frontmatter + Context + Signature + Semantics +
Negative fusion observation + Algebraic laws + Dependencies + Variant axes +
Status + Lowers to + Lifts from + Evidence + L2 vs L1 distinction). Inner code
samples are rendered as 4-space-indented blocks (no nested ` ``` ` fences) per the
`convert-nested-fences-to-indented-code-in-proposed-changes-block` discipline.

Key formalization decisions:

- **Standalone constructed-operator gate, fork-INDEPENDENT, NO fold-parent** —
  stated explicitly in the one-line intro, §Context, the `fold_parent: none`
  frontmatter, §Dependencies, §Status, and the dep-map row. `jacobi-smoother` is
  not a member of the `linear_combination` / `inner_product` fold cohort, so the
  cycle-041 `dot-l2-leaf-floor-vs-fold-only-design` leaf-vs-fold fork (pending the
  batch-12 meta-phase) does not apply to it.
- **Genuine fusion beyond identity: NONE — and the negative observation is the
  fusion-rotation content.** The Jacobi apply is one elementwise product with no
  fused multi-operation kernel to unfold; the complex four-multiply form is a
  single elementwise complex product, not a fused composition of separable L2
  primitives. This is the degree-zero fixed point of the fusion rotation that
  produces `chebyshev-iteration`'s three-term recurrence. Recorded in a dedicated
  §Semantics "Negative fusion observation" subsection and as a deliberate non-law.
- **`Apply<Transpose=true>` dead-code consumer branch** (`jacobi.cpp:61-69`)
  recorded as a recognition-rule caveat / non-law (NOT a status reduction), tying
  to OQ `reciprocal-elementwise-product-mr-dead-code-transpose-consumer-branch`.
- **L2 vocabulary, high→low.** Semantics / signature / laws defined in L2 base-algebra
  vocabulary; the L2>L1 (`jacobi-smoother-apply-identity`, D8) and L3>L2 (in-line
  identity) themes narrate the rotations; the substantive rotation is named as the
  L1>L0 leaf-mutation theme, not re-derived here.

## Supporting evidence

All L0 anchors self-verified on-disk via `tools/citecheck/citecheck.py --anchor`
(2026-06-01): `jacobi.cpp:38` (`Y[i] = DI[i] * X[i]`), `:79` (`AssembleDiagonal`),
`:80` (`Reciprocal`), `:92` (`dinv *= omega`), `:102` (`initial_guess`), `:103`
(`Apply(dinv, x, y)`), `:106` (`JacobiSmoother<Operator>`); `jacobi.hpp:19`/`:28`/`:43`;
`ksp.cpp:198` (`JACOBI`); `errorestimator.cpp:76` (`JacobiSmoother`). The complex
transpose branch `:61-69` confirmed by direct read (conjugate-`dinv` form
`YR[i] = DIR[i]·XR[i] + DII[i]·XI[i]`, the negated-off-diagonal conjugate apply).

`citecheck --scan` bounds/path-hygiene: all citations carry the full
`palace/linalg/...` path (no `AMBIG`); all ranges in-bounds for the on-disk files.

## Open questions / caveats

1. **No L2 `elementwise_product` / `reciprocal` floor entry exists** (genuine gap,
   not a defect of this entry). Both primitives are firm at L1 and L3 (cycle-038)
   but the L2 floor for the elementwise-primitives cohort has not been built. The
   Jacobi apply's body — one elementwise product `op.dinv ⊙ x` — is therefore
   recorded as a single base field operation below the L2 layer's current
   resolution (the same situation as the L3 entry, where the elementwise product
   is below L3's resolution). Once an L2 `elementwise_product` floor lands, this
   entry's body becomes the single call `elementwise_product(op.dinv, x)` and the
   dep-map row should add it as a same-layer dependency. The
   forward-reference is plain-text (no live link — target file does not exist).
   **Proposed plan item: `l2-floor-elementwise-product-reciprocal`** — floor the
   `elementwise_product` / `reciprocal` L2 entries under the firm L3 cohort, the
   same `l2-floor-under-l3-*` foundation-first pattern as the BLAS-1 cohort. This
   is the constructed-operator-gate analog of the cycle-041 BLAS-1 floor work.

2. **The `dot-l2-leaf-floor-vs-fold-only-design` fork does NOT reach
   `jacobi-smoother`** (recorded for the batch-12 meta-phase's awareness). The
   fork governs whether the BLAS-1 *leaves* get same-named L2 floors or fold-only
   treatment; `jacobi-smoother` is a constructed-operator gate with no fold-parent
   on either codomain, so its L2 floor is unconditional regardless of how the
   fork resolves. *However*, the analogous question DOES apply to the OQ-1
   `elementwise_product` L2 floor: `scal(α,x) = elementwise_product(broadcast(α,N), x)`
   makes `scal` a special case of `elementwise_product`, so if the meta-phase
   adopts a "broadcast-primitive floor" view, the `elementwise_product` floor and
   the `scal` floor interact. Flagged so the meta-phase considers the
   elementwise-cohort floor design alongside the BLAS-1 leaf-vs-fold fork.

3. **`Apply<Transpose=true>` dead-code consumer branch**
   (`palace/linalg/jacobi.cpp:61-69`) — the conjugate-`dinv` Hermitian-transpose
   kernel exists but no consumer instantiates `Apply<true>` (`MultTranspose`
   aliases `Mult`). Recorded as a non-law / recognition-rule caveat, NOT a status
   reduction; ties to OQ
   `reciprocal-elementwise-product-mr-dead-code-transpose-consumer-branch`. No new
   action required at L2 — inherited unchanged from L1.

4. **L2 index intro / Working-Notes refresh** (layer-intro-author's domain, not
   harvester's): the L2 index Working-Notes "Cycle-041 BLAS-1-floor cohort" bullet
   and firm-count tally (currently 12 firm + 1 partly-constructive) will need a
   "cycle-042 constructed-operator-gate floor" note and a count bump to 13 firm.
   The tally is **D11-owned this cycle** (count-ownership partition) — I have
   appended only my dep-map row, body, and SUMMARY entry, and deferred the tally
   and the Working-Notes narrative bullet to D11 / the layer-intro-author.
