...
inputs:
  ...
  - book/src/L3/apply_linop.md (firm; identity-row TEMPLATE — opaque-operator-gate precedent)
  - book/src/L3/krylov-step.md (firm; identity-row TEMPLATE — constructed-operator backfill precedent)
  - book/src/L3/dot.md, book/src/L3/scal.md (firm; thinnest identity-row form)
  - book/src/L3/index.md (dep-map shape; c036 D2 audit verdict at :38-43 routes this backfill)
  - book/src/L1-L0/reciprocal-elementwise-product-mutation-rotation.md (downward leaf-lowering context; referenced plain-text/forward, NOT depended on)
  - cycle-037 dispatch D2 (this dispatch)
  - cited evidence self-verified via tools/citecheck/citecheck.py --anchor this invocation
---

# CYCLE: Formalize jacobi-smoother at L3

## Summary

This dispatch authors `book/src/L3/jacobi-smoother.md` as the iteration-rotation-layer (L3) entry for the Jacobi smoother apply — an **identity-in-form L3 backfill** of the firm L1 [`jacobi-smoother`](book/src/L1/jacobi-smoother.md). The Jacobi smoother's per-call action is a single `elementwise_product` of the damped inverse diagonal `op.dinv = ω·D⁻¹` against the input `x` (`y = op.dinv ⊙ x`, witnessed by `Y[i] = DI[i] * X[i]` at `palace/linalg/jacobi.cpp:38`). The apply is inner-product-free, iteration-free, and reduction-free — there is **no loop-recurrence obstruction in the apply itself** — so it is the thinnest possible constructed-operator gate at L3, structurally identical to the firm `apply_linop` opaque-operator-gate precedent and a sibling of the firm `ksp_solve` / `divfree-projector` constructed-operator backfills. The L3 form is value-thread-isomorphic to the L1 form; the entry lands `firm`. This is the sixth of the six (A) firm identity-in-form L3 backfill candidates the cycle-036 D2 cross-layer-cross-cutter audit verdict identified at `book/src/L3/index.md:39` (the "thinnest constructed-operator gate, one `elementwise_product`" candidate), routed to cycles 036-038+ under OQ `l3-cohort-growth-audit-c036-verdict`.

The entry is defined in **L3 vocabulary** (whole-tensor field operation, no element loop exposed; constructed-operator-gate framing per the firm L3 `apply_linop` / `ksp_solve` precedent), NOT in terms of L2/L1 primitives. The downward lowering is narrated forward L3→L1 as identity-in-form on the constructed-operator-gate apply, annotated in-line (no `L3-L2/` or non-adjacent `L3-L1/` directory, per the cycle-012 non-adjacent-identity convention; the substantive leaf-mutation rotation `elementwise_product → forall_switch` lives at the firm L1>L0 `reciprocal-elementwise-product-mutation-rotation` theme, referenced forward).
...
The full firm chapter is authored inside the `edit:book/src/L3/jacobi-smoother.md` fence above. Key structural decisions:

- **Slug + one-line**: `jacobi-smoother` — diagonal (Jacobi) preconditioner action as a whole-tensor field operation at L3; the thinnest constructed-operator gate, one elementwise product.
- **Signature** (4-space-indented, Haskell `::` form): `jacobi_smoother :: (op: JacobiSmoother[N], x: Tensor[N]) -> Tensor[N]` with `jacobi_smoother op x = op.dinv ⊙ x = (ω · diag(A)⁻¹) ⊙ x`. Shape contract with named axes; the opaque constructed type, the absorbed setup, the three pieces of absent L4 wrapper machinery.
- **Semantics**: inner-product-free / iteration-free / reduction-free single whole-tensor elementwise product; the iteration-rotation marker establishing **no L3 obstruction** (the sharpest contrast in the gate family); the matrix-free-approximate-`dinv` non-law caveat.
- **Algebraic laws**: the six that hold (linearity, zero-vector annihilation, `assemble_diagonal` round-trip, damping absorption, estimated-damping degenerate case, self-transpose), inherited unchanged from L1; three non-laws (dead-code Hermitian non-realisation, no-iteration non-equivalence, representation bit-determinism).
- **Status**: `firm` — value-thread-isomorphic to firm L1; firm-on-positive-structure (no `test-jacobi.cpp` does not gate firm).
- **Evidence**: all citations self-verified via `citecheck --anchor` (`jacobi.cpp:38, :79-93, :99-104, :106-107`; `jacobi.hpp:43`; `ksp.cpp:198-200`; `L3/index.md:13, :39`).

## Supporting evidence

- Identity-row TEMPLATE precedents matched: `book/src/L3/apply_linop.md` (opaque-operator gate, firm), `book/src/L3/krylov-step.md` (constructed-operator backfill, firm), `book/src/L3/dot.md` / `book/src/L3/scal.md` (thinnest identity-row form). All establish: identity-in-form rotation noted in-line, no `L3-L2/` or non-adjacent `L3-L1/` directory, "L3 vs L1 distinction" + "Lowers to" + "Lifts from" sections, layer-coherence-anchor framing.
- The cycle-036 D2 cross-layer-cross-cutter audit (`book/src/L3/index.md:38-43`) is the dispatch rationale: `jacobi-smoother` is the sixth (A) firm identity-in-form L3 backfill candidate, "thinnest constructed-operator gate, one `elementwise_product`" (`:39`).
- The firm L1 home (`book/src/L1/jacobi-smoother.md`, 548 lines, `firm`) supplies the inherited signature/laws/variant-axes; the L3 entry transports them in L3 vocabulary (constructed-operator-gate framing) rather than re-deriving.
- The L1>L0 downward leaf-lowering (`book/src/L1-L0/reciprocal-elementwise-product-mutation-rotation.md`, sub-pattern B) is referenced forward as the substantive rotation — NOT depended on (D3 this cycle is auditing it; this entry does not consume D3's output).
- Citation self-verification: all load-bearing L0 pinpoints run through `tools/citecheck/citecheck.py --anchor` this invocation (`jacobi.cpp:38` → `[ok]` line 38; `:79`/`:80`/`:90-93` → `[ok]`; `:99-104` anchor `Apply(dinv, x, y)` → line 103; `:106-107` → `[ok]`; `jacobi.hpp:43` → `[ok]`; `ksp.cpp:198-200` → `[ok]`; `L3/index.md:13`/`:39` → `[ok]`).

## Open questions / caveats

- **L3 firm-operator count update.** With this landing, the L3 cohort reaches **10 firm + 2 `partial-obstruction`** (firm: `krylov-step` c010, `apply_linop` + `axpy` + `axpby` + `axpbypcz` + `dot` + `nrm2` + `scal` c011, `ksp_solve` c020, `jacobi-smoother` c037; partial-obstruction: `chebyshev` c013, `eigsolve` c024) — plus the parallel cycle-037 D1 `divfree-projector` backfill (a distinct row, appended by D1). The integrator-finalize should reconcile the count in the L3 index Working Notes if both D1 and D2 land. **Not authored here** (one operator per dispatch; the Working-Notes count line is integrator/layer-intro-author territory).
- **`jacobi-smoother-mutation-rotation` L1>L0 theme reference.** (REPAIRED cycle-037 repair phase.) The downward "Lowers to" section's three `jacobi-smoother-mutation-rotation` references (§Downward, §Lowers-to, and the L3-index dep-map cell) are now LIVE links to `../L1-L0/jacobi-smoother-mutation-rotation.md` — the repairer confirmed `book/src/L1-L0/jacobi-smoother-mutation-rotation.md` exists on disk (`ls`) and applied the skill `upgrade-plain-text-ref-to-live-link-when-target-on-disk`. (Originally left plain-text because this dispatch did not verify on-disk presence.)
- **L4 verdict carried, not re-audited.** The "no L4 entry" verdict for `jacobi-smoother` is asserted by analogy to the firm `apply_linop` / `ksp_solve` constructed-operator-gate L4 verdicts (no monadic effect / state-stratification / outer-driver structure). No cycle-010-style dedicated cross-layer L4-candidacy audit exists *specifically* for `jacobi-smoother`; the analogy is strong (the gate is strictly thinner than `apply_linop`, which is itself confirmed-not-needed at L4) but is not a separately-recorded audit verdict. Flagged for a future cross-layer-cross-cutter pass if L4 constructed-operator-gate policy is ever revisited.
- **Layer-intro refresh deferred.** The L3 `index.md` §Semantics overlay (`:11-16`) enumerates the obstruction shapes but does not yet name the constructed-operator-gate sub-family (`ksp_solve` / `eigsolve` / now `jacobi-smoother` / parallel `divfree-projector`) as a coherent vocabulary class. A layer-intro-author refresh distinguishing "obstruction-carrying gates" (`ksp_solve`, `eigsolve`) from "obstruction-free gates" (`jacobi-smoother`) would aid navigation. **Not authored here** (layer-intro-author's domain); noted per harvester discipline.
