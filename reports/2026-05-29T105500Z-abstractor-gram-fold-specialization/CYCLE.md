...
inputs:
  ...
  - book/src/L2-L1/inner-product-fold-specialization.md (firm — sibling precedent: the scalar fold this lifts)
  - book/src/L1/dot.md (firm — the per-cell RHS leaf; arg-1-conjugated convention)
  - book/src/L1/bilinear-form.md (rough-in — the B-weighted hook leaf)
  - palace/linalg/nleps.cpp:524-531 (the sole literal XᴴX Gram-build site; cell body :529)
  - palace/linalg/nleps.cpp:354-362,515-518,520-523,532-535,613-619 (literature anchors, empty-basis, coords, consumer, incremental growth)
---

# CYCLE: L2>L1 theme sketch — gram-fold-specialization

## Summary

The firm L2 `gram` operator (`book/src/L2/gram.md`, cycle-022) is the **all-pairs lift** of the
firm L2 `inner_product` fold: `gram dot X [i,j] = inner_product(X[j], X[i]) = X[j]ᴴ X[i]`. Its
chapter explicitly forward-references "how the L2 all-pairs fold lowers onto Palace's
`nleps.cpp:524-531` double-`linalg::Dot` loop" as forthcoming abstractor work (`gram.md:242-246`).
This theme is that lowering. It lowers the L2 `gram` fold into **per-cell L1 leaves** by reusing
the sibling `inner-product-fold-specialization`'s dispatch keys (conjugation / element-type /
weight) **per Gram cell**, and adds the two structures specific to the matrix lift: (1) the
**double-loop materialization** (`Matrix[k,k] = forall (i,j). leaf(X[i], X[j])`, the `:525-531`
nested `for`), and (2) the **symmetry-exploitation transparent note** + **per-cell pinned
reduction tree** (the load-bearing residue the `gram` IEEE-754 non-law defers here,
`gram.md:166-176`). The rewrite shape: one L2 fold over a `k`-column basis re-fuses downward into
a `k×k` grid of `dot` calls (NLEPS uses the canonical Hermitian hook only); the cross-Gram member
and the B-weighted-hook arm reuse the sibling theme's `bilinear_form` dispatch. **No new
operators** — both sides are existing vocabulary (`gram` firm LHS; `dot`/`tdot` firm + the
sibling theme's already-firm dispatch on the RHS). The theme is `firm` (algebraic; the dispatch
IS the `gram` all-pairs definition law read as a lowering, plus the inherited per-cell
conjugate-pair re-order from the sibling theme).

## Proposed changes

````new:book/src/L2-L1/gram-fold-specialization.md
...
APPEND one row to the "## Theme list" table, after the orthogonalize-composition-lowering row
(line 16). Insert exactly this row (do NOT modify the existing four rows):
...
NOTE for integrator: the parallel cycle-024 `deflate-composition-lowering` abstractor also appends
a row to this table. The two rows are distinct (gram vs deflate); append both, preserving the
chebyshev / linear-combination / inner-product / orthogonalize rows above.
```

```edit:book/src/SUMMARY.md
APPEND one chapter entry under the "# L2 > L1 — Lowering" Part, after the
orthogonalize-composition-lowering entry (line 56). Insert exactly this line (do NOT modify the
existing entries):

- [gram-fold-specialization](./L2-L1/gram-fold-specialization.md)

NOTE for integrator: the parallel cycle-024 `deflate-composition-lowering` abstractor also appends
a SUMMARY entry under this Part. The two entries are distinct (gram vs deflate); append both.
```

## Speculative operators proposed

**None.** This theme proposes no new L1 operators. Both sides of the lowering edge are existing
vocabulary:

- **LHS:** `book/src/L2/gram.md` — firm (cycle-022).
- **RHS:** `book/src/L1/dot.md` (firm; `dot` + `tdot`) and `book/src/L1/bilinear-form.md`
  (rough-in; the B-weighted hook member).

The theme is built entirely by **lifting the already-firm sibling scalar theme**
`inner-product-fold-specialization`'s per-cell dispatch over the two basis index axes — there is
no new operator to hand to harvester. (Per the abstractor discipline: rough-in operators are
fine, but here the lowering edge is between operators that already exist on both sides, so the
correct output is `Speculative L1 operators: None`, exactly as the sibling scalar theme records.)

## Supporting evidence

All Palace L0 ranges self-verified this invocation via `palace-codemap` `read_range` /
`search_text` (paths relative to `reference/`):

- `palace/linalg/nleps.cpp:524-531` — the double-`Dot` Gram-build loop; cell body `SS(i, j) =
  linalg::Dot(GetComm(), X[i], X[j])` on `:529` (search_text `linalg::Dot\(GetComm\(\), X` over
  `nleps.cpp` → exactly one hit at line 529). The sole literal `XᴴX` build site.
- `palace/linalg/nleps.cpp:504-537` — the `deflated_solve` lambda enclosing the build; block
  comment `SS = (B - A T^-1 U) = - X^* X S^-1` (`:512-513`); the consumer Schur/LU solves
  (`:532-535`).
- `palace/linalg/nleps.cpp:515-518` (empty-basis early-return), `:520-523` (coordinate extraction
  `x2(j) = b2(j) - linalg::Dot(GetComm(), x1, X[j])`), `:561-569` (residual-deflation second
  consumer), `:613-619` (incremental basis growth `X.resize(k+1); X[k]=v; ... k++`), `:354-362`
  (Jarlebring/Koskela/Mele 2018 + Effenberger 2013 + SLEPc-NEP minimality-index-1 literature
  anchors). All self-verified via read_range.
- `search_text fullPivLu|Gram|deflat|SS\(` over `palace/**/*.cpp` → hits only in `nleps.cpp`
  (confirming the single-Gram-build-site coverage caveat inherited from `L2/gram`).

Artifact anchors read this invocation: `book/src/L2/gram.md` (the firm LHS, including its explicit
forward-reference to this theme at `:242-246`); `book/src/L2-L1/inner-product-fold-specialization.md`
(the firm sibling scalar theme this lifts); `book/src/L1/dot.md`, `book/src/L1/bilinear-form.md`
(the RHS leaves); `book/src/L2/inner_product.md:46-102` (the pinned conjugation convention);
`book/src/L2/index.md:38,59` (the firm `gram` dep-map row); `book/src/SUMMARY.md:51-56` (the L2>L1
Part structure); `book/src/L2-L1/index.md:11-16` (the theme-list table).

## Open questions / caveats

- **New OQ to surface: `gram-percell-dot-vs-fused-matmul-tree-loadbearing`.** Palace builds the
  Gram as `k²` independent per-cell `linalg::Dot` reductions (no cross-cell accumulation). A burn
  port using a single fused `XᴴX` matmul would pin a different (matrix-level) reduction tree and
  would not bit-reproduce Palace's per-cell-`Dot` Gram. Whether the per-cell-`Dot` structure is
  load-bearing (NLEPS depends on exact Gram bits) or transparent (NLEPS only needs LU-solve
  tolerance) is not resolvable from the source alone — needs an NLEPS convergence-sensitivity
  analysis. Recommend a `lowering-verifier` / same-layer-cross-cutter follow-up. **Not blocking**:
  the value-level lowering is firm regardless of the answer; this is the bit-reproduction question.

- **Carry-forward from the sibling scalar theme — all inherited, none folded in.** Reviewed
  `inner-product-fold-specialization` §Open questions: its lifting note (mirrored here for the
  matrix lift), its weighted-member two-stage reduction-tree caveat (inherited verbatim by the
  B-weighted Gram arm; tracked under existing OQ `apply-linop-lowering-verifier-audit-cohort`), and
  its conjugate-pair re-order per-site audit OQ (this theme adds the data point that
  `nleps.cpp:529` is observable-unweighted, already in the scalar theme's caller inventory). This
  theme consumes none of them as blockers and adds no new burden to them.

- **Forward-reference closure (recommendation, NOT actioned — dispatch-phase write discipline).**
  `book/src/L2/gram.md:242-246` forward-references this theme as "forthcoming". Once integrated,
  that cross-reference resolves; a lifter / layer-intro-author cross-reference refresh naming
  `gram-fold-specialization` is the standard follow-up, not abstractor work.

- **No reverse-direction (lifting) content in the formal chapter body.** Per the high→low
  layer-definition discipline, the chapter narrates only L2 → L1; the lift-direction note (how an
  L1 leaf grid lifts up to the `gram` fold, and what it loses) lives in the chapter's §Open
  questions as a working note, not in the rewrite-rule body. Confirmed: the formal sections (L2
  form / L1 form / dispatch rewrite / re-order / summation-order / applicability / justification)
  are all narrated forward.

- **Shared-file coordination (integrator).** `book/src/L2-L1/index.md` + `book/src/SUMMARY.md` are
  shared with the parallel `deflate-composition-lowering` L2>L1 abstractor. My proposed rows/entries
  are explicitly append-only and distinct (gram vs deflate); the proposed-changes blocks above
  carry inline integrator notes to apply both without collision.
