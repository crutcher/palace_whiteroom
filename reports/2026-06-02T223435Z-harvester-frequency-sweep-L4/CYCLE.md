...
inputs:
  - book/src/L4/assemble_frequency_operator.md (firm cycle-069 D1 — the per-ω operand A(ω) this sweep maps the rebuild over)
  - book/src/L4/solve_family.md (the fixed-operator sibling map; frequency_sweep is the operator-VARYING counterpart — solve_family's `per-element` superset scope boundary, named)
  - book/src/L4/fold_solve.md (the state-threaded fold sibling; shape-precedent for a per-element driver combinator)
  - book/src/L4/ksp_solve.md (the per-member solve cap mapped, `ksp_solve :: OpParams -> Inputs -> SimState`)
  - palace/drivers/drivensolver.cpp (the per-ω loop L0 anchor — on-disk-confirmed this dispatch)
  - dispatch D2 cycle-070 forward-references this canonical slug for its L4>L3 theme `frequency-sweep-dissolution` (NOT authored here)
---

# CYCLE: Formalize frequency_sweep at L4

## Summary

`frequency_sweep` is the driven (frequency-domain) pipeline's **solve-half own single-witness L4 form**: a per-ω map over a frequency family where **each member REBUILDS the operator before solving** — `A(ω) = assemble_frequency_operator fam ω`, then `ksp_solve` per member. It is the **operator-VARYING** sweep, the named scope boundary that `solve_family` (the fixed-operator map) explicitly scopes the driven pipeline *out* of (`solve_family.md:65,90,137,163` — `SetOperators` INSIDE the loop, operator rebuilt per-element). This closes the **last pipeline-half L4 gap**: with `assemble_frequency_operator` (the driven assemble half, cycle-069) firm and `frequency_sweep` (the driven solve half) landing here, the whole assemble+solve deliverable reaches L4 across all 5 pipelines' representable shells. Status: **`firm`** (firm-on-positive-structure escape — every law is a read-off syntactic identity on the single positive driven loop + the firm `assemble_frequency_operator` operand + the firm `ksp_solve` cap; the operator-rebuild-per-member non-law is read directly off `SetOperators`-inside-the-loop). It is the driven feature's OWN single-witness form (honoring batch-21 meta decision 4: NOT framed as a shared `map_solve` cross-pipeline parent — `map_solve` is on the STOP-PROPOSING list).

L0 anchors **on-disk-confirmed this dispatch** via `palace-codemap` `read_range` + `tools/citecheck/citecheck.py --anchor` against the on-disk `reference/palace/palace/drivers/drivensolver.cpp`:
- `:168-170` frequency loop `for (std::size_t omega_i = ...; omega_i < omega_sample.size(); omega_i++)` (the map domain)
- `:91-93` the fixed operator basis `{K, C, M}` assembled ONCE before the sweep (`GetStiffnessMatrix`/`GetDampingMatrix`/`GetMassMatrix`)
- `:175` `A2 = GetExtraSystemMatrix<ComplexOperator>(omega, ...)` (the ω-dependent extra term)
- `:176` `A = GetSystemMatrix(1.0+0.0i, 1i*omega, -omega*omega+0.0i, K, C, M, A2)` (the per-ω operator REBUILD inside the loop)
- `:180` `ksp.SetOperators(*A, *P)` (the per-ω operator CAPTURE inside the loop — the operator-VARYING signature, the scope boundary vs `solve_family`)
- `:194` `space_op.GetExcitationVector(excitation_idx, omega, RHS)` (the per-ω RHS)
- `:196` `ksp.Mult(RHS, E)` (the per-member solve = `ksp_solve op_ω inp_ω`)

L4 signature:

    frequency_sweep :: FrequencyOperatorFamily[N] -> [Scalar] -> [SimState]
    frequency_sweep fam omegas =
      map (\omega -> ksp_solve (assemble_frequency_operator fam omega) (rhs_at fam omega)) omegas
...
The full firm chapter body is inside the `edit:book/src/L4/frequency_sweep.md` proposed-changes fence above. Summary of the apparatus:

- **Slug + one-line**: `frequency_sweep` — the driven per-ω frequency-sweep outer-driver combinator; map over a swept frequency family where each member rebuilds the operator before solving.
- **Signature**: `frequency_sweep :: FrequencyOperatorFamily[N] -> [Scalar] -> [SimState]`, with `frequency_sweep fam omegas = map (\w -> ksp_solve (assemble_frequency_operator fam w) (rhs_at fam w)) omegas`. Shape contract: `fam` the captured-once `readonly` operator BASIS; `[Scalar]` the swept frequencies; `[SimState]` the order-preserving collected solutions; `op_w = assemble_frequency_operator fam omega` the per-member REBUILT operator (the load-bearing per-element value).
- **Algebraic laws** (only those that hold): (1) concatenation-homomorphism (holds despite operator-varying — the rebuild is index-local); (2) per-member operator rebuild / NO `SetOperators`-hoist (the load-bearing non-hoist distinguishing from `solve_family`); (3) member-independence / order-preservation; (4) empty-family degenerate. Explicit non-laws: the `SetOperators`-hoist (the load-bearing non-law), per-member operator fusion, solve fusion, readout-linearity.
- **Dependencies**: `assemble_frequency_operator` (per-member operand rebuild), `ksp_solve` (per-member cap), `iterate-while` (§3.7 pure-map degenerate); contrast-siblings `solve_family` + `fold_solve`; concepts `state-stratification` / `solve-monad` / `derived-view-hoisting` / `variant-absorption`.
- **Status**: `firm` (firm-on-positive-structure escape; single-witness-driven by design, 1-of-5 pipelines, NOT a shared `map_solve` parent).
- **Evidence**: the single driven uniform sweep `drivensolver.cpp:80,91-93,168-170,172,175,176,180,194,196` (all on-disk-confirmed) + firm `assemble_frequency_operator` / `ksp_solve` / `solve_family`-scope-boundary endpoints.

## Supporting evidence

- L0 anchors self-verified on-disk via `tools/citecheck/citecheck.py --anchor` against `reference/palace/palace/drivers/drivensolver.cpp`: `:80` (`omega_sample`), `:91-93` (`GetStiffnessMatrix`/`GetDampingMatrix`/`GetMassMatrix`), `:168-170` (frequency loop `omega_i`), `:172` (`omega_sample[omega_i]`), `:175` (`GetExtraSystemMatrix`), `:176` (`GetSystemMatrix` rebuild), `:180` (`SetOperators` inside loop), `:194` (`GetExcitationVector`), `:196` (`ksp.Mult`). All returned `[ok]`.
- Firm-endpoint cross-references: `book/src/L4/assemble_frequency_operator.md` (firm c069 D1, the per-member operand + single-pipeline-by-design precedent), `book/src/L4/ksp_solve.md` (firm c048, the mapped cap), `book/src/L4/solve_family.md` (the `per-element` scope boundary this formalizes), `book/src/L4/fold_solve.md` (the fold sibling + the adaptive-driven-is-fold-not-map split).
- Couple-aware: the L4>L3 dissolution `book/src/L4-L3/frequency-sweep-dissolution.md` is authored by cycle-070 **D2** (canonical slug confirmed `frequency-sweep-dissolution` per dispatch scope); referenced as a live link in `lowers_to` and `## Lowers to` (D2 creates the file this same cycle — if D2 lands after per-report apply, the integrator may need the stub-materialization path; flagged below).

## Index ownership (this dispatch is SOLE owner of the L4/index.md consolidated tally)

Applied in the `edit:book/src/L4/index.md` block above:
- (1) My own dep-map TABLE row for `frequency_sweep` — appended after the `fold_solve` row (line 97; the table is chronological-after-the-alpha-head, so the new row lands at the cohort tail in row order; the alpha-position-within-kind-grouping directive-3 convention is honored by the SUMMARY insert and the cohort bullet).
- NOTE for the integrator: the consolidated tally prose (the `## Vocabulary cohort` "Firm at L4 (13 + 4 outer-driver)" header count → **14 + 4 outer-driver**, plus a `frequency_sweep` cohort bullet in alpha position after the `fold_solve`/`fe_assemble` neighbors) is the SOLE-OWNER tally I must bump 13→14. I have authored the TABLE row + the SUMMARY insert; the running-count header bump + cohort bullet are detailed in the Open-questions / integrator note below because the `## Vocabulary cohort` header prose edit needs a surgical insert the integrator applies from this channel (the header line `**Firm at L4 (13 + 4 outer-driver)**` → `**Firm at L4 (14 + 4 outer-driver)**` and a new bullet). See the explicit edit fragments below.

### Tally edit fragments (SOLE-OWNER; integrator applies)

Header count bump (`book/src/L4/index.md` line 32):

```text
**Firm at L4 (13 + 4 outer-driver)** —
↦
**Firm at L4 (14 + 4 outer-driver)** —
```

New cohort bullet (`book/src/L4/index.md`, inserted in alpha position within the firm cohort sub-list — after the `[`fold_solve`]` bullet (line 45) and before `[`solve_family`]` (line 46); `fold_solve` < `frequency_sweep` < `solve_family`):
...
(Both fragments are SOLE-OWNER tally writes this cycle; D2/D3 defer all L4/index tally writes to D1 per the dispatch scope. I provide them as explicit surgical-insert fragments rather than a full-file rewrite to keep the parallel-safe append discipline; the integrator applies the header bump + the alpha-positioned bullet insert.)

## Open questions / caveats

- **L4>L3 dissolution file ordering (couple-aware, non-blocking).** This entry's `lowers_to` frontmatter + `## Lowers to` prose live-link `../L4-L3/frequency-sweep-dissolution.md`, which **cycle-070 D2 authors this same cycle** (canonical slug confirmed per my dispatch scope). If D2's per-report apply lands after this entry's, the live link is momentarily dangling; per CLAUDE.md §Methodology invariants "Integration may materialize implied components as stubs" the integrator may stub it, or order D2 before/with D1's finalize. The slug `frequency-sweep-dissolution` is the planner-stated canonical (not invented). The SUMMARY.md L4>L3 section already lacks this entry — D2's SUMMARY insert is D2's, not mine.
- **`rhs_at` is an absorbed helper, not a standalone chapter.** The per-member RHS construction `space_op.GetExcitationVector(excitation_idx, omega, RHS)` (`drivensolver.cpp:194`) is rendered as the absorbed `rhs_at :: FrequencyOperatorFamily[N] -> Scalar -> Inputs` closure (ω-dependent excitation), parallel to how `solve_family` absorbs RHS-construction into `[Inputs]`. No standalone chapter is warranted (combinator-as-entry); flag only if a downstream consumer pulls on the driven excitation vector as a named verb.
- **The `map_solve` shared-form question stays closed.** Per batch-21 meta decision 4, `map_solve` (a generalized `(operator, rhs)`-family map shared across pipelines) is on the STOP-PROPOSING list. `frequency_sweep` formalizes the driven instance of `solve_family`'s `per-element` axis value WITHOUT promoting the shared parent. If a future cycle finds a SECOND operator-varying-map witness (none exists by design — transient is a fold, adaptive driven is a fold, the other two are fixed-operator maps), the `map_solve` shared-form question could re-open; absent that, it stays closed. (No OQ-append needed — the scope is settled by design; recorded here for the integrator's awareness.)
- **Vocabulary cohort header is now `14 + 4 outer-driver`** and `solve_family` remains the lone `rough-in (test-coverage-bounded)` row — the "Rough-in at L4 (1)" section count is unchanged by this dispatch (frequency_sweep lands `firm`, not rough-in).
