...
inputs:
  ...
  - book/src/L3/index.md:41,53 (cycle-036 D2 L3-cohort-growth audit; elementwise_product is one of the four remaining (A) identity-in-form backfills)
  - book/src/L3/assemble-diagonal.md + book/src/L3/jacobi-smoother.md (cycle-037 firm; freshest L3 identity-row template precedents)
  - book/src/L3/scal.md + book/src/L3/apply_linop.md (cycle-011 BLAS-1 / opaque-operator-gate leaf-primitive precedents)
  - cross-cutter L3-cohort-growth audit reports/2026-05-31T200500Z-cross-layer-cross-cutter-l3-cohort-growth-audit/CYCLE.md
---

# CYCLE: Formalize elementwise_product at L3

## Summary

This dispatch authors `book/src/L3/elementwise_product.md` — the **iteration-rotation-layer (L3)** entry for the Hadamard (elementwise) binary product, `result = a ⊙ b`. The operator already has a firm L1 home (`book/src/L1/elementwise_product.md`, the diagonal-operator-action / per-call smoother kernel) whose signature is whole-tensor / no-element-loop and therefore L3-native by construction. The L3 entry is an **identity-in-form layer-coherence backfill**: the L3 body is value-thread-isomorphic to the firm L1 form (per-element binary map, no loop-recurrence obstruction), so the rotation L3 → L1 is the identity on the operator's body and signature. It enacts the cycle-036 D2 cross-layer-cross-cutter L3-cohort-growth audit's **(A) identity-in-form** verdict (`book/src/L3/index.md:41`, `:53`) — `elementwise_product` is one of the four remaining (A) firm backfills after `assemble-diagonal` and `jacobi-smoother` landed cycle-037. Status is **`firm`** (firm-on-positive-structure): every algebraic law is a syntactic identity on the positive multiply lambdas at the canonical `BaseDiagonalOperator::Mult` site, so the absent dedicated test does not gate firm — the same precedent as `apply_linop` / `assemble-diagonal` / `scal`. The entry carries **no sequential obstruction**: elementwise multiplication is embarrassingly parallel, the sharpest-contrast clean field operation against the `chebyshev` / `eigsolve` `partial-obstruction` rows.

## Proposed changes

```new:book/src/L3/elementwise_product.md
---
layer: L3
...
- **Inverse (multiplicative)**: there is no general two-sided inverse — `elementwise_product(a, b) = 𝟙` has the solution `b[i] = 1 / a[i]` only when **every** `a[i] ≠ 0`. The partial inverse is realised by composing with `reciprocal` (the elementwise-inverse sibling; an L3 backfill candidate per the c036 (A) list — referenced as plain text, not yet authored): `elementwise_product(a, reciprocal(a)) = 𝟙` when `a` has no zero entries. This is the algebraic shape of the `assemble_diagonal → reciprocal → elementwise_product` preconditioner chain.
...
- `reciprocal` (the elementwise self-map; an L3 (A) backfill candidate per `book/src/L3/index.md:53`, not yet authored — plain text) composes with `elementwise_product` to form the `assemble_diagonal → reciprocal → elementwise_product` diagonal-preconditioner apply chain.
...
The full firm chapter body is authored inside the `new:book/src/L3/elementwise_product.md` fence above. Key facets:

- **Slug + one-line**: `elementwise_product` — whole-tensor Hadamard (elementwise) binary product `result = a ⊙ b` at the iteration-rotation layer; the diagonal-operator-action field operation, broadcast-generalisation of `scal`, apply kernel of `jacobi-smoother`/`chebyshev`.
- **Signature**: `elementwise_product :: (a: Tensor[N], b: Tensor[N]) -> Tensor[N]` (Haskell `::` strawman form; bunsen-style named-axis shape contract; conjugate variant `elementwise_product_conj :: (ComplexTensor[N], ComplexTensor[N]) -> ComplexTensor[N]` as a sub-axis, not a separate primitive). Identical in form to the L1 signature — identity-in-form rotation.
- **Semantics**: element-local, reduction-free, rank-local; no iteration view (leaf field operation, not a step body); **no sequential obstruction** (embarrassingly parallel — the sharpest-contrast clean field operation against the `chebyshev`/`eigsolve` partial-obstructions).
- **Algebraic laws**: the ten that hold at L1 transport unchanged (commutativity, associativity, all-ones identity, all-zeros absorption, distributivity, scalar absorption, broadcast-subsumption-of-`scal`, negation, diagonal-operator-action identity, conjugation involution) + the inherited non-laws (idempotence, multiplicative inverse, conjugate-variant commutativity, reduction-reordering vacuity).
- **Dependencies**: none at L3 (leaf); sibling-subsumption (not dependency) of `scal` and `apply_linop`; consumers `jacobi-smoother`/`chebyshev`/L2-`chebyshev-iteration`.
- **Status**: `firm` (firm-on-positive-structure; no caveat — Hadamard product is exact per element, unlike `assemble-diagonal`'s load-bearing exact-vs-approximate non-law).
- **Evidence**: L1 home + sibling L3 precedents + the canonical `BaseDiagonalOperator::Mult` / `MultHermitianTranspose` / `jacobi.cpp` consumer-duplicate L0 sites (transitive through L1; all re-verified on-disk this dispatch).

## Supporting evidence

All load-bearing citations were self-verified on-disk this dispatch via `tools/citecheck/citecheck.py --anchor`:

- `reference/palace/palace/linalg/operator.cpp:478-487` (`BaseDiagonalOperator` at :479), `:486` (`Y[i] = D[i] * X[i]`), `:489-507` (`BaseDiagonalOperator` complex), `:504-505` (`YR`), `:545-568` (`MultHermitianTranspose`), `:564-565` (`YR` conjugate) — all `[ok]`.
- `reference/palace/palace/linalg/jacobi.cpp:30-39` (`forall_switch`), `:38` (`Y[i] = DI[i] * X[i]`), `:74-93` (`SetOperator`) — all `[ok]`.
- `book/src/L1/elementwise_product.md:41` (`scal`), `:43` (`Algebraic laws`), `:99` (`firm`) — all `[ok]`.
- `book/src/L3/index.md:12` (`Whole-tensor field operations`), `:41` (`elementwise_product` / `normalize`), `:53` (`reciprocal`) — all `[ok]`.
- `book/src/L3-L2/krylov-step-body-identity.md:97` (`L3-native`) — `[ok]`.

Forward-reference hygiene (per the rough-in-forward-reference convention; `ls`-verified):
- `book/src/L3/reciprocal.md`, `book/src/L3/normalize.md`, `book/src/L3/divfree-projector.md` do **NOT** exist on disk → references to the L3 `reciprocal` sibling are written **plain-text** (no live link).
- `book/src/L1/reciprocal.md` exists → live link used.
- `book/src/L1-L0/reciprocal-elementwise-product-mutation-rotation.md` exists → live link used.
- All concept pages (`elementwise-product`, `variant-absorption`, `sequential-obstruction`, `constructed-operators`) exist → live links used.

SUMMARY.md insertion: `elementwise_product` placed after `scal` (the BLAS-1 leaf cohort) and before `jacobi-smoother` (its first consumer) — grouping the elementwise leaf with the field operations it generalises and feeds.

## Open questions / caveats

- **L3 `reciprocal` / `normalize` / `divfree-projector` backfills remain** (the other three of the six (A) identity-in-form candidates per `book/src/L3/index.md:53`). The `elementwise_product` non-law "multiplicative inverse" references the L3 `reciprocal` as plain text; once `book/src/L3/reciprocal.md` lands, the `reciprocal` plain-text references in this entry (§Algebraic-laws non-law, §Dependencies) and in the index dep-map row become eligible for live-link upgrade (skill `upgrade-plain-text-ref-to-live-link-when-target-on-disk`). Tracked under OQ `l3-cohort-growth-audit-c036-verdict`.
- **L3 firm-operator count** after this lands: **12 firm + 2 `partial-obstruction`** (adds `elementwise_product` to the cycle-037 count of 11 firm). The index Working-Notes line at `book/src/L3/index.md:53` records "Four of the six (A) backfills remain (`reciprocal`, `elementwise_product`, `normalize`, `divfree-projector`)" — after this dispatch, three remain (`reciprocal`, `normalize`, `divfree-projector`). The layer-intro-author should refresh that count line and add a cycle-038 Working-Note bullet (out of harvester scope — noted here per the "layer intros are layer-intro-author's job" discipline).
- **L2 `elementwise_product` entry**: deliberately absent (the L3>L1 hop is direct, mirroring `apply_linop` / `assemble_diagonal` / `scal`). The primitive is referenced from the L2 `chebyshev-iteration` composition (`dinv ⊙ r`) but does not get a standalone L2 entry — no algebraic novelty in the L2 rotation. No action needed; recorded for disambiguation.
