...
scope: L2 operator: axpby
status: pending
inputs:
  - book/src/L1/axpby.md (firm; source-of-truth L1 leaf — signature, 9 laws, 4 non-laws, 2 variant axes)
  - book/src/L3/axpby.md (firm; iteration-rotation rendering — the L3 consumer this floor supports)
  - book/src/L2/linear_combination.md (firm cycle-018; fold-parent — axpby = arity-2 specialization)
  - book/src/L2/scal.md (firm cycle-041 D3; sibling floor pattern this entry mirrors)
  - book/src/L2/index.md (dep-map table; §"Fold-cohort boundary"; the leaf-vs-fold fork note)
  - dispatch: cycle-043 D4 under l2-floor-under-l3-leaf-cohort (fork resolved: keep leaf-floor (b))
  - L0 anchors (self-verified via citecheck --anchor + on-disk sed, 2026-06-01): palace/linalg/vector.hpp:130-131, :309-311; vector.cpp:726-730, :732-737, :739-743
integrated_at: 2026-06-01T140000Z
integration_commit: 3f9a7d0
integration_notes: "cycle-043 batch integration (cohort-completing L2-floor build); D4 L2 axpby floor (firm; arity-2 fold-member); applied clean; see reports/2026-06-01T140000Z-integrator-finalize-cycle-43/CYCLE.md + cycle-043 STAGING row."
---

# CYCLE: Formalize axpby at L2

## Summary

`axpby` is the BLAS-1 fused two-scalar two-vector update `y ← α·x + β·y`, firm at L1
(`book/src/L1/axpby.md`, cycle-003) and L3 (`book/src/L3/axpby.md`, cycle-011), but with
no L2 floor entry present — the L3 leaf rested on the L1 leaf directly and `axpby` was
referenced at L2 only as the **arity-2 specialization** of the firm
`linear_combination` fold (`book/src/L2/linear_combination.md:70`,
`axpby(α, x, β, y) = linear_combination [(α, x), (β, y)]`). This dispatch (cycle-043 D4)
floors `axpby` at L2 as a **thin identity-in-form leaf floor** under the foundation-first
directive `l2-floor-under-l3-leaf-cohort`, mirroring the co-dispatched D3 `scal`/`axpy`
sibling floors. The leaf-vs-fold design fork is resolved in favor of keeping the
leaf-floor reading (b): `axpby` has its own L2 chapter, cited as the arity-2 *member of*
`linear_combination` but **NOT merged** (the §"Fold-cohort boundary" do-NOT-merge note is
load-bearing). The L2↔L1 rotation is identity-in-form (whole-tensor in / whole-tensor out
at both layers; the only fusion content — the single aligned `add(α,x,β,y,y)` pass — is
the arity-2 case of the fold's §"Fusion note", deferred to the fold-parent, NOT
re-authored here). The nine algebraic laws and four non-laws transport from L1 unchanged;
the output-aliasing variant axis belongs to the FOLD, not to this leaf.

## Proposed changes

```new:book/src/L2/axpby.md
---
layer: L2
operator: axpby
firmness: firm
...
The full firm chapter body is authored inside the `new:book/src/L2/axpby.md` block above.
Key apparatus:

- **Slug + one-line**: `axpby` — the L2 fusion-rotation rendering of the fused
  two-scalar two-vector update `y = α·x + β·y`; the arity-2 member of the
  `linear_combination` fold (cited, not merged).
- **Signature** (`axpby :: Scalar -> Tensor[N] -> Scalar -> Tensor[N] -> Tensor[N]`) with
  bunsen-style named-axis shape contract (`N` shared across `x`, `y`, result; scalar pair
  `(α, β)` sharing one type and the vector element type).
- **Semantics**: element-local, reduction-free, rank-local, pure/out-of-place; the single
  fused pass is the arity-2 case of the fold's fusion note (deferred to fold-parent).
- **Algebraic laws**: the nine that hold at L1 transport unchanged (subsumption of `axpy`,
  the two `α`/`β` identities, bilinearity, the two distribution laws, scalar absorption,
  the chained-collapse law) + the fold-specialization identity; four inherited non-laws
  (tensor-commutativity, associativity-ill-typed, FP-summation-order, scal+axpy fusion
  bit-identity).
- **Dependencies**: none same-layer (leaf); fold-parent `linear_combination` (cited, not
  merged); concept `scalar-promotion`; sibling subsumptions (`axpy` β=1, `scal` α=0/β=0,
  `axpbypcz` γ=0).
- **Status**: `firm` (firm-on-positive-structure; L2 floor backfill cycle-043 D4 under
  `l2-floor-under-l3-leaf-cohort`, leaf-floor fork resolved (b)).
- **Evidence**: L1/L3/fold-parent/sibling-floor artifact citations + five self-verified
  L0 anchors in `palace/linalg/vector.{hpp,cpp}`.

## Supporting evidence

- **L0 anchor self-verification (2026-06-01)**: all five `AXPBY`-family anchors verified
  via `python3 tools/citecheck/citecheck.py <path:lo-hi> --anchor AXPBY` (all `1 ok, 0
  failing`) and confirmed on-disk:
  - `vector.hpp:130` — `// In-place addition (*this) = alpha * x + beta * (*this).`;
    `:131` — `void AXPBY(std::complex<double> alpha, const ComplexVector &x, std::complex<double> beta);`
  - `vector.hpp:309` — `// Addition y = alpha * x + beta * y.`; `:310-311` — template decl
    `AXPBY(ScalarType alpha, const VecType &x, ScalarType beta, VecType &y)`.
  - `vector.cpp:726-730` — real-real `AXPBY` → `add(alpha, x, beta, y, y)`.
  - `vector.cpp:732-737` — complex-complex `AXPBY` → `y.AXPBY(alpha, x, beta)`.
  - `vector.cpp:739-743` — real-scalar-on-complex `AXPBY` → `y.AXPBY(alpha, x, beta)`
    (implicit promotion).
- **Pattern source**: mirrors the firm `book/src/L2/scal.md` (cycle-041 D3) sibling
  leaf-floor structure exactly (frontmatter `fold_parent`, §Context fold-membership-cited-
  not-merged framing, §Status firm-on-positive-structure + floor-backfill provenance,
  §Lowers-to / §Lifts-from in-line identity-rotation annotation).
- **Fork resolution**: dispatch directs keeping leaf-floor (b), consistent with the c042
  cross-cutter audit recommendation recorded at `book/src/L2/index.md` §Working-Notes
  (`dot-l2-leaf-floor-vs-fold-only-design`).

## Open questions / caveats

- **COUNT-OWNERSHIP (deferred to D2, per dispatch)**: this report appends ONLY the
  `axpby` dep-map row (after the `scal` row), the chapter body, and the SUMMARY entry. It
  does **NOT** touch the L2/index consolidated firm running-count tally — D2 owns it this
  cycle. The integrator should reconcile the absolute firm count when D2's tally lands; do
  not increment from this report.
- **axpby L3 staleness → c044 sweep (NOT here)**: per dispatch, the firm
  `book/src/L3/axpby.md` (cycle-011) predates the firm `linear_combination` fold-parent
  (cycle-018) and the L2 floor cohort, so its dependency framing ("no L2 intermediate
  because the BLAS-1 primitives are L1 leaves not L2 compositions",
  `book/src/L3/axpby.md:6,101,118`) is now stale — the L3 form should lower to the present
  L2 floor, not skip to L1. This is a follow-up sweep item for cycle-044 (re-anchor the
  L3 BLAS-1 cohort `lowers_to` framing to the new L2 floors), **not in scope here**.
  Surfaced for the OQ ledger.
- **Forward-references left plain-text (intentional)**: `L2-L1/axpby-fusion` and
  `L3-L2/axpby-body-identity` do not yet exist (D7 this cycle authors the L2>L1 themes;
  the L3-L2 theme is not yet scheduled — only `scal-body-identity` exists among the BLAS-1
  cohort). Referenced as plain text per the
  `rough-in-forward-reference-must-be-plain-text-not-live-link` convention; upgrade to live
  links when the targets land.
- **`concepts/axpby` does not exist**: only `concepts/axpy` exists; an `axpby` concept
  page (BLAS-1 background + call-site role) is a candidate but not blocking. Noted for the
  plan, not authored here.
- **Index intro refresh (layer-intro-author's job)**: the L2/index §Working-Notes
  leaf-vs-fold fork note (`dot-l2-leaf-floor-vs-fold-only-design`) and the
  fold-parented-floors §motif bullet now cover three floors (`dot`/`nrm2`/`scal`) plus the
  arity floors (`scal`/`axpby` landed, `axpy` co-dispatched, `axpbypcz` pending); a
  consolidated refresh naming the arity-floor cohort is a layer-intro-author follow-up,
  flagged not enacted.
