...
scope: L2 operator: axpy
status: pending
inputs:
  - book/src/L1/axpy.md (firm; source of truth — signature, six laws, two inherited non-laws, variant axes, L0 evidence chain)
  - book/src/L3/axpy.md (firm cycle-011; the L3 consumer this floor supports)
  - book/src/L2/scal.md (firm cycle-041 D3; the sibling arity-1 floor — exact template; mirrored)
  - book/src/L2/linear_combination.md (firm cycle-018; the fold-parent — cited, NOT merged)
  - book/src/L2/index.md (L2 Part overview + dep-map + §"Fold-cohort boundary" load-bearing do-NOT-merge note + leaf-vs-fold fork (b)-ratified)
...
# CYCLE: Formalize axpy at L2

## Summary

This dispatch (cycle-043 D3) lands the **L2 fusion-rotation floor** for the BLAS-1 `axpy` leaf (`y ← α·x + y`) under the foundation-first directive `l2-floor-under-l3-leaf-cohort`. The firm L3 [`axpy`](../L3/axpy.md) (cycle-011) currently skips a layer down to L1 — there is no adjacent same-named L2 parent. Per the methodology invariant **Identity-lowerings still require both L levels** (CLAUDE.md §Methodology invariants), the L3 leaf must rest on a present L2 floor. `axpy` is the **arity-2 member of the firm [`linear_combination`](./linear_combination.md) fold** (`scal`/`axpy`/`axpby`/`axpbypcz` = arity-1/2/2/3; `axpy(α,x,y) = linear_combination [(α,x),(1,y)]`, second coefficient fixed to 1) — cited as a fold-specialization but **NOT merged** (the fold-cohort boundary is load-bearing for the one-to-one L1↔L0-symbol mutation rotation). This is a **thin identity-in-form floor**: value-thread-isomorphic to the firm L1 leaf, six laws + two inherited non-laws (with the IEEE-754 FP-summation non-law made explicit at L2 — three non-laws total in the body) + two variant axes inherited unchanged, the only fusion content deferred to `linear_combination` §"Fusion note". It mirrors the cycle-041 [`L2/scal.md`](./scal.md) sibling-floor precedent exactly (the arity-1 floor that already landed under the same directive). The leaf-vs-fold fork is **RESOLVED** (keep leaf-floor (b), batch-12 meta-phase ratification per the c042 cross-cutter audit); this entry presupposes the (b) "same-named floor" realization, consistent with the cohort. **Firm-on-positive-structure**: every law is a syntactic identity on the small fully-present `AXPY` free-function + `ComplexVector::AXPY` surface, so the absence of a dedicated `axpy` unit test does not gate firm.

## Proposed changes

```new:book/src/L2/axpy.md
---
layer: L2
operator: axpy
firmness: firm
...
The full firm chapter body is authored inside the `new:book/src/L2/axpy.md` proposed-changes block above. Key shape:

- **Slug + one-line**: `axpy` at L2 — fusion-rotation rendering of `y_new = α·x + y_old`; the arity-2 member (second coeff fixed to 1) of the `linear_combination` fold, cited not merged.
- **Signature**: `axpy :: Scalar -> Tensor[N] -> Tensor[N] -> Tensor[N]`, positional, identity-in-form to L1, with bunsen-style named-axis shape contract (`α` scalar; `x`, `y` `Tensor[N]` read-only; result `Tensor[N]`).
- **Semantics**: element-local, reduction-free, rank-local, pure/out-of-place; single aligned-pass fusion note deferred to `linear_combination` §"Fusion note"; the `α == 1.0` L0 transparent trick erased at L1.
- **Algebraic laws**: the six L1 laws inherited verbatim + the fold-specialization identity `axpy(α,x,y) = linear_combination [(α,x),(1,y)]`; three non-laws (tensor-arg non-commutativity, ternary non-associativity, IEEE summation non-associativity).
- **Dependencies**: no same-layer constituents (leaf); fold-parent `linear_combination` (cited NOT merged); concepts `scalar-promotion` + `axpy`; sibling subsumptions `axpy = axpby(·,·,1,·)` and `scal = axpy(·,·,0)`.
- **Status**: `firm` (firm-on-positive-structure; leaf-vs-fold fork resolved keep-(b)).
- **Evidence**: firm L1/L3/fold-parent/sibling-floor artifact cites + six self-verified L0 anchors.

## Supporting evidence

- **Template mirrored**: `book/src/L2/scal.md` (cycle-041 D3) — the sibling arity-1 floor under the same directive. This entry follows its section structure (Context / Signature / Semantics / Algebraic laws / Dependencies / Variant axes / Status / Lowers to / Lifts from / Evidence / L2 vs L1) exactly, adapting arity-1→arity-2 (the `y`-term with fixed-1 coefficient is the arity-2-specific content).
- **Fold-parent**: `book/src/L2/linear_combination.md:69` — `axpy(α, x, y) = linear_combination [(α, x), (1, y)]` (the arity-2-coeff-1 specialization); §Algebraic-laws law 6; §Variant axes axis 1 (output-aliasing is the fold's axis).
- **L0 anchors self-verified 2026-06-01** via `tools/citecheck/citecheck.py --anchor` + `Read`: `palace/linalg/vector.cpp:702-712` (`α==1.0` fast-path at `:704`), `:714-718` (real-α-on-complex forwarding overload), `:720-724` (complex-α overload), `:276-311` (`ComplexVector::AXPY` def), `vector.hpp:115-118` (member decl), `:305-307` (free-fn template decl). All returned `[ok]`.
- **Directive**: `l2-floor-under-l3-leaf-cohort` (2026-05-31), leaf-vs-fold fork resolved keep-(b) (batch-12 meta-phase per the c042 cross-cutter audit `reports/2026-06-01T063231Z-cycle-042-cross-cutter-leaf-vs-fold-audit/`).

## Open questions / caveats

- **The firm L3 `axpy` entry (`book/src/L3/axpy.md`) will go stale once this floor lands** — its `lowers_to` frontmatter and §"Lowers to" / §"Lifts from" prose currently assert a direct L3>L1 identity rotation with "no L2 intermediate because the BLAS-1 primitives are L1 leaves not L2 compositions" (`L3/axpy.md:6`, `:97`, `:114`). With this L2 floor present, the L3 leaf now rests on an adjacent same-named L2 parent; the L3 entry should re-anchor its lowering target L1 → L2 (mirroring how `L3/scal.md` was/should be swept after the cycle-041 `L2/scal` floor). **Route to the c044 L3-re-anchor sweep — do NOT fix here** (one operator per invocation; the L3 entry is out of this dispatch's scope). This is the same staleness the cycle-041 `scal` floor created for `L3/scal.md`; track them together.
- **Output-aliasing axis ownership** (`arity-family-leaf-floors-output-aliasing-axis-is-the-folds`): this entry treats output-aliasing (in-place vs out-of-place) as the **fold's** variant axis (per `linear_combination.md` §Variant axes axis 1), carried by reference, NOT as an `axpy`-leaf-specific axis — so the leaf lists only the two element-type axes. The sibling `scal` floor took the same stance implicitly. Confirm this is the intended cohort-wide convention for all arity-family leaf floors (the meta-phase / index owner may want a one-line note in §"Fold-cohort boundary" generalizing it, alongside the leaf-vs-fold keep-(b) generalization the c042 audit already recommended).
- **L2/index consolidated tally NOT touched** (count-ownership): per dispatch instruction, I appended only my own dep-map ROW (the `axpy` row, inserted after `linear_combination` and before `scal`) + the SUMMARY registration. The L2/index §"Vocabulary cohort" running firm-count tally (currently 17 firm + 1 partly-constructive) is **D2 (layer-intro-author)'s to update this cycle** — deferred, not written here.
- **L2>L1 `axpy-fusion` theme is a forward-reference** (plain text, not a live link) — the D6 dispatch this cycle authors the L2>L1 BLAS-1 floor themes; the file does not yet exist, so the in-chapter reference is plain text per `rough-in-forward-reference-must-be-plain-text-not-live-link`.
- **SUMMARY placement**: registered `axpy` between `linear_combination` and `scal` to keep the fold-family grouping visible (fold-parent then its arity-1/arity-2 leaves). If the layer-intro-author prefers strict alphabetical or arity order, that is a cosmetic re-order for the integrator/D2 to settle — the chapter content is order-independent.
