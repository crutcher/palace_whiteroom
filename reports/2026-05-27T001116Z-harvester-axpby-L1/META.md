...
**(low-severity)** — Open-question status update is recommended in prose (CYCLE.md:187): `axpby-axpy-scal-decomposition-decision` should be marked `answered` by the integrator. The report cannot self-promote that ledger entry (write-authority partition: harvester writes only to `reports/<id>/`). Flagging for the integrator handoff — no fault.
