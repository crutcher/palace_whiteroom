...
inputs:
  - book/src/L4/domain_energy_reduce.md (firm structural model — a domain-RESTRICTED reduce composing firm primitives)
  - book/src/L4/inner_product.md:154-157 (firm split-additivity / shape-concatenation monoid-homomorphism law)
  - book/src/L4/linear_combination.md:146-151 (firm concatenation-homomorphism law)
  - book/src/L4/gram_reduce.md (firm per-pair map-homomorphism reduce), book/src/L2/gram.md
  - c133 WAVE-1 hard non-destabilization probe (CLEARED both arms)
  - reference/palace/palace/linalg/rap.{cpp,hpp} + utils/geodata.cpp (deferred MPI MECHANISM only — cited, NOT lifted)
---

# CYCLE: L4 roadmap_goal sketch — sharding-decompose-reduce

## Summary

The c133 WAVE-1 hard non-destabilization probe CLEARED on both arms: the firm reduce
primitives (`inner_product`, `linear_combination`, `gram`/`gram_reduce`,
`domain_energy_reduce`) ALREADY carry the split/concatenation **monoid-homomorphism over
the index set** as standing firm laws, so a partition-of-the-index-set restriction is a
DERIVED consumer (`reduce ∘ restrict-to-block`), not a re-root. This dispatch authors ONE
new **rank-0 `roadmap_goal`** chapter, `book/src/L4/sharding-decompose-reduce.md`,
sketching the sharding-as-decomposition-abstraction MATH: the sub-domain
**restrict → per-sub-domain reduce/solve → homomorphic compose** abstraction, with the core
`subdomain_reduce = reduce ∘ restrict-to-block` recovered globally by the firm reduce verbs'
concatenation-homomorphism. `domain_energy_reduce` is the structural model (itself a firm
domain-restricted reduce composing firm primitives). This is a speculative FUTURE-DIRECTION
sketch — claim-free, carrying intent + pulled-by provenance (the batch-43 (C) directive + the
c133 gate-CLEAR) + declared deps + accreting working context. **The hard tripwire is
honored:** the firm roots it composes are listed under `reference:` ONLY (never `depends-on`),
so no rank violation is manufactured (`rank(u) ≤ min(deps)`; a rank-0 node `depends-on` a
rank-3 firm node would violate the baseline `rank_violations=0`). MPI/distributed mechanics
(`ParOperator`/RAP, mesh `Partition`/distribution) are CITED as the deferred-future
realization mechanism, NOT lifted as active work (DIRECTIVE-1).

The default ONE-chapter shape was kept (the author's-judgment bifurcation escape was
considered and declined — for a rank-0 speculative sketch the restrict/compose pair reads
more clearly as two combinators sketched in one home than as two thin separate roadmap_goal
chapters; if a real consumer later pulls them apart, the split is cheap).

## Proposed changes

```new:book/src/L4/sharding-decompose-reduce.md
---
layer: L4
...
These are the rank-0 speculative combinators the roadmap_goal sketch names; they do NOT need to
exist yet and land inside this `roadmap_goal` chapter as intent (NOT as stranded report
sketches, NOT promoted to firm operators — harvester would only promote them if a real consumer
pulls them).

- **`restrict_to_block`** — `IndexBlock -> Tensor[(S: ...)] -> Tensor[(Sb: ...)]`. Restrict a
  global field (or operator) to one block of an index-set partition (sub-shape `Sb ⊑ S`). The
  rank-agnostic generalization of `domain_energy_reduce`'s per-domain operator-restriction
  (`M_idx` restricted to domain `idx`). Speculative; mechanism realization = the
  `Partition`/`ParOperator` machinery (deferred).
- **`subdomain_reduce`** — `(Tensor[(Sb: ...)] -> r) -> Partition -> Tensor[(S: ...)] -> r`.
  The decomposition combinator: `mconcat ∘ map (reduce ∘ restrict_to_block) ∘ blocks`. Closes
  over a firm reduce verb; recovers the global reduction by the firm verb's standing
  concatenation-homomorphism over the partition. The core sharding-MATH abstraction.

(The author's-judgment bifurcation escape — splitting `restrict_to_block` and `subdomain_reduce`
into two separate roadmap_goal chapters — was considered and DECLINED: for a rank-0 speculative
sketch the pair reads more clearly in one home; if a real consumer later pulls them apart the
split is cheap.)

## Supporting evidence

- The firm structural model `book/src/L4/domain_energy_reduce.md` (read in full this dispatch):
  a firm domain-RESTRICTED reduce composing firm primitives, carrying both the
  concatenation-homomorphism fold law (:147-152) and the config-conditional partition-of-unity
  NON-law (:172-178) — the exact two patterns this sketch follows.
- The firm homomorphism laws: `inner_product.md:154-157`, `linear_combination.md:146-151`,
  `gram_reduce.md:119-120` — the lateral-arm gate-CLEAR basis (the firm reduce verbs ARE monoid
  homomorphisms over index-set concatenation; a partition is a `++`-decomposition).
- Deferred MPI mechanism citations all citecheck-`--anchor`-verified on-disk this dispatch
  (`geodata.cpp:262`, `:3230-3242`; `rap.hpp:24`; `rap.cpp:116-126`).

## Open questions / caveats

- **SUMMARY wiring + group placement.** I placed the new chapter in the "Data-algebra
  combinators & named verbs" group, alpha-positioned between `nrm2` and `sparameter_reduce`
  (SUMMARY.md:81-82) — `sharding` < `sparameter` (second char `h` < `p`) — matching the
  alpha-within-kind-group convention.
  This is a reduce-family combinator, so the data-algebra group is correct. If the
  layer-intro-author judges a roadmap_goal belongs in a distinct grouping (e.g. a future-direction
  sub-group), flag for re-placement — but the data-algebra group is the natural home given it
  composes the data-algebra reduce verbs. **No L4 `index.md` dep-map row was added** (the
  dep-map is the firm/rough-in vocabulary table; a rank-0 roadmap_goal is wired via SUMMARY +
  its own frontmatter edges, not the firm dep-map — confirm with layer-intro-author whether the
  index wants a separate roadmap_goal listing).
- **The reference-not-depends-on tripwire is the load-bearing constraint.** I verified the
  frontmatter lists all five firm roots under `reference:` only. The integrator/critic should
  re-confirm `rank_violations=0` after this lands (the graded-stack rank linter); a `depends-on`
  slip here is the one thing both probe arms turned on.
- **The solve generalization is recorded as open intent, NOT a claim.** Only the REDUCTION case
  has the firm homomorphism law today. The per-sub-domain solve / additive-Schwarz generalization
  (the eventual point of "sharding") needs interface/overlap handling (the deferred mechanism) and
  is carried in the accreting working context as speculative, not asserted. A real
  domain-decomposition-preconditioner consumer is the eventual promotion pull.
