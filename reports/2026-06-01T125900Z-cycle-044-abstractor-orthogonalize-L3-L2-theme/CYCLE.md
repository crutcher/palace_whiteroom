...
scope: L3>L2 theme sketch — orthogonalize-variant-split (the FIRST substantive L3>L2 rotation theme for a partial-obstruction operator)
status: pending
inputs:
  - book/src/L3/orthogonalize.md (firm partial-obstruction, cycle-040 — the LHS)
  - book/src/L2/orthogonalize.md (firm, cycle-012 — the RHS)
  - book/src/L3-L2/ksp-solve-outer-driver.md (the substantive-theme precedent, cycle-021)
  - book/src/concepts/sequential-obstruction.md (:22, :37-48 — the MGS obstruction)
...
# CYCLE: L3>L2 theme sketch — orthogonalize-variant-split

## Summary

The firm L3 `orthogonalize` entry (cycle-040) is the **first `partial-obstruction` operator whose obstruction is variant-conditional**: at L3 the per-step body lifts for all three Gram-Schmidt variants, the CGS/CGS2 *loop* also lifts (batched `H = Vᴴw` / `w' = w − VH` global statements), but the MGS `j`-loop is a Palace-authored numerical-stability `sequential-obstruction` over the basis index that does **not** lift. The firm L2 `orthogonalize` entry (cycle-012) is the named `project ▷ subtract` composition that erases the iteration view, recording the same split as the **collective-shape residual axis** (`m×1` MGS / `1×m` CGS / `2×m` CGS2) and the MGS **column-order non-commutativity** non-law. This theme narrates the **substantive (non-identity)** L3→L2 hop forward: the L3 `case op.variant` (CGS/CGS2 straight-line global statements + the explicit MGS `jloop` tail recursion carrying the named obstruction) **dissolves** into the L2 `project ▷ subtract` per-variant-sequenced composition, and the named MGS obstruction is **erased to its L2 shadow** (the column-order-non-commutativity non-law + the residual-axis disclosure) — exactly as the `ksp-solve-outer-driver` precedent erases the outer-loop obstruction to the L2 fold non-laws, but here the erasure is **variant-conditional** (it shadows the MGS branch only; the CGS/CGS2 branches are clean lifts on both sides of the hop). This is the structural analogue of `ksp-solve-outer-driver` (substantive iteration-rotation erasure) at the orthogonalize family. No new L3 vocabulary; both endpoints are firm. Status `firm`.

## Proposed changes

```new:book/src/L3-L2/orthogonalize-variant-split.md
...
The §Vocabulary-cohort bullet (the second of the two dual-registration sites per the c043 convention). This is a **substantive** theme, so it gets its own cohort grouping note after the fork-INDEPENDENT cohort block — appended after the `normalize-body-identity` bullet (the last line of the Vocabulary-cohort section, line 51):
...
**None.** This theme is a substantive iteration-rotation erasure between two firm endpoints (firm `L3/orthogonalize` cycle-040 partial-obstruction; firm `L2/orthogonalize` cycle-012). No new L3 vocabulary is introduced — the per-step body's `dot`/`axpy` primitives are firm at both layers, and the `case op.variant` / `project ▷ subtract` forms are reproduced from the firm endpoints. Harvester has nothing to pick up here.

## Supporting evidence

- `book/src/L3/orthogonalize.md` (firm `partial-obstruction`, cycle-040) — the LHS. §"Value-threaded form (L3 rendering)" (the `case op.variant` with the MGS `jloop`), §"Iteration-rotation marker", §"CGS / CGS2 — the global lift", §"MGS — the sequential obstruction", §"L3 vs L2 distinction" / §"Downward" (records the L3>L2 hop as iteration-view-erasing, body identity-in-form — the rotation this theme narrates forward; the L3 entry's §"Downward" had said "no `L3-L2/` theme file — in-line per the cycle-012 non-adjacent-identity convention," which was the *body-identity* part; the *substantive loop-erasure* part is what this theme files — see Open questions).
- `book/src/L2/orthogonalize.md` (firm, cycle-012) — the RHS. §Semantics (the per-variant sequencing disclosed as the residual axis; the MGS bullet's explicit "the interleaving is the `sequential-obstruction` that blocks MGS's global tensor-field form at L3"), §"Algebraic laws" non-laws.
- `book/src/L3-L2/ksp-solve-outer-driver.md` (firm, cycle-021) — the substantive-theme precedent.
- `reference/palace/palace/linalg/orthog.hpp:41-89` — MGS (`:41-53`) / CGS+CGS2 (`:57-89`) bodies, read in full and self-verified this dispatch (MGS interleaving `:46-52`; CGS dots-against-original-`w` `:66-74`; CGS2 refine `:75-88`).
- `reference/palace/palace/linalg/iterative.cpp:308-325` — `OrthogonalizeIteration` variant dispatch (`switch (type)` `:313-323`), self-verified.
- `book/src/concepts/sequential-obstruction.md:22, 37-48` — the MGS-as-sequential-obstruction structural argument (the L3 obstruction named first-class; erased at L2).

All load-bearing L0 citations self-verified against on-disk `reference/palace/` via `read_range` + `citecheck --anchor` this dispatch (citecheck is the on-disk source of truth per the cycle-027 codemap brace-drift guard; all anchors returned `[ok]`).

## Open questions / caveats

- **First substantive L3>L2 theme for a `partial-obstruction` operator — taxonomy implication for the meta-phase.** This is the second substantive L3>L2 theme overall and the first whose LHS is a `partial-obstruction` L3 entry. It introduces a **variant-conditional** substantive-erasure shape distinct from the `ksp-solve-outer-driver` **unconditional** shape. The remaining substantive L3>L2 rotations are `eigsolve` (`partial-obstruction`, opaque-library-owned loop — a third erasure root: the loop is library-owned, not Palace-authored) and `chebyshev` (in-line at `chebyshev-iteration` already; unconditional, numerical-stability-rooted). Whether the "substantive erasure scope/root" axis (unconditional-Palace-authored / variant-conditional-Palace-authored / opaque-library) wants a named taxonomy across the substantive L3>L2 cohort is a meta-phase / cross-cutter question — I did not name it (concept-page / index-taxonomy authoring is layer-intro-author / cross-cutter territory). Flagged.
- **The L3 `orthogonalize` entry §"Downward" / §"L3 vs L2 distinction" says "No `L3-L2/` theme file — in-line per the cycle-012 non-adjacent-identity convention."** That statement is correct for the *body-identity* part of the hop (the per-step `dot`+`axpy` body is identity-in-form, and the cycle-012 convention covers identity rotations). But the *loop* part of the hop — the MGS iteration-view erasure + obstruction shadow — is **substantive**, and the cycle-012 non-adjacent-identity convention does **not** cover substantive rotations (it is for identity-in-form edges only; precedent `ksp-solve-outer-driver` got a dedicated theme file for exactly this reason). So a dedicated `L3-L2/orthogonalize-variant-split` theme file IS warranted (the substantive content is not an identity that can be annotated in-line). I did NOT edit the L3 entry's §"Downward" prose to point at this new theme file (that is a carry-forward correction to an L3 *operator* entry — harvester/lifter territory, NOT abstractor write-scope; and per the discipline it is a change to *propose*, not apply). **Flagging the L3-entry §"Downward" cross-reference update** (it should gain a pointer to this theme for the substantive loop-erasure half, while keeping the in-line body-identity note) for a follow-up harvester/lifter dispatch or the integrator's implied-component handling. This is the only residual inconsistency; it does not block this theme (the theme is self-contained and both endpoints are firm).
- **No firm L4 `orthogonalize`.** The `L4/orthogonalize.md` chapter is unauthored; the natural L4 target is the Arnoldi-step-monad auxiliary stage that `krylov-step` carries as `op.orthog`. This theme is L3>L2 only; the L4>L3 edge is a future dispatch. Out of scope here (one-theme discipline). Inherited from the L3 entry's same open question.
