...
scope: L2 operator: reciprocal
status: pending
inputs:
  - book/src/L1/reciprocal.md (firm; source of truth — signature, 8 laws, partiality, variant axis, full L0 evidence)
  - book/src/L3/reciprocal.md (firm cycle-038; the L3 leaf this L2 floor goes under)
  - book/src/L2/dot.md, book/src/L2/scal.md (cycle-041 BLAS-1-floor siblings — form precedent)
  - book/src/L2/index.md §"Identity-in-form BLAS-1 floors" + §"Fold-cohort boundary"
  - L0 anchors (all self-verified on-disk via tools/citecheck/citecheck.py --anchor, 2026-06-01):
    palace/linalg/vector.cpp:248-261 (ComplexVector::Reciprocal), :257-259 (XR/XI kernel);
    palace/linalg/vector.hpp:20 (mfem::Vector alias), :107-108 (complex decl + doc);
    palace/linalg/jacobi.cpp:80 (consumer), :16 (SPD precondition);
    palace/linalg/chebyshev.cpp:178, :241 (consumers);
...
# CYCLE: Formalize reciprocal at L2

## Summary

This dispatch (cycle-042 D2) builds the L2 fusion-rotation floor entry for the elementwise multiplicative-inverse leaf `reciprocal` (`y[i] = 1/x[i]`), so the firm L3 [`reciprocal`](../L3/reciprocal.md) (cycle-038) rests on a *present adjacent same-named L2 parent* per the methodology invariant **Identity-lowerings still require both L levels**, rather than the L3 leaf skipping a layer down to L1. It is a **thin identity-in-form floor** under the 2026-05-31 foundation-first directive `l2-floor-under-l3-blas1-cohort`: the L2 form is value-thread-isomorphic to the firm L1 [`reciprocal`](../L1/reciprocal.md) — the fusion rotation is a no-op on the primitive (the L0 complex kernel is already the unfolded single-pass elementwise reciprocation, and the only "fusion" present — the `s = 1/|z|²` intermediate reused across the two real components — is a transparent factoring of the closed form `z̄/|z|²`, not a multi-operation kernel fusion to de-fuse). The eight algebraic laws and the single variant axis (element-type) transport unchanged from L1; the partiality precondition (`x[i] ≠ 0`) is recorded once.

**Design-finality statement (per dispatch directive).** `reciprocal` is a **standalone elementwise leaf with NO fold-parent** — it is NOT a member of `inner_product` (it does not reduce over the length axis to a scalar) and NOT a member of `linear_combination` (it is not a scalar-weighted-sum term; it is a *nonlinear* elementwise self-map, `1/(a+b) ≠ 1/a + 1/b`). The **leaf-vs-fold design fork** under batch-12 meta-phase adjudication (`dot-l2-leaf-floor-vs-fold-only-design`, recorded at `book/src/L2/index.md:91`) **does not apply to this floor**: there is no fold-parent for the fork to re-anchor this leaf into. Whether the meta-phase adopts the (a) fold-only or (b) same-named-floor reading for `dot`/`scal`, `reciprocal`'s L2 floor is design-final — it can only ever be a same-named standalone leaf, because no `inner_product`/`linear_combination` fold-parent subsumes it. This is the structural distinction that makes `reciprocal` (and `nrm2` on the fold-membership question, though `nrm2` rides the floor-directive decision differently) cleaner than `dot`/`scal` under the fork.

No genuine fusion content beyond identity was found: the complex kernel's `s = 1/|z|²` reuse is the same transparent factoring the L1 entry already records, and `reciprocal` composes no other L2 operator. The entry stays a thin floor.

## Proposed changes

```new:book/src/L2/reciprocal.md
---
layer: L2
operator: reciprocal
firmness: firm
...
The full firm chapter body is authored inside the `new:book/src/L2/reciprocal.md` proposed-changes fence above. Key features:

- **Slug + one-line**: `reciprocal` — elementwise multiplicative-inverse `y[i] = 1/x[i]` as the L2 fusion-rotation floor.
- **Signature**: `reciprocal :: Tensor[N] -> Tensor[N]`; shape contract with named axis `N`, result element type tracks input; partiality precondition `x[i] ≠ 0` recorded.
- **Semantics**: element-local, reduction-free, rank-local; pure/out-of-place; real `1/x` vs complex `z̄/|z|²` closed form; the `s = 1/|z|²` transparent factoring isolated in a §"Fusion note".
- **Algebraic laws**: eight laws (involution, multiplicative-inverse identity, scalar-factor distribution, multiplicative-distributivity, complex closed-form, conjugate-reciprocal commutation, all-ones fixed point, negation), inherited unchanged from L1; non-laws (partiality, nonlinearity, IEEE-754 caveats) catalogued explicitly.
- **Dependencies**: none same-layer (leaf); **fold-parent NONE**; `elementwise_product` sibling (plain text); consumers `krylov-step` + FE-assembly multiplicity.
- **Status**: `firm` — value-thread-isomorphic to firm L1 leaf; firm-on-positive-structure on the complex kernel; thin floor; design-final on the leaf-vs-fold fork.
- **Evidence**: L1/L3 anchors + the cycle-041 floor siblings + all L0 anchors self-verified on-disk this invocation.

## Supporting evidence

All L0 anchors self-verified on-disk via `tools/citecheck/citecheck.py --anchor` (2026-06-01), all `[ok]`:

- `palace/linalg/vector.cpp:248-261` (anchor `Reciprocal` @ :248), `:257-259` (anchor `XR` @ :257-258) — the complex `ComplexVector::Reciprocal()` kernel.
- `palace/linalg/vector.hpp:20` (anchor `mfem::Vector`), `:107` (anchor `reciprocal`), `:108` (anchor `Reciprocal`).
- `palace/linalg/jacobi.cpp:80` (anchor `Reciprocal`), `:16` (anchor `SPD`).
- `palace/linalg/chebyshev.cpp:178`, `:241` (anchor `Reciprocal`).
- `palace/fem/bilinearform.cpp:278` (anchor `Reciprocal`).

These are exactly the anchors the firm L1 and L3 `reciprocal` entries cite; no drift found against either source-of-truth entry.

Insertion point: the L2/index dep-map row is appended immediately after the `nrm2` row (keeping the cycle-041 BLAS-1-floor cohort contiguous: `dot`, `nrm2`, then `reciprocal`; `scal` already precedes them earlier in the table). The SUMMARY.md chapter entry is inserted after `[nrm2]` and before `[orthogonalize]`, matching the dep-map ordering and the floor-cohort grouping.

## Open questions / caveats

- **COUNT-OWNERSHIP (deferred to D11, per dispatch directive).** This report does NOT touch the L2/index consolidated firm/partly-constructive running-count tally (currently "firm 9 → 12" at `book/src/L2/index.md:90` plus the §"Vocabulary cohort" firm list header). D11 owns the consolidated count this cycle. With this landing the firm L2 cohort becomes 13 (12 + `reciprocal`); D11 should reconcile the tally and add `reciprocal` to the §"Vocabulary cohort" firm bullet list + the §"Identity-in-form BLAS-1 floors" sub-list (flagging it as the fold-parent-free member). I have added ONLY my own dep-map row + SUMMARY registration.

- **L2>L1 lowering theme (`reciprocal` edge) is D10's, forward-referenced as plain text.** This entry forward-references a not-yet-existing `L2-L1/reciprocal-elementwise-identity` theme as plain text (no live link), per the `rough-in-forward-reference-must-be-plain-text-not-live-link` convention. D10 authors the L2>L1 and L3>L2 `reciprocal` identity themes this cycle. If D10 chooses a different slug, the in-line plain-text reference in §"Lowers to" needs no edit (it is plain text), but the meta-phase cohort-naming-normalization OQ (`dot`-cohort L2>L1 slug split, `book/src/L2/index.md:92`) should consider `reciprocal`'s theme slug in the same normalization pass — `reciprocal` is fold-parent-free, so a `-fold-specialization` suffix would be doubly mis-naming (no fold AND identity-leaf); the natural slug is `-elementwise-identity` or `-leaf-identity`.

- **`elementwise_product` L2 floor not yet authored.** Referenced as plain text (sibling elementwise leaf, also fold-parent-free). The diagonal-preconditioner-apply chain `assemble_diagonal → reciprocal → elementwise_product` is now floored at the `reciprocal` step but not the `elementwise_product` step; `elementwise_product` is a clear L2-floor candidate (firm at L1 and L3 already) for a future cycle. Not in this dispatch's scope.

- **Leaf-vs-fold fork (`dot-l2-leaf-floor-vs-fold-only-design`, batch-12 meta-phase) — `reciprocal` is OUTSIDE it.** Recorded affirmatively in the entry (§"No fold-parent", §Status): because `reciprocal` has no `inner_product`/`linear_combination` fold-parent, neither the (a) fold-only nor the (b) same-named-floor reading changes its floor (it can only ever be a standalone same-named leaf). This is a *data point for* the meta-phase adjudication: the fork is specifically about fold-*member* leaves; the fold-parent-free elementwise leaves (`reciprocal`, and the forthcoming `elementwise_product`) are design-final same-named floors regardless. Worth the meta-phase noting that the floor cohort is heterogeneous (fold-leaf `dot`/`scal` vs fold-consumer `nrm2` vs fold-free `reciprocal`/`elementwise_product`), and the fork only touches the first sub-group.
