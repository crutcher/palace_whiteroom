...
inputs:
  ...
  - book/src/L2/inner_product.md (firm; the scalar fold gram lifts to a matrix)
  - book/src/L2/index.md (the rough-in dep-map row being flipped to firm)
  - book/src/L2/orthogonalize.md (firm; the named-composition / L2-placement / over-unification-guard precedent)
  - book/src/L1/dot.md (the pinned arg-1-conjugated ⟨x,y⟩ = xᴴ y convention)
  - palace/linalg/nleps.cpp:524-531 (the sole literal Gram-build site)
---

# CYCLE: Formalize gram at L2

## Summary

`gram` is the **all-pairs `inner_product` fold** that builds the `k×k` Gram matrix
`G = XᴴX` (entry `G[i,j] = ⟨X[j], X[i]⟩ = X[j]ᴴ X[i]`) from a `k`-column basis `X`. It is the
matrix-valued lift of the firm L2 scalar fold [`inner_product`](./inner_product.md): where
`inner_product` reduces two tensors to one scalar, `gram` materializes the cartesian product
of two basis index sets into a matrix of those scalars. Cycle-021's combinator-miner proposed
it as a plain-text rough-in dep-map row in `book/src/L2/index.md:54`
(`reports/2026-05-29T051532Z-combinator-miner-deflate-gram/CYCLE.md`); this dispatch firms it.

The promotion is to **`firm`**. The single literal Gram-build site in the entire Palace tree
is `nleps.cpp:524-531` (the SLEPc-NEP deflated quasi-Newton solver's deflation Gram matrix
`SS(i,j) = linalg::Dot(GetComm(), X[i], X[j])`), and `gram`'s entire algebraic-law set is
**syntactic identities on the firm `inner_product` fold** — Hermitian symmetry, PSD,
`gram(X)[i,j] = inner_product(X[j], X[i])`, the incremental-Gram block law — every one of which
is operator-algebra inherited from `inner_product` (whose own laws are firm and partly
test-anchored). Per the `apply_nonlinear_pencil` / `apply_linop` firm-on-syntactic-identity
precedent (cycle-021 §Status-tier codification: "an entry whose laws are syntactic identities
on fully-specified positive source is `firm` even when the surrounding area has no test"), the
absence of a dedicated NLEPS/deflation unit test (`search_text` over `palace/test/**` for
`nleps|deflat|MatVecMult|invariant pair` returns zero hits) does **not** gate firmness: the
laws are not convergence-semantics that need a behavioral test, they are matrix-algebra facts
about an `inner_product`-of-pairs. The single-algorithm concentration is recorded as a coverage
caveat (it does not reduce the status, mirroring `inner_product`'s own `tdot` member caveat).

The IEEE reduction-tree associativity non-law is **inherited** from `inner_product` (each entry
is one `dot`; bit-reproduction of an L0 Gram pins each entry's reduction tree).

## Proposed changes

```new:book/src/L2/gram.md
# gram
