---
agent: abstractor
invoked_at: 2026-05-29T23:45:06Z
...
inputs:
  ...
  - `book/src/L1-L0/minres-iteration.md` (obstruction-theme precedent — MFEM_ABORT-anchored)
  - `book/src/L1-L0/bicgstab-iteration.md` (obstruction-theme precedent — MFEM_ABORT-anchored, with verified_against negative-anchor YAML)
  - `book/src/L3/index.md:7` (already names "certain triangular solves" as a canonical L3 obstruction — this theme is the concrete L0 evidence behind that note)
  - `book/src/L1/back_solve.md` (firm c027 — the small-dense GMRES/FGMRES restart-correction back-substitution; sibling, NOT a general trsv)
  - cycle-029 dispatch-1 `back-solve-mutation-rotation` L1>L0 theme (NOT yet on disk at dispatch time — plain-text reference, integrator-finalize upgrades to live link after applying dispatch-1)
---

# CYCLE: L1>L0 obstruction theme sketch — `triangular-solve-obstruction`

## Summary

Cycle-028's harvester localization established (two exhaustive zero-hit codemap text searches across the entire Palace tree, critic-reproduced) that **Palace has no standalone general triangular-solve primitive** (`trsv` / `trsm` / `TriangularSolve` / `SpTrSV`): the small-dense matrix utility module `palace/linalg/densematrix.hpp:24-36` exposes only `MatrixSqrt` / `MatrixPow` / `SingularValueMax` / `SingularValueMin` / `Mult`, no triangular solve and no LU/Cholesky factorization. Triangular solves do exist in Palace, but **only as internal substitution sweeps inside opaque library calls** — HYPRE-internal Gauss-Seidel / SSOR relaxation sweeps selected by integer `relax_type` config flags (`palace/linalg/amg.cpp:19`, `palace/linalg/ams.cpp:162`), and forward/back substitution inside the MUMPS / SuperLU / STRUMPACK direct-solver factorizations (external-library wrappers at `palace/linalg/strumpack.hpp:21`, `superlu.hpp:22`, `mumps.hpp:21`). Palace-authored smoothers are GS-free (Jacobi + Chebyshev only); the Chebyshev choice over Gauss-Seidel is explicitly Adams-et-al.-2003-cited (`palace/linalg/chebyshev.hpp:82`), and the GPU AMG default flips `relax_type` from `8` (l1-symm. GS) to `18` (l1-Jacobi) (`palace/linalg/amg.cpp:24`) precisely because the GS triangular sweep is sequential and GPU-hostile. The block lower-triangular preconditioner (`palace/linalg/blockprecond.hpp:16-29`) is a 2×2 BLOCK forward solve — `z0 = P0⁻¹r0; z1 = P1⁻¹(r1 − L10 z0)` — applying sub-solvers to whole blocks, NOT a scalar triangular substitution; documented here as a red-herring non-example. This theme follows the cycle-004 MINRES / BiCGStab obstruction-theme precedents: claim-free documentation, `justification kind: obstruction`, negative anchors only, no constructive L1 form. The one concrete triangular-system component Palace DOES implement is the small-dense GMRES / FGMRES restart-correction back-substitution (firm cycle-027 `back_solve` L1 leaf), cross-referenced here as the sibling distinction; it is **not** a general `trsv`. Resolves the `trsv` leaf of OQ `l3-vocabulary-inventory-gap` as **resolved-by-obstruction** rather than perpetually BLOCKED, giving the L3 index's `:7` "certain triangular solves" line a citable home.

## Proposed changes

```new:book/src/L1-L0/triangular-solve-obstruction.md
...
- [`back_solve`](../L1/back_solve.md) (firm c027) — the **small-dense**
  ...
  *small-dense-triangular* sibling of [`lu_solve`](../L1/lu_solve.md), NOT a
  ...
  leaf does not.
- The forthcoming `back-solve-mutation-rotation` L1>L0 theme (cycle-029
  dispatch-1, sibling theme being applied in the same cycle as this one) —
  the rotation of the `back_solve` leaf into its L0 in-place back-substitution
  loop. **Not yet on disk** at the time this theme is authored; the
  integrator-finalize upgrade-pass should convert this plain-text reference
  to a live link `../L1-L0/back-solve-mutation-rotation.md` once dispatch-1
  has landed (skill `upgrade-plain-text-ref-to-live-link-when-target-on-disk`).
  Cross-referenced here only to make the sibling distinction concrete; that
  theme also covers only the GMRES/FGMRES restart-correction case, NOT a
  general `trsv`.
...
- [`book/src/L1/back_solve.md`](../L1/back_solve.md) — the firm c027
  ...
  length-`N` field).
- `back-solve-mutation-rotation` (cycle-029 dispatch-1; sibling theme being
  applied in the same cycle as this one; NOT yet on disk at the time this
  theme is authored — integrator-finalize upgrade-pass converts this
  plain-text reference to a live link
  `../L1-L0/back-solve-mutation-rotation.md` once dispatch-1 has landed).
...
[old]: | [bicgstab-iteration](./bicgstab-iteration.md) | (speculative — `bicgstab_step`, …) | (no Palace anchor — `MFEM_ABORT` at `ksp.cpp:53-57`) | obstruction |
[new]: | [bicgstab-iteration](./bicgstab-iteration.md) | (speculative — `bicgstab_step`, …) | (no Palace anchor — `MFEM_ABORT` at `ksp.cpp:53-57`) | obstruction |
...
[old]: - [minres-iteration](./L1-L0/minres-iteration.md)
[new]: - [minres-iteration](./L1-L0/minres-iteration.md)
- [triangular-solve-obstruction](./L1-L0/triangular-solve-obstruction.md)
```

## Speculative operators proposed

**None.** This is an obstruction theme; per CLAUDE.md §Scope ("Unimplemented Palace components are NOT direct implementation targets") and the cycle-004 [`bicgstab-iteration`](../../../book/src/L1-L0/bicgstab-iteration.md) / [`minres-iteration`](../../../book/src/L1-L0/minres-iteration.md) precedent for negative-result themes, no constructive L1 operator is proposed. The literature `trsv :: (R: Triangular[N,N], s: Tensor[N]) -> Tensor[N]` signature is named in the theme for cross-referencing only and explicitly carries the "do not promote" caveat. Distinct from the cycle-004 obstruction themes (which DID name three speculative operators each on the rationale that an Arnoldi-step variant axis or BiCGStab `omega_update` might later inform an L2 combinator); for `trsv` there is no upstream consumer the artifact uses that would be simplified by a `trsv` leaf — the Palace consumers (HYPRE relaxation, external direct solvers, Chebyshev polynomial smoothing) do not factor through a `trsv` step. The harvester should not promote a speculative `trsv` operator on the basis of this theme.

## Supporting evidence

Citation self-verification — every load-bearing pinpoint cited in the theme body was checked against on-disk via `tools/citecheck/citecheck.py --anchor`. Codemap was used for localization only (per CLAUDE.md §Methodology invariants — "the codemap is localization-only; `citecheck` / the on-disk `reference/` file is the citation SOURCE OF TRUTH"). Re-verifications this dispatch:

- `reference/palace/palace/linalg/amg.cpp:19` anchor `'l1-symm. GS'` — OK (line 19).
- `reference/palace/palace/linalg/amg.cpp:24` anchor `'relax_type = 18'` — OK (line 24). (The c028 report's loose phrasing "GPU default flips `18` = l1-Jacobi, `amg.cpp:24`" pointed at the assignment line `relax_type = 18;`; the `l1-Jacobi` text itself sits at line 19 in the enum comment. The theme anchors line 24 with `'relax_type = 18'` to match the actual line content; the `l1-Jacobi` semantic enum value is supplied by the line-19 comment.)
- `reference/palace/palace/linalg/amg.cpp:29` anchor `'HYPRE_BoomerAMGSetRelaxType'` — OK (line 29).
- `reference/palace/palace/linalg/ams.cpp:158` anchor `'amg_relax_type'` — OK (line 158).
...
- `reference/palace/palace/linalg/ams.cpp:179` anchor `'coarse_relax_type'` — OK (line 179).
- `reference/palace/palace/linalg/jacobi.hpp:19` anchor `'JacobiSmoother'` — OK (line 19). (Re-anchored from the c028 report's `jacobi.hpp:15` after a citecheck-detected +4 drift: the class declaration is at line 19, lines 15–16 are the descriptive comment block immediately above it.)
- `reference/palace/palace/linalg/chebyshev.hpp:23` anchor `'ChebyshevSmoother'` — OK (line 23).
- `reference/palace/palace/linalg/chebyshev.hpp:82` anchor `'polynomial versus Gauss'` — OK (line 82).
- `reference/palace/palace/linalg/strumpack.hpp:18` anchor `'STRUMPACK'` — OK (line 18).
- `reference/palace/palace/linalg/strumpack.hpp:21` anchor `'StrumpackSolverBase'` — OK (line 21).
- `reference/palace/palace/linalg/superlu.hpp:22` anchor `'SuperLUSolver'` — OK (line 22).
- `reference/palace/palace/linalg/mumps.hpp:21` anchor `'MumpsSolver'` — OK (line 21).
- `reference/palace/palace/linalg/blockprecond.hpp:16-29` anchor `'Block lower-triangular'` — OK (anchor at line 16 within range 16-29).
- `reference/palace/palace/linalg/densematrix.hpp:24-36` — range in bounds (file has 40 lines); read confirms 5 declarations only (MatrixSqrt ×2, MatrixPow ×2, SingularValueMax, SingularValueMin, Mult), no triangular solve, no factorization.

Sources beyond the c028 localization that were consulted this dispatch:

- `book/src/L1-L0/index.md` — the L1>L0 theme dep-map, to confirm row position (insertion after the existing `bicgstab-iteration` row at the end of the table).
- `book/src/SUMMARY.md` — registration order; the new entry goes after `minres-iteration` line 98 (per the obstruction-theme precedent ordering: `bicgstab` `:97` / `minres` `:98`; `triangular-solve-obstruction` slots in after `minres` to keep the obstructions clustered).
- `book/src/L1/back_solve.md` (firm c027) — the sibling-distinction grounding; the §"Why this is NOT a general `trsv`" section (lines 44–61) and the §"General-`trsv` membership" non-law (lines 255–262) explicitly frame `back_solve` as the firm sibling to this obstruction (and the §Status paragraph at line 366 explicitly says this theme **relates to but does not close** the `trsv` gap — closing it is the role of this theme).
- `reference/palace/palace/linalg/jacobi.hpp:12-26` (read on-disk) — to confirm the `JacobiSmoother` declaration is at line 19, not 15 (drift detected via citecheck during pre-emit self-verification).

## Open questions / caveats

- **OQ `l3-vocabulary-inventory-gap` — `trsv` leaf**: this theme is the **resolution** of the `trsv` leaf. The integrator-per-report should mark the `trsv` leaf as **resolved-by-obstruction** (matching the c028 localization's recommendation) and close the parent OQ as fully complete (all four leaves done: `gemv` / `ksp_solve` / `eigsolve` / `trsv`). The OQ ledger entry's `resolution` field can cite `book/src/L1-L0/triangular-solve-obstruction.md` as the home.
- **Integrator-finalize upgrade**: the plain-text reference to `back-solve-mutation-rotation` in the theme body and in the index.md row's `back_solve` mention is intentionally plain-text — that L1>L0 theme is being authored concurrently in cycle-029 dispatch-1. Per CLAUDE.md §Methodology invariants (the `rough-in-forward-reference-must-be-plain-text-not-live-link` convention), this avoids the `linkcheck2` build break. Once dispatch-1 has landed on disk during integrator-per-report processing, the integrator-finalize upgrade-pass (skill `upgrade-plain-text-ref-to-live-link-when-target-on-disk`) converts the plain-text `back-solve-mutation-rotation` reference to a live link `../L1-L0/back-solve-mutation-rotation.md`. Recorded so the integrator does not flag the plain-text reference as a defect.
- **L3 index back-reference (out of dispatch scope)**: `book/src/L3/index.md:7` ("certain triangular solves") may want a back-reference to this theme file, paralleling the `eigsolve` partial-obstruction cross-reference at `:31` / `:45`. This is a layer-intro-author follow-on; not blocking and not authored here. Suggested OQ: `l3-index-line-7-add-triangular-solve-obstruction-backref`.
- **The c028 report's negative-anchor citation `jacobi.hpp:15`** (in its §"Characterization (b2) Palace-authored smoothers are GS-free" bullet) **was drifted off-by-4**: the `JacobiSmoother` class declaration is at `jacobi.hpp:19`, with lines 15–18 carrying the descriptive comment block immediately above the class. This theme uses the corrected `:19` anchor. Recorded as a producer-citation-drift instance the c028 localization's own pre-emit self-verification block (its "Citation self-verification" section) did not catch — that section verified the *load-bearing* pinpoints (the five citations explicitly listed there) but not the supporting `:15` mention. Friction-ledger candidate: `producer-citation-drift-verify-not-self-invoked` (recurrence). The negative finding's substance is unaffected (the `JacobiSmoother` class IS GS-free; only the line number was off).
- **Justification-kind variant**: this is an **opaque-library-ownership** obstruction (the substitution exists in a Palace run but is owned by HYPRE / STRUMPACK / SuperLU / MUMPS), distinct from the cycle-004 MINRES / BiCGStab obstructions which are **enum-only-stub** obstructions (the substitution does NOT exist in any Palace run — every invocation aborts at `MFEM_ABORT`). Both are `justification kind: obstruction`, but with materially different substructure. If the meta-phase finds the distinction load-bearing for friction-clustering, an obstruction-sub-kind refinement (`opaque-library-ownership` vs `enum-only-stub`) would be the natural codification. Not proposed as an immediate methodology change — flagged for the meta-phase to consider when it surveys the obstruction-theme cohort.
- **No specialized L1 operator section in the theme** (unlike the cycle-004 obstruction themes which proposed 3 speculative operators each): this is the deliberate divergence — those themes proposed operators on the rationale that a hypothetical Arnoldi-step variant axis or BiCGStab `omega_update` might later simplify an L2 combinator. For `trsv` there is no upstream consumer Palace uses that would factor through a `trsv` leaf (HYPRE relaxation, external direct solvers, Chebyshev polynomial smoothing all bypass it). The empty Speculative-operators section is a *first-class output* of this theme — it documents that promotion is not warranted, not that promotion was deferred or forgotten.
- **Phase-1-slice-reduction candidacy for `book/src/spec/slices/sparse_triangular_solve.md`** (out of this dispatch's scope; cycle-repairer-added per cross-reference-integrity finding): now that this L1>L0 theme lands as the layered-artifact home for the same negative result, the slice's reduction status (currently "annotated-and-retained" per cycle-013+ corpus-reduction policy) may merit re-evaluation. The slice's live §"Live OQs" question ("rename to `sparse_direct_solver_wrapper` + re-push to L1 against the wrapper-level contract; whether an MFEM/SuperLU-level slice family owns the factor-Allgatherv / residual-of-triangular-solve framing") is arguably *partly answered* by this theme (the absence is now layered-artifact-documented; the wrapper-level rotation question remains open). The repairer's per-CLAUDE.md "do not enact phase-1-slice reduction" directive (separate same-layer-cross-cutter dispatch) is followed; recorded as a follow-up OQ candidate: `sparse-triangular-solve-slice-reduction-after-l1l0-theme-lands` — for a future same-layer-cross-cutter dispatch to audit whether the slice should reduce to a stub pointing at this L1>L0 theme + the two concept canonical-instance sections, or remain as the wrapper-level carry-through home.
