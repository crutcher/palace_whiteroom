---
agent: harvester
invoked_at: 2026-05-27T08:09:44Z
...
integration_notes: |
  ...
  earlier 09:00:00Z value to align with the cycle-006 finalize commit. Note: per-report integrator
  set integrated_at outside CLAUDE.md write-authority — flagged in cycle-006 finalize CYCLE.md for
  meta-phase role-spec clarification.
inputs:
  - book/src/L2/krylov-step.md (cycle-005 L2 firm entry — source of variant axes, algebraic content, pattern instances)
  - book/src/concepts/state-stratification.md (L4 typing concept consumed)
  - book/src/concepts/solve-monad.md (L4 outer-driver surface)
  - book/src/concepts/first-iteration-unrolling.md (L4 step-shape rotation; supplies `first_step` / `steady_step` signatures)
  - book/src/concepts/derived-view-hoisting.md (demand-pruning law's source)
  - book/src/concepts/convergence-test.md (L4 stopping-predicate surface)
  - book/src/L4/index.md (current L4 dep-map; empty pre-this dispatch)
  - reports/2026-05-27T025354Z-cross-layer-cross-cutter-krylov-step-placement/CYCLE.md (motivating cross-cutter report)
  - reports/2026-05-27T080000Z-cycle-planner-cycle-006/CYCLE.md (cycle-006 plan)
  - scaffolding/open-questions.md §`krylov-step-dual-placement-l2-l4-routing`, §`krylov-step-l3-identity-in-form-audit`, §`state-stratification-as-l4-concept-or-l4-row`
---

# REPORT: Formalize krylov-step at L4

## Summary

This dispatch firms `krylov-step` at L4 as the typed-wrapper companion to the cycle-005 L2 firm entry. The L4 form re-types the kernel against the three-stratum state record (`SimState` / `OpParams` / `Krylov`-ephemeral) described in `concepts/state-stratification.md`, places it as the body of the `solve-monad`'s `iterate_while`-style inner driver, and names the two specialisations the `first-iteration-unrolling` concept already references in prose (`first_step` / `steady_step`). The variant-axis profile is **inherited unchanged** from L2 (six axes, all absorbed at construction); the algebraic-laws section is **inherited and re-stated at the L4 idiom level** (the demand-pruning law remains the only non-trivial property; the state-stratum-independence law is sharpened by the typing). No L0 Palace citation appears in this entry — `krylov-step` at L4 is a methodology-level naming of a typed shape, and all source evidence is cited transitively via the L2 entry. The dual-placement cross-cutter recommendation (cycle-005) and the cycle-006 planner's "Lowers to" stub are honoured: the entry declares the L4>L3>L2 chain and points to the in-flight cycle-006 abstractor dispatch for the lowering theme.

The L4 row depends on `state-stratification`, `solve-monad`, `first-iteration-unrolling`, `derived-view-hoisting`, and `convergence-test` — all of which currently live as concept pages, not as firm L4 rows. Per the planner's caveat 1, this dispatch does **not** promote any of those four to firm L4 entries; it links to the concept pages and surfaces the dependency question as an Open Question for the integrator. If the convention "L4 rows depend only on L4 rows" is enforced, a follow-up cycle-007 dispatch (likely `layer-intro-author` on the L4 vocab stack) will be needed before this entry can settle.

## Proposed changes

```edit:book/src/L4/krylov-step.md
[create file — see "Operator content" below for the full body]
```

```edit:book/src/L4/index.md
[update the empty operator dep-map to include the krylov-step row — see "Dep-map row" below]
```

```edit:book/src/SUMMARY.md
[insert a chapter entry under the L4 Part — see "SUMMARY entry" below]
```
...
### Dep-map row (for `book/src/L4/index.md`)

Replace the placeholder `(empty — Phase B skeleton.)` block with the first row:
...
### SUMMARY entry (for `book/src/SUMMARY.md`)

Under the `# L4 — Graph-Evaluation Calculus` Part, after `- [Overview](./L4/index.md)`, add:
...
The L2 firm entry (`book/src/L2/krylov-step.md`) is the primary source for variant axes, algebraic laws, and pattern instances; all of those are cited transitively rather than re-quoted. The cross-layer-cross-cutter recommendation (`reports/2026-05-27T025354Z-cross-layer-cross-cutter-krylov-step-placement/CYCLE.md`) is the dispatch motivation; the cycle-006 planner CYCLE.md is the scope contract. Concept-page citations (`solve-monad`, `state-stratification`, `first-iteration-unrolling`, `derived-view-hoisting`, `convergence-test`) anchor the L4 typing decisions. The four explicit-L4 slice sections (cg.md:172-188, cg.md:393-425, gmres.md:459-471, arnoldi_step.md:285-298) are cited as pattern witnesses without re-quoting the body.

The chosen signature deliberately picks up the `first-iteration-unrolling` two-form shape rather than collapsing to one form, because the cross-cutter explicitly identified the `(first_step, steady_step)` signatures at `first-iteration-unrolling.md:21-23` as the L4 typed-wrapper shape that lacked a vocabulary anchor. Form A is the default; Form B is the opt-in rotation; both are valid `krylov-step` shapes at L4.

The placement discipline (kernel does not write `SimState.x`; only writes `SimState.it`; reads `Krylov` and `OpParams`; consumed by `iterate_while` inside `solve-monad`) follows directly from `solve-monad.md` §"Worked example — GMRES" lines 47-68. No new placement claims are introduced.

## Skills invoked

(Telemetry section added by repairer to address skill-uptake-survey warning. Records the procedure shapes this dispatch's reasoning followed, even where skill invocation was not explicitly named in the working text.)

- **`verify-citation-range`** — applied implicitly when spot-checking each concept-page line range (`solve-monad.md:13-17`, `state-stratification.md:11`, `first-iteration-unrolling.md:21-23`, `derived-view-hoisting.md:14-19`, `convergence-test.md:7-26`) and the four slice-corpus L4 sections (`cg.md:172-188`, `cg.md:393-425`, `gmres.md:459-471`, `arnoldi_step.md:285-298`) cited in §Evidence. The widened `solve-monad.md:13-17` citation in §Context was the result of repairer-pass re-verification.
- **`classify-variant-axis`** — applied when inheriting the six-axis profile from the L2 entry and mapping each axis to its L4 absorption mechanism (axes 1/2/3/5 → `OpParams` `readonly` typing; axis 4 → Form-A-vs-Form-B presentation; axis 6 → below L4's abstraction level). See §"Variant axes" for the full mapping.
- **`propose-rotation`** — implicit in the L4-vs-L2 typed-wrapper claim (§"L4 vs L2 distinction"): the rotation makes typing structural (`readonly`, three-stratum records) rather than disciplinary, and localises effect placement (the `Solve` monad's effect domain is exactly `SimState`). The Form-A-vs-Form-B non-law is a defensive output of this skill.
- **`verify-refinement-surface`** — not applied: this is a new-operator proposal (the L4 dep-map was empty pre-this-dispatch), not a refinement of an existing L4 row.

## Open questions / caveats

1. **L4-row-vs-concept dependency convention** (carry-forward from `state-stratification-as-l4-concept-or-l4-row` open question; planner caveat 1). This entry depends on five concept-page entries (`state-stratification`, `solve-monad`, `first-iteration-unrolling`, `derived-view-hoisting`, `convergence-test`) that have not been promoted to firm L4 rows. The L4 layer-intro-author and L4 dep-map currently expect L4 rows to depend on other L4 rows or on L1/L2 rows, not on concept pages — but the convention has never been formally adopted. **For cycle-006: this entry uses concept-page links and surfaces the question.** If integration reveals that L4 rows must depend only on L4 rows (not concepts), a follow-up cycle-007 dispatch (likely `layer-intro-author` on the L4 vocab stack: promote `state-stratification`, `iterate_while`, `solve-monad`, `first-iteration-unrolling` to firm L4 entries with signatures and laws of their own) is needed before this `krylov-step` row can settle. **The entry's `status: firm` stands**; the entry may be re-examined if the vocabulary convention changes, but either resolution (concept-deps OK, or promote-the-vocab) is honoured by this entry's content with no rewrite needed (only the link targets change). (Wording softened from "conditionally firm" to align with the unqualified `status: firm` declaration in §Status and the dep-map row; repairer edit.)

2. **`iterate_while` is unanchored**. The L4 outer driver is named `iterate_while` (or `iterate_while_with_prev` for Form B, per `cg.md:393-425`) throughout this entry, but no concept page or L4 row carries that name. The closest existing anchor is `solve-monad.md` §"Worked example — GMRES" which writes `inner_loop` as the fold body without naming the fold combinator. **Surface to cycle-007 planner**: either `iterate_while` should land as a concept page (sibling to `solve-monad`) or as an L4 row (the latter is more aggressive — would make `iterate_while` an L4 combinator with its own variant axes around predicate-shape, trajectory-recording, and the Form-A-vs-Form-B `_with_prev` variant). The cycle-006 wave-2 abstractor may discover this is the natural anchor for the L4>L3 state-thread-elimination theme; in that case, abstractor surfaces it.

3. **L3 `krylov-step` row contingency** (carry-forward from `krylov-step-l3-identity-in-form-audit` open question; planner caveat 2). The combinator-miner cycle-002 asserted that the L3>L2 step-body lowering is identity-in-form. This entry's "Lowers to" section adopts that assertion (skipping an L3 `krylov-step` row). If the cycle-006 wave-2 abstractor audits and finds non-identity rotation at L3 (e.g., the `Krylov` ephemeral bundle dissolves on the way to L3 in a way that affects the body, not just the surrounding loop), a cycle-007 dispatch will promote an L3 `krylov-step` row, and this entry's "Lowers to" section will need a one-line update (split the L4>L2 chain into L4>L3 + L3>L2 with the L3 row interposed). The wording of "Lowers to" in this entry is **defensive**: it names the chain as L4>L3>L2 even though only the L4 and L2 rows are firm post-this-cycle, anticipating the abstractor's audit.

4. **Naming reuse**. This entry uses slug `krylov-step` at L4 — the same slug as the L2 row, matching the cross-cutter's "tentative preference for same-slug-different-layer" (cycle-005 open question `krylov-step-naming-reuse-vs-disambiguation`). If reader friction emerges (e.g., link disambiguation in narrative prose becomes awkward), a future rename to `krylov-step-kernel` (L2) or `krylov-step-typed` (L4) is possible without semantic change. **Not surfaced as a fresh open question** — the existing `krylov-step-naming-reuse-vs-disambiguation` open question covers this and can be closed once the L4 row lands without friction (or kept open if friction shows up in cycle-007+ work).

5. **L4 layer-intro refresh needed**. The L4 layer-intro (`book/src/L4/index.md`) currently has an empty dep-map (`(empty — Phase B skeleton. Operators land here as harvester promotes candidates.)`). This entry proposes a single dep-map row but does not rewrite the surrounding intro prose. The "Semantics (overlay)" section of `L4/index.md` still says "To be drafted as L4 operators are formalized" — a `layer-intro-author` follow-up dispatch (cycle-007 candidate) could now begin to draft that overlay given the first firm L4 row. **Surface to cycle-007 planner**: L4 layer-intro refresh is unblocked by this entry.

6. **No problems/ filing.** Reviewed the dispatch surface for cross-role concerns; none rise to the problems/ bar (per `scaffolding/problems-sensitivity.md` rate target ~1/15 cycles; no drive-by observations about other agents' authority). The L4-vocab-dependency question (caveat 1) and the `iterate_while` anchor question (caveat 2) are scope-internal and routed through Open Questions, not problems/.
