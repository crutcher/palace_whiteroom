---
agent: abstractor
invoked_at: 2026-05-27T23:47:30Z
...
inputs:
  ...
  - book/src/L1-L0/ksp-solve-mutation-rotation.md (cycle-008 sister theme; structural precedent)
  - reports/2026-05-27T220558Z-lifter-eigsolve-linear-solve-failed-anchor/CYCLE.md (cycle-010 lifter, 10 callsite citations)
  - reference/palace/palace/linalg/arpack.cpp (ArpackEPSSolver + ArpackPEPSolver bodies; 4 opInv->Mult callsites)
  - reference/palace/palace/linalg/slepc.cpp (SlepcEPSSolverBase::Solve + 5 shell-matrix opInv->Mult callsites)
  - reference/palace/palace/linalg/nleps.cpp (QuasiNewtonSolver::Solve + deflated_solve opInv->Mult callsite)
  - reference/palace/palace/linalg/ksp.cpp:297-310 (BaseKspSolver::Mult void return, Mpi::Warning)
  - reference/palace/palace/linalg/eps.hpp:22-141 (EigenvalueSolver abstract base)
  - reference/palace/palace/drivers/eigensolver.cpp (driver-side composition + double-solve refinement)
  - reference/palace/palace/models/modeeigensolver.cpp (backend dispatch site; sort-by-target re-ordering)
---

# CYCLE: L1>L0 theme sketch — eigsolve-mutation-rotation

## Summary

This dispatch authors the **`eigsolve-mutation-rotation` L1>L0 theme** at `book/src/L1-L0/eigsolve-mutation-rotation.md`, documenting how the L1 `eigsolve(E, control) -> EigResult` form (cycle-009 rough-in, cycle-010 lifter LinearSolveFailed annotation) lowers into the Palace L0 eigensolver source: three concrete `EigenvalueSolver` subclass families (`ArpackEigenvalueSolver`, `SlepcEigenvalueSolver`, `NonLinearEigenvalueSolver::QuasiNewtonSolver`) coupled to an inner `ComplexKspSolver` via 10 `opInv->Mult` callsites. The theme follows the structural-precedent shape of `ksp-solve-mutation-rotation` (cycle-008), decomposed into **four sub-patterns** (A: setup/backend selection, B: inner-solve mutation-rotation, C: result-status flow, D: teardown). The theme is **firm** at the structural level (the four-sub-pattern decomposition is well-supported by direct source reading); the per-status materialisation for `LinearSolveFailed` is documented as a **partly-constructive lowering** (per cycle-010 lifter's defer-verdict: "the L0 form does not have the variant; the lowering says 'if Palace were to plumb the inner-solver status, here is the rewrite shape'"). The theme acknowledges 3 sibling cycle-009 eigsolve OQs (`eigsolve-scaling-coordinate-convention`, `eigsolve-initial-space-axis-placement`, `eigsolve-iteration-count-result-field`) as out-of-scope (separate wave-2 dispatch #8 lifter).

The dispatch adds 3 proposed-changes: (a) create the new theme file, (b) append theme entry to `book/src/L1-L0/index.md` dep-map, (c) add chapter entry to `book/src/SUMMARY.md` under the L1>L0 Part. No new L1 operators are speculated — the theme decomposes into existing firm L1 vocabulary (`eigsolve`, `ksp_solve`, `apply_linop`, plus transitively `dot`/`nrm2`/`axpy`/`axpby`).

## Proposed changes

### Edit 1: create `book/src/L1-L0/eigsolve-mutation-rotation.md`
...
### Edit 2: append theme entry to `book/src/L1-L0/index.md` dep-map

```edit:book/src/L1-L0/index.md
[old]:| [ksp-solve-mutation-rotation](./ksp-solve-mutation-rotation.md) | `L1/ksp_solve` (firm) | `palace/linalg/ksp.cpp`, `palace/linalg/iterative.{hpp,cpp}` | rough-in *(firmed cycle-008)* |
...
[new]:| [ksp-solve-mutation-rotation](./ksp-solve-mutation-rotation.md) | `L1/ksp_solve` (firm) | `palace/linalg/ksp.cpp`, `palace/linalg/iterative.{hpp,cpp}` | rough-in *(firmed cycle-008)* |
...
### Edit 3: add chapter entry to `book/src/SUMMARY.md` under L1>L0 Part

```edit:book/src/SUMMARY.md
[old]:- [ksp-solve-mutation-rotation](./L1-L0/ksp-solve-mutation-rotation.md)
- [bicgstab-iteration](./L1-L0/bicgstab-iteration.md)
- [minres-iteration](./L1-L0/minres-iteration.md)
[new]:- [ksp-solve-mutation-rotation](./L1-L0/ksp-solve-mutation-rotation.md)
...
**None.** The theme decomposes entirely into existing firm L1 vocabulary
(`eigsolve`, `ksp_solve`, `apply_linop`, plus transitively
`dot`/`nrm2`/`axpy`/`axpby`). No new L1 operators are needed.

The `LinearSolveFailed` constructive-introduction is **internal to the
existing `eigsolve` L1 form** (the `EigStatus` sum type already carries
the variant per cycle-010 lifter annotation); the theme materialises the
lowering of this case, not a new operator. The partly-constructive
shape of Sub-pattern B records the rewrite forward-looking against a
future Palace refactor; no L1 speculation is required to support this.

## Supporting evidence

The theme cites 23+ L0 source ranges across `palace/linalg/eps.hpp`,
`palace/linalg/arpack.cpp`, `palace/linalg/slepc.cpp`,
`palace/linalg/nleps.cpp`, `palace/linalg/ksp.cpp`,
`palace/drivers/eigensolver.cpp`, and
`palace/models/modeeigensolver.cpp`. Each range was directly read via
`mcp__palace-codemap__read_range` during this dispatch and confirmed to
support the cited claim.

Key evidence anchors:

- **Backend dispatch site** —
  `palace/models/modeeigensolver.cpp:1030-1053` (ARPACK / SLEPc
  branch).
- **Ten `opInv->Mult` callsites** — per cycle-010 lifter enumeration,
  confirmed by reading each:
  - ARPACK 4: `arpack.cpp:574, 580, 761, 778`.
  - NLEPS 1: `nleps.cpp:514`.
  - SLEPc shell 5: `slepc.cpp:1858, 1965, 1978, 2076, 2159`.
- **`BaseKspSolver::Mult` body** — `ksp.cpp:297-310` (void return +
  Mpi::Warning; negative anchor for LinearSolveFailed).
- **Eigensolver Solve bodies** — ARPACK at `arpack.cpp:513-560`,
  SLEPc at `slepc.cpp:687-709`, QuasiNewton at `nleps.cpp:351-805`.
- **Driver-side composition** — `eigensolver.cpp:280-410` (setup +
  Solve + optional QuasiNewton refinement).
- **Per-pair extraction surface** — `eps.hpp:124-140`
  (`Solve` / `GetEigenvalue` / `GetEigenvector` / `GetError`) plus
  `slepc.cpp:711-716` (Higham un-scaling at accessor).

## Open questions / caveats

- **Partly-constructive lowering pattern needs methodology-level
  treatment.** Sub-pattern B's `LinearSolveFailed` materialisation is
  "the L0 form does not have the variant; the lowering says 'if Palace
  were to plumb the inner-solver status, here is the rewrite shape;
  current source is silent on this case'" — a shape the cycle-010
  lifter flagged for cycle-012 meta-phase consideration. This theme
  uses the pattern; whether the methodology should formalise it as a
  distinct theme-status (alongside `firm` / `rough-in` /
  `obstruction`) is a meta-phase question. Flagging for cycle-012
  meta-phase recurrence-2 (cycle-010 lifter was recurrence-1).

- **Three sibling cycle-009 eigsolve OQs remain open** —
  `eigsolve-scaling-coordinate-convention` (Algebraic-laws §5
  routes; affects Sub-pattern C's scaling passthrough), `eigsolve-initial-space-axis-placement` (affects Sub-pattern A's
  Stage A2 / per-call control surface), `eigsolve-iteration-count-result-field`
  (affects whether `EigResult` should carry an `iterations` field for
  driver-side reporting). These are explicitly out of scope per "one
  theme per invocation"; cycle-011 wave-2 dispatch #8 (lifter on the
  three OQs as a cluster) is the natural next step. Flag to
  cycle-011 wave-2 planner.

- **SLEPc EPSConvergedReason mapping is a sub-theme candidate.** SLEPc
  exposes a richer convergence-reason enum (`EPSConvergedReason`) than
  what `BaseKspSolver::Mult` surfaces. The current theme records the
  per-status mapping at sub-pattern C narrative-level but does not
  enumerate the full reason → `EigStatus` mapping. A future
  `slepc-convergence-reason-lift` sub-theme (cycle-012+ candidate)
  would carry the full table, including the
  `EPS_DIVERGED_BREAKDOWN` / `EPS_DIVERGED_SYMMETRY_LOST` →
  `LinearSolveFailed` mapping that the partly-constructive
  materialisation in Sub-pattern B references.

- **Driver-side double-solve composition (QuasiNewton refinement) is
  out of scope.** `palace/drivers/eigensolver.cpp:377-407` shows a
  pattern where the linear eigensolve's result is consumed as initial
  guesses by a subsequent QuasiNewton refinement — a *higher-level*
  composition that the L1 `eigsolve` form does not capture. This
  composition is more naturally an L2 / L4 monadic-composition
  pattern (`eigsolve` followed by `eigsolve` with initial guesses
  from the prior result); flagging as a cycle-012+ same-layer-cross-cutter
  observation candidate.

- **Sub-pattern recognition is structural-only at this dispatch.**
  Full per-step sub-rewrite verification — i.e., walking each line of
  each backend's body and confirming the per-step kernel decomposes
  into the cited sister-theme primitives — is deferred to a
  `lowering-verifier` cycle. This is the same approach as
  `ksp-solve-mutation-rotation`'s cycle-008 firm-promotion: the
  theme stands on structural-level coverage; per-line verification is
  a follow-up audit.
