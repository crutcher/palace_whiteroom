...
inputs:
  ...
  - book/src/L1-L0/matrix-weighted-norm-mutation-rotation.md (the firm PAIRED sibling theme)
  - book/src/L1/matrix-weighted-norm.md (the diagonal-sibling L1 operator)
  - palace/linalg/operator.hpp:386-394 (the two free-function bilinear-form decls)
  - palace/linalg/operator.cpp:621-639 (the two specializations)
  - palace/models/boundarymodeoperator.cpp:85, :90 (Poynting + cross-coupling callsites)
---

# CYCLE: L1>L0 theme sketch — bilinear-form-mutation-rotation

## Summary

Authors the missing L1>L0 mutation-rotation theme for the L1 operator
`bilinear-form` (the matrix-weighted inner product `xᴴ M y`, rough-in
test-coverage-bounded). The L0 surface is the two `linalg::Dot(comm, x, A, y)`
free-function overloads at `palace/linalg/operator.hpp:386-394` /
`palace/linalg/operator.cpp:621-639`, which differ only in the element-type of
the weight operator (`Operator` real / `ComplexOperator` complex). Both compute
the same three-step composition `A.Mult → Dot → return`, but with
**internally-allocated** workspace `Ax` (Category-4 synthetic workspace per
`mutable-workspace-pattern`) — distinct from the **caller-supplied** workspace
`Bx` of the sibling `matrix-weighted-norm` theme. The theme **reuses** two
already-firm sibling sub-themes (`apply-linop-mutation-rotation` Sub-pattern A
for the leading `A.Mult`; `dot-mutation-rotation` Sub-pattern A for the inner
`Dot`) rather than restating them. What this theme adds is the bilinear-form
machinery: the **internal `Ax` workspace** (ownership / lifetime / per-call
allocation), the **real-`A`-on-complex-`x` real/imaginary lane split** (same
plumbing as Sub-pattern B of `matrix-weighted-norm`), the **conjugation-
asymmetry reconciliation** between the L1 `xᴴ M y` convention and Palace's
arg-2-conjugated L0 `Dot` (resolved by Palace's argument-position choice
`Dot(comm, Ax, y) = yᴴ A x`), and the **non-Hermitian-`M` admissibility**
(distinct from the SPD precondition of `matrix-weighted-norm`). Status `firm`:
the rewrite is structurally exhaustive, self-verified against on-disk Palace,
and rests on the same positively-anchored ingredients as the sibling theme. The
L1 operator's `rough-in (test-coverage-bounded)` status is upstream and does
not gate this theme (precedent: `matrix-weighted-norm-mutation-rotation` is
firm over its rough-in L1).

## Proposed changes

```new:book/src/L1-L0/bilinear-form-mutation-rotation.md
...
[insert one row after the existing `matrix-weighted-norm-mutation-rotation` row at line 30 (the rows are roughly grouped by L1-operator-cohort; bilinear-form belongs next to matrix-weighted-norm because they share the L0 file block and inherited sub-themes). New row:
...
[insert one entry under the L1 > L0 Part, immediately AFTER the existing `matrix-weighted-norm-mutation-rotation` line at line 106. New entry:

- [bilinear-form-mutation-rotation](./L1-L0/bilinear-form-mutation-rotation.md)

The insertion places it adjacent to its diagonal-sibling theme, mirroring the L1 layer's adjacent placement of `matrix-weighted-norm` (line 74) and `bilinear-form` (line 75).]
```

## Speculative operators proposed

**None.** This theme lowers the existing L1 `bilinear-form` operator (rough-in
test-coverage-bounded) into existing firm L1 vocabulary (`apply_linop`, `dot`).
It proposes no new L1 vocabulary.

## Supporting evidence

L0 source (self-verified via `tools/citecheck/citecheck.py --anchor` against on-disk
`reference/palace/`):

- **`palace/linalg/operator.hpp:386-394`** — the two bilinear-form overload declarations.
  Real-`A`: `:386-389` (comment `:386-387`, decl `:388-389`). Complex-`A`: `:391-394` (comment
  `:391-392`, decl `:393-394`). Both comments document the form as `yᴴ A x` (the L0 convention).
  `--anchor 'Compute the bilinear form'` lands on `:386, :391`.
- **`palace/linalg/operator.cpp:621-629`** — real-`A` overload body. `ComplexVector Ax(A.Height())`
  (`:624`), `Ax.UseDevice(true)` (`:625`), `A.Mult(x.Real(), Ax.Real())` (`:626`),
  `A.Mult(x.Imag(), Ax.Imag())` (`:627`), `return Dot(comm, Ax, y)` (`:628`).
  `--anchor 'ComplexVector Ax'` lands on `:624` within range.
- **`palace/linalg/operator.cpp:631-638`** — complex-`A` overload body. `ComplexVector
  Ax(A.Height())` (`:634`), `Ax.UseDevice(true)` (`:635`), `A.Mult(x, Ax)` (`:636`),
  `return Dot(comm, Ax, y)` (`:637`). `--anchor 'ComplexVector Ax'` lands on `:634` within range.
- **`palace/models/boundarymodeoperator.cpp:85`** — Hermitian-`A` callsite (Poynting power):
  `0.5 * std::conj(kn) / omega * linalg::Dot(comm, et, *Bttr, et)`. `--anchor 'linalg::Dot'` OK.
- **`palace/models/boundarymodeoperator.cpp:90`** — non-Hermitian-`A` callsite (cross-coupling):
  `std::complex<double>(0.0, 1.0) / (2.0 * omega) * linalg::Dot(comm, en, Atn, et)`.
  `--anchor 'linalg::Dot'` OK.
- **`palace/linalg/nleps.cpp:675`** — informational Newton-denominator unweighted-`Dot` site
  (NOT a bilinear-form callsite; included only to confirm the L1 entry's note about callers
  inlining the unfolding manually). `--anchor 'Dot'` OK.

L1 entries and cross-theme precedents:

- `book/src/L1/bilinear-form.md` — the L1 operator (rough-in test-coverage-bounded).
- `book/src/L1/matrix-weighted-norm.md` — the diagonal-sibling L1 operator.
- `book/src/L1-L0/matrix-weighted-norm-mutation-rotation.md` — the firm paired sibling theme;
  structural template followed throughout. Key in-line citations: §"The caller-owned workspace
  `Bx`" (the contrasting workspace-ownership boundary), Sub-pattern B (the same real-`B`-on-
  complex-`x` lane-split machinery), §"Speculative L1 operators" (which explicitly defers
  authoring of `bilinear-form-mutation-rotation` to this theme).
- `book/src/L1-L0/apply-linop-mutation-rotation.md` Sub-pattern A — the inherited `A.Mult(x, y)`
  lowering.
- `book/src/L1-L0/dot-mutation-rotation.md` Sub-pattern A — the inherited inner `Dot(comm, a, b)`
  lowering; §"The conjugation asymmetry" — the inherited L1/L0 conjugation reconciliation.

## Open questions / caveats

1. **L0 surface naming-and-comment audit, narrow.** The L0 comments at
   `palace/linalg/operator.hpp:386, :391` both write `yᴴ A x` (the Palace-internal arg-2-conjugated
   form), but readers of the L1 entry may be confused by the L1 `xᴴ M y` form vs the L0 `yᴴ A x`
   form. This theme's §"Conjugation asymmetry" addresses it explicitly, but a future
   `lowering-verifier` audit may want to add a one-line callout-box at the top of the L1 entry
   stating "L1 uses arg-1 conjugation (`xᴴ M y`); L0 uses arg-2 conjugation (`yᴴ A x`); the
   lowering swaps argument positions". Filed as a downstream polish item, not a gate.

2. **Lifting note (working-notes-only, NOT for the theme prose).** The high→low discipline
   (CLAUDE.md "Layers are defined high→low") keeps lifting notes in working notes. **For OQ
   ledger record only**: lifting from the L0 callsite cohort (`boundarymodeoperator.cpp:85, :90`
   + `nleps.cpp:675`) to an L2 combinator would be cleanest if there were an L2 "weighted-inner-
   product reduction" combinator that subsumed both the diagonal weighted-norm case and the
   off-diagonal bilinear-form case under a single shape `\(x, M, y) -> reduce(M·something, dot)`.
   This is a candidate L2 lifting target, surfaced here as a downstream observation; not part of
   this theme.

3. **Variant-axis-coverage gaps remain L1-entry-owned, not theme-owned.** The two L1 entry's
   `rough-in (test-coverage-bounded)` gates (real-`x`/real-`M`/real-`y` variant absent;
   Cauchy–Schwarz tight case at `y = x` unexercised) are upstream conditions on the L1 operator
   that this theme **faithfully records** but does not attempt to close. A future direct unit
   test exercising `linalg::Dot(comm, x, A, y)` on a known weight (identity, diagonal positive,
   non-Hermitian) would close the L1 entry's gate without requiring any change to this theme's
   status.

4. **Lowering-verifier audit is the natural follow-up.** Per the sibling-theme convention, a
   `lowering-verifier` dispatch on this theme would (a) attach a `verified_against:` YAML block
   confirming each citation's `verdict: supports`, (b) verify the surface-form recognition is
   exhaustive (no un-cited `Dot(comm, x, A, y)` overload — the codemap surface confirms only the
   two overloads documented here), and (c) confirm the inherited-sub-theme boundaries hold. This
   is the standard cycle-N+1 follow-up, not a gate on this theme's `firm` status.
