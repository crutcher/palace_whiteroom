...
inputs:
  ...
  - palace/linalg/nleps.cpp:533-535,562-563,665-667 (S.fullPivLu().solve — NLEPS deflation)
  - palace/models/romoperator.cpp:765 (Ar.fullPivHouseholderQr().solve — ROM PROM online), :757-758 (disabled LDLT), :762-764 (stability comment)
  - book/src/L1-L0/dot-mutation-rotation.md (sibling BLAS-direct-solve theme template)
  - book/src/L1-L0/index.md, book/src/SUMMARY.md (registration sites)
---

# CYCLE: L1>L0 theme sketch — lu-solve-mutation-rotation

## Summary

The firm L1 `lu_solve` operator (landed cycle-022; `book/src/L1/lu_solve.md`) is the small-dense
direct solve `x = lu_solve(A, b)` — the coordinate-space counterpart of the large-sparse
`ksp_solve`. It carries a forward-reference to a not-yet-written `lu_solve-mutation-rotation`
L1>L0 theme (`book/src/L1/lu_solve.md:14,90`). This dispatch authors that theme: how the pure
L1 `lu_solve(A, b)` lowers into its L0 Palace source form — an **inline Eigen dense
factorize-and-solve overwriting the RHS buffer in place** (`S.fullPivLu().solve(SS)` at NLEPS,
`Ar.fullPivHouseholderQr().solve(RHSr)` at ROM). There is **no `palace/linalg/lu.cpp`**; the
operator is realized inline at the two L0 use sites via Eigen dense-decomposition calls. The
theme is `firm`, justification `structural` (+ one load-bearing-numerical recording for the
factorization-kernel axis). Two sub-patterns track the distinct call sites: **Sub-pattern A**
(NLEPS deflation, full-pivot LU, single- and multi-RHS) and **Sub-pattern B** (ROM PROM online,
full-pivot Householder QR, with the disabled-LDLT rejected alternative). The factorization-kernel
variant axis (LU vs QR vs rejected LDLT) is recorded as a **load-bearing numerical choice** —
the ROM source comment (`romoperator.cpp:762-764`) chooses QR "for maximal stability", which is
part of the algorithm, not a transparent trick. All L0 citations self-verified via
`palace-codemap` `read_range`/`search_text` this invocation.

## Proposed changes

```new:book/src/L1-L0/lu-solve-mutation-rotation.md
...
(The dep-map insert: the new `lu-solve-mutation-rotation` row goes after the existing
`assemble-diagonal-mutation-rotation` row; the two preceding rows are shown for anchoring. The
`minres-iteration` / `bicgstab-iteration` obstruction rows stay at the table tail.)
...
(SUMMARY insert: append the `lu-solve-mutation-rotation` chapter entry under the `# L1 > L0 —
Lowering` Part, after the existing `matrix-weighted-norm-mutation-rotation (stub)` entry at the
tail of the Part. The two preceding entries are shown for anchoring.)

## Speculative operators proposed

**None.** This theme lowers the already-firm L1 `lu_solve` operator (landed cycle-022). No new L1
vocabulary is proposed: the factorization kernel (full-pivot LU / full-pivot Householder QR /
rejected LDLT) is a **contracted variant axis of the existing `lu_solve` operator**, already
recorded in `book/src/L1/lu_solve.md` §Variant axes — not a new operator. No harvester promotion
is needed for this theme.

## Supporting evidence

All L0 citations were self-verified this invocation via `palace-codemap` (`read_range` /
`search_text` / `get_symbol_def`) — see the theme's §Verified-against for the per-citation
verification notes. Key findings:

- **No `palace/linalg/lu.cpp`.** `search_text` for `fullPivLu\(\)\.solve|fullPivHouseholderQr\(\)
  \.solve|ldlt\(\)\.solve|selfadjointView` over `palace/**/*.cpp` returns exactly 9 hits, all
  inline at two files: `nleps.cpp:533,534,535,563,665,667` and `romoperator.cpp:757,758,765`.
  `lu_solve` is realized inline via Eigen dense-decomposition method chains, not a standalone
  source file. The scope-correction in the dispatch is confirmed.
- **All solve-call line numbers verify exactly** (no drift): NLEPS LU at `533/534/535/563/665/667`;
  ROM QR active at `765`; ROM LDLT disabled at `757/758`; ROM stability comment at `762-764`;
  `if constexpr (false)` gate at `754`.
- **Factorization-kernel load-bearing-numerical evidence is positively anchored.** The active LU
  (`nleps.cpp:533`), the active QR (`romoperator.cpp:765`), the *disabled* LDLT
  (`romoperator.cpp:757-758`, under `if constexpr (false)`), and the "QR solve, for maximal
  stability …" comment (`romoperator.cpp:762-764`) are all read from positive source. The kernel
  axis is a stated stability decision (a deliberately-not-taken faster LDLT path kept in-source) —
  the canonical marker of a load-bearing numerical trick per the CLAUDE.md taxonomy.
- **Minor L1-file citation drift noted (not corrected here — out of dispatch scope).** The firm
  L1 `book/src/L1/lu_solve.md:103` cites the ROM assembly as `:728-735`; the verified active
  assembly is `Ar += Kr` (`:729`), `Ar += (1i*omega)*Cr` (`:732`), `Ar += (-omega*omega)*Mr`
  (`:734`), with `Vᴴ A2 V` via `ProjectMatInternal` at `:723`. This theme uses the corrected
  range `:720-734`. The L1-file range is a ±few-line approximation, not a wrong claim; flagged in
  §Open questions for a lifter/harvester pass, not edited (write-scope is this theme only).

## Open questions / caveats

1. **L1-file ROM-assembly citation `:728-735` is a ±few-line approximation.** `book/src/L1/
   lu_solve.md:103` cites `Ar += Kr + iω Cr − ω² Mr + Vᴴ A2 V` at `:728-735`; the verified active
   scalar-assembly lines are `729/732/734` with the `Vᴴ A2 V` projection at `723`. This theme uses
   the corrected `:720-734`. Not a content error (the assembly *is* in that region) — a candidate
   for a lifter/harvester citation-tightening pass on `lu_solve.md`. Recorded as a working note,
   not a chapter edit (out of this dispatch's write scope).
2. **No dedicated `.solve()` unit test** (inherited from the firm L1 operator). `test/unit/
   test-romoperator.cpp` exercises PROM matrix *assembly* (`CalculateNormalizedPROMMatrices` /
   `UpdatePROM`), not `SolvePROM`'s solve; NLEPS has zero dedicated unit tests. Per CLAUDE.md
   status guidance the absent test does not gate the syntactic-identity laws (the lowering is a
   structural expansion, not a convergence claim) — hence `firm`. A `lowering-verifier` audit
   attaching a `verified_against:` block is the standard follow-up.
3. **Real element-type variant is permitted-but-unwitnessed.** Every Palace site is complex
   (`Eigen::MatrixXcd` / `VectorXcd`); the real case is element-type-agnostic in the factorization
   but has no Palace witness. Flagged as a non-surfaced variant, not a separate sub-pattern
   (mirrors the L1 operator's treatment).
4. **Reverse-direction (L0→L1 lifting) note, working-notes only (high→low discipline).** The lift
   of the inline-Eigen solve back into the pure `lu_solve` form requires recognizing (a) the
   destination-is-RHS aliasing as a dead-buffer reuse (so the RHS can be treated read-only at L1)
   and (b) the transient decomposition object as having no observable lifetime at L1. These lifting
   prerequisites are recorded here as working notes; the formal chapter narrates only the forward
   (L1→L0) rewrite, per the CLAUDE.md "Layers are defined high→low" invariant.
