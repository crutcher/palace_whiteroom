...
algebra---\code{electrostatic} $=$ \code{gram\_reduce}$(w{=}1)\circ$\allowbreak%
\code{solve\_family}$\circ$\allowbreak\code{fe\_assemble}; \code{magnetostatic} the same with the
weight $1/(I_iI_j)$; \code{driven} $=$ \code{sparameter\_reduce}$\circ$\allowbreak%
\code{frequency\_sweep}$\circ$\allowbreak\code{fe\_assemble}$^{\times3}$; \code{transient} $=$
\code{fold\_solve}$\circ$\allowbreak\code{fe\_assemble}$^{\times3}$; \code{eigenmode} and
\code{boundary\_mode} $=$ \code{map readout}$\circ$\allowbreak\code{eigsolve}$\circ$\allowbreak%
