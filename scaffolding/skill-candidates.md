...
- **status**: candidate (batch-9 meta-phase default-accept eligible)
- **addressed pattern**: cycle-planner / orchestrator recruiting a dispatch off a stale `priorities.md` plan line whose target is already firm-landed on disk → no-op dispatch (cycle-031 had TWO in the D6 slot: `nleps-jacobian-action-mutation-rotation` firm c025, `concepts/eigsolve` authored c025). OQ `cycle-planner-pre-dispatch-existence-check-of-target-artifact`.

**Sketch of procedure** (cycle-planner-facing, pre-emission): before authoring a dispatch whose deliverable is a named artifact slug (an L_n operator, an L_{n+1}>L_n theme, or a `concepts/<slug>` page), the planner runs a one-line existence + maturity check on the target:
1. `ls book/src/<layer>/<slug>.md` (or `grep -rl '<slug>' book/src/<layer>/`) — does the target file already exist?
2. If it exists, read its `## Status` line: `firm` / `partly-constructive` / `rough-in` / `obstruction` / `stub`. A `firm` target with the proposed deliverable already present is a no-op — do NOT recruit; mark the plan line discharged.
3. Cross-check the cited OQs: if the priorities line names OQs that are already in the `scaffolding/open-questions.md` Closed-index, the line is stale.
4. Only recruit when the target is genuinely absent OR present-but-below-the-proposed-maturity (e.g. stub→firm, rough-in→firm with a real promotion route).
