...
- **proposed_by**: abstractor (cycle-031 D6 no-op) + layer-intro-author (cycle-031 D6-reroute no-op)
- **status**: promoted
- **promoted_to**: skills/verify-dispatch-scope-not-already-discharged/SKILL.md
- **promoted_at**: cycle-033 meta-phase (batch-9) / 2026-05-30
- **promotion_note**: promoted on recurrence-3 across batch-9 (c031 file-existence-staleness, c032 broader 4-of-6 deliverable-presence-staleness, c033 working precedent — the c033 cycle-planner ran the deeper check inline per-dispatch and landed 3/3 genuinely-open). Default-accept under low-bar policy (concrete procedure + recurrence ≥2). The c031 candidate sketched the file-existence check; the c032 OQ promoted it to deliverable-presence; the c033 working precedent realised the four-step procedure. Companion enactment: cycle-planner role-spec §Discipline ENFORCEMENT bullet wiring the procedure as MANDATORY pre-dispatch for any named-artifact-slug scope, AND friction-ledger `cycle-planner-stale-priorities-line-recruitment` codifying the friction. The original c031 sketch was the file-existence-only step; the promoted skill extends to the four-step deliverable-presence sequence (file existence + maturity / `verified_against:`-block presence + OQ-RESOLVED-grep + structural-block check).

- **addressed pattern**: cycle-planner / orchestrator recruiting a dispatch off a stale `priorities.md` plan line whose target is already discharged in one of four shapes the c027 file-existence bullet does not catch: (i) firm-on-disk at-or-above proposed maturity; (ii) audit-already-discharged (`verified_against:` block present); (iii) slice-audit RESOLVED in immediately-prior cycle; (iv) structurally test-coverage-gated promotion. The c027 bullet checks the cycle-record trail (`counts_after`/STAGING/struck-Backlog markers); this skill checks the artifact itself, which is the ground truth.

**Sketch of procedure** (cycle-planner-facing, pre-emission per-dispatch): see `skills/verify-dispatch-scope-not-already-discharged/SKILL.md` for the full procedure. Summary: four-step deliverable-presence sequence — (1) `ls book/src/<layer>/<slug>.md`; (2) if file exists, read `## Status` line + grep for `verified_against:` blocks; (3) `grep '<slug>.*RESOLVED\|<slug>.*CLOSED' scaffolding/open-questions.md`; (4) structural-block check (`rough-in (test-coverage-bounded)` / `partly-constructive` / `partial-obstruction` / `obstruction (opaque-library-ownership)` gates). Only recruit when ALL four pass. Record the check outcome inline per-dispatch in the plan (`reports/2026-05-30T150000Z-cycle-planner-cycle-033/CYCLE.md` §"Deliverable-presence verification" is the canonical worked example).

Complements the c027 cycle-planner §Discipline bullets (verify-candidate-is-genuinely-open via counts_after/STAGING/struck-Backlog) by adding the **direct on-disk artifact check** (the c027 bullets check the cycle-record/staging trail; this checks the artifact itself, which is the ground truth). Cheap (one `ls`/`grep` per scope). Promotion route: a cycle-planner §Discipline bullet + this skill.
...
**Sketch of procedure** (producer- AND critic-facing): for each `git mv old.md new.md` slug rename, (1) `grep -rn '<old-slug>' book/src/` for the COMPLETE occurrence set; (2) partition into path-links (`](...<old>.md)` — build-fragile) vs. bare slug-mentions; (3) map every occurrence to a report `[old]`/`[new]` edit (or the `git mv` for the file's own H1/§Slug); (4) explicitly enumerate same-suffix sibling slugs that are NOT being renamed (substring-collision guard) and confirm none are accidentally rewritten and none reference the renamed target via a now-stale path; (5) assert post-apply `grep -rn '<old-slug>' book/src/` returns zero (modulo the not-renamed siblings). Producer pastes the inventory + the zero-residual assertion; critic re-runs the grep independently and re-maps before clearing cross-reference-integrity.
...
**Motivating observation.** Cycle-051 D1 (a multi-file `delete:` of 8 themes) handled inbound live links from the 4 re-expressed leaves, the combinator home, and 3 analogy-gate files — but MISSED `book/src/L3/index.md`, which carried 6 live links (2 each on 3 rows) to the deleted `*-body-identity` slugs. Those would be hard `linkcheck2` build errors at integration. This is the cycle-019-class build-breakage pattern: a deletion whose inbound-live-link cleanup is done ad-hoc per recalled reference rather than by mechanical enumeration, and one surviving table is forgotten. The producer's own (c)/(e) de-link reasoning was thorough but incomplete precisely because there was no enumerate-then-check step.

**Sketch of procedure.** For every slug being deleted in a report's `delete:` fences, run a mechanical inbound sweep BEFORE asserting de-link completeness: `grep -rnoE "\[[^]]*\]\((\.\./)*<dir>/<slug>\.md\)" book/src` for each deleted slug, then subtract (i) the files themselves being deleted (their internal cross-links self-resolve) and (ii) the files the report's proposed-changes already edit. Any residual surviving file with a live link is an unhandled build-breaker → flag `cross-reference-integrity: fail` with the exact `file:line`. Code-span / path references (backticked slug, no `](...)`) are a separate LOW stale-not-breaking tier. Companion to `proposed-changes-fence-encloses-full-body-guard` (which guards fence shape, not inbound links).

### disciplined-cross-pipeline-combinator-mining-gate

**PROMOTED cycle-054 (batch-16 meta-phase, default-accept under low bar).** Promotion bar met three ways: (1) pattern observed ≥2 cycles (the single-witness → 2nd-pipeline-probe → discharge → mine gate ran end-to-end across c052 D6 / c053 D1 / c054 D1); (2) the sketch was concrete enough to write as SKILL.md directly (it became the four-point procedure verbatim); (3) the c054 D1 critic's `skill-uptake-survey: warning` is the exact telemetry the skill closes — the procedure was followed but uncited because no skill named it. `promoted_to: skills/disciplined-cross-pipeline-combinator-mining-gate/SKILL.md`. Owners: combinator-miner + cross-layer-cross-cutter. The batch-17 superset probe (`solve_family` general form) + the transient fold-vs-map check are the next live invocation.

**Motivating observation.** Cycle-054 D1 (`solve_family` combinator-miner) followed an exemplary but un-skilled discipline: it honored a ≥2-witness bar before authoring a cross-pipeline combinator, carried a load-bearing scope caveat (driven breaks shared-operator-capture → the fixed form is scoped, the general form is superset-deferred), used the negative 3rd-pipeline witness to *sharpen* scope rather than as supporting evidence, and explicitly guarded against over-unifying a `fold` (transient time-stepping carries state) into a `map`. `log/cycle-053.md:13` already names "single-witness → 2nd-pipeline-probe → discharge" as a reusable disciplined-mining gate, but no skill encodes it, so the critic's skill-uptake-survey can only mark a `warning` (procedure followed, none cited).

**Sketch of procedure.** When a combinator-miner report proposes a cross-pipeline combinator: (i) confirm ≥2 positive witnesses that are structurally identical at the load-bearing shape (codemap-verify each pinpoint); (ii) require any negative/break witness to be classified as a *scope boundary* (a distinct combinator / superset), NOT folded silently into a variant axis; (iii) check that no unprobed pipeline is implicitly claimed — each unprobed pipeline must be named as deferred, with a flag for the over-unification hazard (is the unprobed shape a `map` or a `fold`?). Emit `skill-uptake-survey: pass` when the report cites this gate; `warning` when the discipline is present but uncited. Companion to `propose-rotation` / `verify-rotation-citation`. status: promoted

**Batch-17 confirmation (cycle-057 meta-phase).** The gate was the load-bearing discipline across the WHOLE batch and worked exactly as designed at the `<2-witness` boundary every time: c055 D1 scoped `solve_family` fixed-operator-only (driven breaks shared-operator-capture → `map_solve` superset deferred); c056 D1 DEFERRED `map_solve` at 1 witness AND held the transient state-threaded fold OUT of the map family (the fold-vs-map guard); c057 D3 classified `SweepAdaptive` as a ROM **fold** (not a 2nd map witness — would have been a step-2 scope-boundary violation to count the frozen-ROM online sweep as an operator-varying map). The two-combinator MAP/FOLD factoring (`solve_family` independent-map vs `fold_solve` sequential-fold, both §3.7 `iterate_while` children) is the structural yield the gate's over-unification guard protected. No over-unification reached the artifact; `map_solve` correctly stays a single-witness coverage finding. Skill confirmed working — no refinement needed.

### by-kind-part-reorg

- **proposed_by**: repairer (cycle-076 layer-intro-author feature-Part by-kind reorg verification)
- **status**: open candidate

**Motivating observation.** The cycle-076 feature-Part by-kind reorg (flat 10-column `# Feature surfaces — entry points` Part → 3 sub-chapter groupings: spine-ROOT / driver-leaf / output-product) cited the cycle-071 layer-Part reorg as its precedent pattern and mirrored `book/src/L4/iteration-combinators-intro.md` for the group-intro convention, but named NO skill — so the critic could only mark `skill-uptake-survey: warning` (procedure followed, none cited). This is now the SECOND witness of the same "one-time by-kind sub-chapter reorg" shape (cycle-071 layer-Part = witness 1; cycle-076 feature-Part = witness 2), and the directive-3/mdBook by-kind grouping convention (`feedback_mdbook_subchapter_grouping_and_alpha_api`) means more Parts will be reorged this way (the book/-reorg-wave is meta-phase-sequenced). The procedure is mechanical and high-stakes for cross-reference-integrity (zero orphaned column links, alpha-within-kind, within-column ordering preserved, SUMMARY old-block must match disk exactly), yet there is no named skill that encodes the checks — so each reorg report draws a presence-gap warning.

**Sketch of procedure** (producer- AND critic-facing). For a one-time by-kind Part reorg: (1) snapshot the flat Part from `SUMMARY.md` and the matrix/dep-map; (2) partition columns/chapters into the directive-named kind groupings (NOT inventing kinds — the grouping taxonomy is fixed by the directive); (3) author one group-intro page per grouping (mirror `iteration-combinators-intro.md`: orientation prose + bulleted member list + ordering note), with member down-links resolving on disk; (4) nest SUMMARY into the groupings, asserting (a) the old-block matches disk EXACTLY, (b) ZERO column/chapter links dropped (no orphans — diff old vs new link sets, the only additions are the N group-intro pages), (c) alpha-within-kind ordering, (d) any deliberate within-cohort ordering exception (e.g. the FEATURE-SURFACE high→low within-column rule) preserved everywhere; (5) re-sort the matrix/dep-map to match + add bold group-header rows linking each group-intro; (6) assert ZERO body/status/citation changes (pure-structural). Emit `skill-uptake-survey: pass` when a reorg report cites this gate; `warning` when the discipline is present but uncited. Companion to `summary-md-surgical-insert` (the per-insert mechanic) — this is the whole-Part-reorg superset.

### foundation-blocker-dischargeability-probe

**PROMOTED cycle-093 (batch-29 meta-phase, default-accept under low bar).** Promotion bar met three ways: (1) pattern observed >=2 cycles (the scoped probe -> two-clean-outcomes -> gated-cascade gate ran end-to-end across c088 structure-side + c089 FP-side on matrix-weighted-norm AND c092 on bilinear-form, all DISCHARGE); (2) the sketch was concrete enough to write as SKILL.md directly (it became the four-step probe + gated-wave procedure verbatim); (3) a friction-ledger entry exists for the addressed pattern (matrix-weighted-norm-full-firm-cascades-thirty-file-reanchor-sweep resolved-via-the-probe-then-wave arc). promoted_to: skills/foundation-blocker-dischargeability-probe/SKILL.md. Owners: cycle-planner (sequences probe-then-wave) + lowering-verifier (runs the probe) + meta-phase (GO/NO-GO on the verdict).

**Motivating observation.** The cheap-probe-before-heavy-wave playbook discharged the long-held matrix-weighted-norm sqrt-foundation-blocker (NO-GO-held batches 25-27 -> scoped probes c088/c089 -> GO -> c091 cascade LANDED CLEAN) and then immediately the next convergent blocker bilinear-form (c092 DISCHARGE -> batch-30 LEAD). The pattern SEPARATES the law-confidence question (cheap, single-file) from the heavy-cascade cost (a separate gated wave), converting NO-GO-held-by-inertia blockers into GO-by-explicit-derivation flips WITHOUT ever forcing the gate. It was un-skilled (named only in the friction-ledger re-weigh notes + the planner reshapes), so the discipline lived as folklore across four probe dispatches.

**Sketch of procedure.** Scope to ONE file; test the firm-on-positive-structure / syntactic-identity escape law-by-law (read-off / exact-arithmetic-theorem-on-provable-premise / FP-constituent-inheritance vs genuinely-needs-a-positive-test); judge the missing test REDUNDANT (escape applies) or LOAD-BEARING (real ceiling); land ONE of two clean outcomes (DISCHARGE: verified_against + Status-narrow, token STAYS rough-in, queue the cascade as a gated wave; CONFIRM-CEILING: explicit-verdict Status note, stays rough-in) -- NEVER flip the token or cascade in the probe. The cascade is a separate dedicated batch-LEAD wave carrying the within-file + whole-book re-anchor disciplines. status: promoted

---

## inbound-link-sweep-before-slice-delete

**Status:** rejected (batch-31 meta-phase, cycle-099) — DUPLICATE of the already-promoted `deleted-slug-inbound-live-link-sweep` (cycle-051, batch-15), which owns the exact enumerate-then-check inbound-grep-before-delete verb. The candidate's load-bearing NEW contribution — the `grep -v <link-target-text>` self-exclusion grep-bug that swallowed 8 inbound links at c098-D1 — was FOLDED into that skill's Procedure step 2 as the "exclude by SOURCE-PATH prefix, NEVER by link-target text" warning, and the friction is recorded as friction-ledger `slice-deletion-inbound-link-sweep-self-exclusion-grep-bug`. A second standalone skill for the same verb would fragment the procedure. **rejected_reason:** duplicate of `deleted-slug-inbound-live-link-sweep`; the sharpening (source-path-prefix exclusion + relative-sibling-link sweep) folded into the existing skill instead.

**Motivating observation (cycle-098 critique, same-layer-cross-cutter orthog-slice-delete).** A slice-deletion report claimed "exactly 2 inbound cites" from a grep `grep 'slices/orthog\|orthog\.md:' | grep -v 'spec/slices/orthog.md' | grep -v 'spec/index.md'`. The `grep -v 'spec/slices/orthog.md'` self-exclusion filter — meant to drop the slice file's own lines — ALSO swallowed every inbound `[..](../spec/slices/orthog.md)` markdown link, because the link-target text literally contains `spec/slices/orthog.md`. Result: 6 external files with live `[..](..)` links to the slice (plus 4 relative `./orthog.md` links from a sibling slice) went unrepointed; PC-3's deletion would produce that many `mdbook-linkcheck2` hard build errors. The report instead repointed the only two backtick-inline-code references (which are NOT linkcheck errors). The exact load-bearing-link defect the deletion was supposed to prevent.

**Sketch of procedure.** To find ALL inbound links to a file `book/src/<dir>/<name>.md` slated for deletion: (1) absolute-path links — `grep -rnE '\]\([^)]*<name>\.md' book/src --include='*.md'`; (2) relative sibling links from the same directory — `grep -rnE '\]\(\./<name>\.md' book/src/<dir> --include='*.md'`; (3) exclude ONLY lines originating in the target file, by SOURCE-PATH prefix (`grep -v '^book/src/<dir>/<name>.md:'`), NEVER by link-target text (which is what the bug did). Then classify each hit: a `[..](..)` markdown link is a linkcheck2 hard error on deletion (MUST repoint or co-delete); a backtick inline-code reference is build-safe (optional repoint). Cross-check the count against `SUMMARY.md` (the TOC row is itself a link and must be removed atomically with the file) and confirm no `depends-on` edge targets the file. Sibling-slice inbound links (e.g. `arnoldi_step.md → ./orthog.md`) count too if the sibling is still a live chapter at build time.

---

## cross-file-rename-completeness-sweep

**Status:** proposed (cycle-139 critique, layer-intro-author synthesis-residual-content-fidelity-followups).

**Motivating observation.** A report renamed `initial_state → initial_eig_state` across all 7 in-file occurrences in `book/src/L4/eigsolve.md`, and asserted in its open-questions "No L3-kernel or L4-sibling cross-file occurrence exists" / "No `eigsolve.md` occurrence was left un-flipped." The first clause was false: the same EigState-threaded eigsolve-cap constructor token also lived UN-FLIPPED in `book/src/L4/index.md:56` + `:132` (the eigsolve bullet + dep-map row) AND was the explicit subject of a deliberate NOTE in `book/src/synthesis/coordination.md:225-229` that pinned `eigsolve.md:44` by line and called the naming "a latent inconsistency to reconcile upstream." The cross-file occurrences were ASSERTED-absent (reasoned about) rather than GREPPED. A scoped rename whose cross-file index/sibling/synthesis-NOTE occurrences are left behind produces exactly the mixed-naming inconsistency the rename was meant to remove.

**Sketch of procedure.** When a report renames a token X→Y in one chapter and claims completeness: (1) `grep -rn '\bX\b' book/src --include='*.md'` for ALL occurrences repo-wide; (2) partition the hits by whether they describe the SAME entity being renamed (e.g. the eigsolve cap, EigState-threaded) vs a legitimately-different same-named entity (e.g. the `ksp_solve`/generic `solve_loop` `initial_state`, SimState-threaded — correctly stays); (3) the SAME-entity hits outside the edited file (its index/dep-map row, its synthesis render, any concept-page or NOTE that pins it by line) are the completeness gap — each is either a token-flip or, for prose NOTEs that DESCRIBE the old inconsistency, a re-phrase (the NOTE's premise goes stale once the fix lands). Flag the report's "no cross-file occurrence exists" claim against the grep, not against reasoning. Output the exact additional file:line list so the repairer can act mechanically.
