...
## Layered-spec progress (added cycle-002; updated cycle-008)

The 6-phase agent loop now builds the L4→L0 layered stack. Per-layer dep-map populations as of cycle-008:

- **L0** — **reference-note overlay bundle 1 landed cycle-005** (priority #10): 6 reference chapters (Conventions: `output-arg-vs-receiver`, `transparent-vs-load-bearing-tricks`, `mfem-vector-types`, `linalg-free-functions`; File overviews: `linalg-vector-file`, `ksp-factory-file`). **Bundle 2 landed cycle-006**: 2 new chapters (`apply-linop-overload-set`, `kspsolver-base-class`) plus new "Overload sets and class interfaces" L0 grouping. **Bundle 3 landed cycle-007**: 3 new chapters (`mfem-wrapper-solver`, `linalg-iterative-file`, `mutable-workspace-pattern`). **Bundle 4 landed cycle-008**: 3 new chapters (`eigensolver-wrapper` — Overload-sets-and-class-interfaces; `par-types-single-rank-reading` — Conventions; `linalg-operator-file` — File overviews). **14 chapters total** (well past initial ~13-chapter target; bundle 5 candidates surfaced — see cycle-008 OQ `l0-bundle-5-candidates`). Cycle-008 housekeeping: 5 L0 chapters had stale forward-declaration italic notes removed (cycle-007 thinning sweep cleanup).
- **L1** — **8 firm operators** (`axpy` pilot-1, `dot` cycle-002, `nrm2` cycle-003, `axpby` cycle-003, `scal` cycle-004, `apply_linop` cycle-004, `axpbypcz` cycle-004, `ksp_solve` cycle-007 — **first L1 op with structured opaque primary argument**, introduces Constructed-operator absorption motif) + **6 rough-in obstruction operators** (`lanczos_step`, `three_term_recurrence_update`, `givens_apply_with_residual_min` from `minres-iteration` theme; `bicgstab_step`, `omega_update`, `stabilisation_update` from `bicgstab-iteration` theme — **decision NOT to promote any landed cycle-005** per `scaffolding/decisions/2026-05-27-krylov-step-speculative-l1-promotion.md`). **Cycle-006 retroactive-thinned 4 L1 entries** (scalar-promotion sweep). **Cycle-007 retroactive-thinned all 7 pre-`ksp_solve` firm L1 entries** (~55% net Context shrink). **Cycle-008 L1/index refresh** added the closing-the-loop pointer at motif 4 to the new `ksp-solve-mutation-rotation` L1>L0 theme + `ksp_solve` dep-map row Status-cell annotated with the L1>L0 link (first cross-link to live in the Status column rather than Dependencies — see cycle-008 OQ `l1-intro-refresh-after-constructed-operator-gate` closure).
- **L1>L0** lowering — **6 themes**: `axpby-mutation-rotation` (cycle-002); `minres-iteration` (cycle-004, **obstruction**); `bicgstab-iteration` (cycle-004, **obstruction**); `apply-linop-mutation-rotation` (cycle-005, 5 sub-patterns A-E); `axpbypcz-mutation-rotation` (cycle-005, 4 sub-patterns + first mixed-justification sub-rule); **`ksp-solve-mutation-rotation` (cycle-008 wave-1 abstractor)** — first L1>L0 theme for a constructed-operator-absorption operator; 4 sub-patterns (A workspace allocation, B initial-guess threading, C statistics-counter mutation, D convergence-warning logging) × {CG, GMRES, FGMRES} outer-loop variants; displaced `(empty — Phase B skeleton.)` placeholder in `L1-L0/index.md` with first firm dep-map table. Mutation-rotation cohort now 4 themes; obstruction cohort 2 themes.
- **`concepts/dot.md`** — rewritten cycle-004.
- **`concepts/scalar-promotion.md`** — landed cycle-005.
- **L2** — **1 firm operator** (`krylov-step`, cycle-005).
- **L3>L2** — **1 firm-rough-in theme** (cycle-007 `krylov-step-body-identity`); status `firm-rough-in` inherits upstream L4>L3 `rough-in`. **Eligible cycle-009 mechanical promotion** to plain `firm` now that cycle-008 wave-1 lifter firmed the upstream L4>L3 theme (status-inheritance auto-eligibility per cycle-007 convention).
- **L4** — **3 firm operators**: `krylov-step` (cycle-006), `iterate_while` (cycle-007), `iterate_while_with_prev` (cycle-007). **Cycle-008 L4/index refresh**: Semantics-overlay placeholder replaced with grounded 4-motif overlay; new `## Vocabulary cohort` subsection added (template adapted: middle slot uses L4>L3 cross-layer themes rather than rough-in same-layer operators, since L4 currently has no rough-in operators; cycle-009+ meta-phase consideration via OQ `vocabulary-cohort-middle-slot-cross-layer-adaptation`); dep-map widened 4→5 columns with new `Lowers to` column splitting cross-layer references out of `Dependencies`. Closes cycle-006 OQ `l4-layer-intro-refresh-unblocked-by-first-firm-row`.
- **L4>L3** — **1 firm theme + 1 rough-in theme**: `krylov-step-typed-wrapper-dissolution` (cycle-006 wave-2 abstractor; **promoted cycle-008 wave-1 lifter** from rough-in → firm with two-form pruned/unpruned sketch + Condition 5 + L3-side Law-1 image reduction rule + 10-citation `verified_against:` YAML block; closes cycle-006 OQ `iterate-while-l3-rendering-trajectory-accumulation-gap`); `gmres-inner-loop-iterate-while-migration` (cycle-008 wave-2 abstractor, **rough-in** — pending upstream gmres.md §L4 v0.6→v0.7 self-rotation; documents migrated L4 form + L3 form + 4-axis variant pass-through; status `answered-by-rough-in-theme` on the source OQ).
- **L2>L1**, **L3** — Part skeletons only.

Forward indicator (post-cycle-008): L1 vocabulary at **8 firm operators** + Constructed-operator absorption motif registered; L1 intro motif 4 closed by the L1>L0 anchor landing this cycle. L2 has 1 firm. **L4 has 3 firm + Vocabulary cohort subsection** (closing the cycle-006 OQ on intro-refresh). **L4>L3 has 1 firm + 1 rough-in** (cycle-008 wave-1 lifter promoted the typed-wrapper-dissolution theme; cycle-008 wave-2 added the GMRES-inner-loop rough-in). **L3>L2 has 1 firm-rough-in theme**, eligible for mechanical promotion cycle-009 via status-inheritance (upstream now firm). L0 bootstrap at **14 chapters** (bundle 4 landed cycle-008). L1>L0 has **6 themes** including the cycle-008 ksp-solve-mutation-rotation. **5 OQs closed cycle-008** (`iterate-while-l3-rendering-trajectory-accumulation-gap`, `eigensolver-wrapper-l0-bundle-4-candidate`, `gmres-inner-loop-iterate-while-migration` via `answered-by-rough-in-theme`, `l1-intro-refresh-after-constructed-operator-gate`, `l4-layer-intro-refresh-unblocked-by-first-firm-row`; plus `ksp-solve-mutation-rotation-l1-l0-theme` flipped at finalize); 4 new OQs opened (1 critical for meta-phase: `abstractor-write-authority-violation-cycle-008`; 3 routine: `eigsolve-l1-operator-rough-in-candidate`, `matrix-weighted-norm-and-bilinear-form-l1-rough-ins`, `l0-bundle-5-candidates`). **Cycle-009 candidates**: mechanical L3>L2 status promotion (lifter or per-report inheritance update); L0 bundle 5; `eigsolve` L1 rough-in candidate; matrix-weighted-norm / bilinear-form L1 rough-ins. **Cycle-009 meta-phase batch aggregation** fires after cycle-009 finalize per 3:1 cadence.
