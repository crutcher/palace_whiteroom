...
- **L3>L2** — **1 firm-rough-in theme landed cycle-007**: `krylov-step-body-identity` (cycle-007 wave-1 abstractor; ratifies cycle-006 audit verdict; status `firm-rough-in` inherits upstream L4>L3 theme's `rough-in` status; auto-promotes to plain `firm` when upstream firms). **First L3>L2 theme of any status.** First cross-edge status-inheritance instance in the artifact.
- **L4** — **3 firm operators**: `krylov-step` (cycle-006), `iterate_while` (cycle-007), `iterate_while_with_prev` (cycle-007). The cycle-006 wave-2 abstractor audit confirms-with-refinement the cycle-002 identity-in-form claim for the L3>L2 body lowering, so no L3 row was promoted. Cycle-007 ratified the audit at L3>L2. The L4 `krylov-step` dep-map row's Dependencies cell was extended cycle-007 to list `iterate_while` + `iterate_while_with_prev` as 2 new L4-row deps (**first L4-row-on-L4-row dependency edge in the artifact**). L4 firm cohort now eligible for Vocabulary-cohort subsection template (≥3 firms per cycle-004 L1 precedent).
- **L4>L3** — **1 firm theme landed cycle-006**: `krylov-step-typed-wrapper-dissolution` (cycle-006 wave-2 abstractor; 4-part wrapper dissolution covering record dissolution, monad dissolution, readonly demotion, Form A/B presentation collapse). Status currently `rough-in` pending the cycle-008+ lifter patch (per cycle-007 wave-2 lowering-verifier audit verdict (c)) that lands the §3.8-citation revision at §"What the L3 form for iterate_while looks like" + Condition 5 + two-form sketch (pruned vs unpruned).
- **L2>L1**, **L3** — Part skeletons only.

Forward indicator (post-cycle-007): L1 vocabulary at **8 firm operators** + Constructed-operator absorption motif registered. L2 has 1 firm. **L4 has 3 firm (cohort growth 1→3 cycle-007)**, L4>L3 has 1 firm theme (rough-in pending lifter), **L3>L2 has 1 firm-rough-in theme** (first L3>L2 entry of any status). L0 bootstrap at **11 chapters**. Pending L1 harvests: `nrm2_B` (energy norm), possibly fused `normalize`. **Cycle-008 PRIORITY**: lifter patch on `book/src/L4-L3/krylov-step-typed-wrapper-dissolution.md` (closes `iterate-while-l3-rendering-trajectory-accumulation-gap` OQ; auto-promotes L3>L2 `krylov-step-body-identity` from `firm-rough-in` to plain `firm`). Other cycle-008+ candidates: `ksp_solve @ L1>L0` abstractor; L1/index refresh; same-layer-cross-cutter on stale L0 forward-declaration notes; L0 bundle-4 (eigensolver-wrapper); GMRES-inner-loop iterate-while migration abstractor.
