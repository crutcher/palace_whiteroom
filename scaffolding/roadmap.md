...
## Layered-spec progress (added cycle-002; updated cycle-010)

The 6-phase agent loop now builds the L4→L0 layered stack. Per-layer dep-map populations as of cycle-010 (first primary cycle of meta-batch-2; cycle-009 closed meta-batch-1):
...
- **L3** — **1 firm operator** (`krylov-step`, cycle-010 wave-1 harvester — **first firm L3 operator in the artifact**; identity-lowering backfill enacting CLAUDE.md §Methodology invariants new bullet "Identity-lowerings still require both L levels" codified cycle-009 meta-phase; L3 layer transitions from placeholder-only to 1-firm-operator status; cycle-006 verdict "no L3 row needed for krylov-step" formally SUPERSEDED). Cohort growth candidates routed via cycle-010 wave-1 pass-2 audit's 4 OQs (priority `l3-backfill-apply-linop-and-blas1-cohort` HIGH CONFIDENCE for `apply_linop` L3 + 6-entry BLAS-1 cohort backfill).
...
- **Phase 1 slice corpus reduction first instance landed cycle-010** — 3 slices reduced (gmres.md 1144→671 -42%; cg.md 506→165 -67%; arnoldi_step.md 330→302 -8%; net 842 lines removed from audited slice subset); first concrete realization of cycle-009-meta-batch-1-closure user-directive "Phase 1 corpus reduces as material is lifted" (priority #19). Audit template established and machine-replayable for cycle-011+ batches; remaining 7 slices routed via cycle-010 OQ `phase-1-corpus-reduction-remaining-7-slices` with priority order orthog → chebyshev → polynomial_recurrence_step → divfree → cg_preconditioning_framework → plane_rotation_stream → sparse_triangular_solve; batch-size suggestion 2-4 slices per dispatch.

Forward indicator (post-cycle-010, first primary cycle of meta-batch-2): L1 vocabulary at **8 firm + 3 rough-in (test-coverage-bounded / lower-layer-shared-vocabulary) + 6 rough-in (obstruction)**. L2 has 1 firm. **L3 has 1 firm** (krylov-step landed cycle-010 — first firm L3 operator). **L3>L2 has 1 firm theme** (krylov-step-body-identity, promoted cycle-009 via status-inheritance). L4 has 3 firm. **L4>L3 has 1 firm + 1 rough-in**. L0 bootstrap at **16 chapters** unchanged. L1>L0 has **6 themes** unchanged. **Phase 1 slice corpus reducing**: 3 slices reduced cycle-010 (~842 lines net removed); remaining 7 slices routed for cycle-011+ batches. **Cycle-010 incremental delta over batch-1 close**: L1 rough-in operators **1 → 3** (+matrix-weighted-norm +bilinear-form); L3 firm operators **0 → 1** (+krylov-step backfill, first firm L3); slice corpus -3 slices reduced (gmres / cg / arnoldi_step); net OQ ledger **3 partial-answer flips + 18 new opens** (net +15 — priority #19 + #20 audit dispatches contribute 8 routing OQs; priority #17 harvester dispatches contribute 8 follow-up OQs). **Cycle-011+ candidates per cycle-010 integrator-signals**: L3 `apply_linop` + BLAS-1 cohort backfill (priority #20 second target follow-on); `L4/index.md:40` SUPERSEDED text drift (small lifter); priority #13 close routing OQ; `eigsolve-mutation-rotation` L1>L0 abstractor dispatch; 3 remaining cycle-009 eigsolve OQ cluster; next phase-1 corpus reduction batch (orthog → chebyshev → polynomial_recurrence_step); FGMRES lifter sequencing per cycle-010 wave-2 #6 routing OQ; `gmres.md §L4 v0.6 → v0.7` self-rotation (still deferred); L0 bundle 6 (still deferred); `l1-orthogonalize` candidate. **MCP codemap pilot retry SUCCESS** cycle-010 wave-2 #6 (14 calls, 0 permission-denied) — friction-ledger entry `mcp-codemap-permission-denied-across-batch-1` is resolution-candidate for cycle-012 meta-phase enactment. **Meta-phase batch-2 aggregation** fires after cycle-012 finalize.
