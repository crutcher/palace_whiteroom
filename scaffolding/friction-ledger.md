...
last_observed: cycle-155
recurrence_count: 2
status: addressed
addressed_by: meta-150 (A–F scan codified) + meta-153 (E-grep widened to the date-less `meta-review #N` arm; the `## Context` orientation-vs-process distinction codified into the finalization-debulk skill) + meta-156 (the converging trajectory reached its fixed point — the genuine-debt count went to 0 via a comprehensive classification, NOT a self-characterized cohort tally)
---
```

**Batch-51 note (meta-156 — the CONVERGENCE: the pattern's predicted fixed point was REACHED; recurrence STAYS 2, status STAYS `addressed`, NOT escalating — this is the *terminating* observation of the pattern, not a new recurrence).** Batch-51 (cycles 154/155/156) drove the finite maintenance backlog to convergence (the human directive). The c154 once-per-batch full-hygiene sweep delivered exactly what this pattern's corrective demands — a **comprehensive book-wide classification** rather than a self-characterized cohort tally: the 61 `untyped` pages = **(a) 35 non-DAG carve-outs + (b) 26 `L0/` ground-truth leaves + (c) 0 genuine-untyped DAG nodes** (the `(c)=0` independently critic-confirmed: no L1/L2/L3/L4/lowering/record node is untyped). This is the pattern functioning *as designed and to completion*: the comprehensive scan (not the campaign's self-claim) established the true residue, which turned out to be exactly zero genuine debt. The c155 convergence then enacted the corresponding lint-definition fix (carve out the by-design outside-DAG set so the `untyped` warning counts genuine edge-typing debt only), bringing genuine `untyped` 61→0 and discharging the LAST finite maintenance item `p1-edge-typing-true-detritus-sweep`. The "61 looked like 61 nodes of debt" surface impression was resolved by the comprehensive classification showing (c)=0 — exactly the lesson of this pattern (a count is not debt until a comprehensive scan classifies it). NOT escalating: recurrence stays 2; this is the pattern reaching its **asymptote** (a fixed-point scan whose "complete" claims are mechanically verifiable), the convergent end-state the batch-50 note predicted. The future-debt-detection probe (a temporary untyped `L1/__probe` STILL surfaces as genuine debt) proves the carve-out is precise, not a blanket warning-off — so the scan stays load-bearing for any *future* genuine residue. No corrective forward-frontier work-item is warranted; the maintenance floor that surfaced and discharged this is the steady state.


**Batch-50 note (meta-153 — RECURRED, recurrence 1→2; CONVERGING, NOT escalating; the residue is asymptotically shrinking and the scan is now self-widening).** The batch-50 D/E/F de-bulk campaign ran to a CLEAN book-wide A–F scan (A=0, B=0, C=0, D=0, E=2-KEEP-files-only, F=0) — the campaign-complete claim is honest *for the defined A–F scan*. But the same pattern recurred in its characteristic form: the comprehensive scan surfaced **two NEWLY-adjacent residue sub-classes the A–F regex does not target** — (1) a **date-LESS `meta-review #N`** E-sub-class (`concepts/dependency-map.md:92-93`; the dated `2026-0X-XX` E-grep misses it), and (2) a **pre-existing DUPLICATE concept body** in `concepts/constructed-operators.md:175-213` (content redundancy / de-dup, NOT process accounting — a *different* defect class entirely). This is the same lesson as batch-49 (each comprehensive scan finds a new adjacent sub-class — first the F-cohort beyond L1/index in batch-49, now the date-less meta-review-#N + duplicate-body sub-classes), but the **trajectory is convergence, not escalation:** the residue tail is the finite, fully-enumerable set of slice-era process forms, and the scan-definition is *self-correcting* — each recurrence widens the standing scan (batch-50 added the `meta-review #[0-9]` E-arm to the skill grep) so the *next* sweep catches the sub-class mechanically. The asymptote: a fixed-point A–F+ scan that catches all known process-residue forms, after which "complete" claims are mechanically verifiable. **Addressed (meta-153 go):** the E-grep widened (`meta-review #[0-9]` arm); the `## Context` orientation-vs-process-narrative distinction codified into the `finalization-debulk` skill (resolving the c151 scoping OQs); the date-less-meta-review-#N residue (2 refs in `dependency-map.md`) + the duplicate-body de-dup migrated to the plan as tiny LOW/hygiene follow-ups. NOT escalating — recurrence 2 is the *expected* shape of an asymptotically-shrinking residue tail being progressively mechanized; the corrective is cheap and standing each time.
...
**Batch-50 note (meta-153 — the FINALIZATION-residue tail is now FULLY EXHAUSTED: the A–F scan is book-wide CLEAN; recurrence STAYS 3, status STAYS `addressed`, NOT escalating; the §CENTRAL ASK returns a 9th time, with the strongest handoff-ready signal yet).** Batch-50 (cycles 151/152/153) was the D/E/F FINALIZATION-residue de-bulk CAMPAIGN — the last finalization debt the batch-49 sweep adjudicated. It ran to COMPLETION: all 26 D/E/F targets de-bulked (pilot c151 + 12 c152 + 13 c153), D-class → 0, and the **book-wide A–F completion scan is CLEAN** (A=0 B=0 C=0 D=0 E=2-KEEP-files F=0). The graded-stack baseline HELD EXACTLY all 3 cycles; both hard invariants held; build EXIT 0; 148th consecutive cycle under the split integrator. This is the 9th consecutive in-scope *forward-frontier* steady-state-complete batch (41→50). The texture shift batch-49 flagged (the floor has real residue-yield) is now RESOLVED in the convergent direction: the residue tail was finite and is now exhausted — the in-scope artifact is not only complete + finalized but **book-wide residue-clean**, the strongest handoff-ready signal in the project's history. Two tiny adjacent sub-classes remain (date-less `meta-review #N`; one duplicate concept body), migrated to the plan as LOW/hygiene follow-ups, captured under `completeness-claim-vs-comprehensive-scan` (above). This does NOT re-escalate the plateau — the forward-frontier R&D remains complete; the strategic §CENTRAL ASK (9th time) is correctly the human's call, and the accumulating evidence (9 in-scope-complete batches + the now-fully-exhausted finalization-residue tail) further STRENGTHENS the standing (C) downstream-burn-handoff recommendation. No corrective forward-frontier work-item is warranted.
...
**Batch-44 instance + new-surface guard (cycle-138 meta-phase — narrow self-resolved recurrence; status stays `resolved`).** The SYNTHESIS `# Synthesis` Part stand-up surfaced the SAME drift-class on a NEW derived surface — the `synthesis/index.md` **matrix-mirror** Status cells, hand-maintained independently from each chapter's frontmatter. c136 normalized the 3 calculus-library chapters' frontmatter (to the no-`status:`-field filled-VIEW convention) but **left 3 stale `stub (Wave 2)` matrix cells** on `synthesis/index.md` (`:37-39`); c137's finalize mopped them up (`stub (Wave 2)` → `navigational (rendered)`) in the same pass that flipped the `drivers` row. This is the identical "the index/matrix mirror cell drifts independently from a chapter's status frontmatter; a finalize that flips a chapter's rendered-status should also check its matrix-mirror cell in the same pass" failure (OQ `synthesis-index-per-library-status-cell-rendered-completeness-convention`, c137; CLOSED-RESOLVED this meta-phase). **Self-resolved within the batch** (c136→c137, one mop-up edit, zero artifact damage) — a narrow new-surface instance, NOT an escalation: the guard is the same promotion-time same-pass-flip discipline. **The guard is now CODIFIED for the Synthesis surface** in `.claude/agents/layer-intro-author.md` §SYNTHESIS (a dedicated bullet: when a `synthesis/` chapter's rendered-status / frontmatter flips, edit the `synthesis/index.md` matrix-mirror cell in the SAME pass; `linkcheck2` does not catch matrix-cell drift). No status change (stays `resolved`); the heavyweight finalize-time consistency check stays UNENACTED. *Re-open trigger unchanged:* a flip landing without the matching matrix-cell touch despite the codified guard.
...
## katex-dollar-sigil-eaten-in-indented-pseudocode

```yaml
---
slug: katex-dollar-sigil-eaten-in-indented-pseudocode
first_observed: cycle-133
last_observed: cycle-139
recurrence_count: 2
status: addressed
addressed_by: batch-43 meta-phase (cycle-135) — (a) post-build assertion codified into integrator-finalize.md step 5c (no `<pre>` may contain `class="katex"`); (b) a `$`-sigil-MUST-be-fenced reminder clause added to the `**Fenced**` bullet of all 4 firm-body producers (harvester/abstractor/lifter/layer-intro-author). Out-of-band root-cause fix already committed `988d2f6` (27 files / 30 blocks, indented→fenced). Memory `project_katex_dollar_sigil_fence_requirement`. // batch-45 meta-phase (cycle-141) — recurrence-2 (c139 D2): added a THIRD guard — a fence-aware **pre-apply lint** at integrator-per-report (new step-5 gate; flags any edit-payload line that is 4-space-indented AND outside a fence AND contains a `$`-sigil token, BEFORE the write), moving the catch from finalize-build-time to per-report-apply-time. The producer prose reminder (b) was already on the abstractor when D2 re-drifted, so prose-only is insufficient; the pre-apply mechanical check is the durable upgrade. The 5c post-build assertion stays as the backstop. STILL `addressed` (not escalating: c139 was caught by 5c, did NOT ship past the gate — the watch-clause escalation trigger was not met).
---
```

**Pattern (the named-shape-group `$`-sigil notation collides with the KaTeX `$...$` math delimiter when it sits in a 4-space-INDENTED pseudocode block).** The mdBook `[preprocessor.katex]` renders `$...$` as inline math. It correctly **skips fenced code blocks AND inline code spans**, but it does **NOT** skip **indented code blocks**. So a pseudocode block written with 4-space indentation (rather than a ` ```text ` fence) that contains the named-shape-group `$`-sigil tokens (`Tensor[$S]`, `LinOp[$S,$S]`, `$N` — user directive 2026-06-06, `project_named_shape_groups_notation`) has its `$...$` runs eaten as inline math, injecting `<span class="katex">…</span>` garbage into the rendered `<pre>`. The defect is invisible to the source author (the markdown looks fine) and to `linkcheck2` (it is not a link) — it manifests only in the built HTML. The CLAUDE.md notation convention ALREADY mandates ` ```text ` fencing for L4/L3 pseudocode; agents had drifted to indented blocks (the indent reads naturally and the pre-`$`-sigil era had no collision, so the drift was harmless until the `$`-sigil notation landed at batch-36/37).

**Evidence (out-of-band this session, root-caused + fixed at `988d2f6`).** The user reported garbage-rendered code blocks in `L4/inner_product` and `L4/fe_assemble`. A repo-wide audit found the collision across **27 files / 30 blocks** — every indented pseudocode block carrying a `$`-sigil token. All were fixed by converting the indented blocks to fenced ` ```text ` blocks (pure fencing — zero content change). Post-fix, the authoritative assertion (`class="katex"` inside any `<pre>` across all built HTML) returns **0** (re-confirmed at the c134 + c135 finalizes). This is recurrence-1 (a single root-cause class, fixed once).

**Decision (batch-43 meta-phase — go, dual durable guard).** Two complementary enactments so the class cannot silently recur: **(a) a post-build assertion** in `integrator-finalize.md` (new step 5c) — after `cargo make book`, assert no `<pre>` block contains `class="katex"`; a hit blocks (build-readiness gate) and the surgical repair is indented→fenced. This is the TRUE detector (it sees the rendered collision; a *source* grep `^    .*\$[A-Z]` false-positives on `$`-sigil lines that are already inside a ` ```text ` fence but indented for alignment, so the source grep is NOT reliable). The integrator already ran this scan informally at the c134/c135 finalizes; 5c makes it a standing step. **(b) a producer reminder** — a `$`-sigil-MUST-be-fenced clause on the `**Fenced**` bullet of harvester/abstractor/lifter/layer-intro-author, explicitly noting it does NOT conflict with the batch-6 "4-space-indent inside a proposed-changes block" rule (that rule is about nested-fence parser-toggle inside the *proposed-changes* fence; the KaTeX rule is about the *landed chapter body* — for `$`-sigil content the resolution is a fenced ` ```text ` sample, which the modern proposed-changes mechanism handles). **Watch / re-open as escalating** if a future cycle SHIPS a `$`-sigil indented block past the 5c assertion (the gate was skipped) — at which point a `tools/`-code pre-build source check (a fence-aware scanner, ask-class) earns its cost; the post-build assertion is the lower-friction durable guard for now. NO session-restart-blocking subtlety: the producer-spec edits + the integrator-finalize edit DO require a session restart before the next cycle (agent-defs changed).

**Decision (batch-45 meta-phase, cycle-141 — go, third guard: a per-report-integrator PRE-APPLY lint).** Recurrence-2 fired at c139: the abstractor (D2) re-introduced a 4-space-indented `$`-sigil signature line (`lanczos_step :: LinOp[(S: ...), $S] -> ...`) in its proposed-changes despite carrying the (b) producer prose reminder — so prose-only is demonstrably insufficient, and the c139 round-trip cost a finalize-time build-repair. The watch-clause's escalation trigger (a `$`-sigil block SHIPPING past the 5c assertion) was NOT met — 5c caught it — so this is NOT yet the ask-class `tools/`-code fence-aware scanner; the lower-cost durable upgrade is a **role-spec pre-apply lint** moving the catch one phase earlier. Enacted: a new `integrator-per-report.md` step-5 gate (`katex-dollar-sigil-pre-apply-fence-lint`) — before writing each ` ````edit:` payload whose target is a `book/src/**` chapter, scan the payload with a FENCE-STATE tracker (toggle on each ` ``` ` line) and flag any line that is (i) outside a fence AND (ii) 4-space-indented (or a markdown indented-code-block continuation) AND (iii) contains a `$`-sigil token (`$S`/`$N`/`$`-letter). This avoids the unreliable bare-grep's false-positive (a `$`-sigil line indented FOR ALIGNMENT inside a ` ```text ` fence is NOT flagged, because the fence-tracker knows it is fenced). On a hit, the per-report integrator wraps the offending block in a ` ```text ` fence per the standing convention (build-repair authority, the indented→fenced surgical fix) and records `katex-dollar-sigil-pre-apply-fenced` in the staging-row Notes; if the wrap would alter load-bearing structure it marks the row `deferred` for the planner. The 5c post-build assertion STAYS as the backstop (catches anything the per-report lint's heuristic misses + non-edit-block drift). Status stays `addressed` (three complementary guards now: pre-apply lint → producer prose reminder → post-build assertion); recurrence_count 1→2. Watch / re-open as `escalating` only if a `$`-sigil indented block still ships past BOTH the pre-apply lint AND the 5c assertion — then the ask-class fence-aware `tools/` scanner earns its cost. The `integrator-per-report.md` edit DOES require a session restart before c142.
