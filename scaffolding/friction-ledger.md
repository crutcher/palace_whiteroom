...
recurrence_count: 4
status: escalating
addressed_by: cycle-015 meta-phase (4 producer role-spec self-verification Discipline bullets: harvester / abstractor / lifter / layer-intro-author + skill `verify-citation-range` §"Producer self-verification before emitting citations" section) — INSUFFICIENT; recurrence-4 fired batch-5; mechanical codemap-backed citation-range checker tool ESCALATED from defer to ASK→go-recommended (batch-5 meta-phase) → **ENACTED (user go, 2026-05-29): `tools/citecheck/`** — a stdlib-only Python lint that resolves every `path:lo-hi` citation against `reference/` (+ `book/src/`), bounds-checks it, and with `--anchor` reports DRIFT(±N) + a suggested corrected range. Modes: inline, `--anchor`/`--regex`, `--scan <CYCLE.md>` (pre-integration bounds + path-hygiene lint), `--batch`, `--json`; non-zero exit on any failure. Validated against batch-5 drift cases (e.g. cycle-019 `vector.cpp:667`→`:668` MFEM_ASSERT drift detected + suggested). **Remaining step: role-spec wiring** — NOT done in this enactment; left for the next meta-phase to fold `citecheck --scan/--anchor` into the producer self-verify bullets + the critic/lowering-verifier/integrator-per-report procedures (write-authority partition). Until then available for manual/dispatch invocation. See `tools/citecheck/README.md` §"Suggested integration points".
---
```
