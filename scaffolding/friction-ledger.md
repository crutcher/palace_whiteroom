...
recurrence_count: 6
status: escalating
addressed_by: cycle-033 meta-phase (batch-9) — (a) c033 cycle-planner WORKING PRECEDENT; (b) skill `verify-dispatch-scope-not-already-discharged`; (c) cycle-planner role-spec ENFORCEMENT bullet. **STRENGTHENED cycle-036 meta-phase (batch-10) — recurrence-3-WITHIN-BATCH-10 after the batch-9 codification proved the prompt-level + skill-level codification insufficient at the planner side**: (d) cycle-planner role-spec bullet AMENDED to require **PASTED INLINE EVIDENCE** (literal `ls`/`grep`/`## Status`-line output, not assertion of compliance); (e) skill `verify-dispatch-scope-not-already-discharged` extended with §"Batch-10 strengthening" + steps 5 (STOP-PROPOSING NEGATIVE LIST consult) + 6 (audit-first vs reflexive-harvest framing); (f) c036 cycle-planner WORKING PRECEDENT (paste-inline-evidence pattern); (g) the STOP-PROPOSING NEGATIVE LIST recorded prominently in `priorities.md` Backlog as the anti-recurrence data feed. **Swap-to-opus stays HELD** — surfaced as a meta-phase ASK to the human (the c036 paste-evidence success suggests haiku CAN run the procedure when required to paste; the cumulative recurrence-6 across batch-7/9/10 is strong enough evidence to ask the human to authorize the model change if recurrence-7 surfaces).
---
```
...
**Why not swap to opus (batch-9 reasoning, batch-10 confirmation).** The c033 working precedent demonstrated **the deeper check CAN be done by haiku** — the c033 planner ran 3/3 clean with the new procedure. The c036 paste-evidence working precedent re-confirmed it — when the planner is required to paste literal evidence, the check actually runs. The fix is in the *procedure*, not the *tier*. Swap-to-opus stays HELD; surfaced as a meta-phase **ASK** to the human for the recurrence-7 threshold (the cumulative recurrence-6 across batch-7 c026/c027 + batch-9 c031/c032 + batch-10 c034/c035/c036 is strong but not yet decisive against the paste-evidence pattern).
...
**Watch (batch-11 inputs).** Confirm c037+ dispatches against the c036 paste-evidence procedure: every multi-dispatch plan should carry a `## Deliverable-presence verification` section with *literal pasted command output* per dispatch (not paraphrased prose; not just "all checks passed"). If recurrence-7 surfaces despite the paste-evidence demand:
- Re-open the swap-to-opus ask (the ask is now surfaced this meta-phase; the human decision sets the trigger threshold).
- Consider the mechanical pre-dispatch gate.
- The cumulative pattern across batch-7/9/10 (6 recurrences after multiple codifications) would be strong evidence the haiku tier has reached its ceiling for this multi-step verification.
