...
**Scope nuance (load-bearing; UPDATED batch-11).** The drift has now been observed on **three distinct boundaries**: the `nleps.cpp` deflation block (re-touched cycles 024/025/026), `operator.cpp:601` (cycle-027), and `iterative.cpp` `CgSolver::Mult` `:360→:361` (cycle-039). All three share the same shape — a comment-line or `template`-declaration line merged with the immediately-following body-open line, producing a +1 on the body anchors. The recurrence on a **third independent boundary** strengthens the read that this is a *systematic codemap-server line-indexing behavior on a specific source-construct class* (non-body line preceding a body-open), not a fluke localized to `nleps.cpp`. It is NOT a tree-wide off-by-one on *every* brace (most citations resolve clean) — it triggers specifically at that construct. The recurrence-count tallies cycles where the drift was *present in the localization hint*; the on-disk discipline has caught all five with zero artifact damage (cycle-039 cost zero repair rounds — the lowering-verifier never transcribed off the hint).
...
**Watch (UPDATED batch-11; the guard is steady-state working).** The cycle-039 recurrence-5 is the *good* outcome the cycle-027 enactment was designed for: a NEW-boundary drift in the planner's localization hint, caught and corrected by on-disk `--anchor` discipline with ZERO artifact damage and ZERO repair round. This is the guard working, not degrading — so NO further enactment this batch (the meta-phase judged the ask-class HARD pinpoint-`--anchor` integrator gate NO-GO: it would need the CYCLE.md anchor-token channel-format change, and there is no escaped-drift evidence to justify it; a citecheck-faithful producer has never landed a drifted citation). The third-independent-boundary datapoint upgrades the *characterization* (systematic-on-a-construct-class, see Scope nuance) but does NOT yet warrant a codemap-server bug report (code change, ask-class) — the on-disk tie-breaker fully neutralizes the tool drift, so the bug is cosmetic-to-the-pipeline. **Re-open / escalate** only if (a) a producer faithfully transcribes a codemap hint and a drifted citation reaches the artifact uncaught (the source-of-truth bullet failed → HARD-gate ask), OR (b) the drift count climbs steeply enough that the localization-hint noise becomes a planner-time cost worth a server-side fix (→ codemap-server bug-report ask). Neither holds at recurrence-5.
...
last_observed: cycle-057
recurrence_count: 1
status: addressed
addressed_by: cycle-057 meta-phase (batch-17) — codified the lightweight promotion-time index-cell guard as a producer-side §Discipline bullet in `.claude/agents/lifter.md` (a status-flip dispatch owns the matching index-table cell in the SAME proposed-changes pass) + sharpened the `.claude/agents/layer-intro-author.md` count-owner survey bullet (count firm from the linked chapter's `## Status` line, NEVER from the drift-prone index-table cells) + a count-owner index-cell-flips-with-the-tally sub-bullet. Promotion-time guard (c056 D2 recommendation) chosen over a heavyweight finalize-time re-sweep.
---
```
