...
4. **mfem-as-l0-substrate-policy** — ASK item surfaced cycle-004 meta-phase: should MFEM headers be admitted as L0 substrate for Krylov solvers (MINRES, BiCGStab, possibly more) that Palace advertises via enum but does not implement? See cycle-004 meta-phase CYCLE.md §Open ask items + friction-ledger `advertised-but-unimplemented-krylov-solvers`. Decision blocks Shared Infrastructure items #5–#8 below.
5. **cross-layer-cross-cutter-krylov-step-layer-placement** — decide L2 vs L4 vs both for `krylov-step` per open question `krylov-step-layer-placement`. Can co-bundle with #1's L2 firm-up to ensure the dual-placement decision is made coherently.
6. **shared-infra-Householder-QR** (user directive 2026-05-27) — slice Householder QR (orthogonalisation sibling). Roadmap §Orthogonalisation. **Pre-grep recommended** (cycle-004 MINRES precedent): check `palace/utils/labels.hpp` + relevant solver classes for enum-only / `MFEM_ABORT`-routed status before harvester dispatch; route to abstractor-obstruction if Palace defines but does not implement.
7. **shared-infra-Jacobi-smoother** (user directive 2026-05-27) — slice Jacobi / damped Jacobi smoother. Roadmap §Smoothers and preconditioners. Same pre-grep recommended.
...
12. **shared-infra-MINRES** + **shared-infra-BiCGStab** — **deferred to #4 ASK outcome.** Currently captured as L1>L0 obstruction themes (`book/src/L1-L0/minres-iteration.md`, `book/src/L1-L0/bicgstab-iteration.md`) per cycle-004 abstractor work. Resolution path: if `mfem-as-l0-substrate` is admitted, retroactively rewrite themes with MFEM L0 anchors; if not, re-categorise explicitly out-of-scope.
...
- **Dispatch target: up to 15 sub-agents per cycle** (was 1–6). Cycle-004 ran **7 wave-1 dispatches** with zero structural conflict at integration (5 wave-mates appending to `book/src/L1/index.md` cleanly). Cycle-005 stretch target: **8–12 dispatches**. See `.claude/agents/cycle-planner.md` Discipline section.
