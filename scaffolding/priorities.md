...
## Now (active) — cycle-021 (THIRD/FINAL primary cycle of meta-batch-5; cycles 019/020/021)

**Batch-5 primary cycles carry forward:**
Cycle-019 landed 5 firm promotions (L2 `orthogonalize` + `inner_product`, L2>L1 `inner-product-fold-specialization`, L1 `assemble-diagonal`, L1>L0 `nrm2-mutation-rotation`) + L0 `fespace-file` anchor. Cycle-020 landed 6 firm promotions (L1>L0 `dot-mutation-rotation` + `scal-mutation-rotation` + `assemble-diagonal-mutation-rotation`, L4>L3 `gmres-inner-loop-iterate-while-migration`, L3 `ksp_solve`, L2 Part-intro refresh) + gmres self-rotation + L3 vocabulary. The cycle-019/020 active heads all COMPLETED and removed per active-head focus discipline. Cycle-021 pulls the final batch-5 slate: BLAS-1 floor close (axpby/axpbypcz themes) + L2 ksp_solve promotion + L3-L2 ksp-solve-outer-driver theme + fgmres lifter (held from cycle-020) + NLEPS large carry-forward + L3 eigsolve inventory + combinator-miner deflate/gram + orthog.hpp:35 surface.

**CYCLE-021 DISPATCH STATUS (9 dispatches; wave-1/2/3 per dispatch plan):**
1. **(lifter) `fgmres-inner-loop-iterate-while-migration` L4>L3 theme firm re-anchor** — Held explicitly from cycle-020 (gmres sibling now firm); re-anchor fgmres against gmres precedent. **fan-out:** unblocks downstream Krylov L2/L3/L4 work. **DISPATCHING cycle-021, wave-1.**
2. **(harvester) `L2/ksp_solve` outer-driver stub→firm** — L3 ksp_solve now firm; reverse maturity-gradient inversion. Substantive non-identity rotation. **fan-out:** gates `L3-L2/ksp-solve-outer-driver` theme + all 5 per-solver pipeline drivers. **DISPATCHING cycle-021, wave-1.**
3. **(abstractor) `L3-L2/ksp-solve-outer-driver` L3>L2 theme firm** — Depends on #2. Closes gap between firm L3 inventory and L2 lowering surface. **fan-out:** krylov-step / ksp_solve kernel/driver architecture. **DISPATCHING cycle-021, wave-2 (after #2 lands).**
4. **(abstractor) `axpby-mutation-rotation` + `axpbypcz-mutation-rotation` L1>L0 themes firm** — Remaining BLAS-1 L1>L0 lowering-floor closure (dot/scal/nrm2/assemble-diagonal completed cycle-019/020). **fan-out:** foundational lowering coverage every solver lowering reuses. **DISPATCHING cycle-021, wave-1.**
5. **(combinator-miner) `deflate` / `gram` deflation-subspace combinator candidate** — nleps.cpp recurrent `X[j]ᴴ·` pattern (:522, :529, :568). **fan-out:** supports NLEPS lowering + polynomial preconditioner variants. **DISPATCHING cycle-021, wave-1.**
6. **(same-layer-cross-cutter) `orthog.hpp:35` `LocalDot`+`GlobalSum` unweighted-inner-product surface** — Second unweighted inner-product surface; Condition-5 coverage gap. **fan-out:** closes Gram-Schmidt coverage gap. **DISPATCHING cycle-021, wave-1.**
7. **(harvester) NLEPS at L1+ (large carry-forward)** — Multi-cycle carry-forward; sustains sustained context. Blocks `check_stop_into_carry` L4 promotion + eigsolve lowering. **fan-out:** unblocks iterate-while/eigsolve L4 pathway + NEP solver coverage. **DISPATCHING cycle-021, wave-1.**
8. **(layer-intro-author) L2 Part-intro refresh** — L2 at 5 firm + 2 live stubs (ksp_solve→firm this cycle, incremental-least-squares). **fan-out:** narrative clarity for fold cohort + L2 vocabulary status. **DISPATCHING cycle-021, wave-1.**
9. **(harvester) `L3/eigsolve` kernel+driver pair L3 backfill** — Next major L3 inventory backfill (mirrors krylov-step/ksp_solve split). **fan-out:** L3 shared-vocabulary weight per cycle-009 directive. **DISPATCHING cycle-021, wave-1.**
8b. **(lowering-verifier) `axpby-mutation-rotation` + `axpbypcz-mutation-rotation` audit** — Depends on #4. Validates BLAS-1 lowering floor. **DISPATCHING cycle-021, wave-3 (after #4 lands); optional if density tight.**
...
- ~~**l2-named-composition-lifts**~~ — **FULLY DISPATCHED cycle-019** (orthogonalize firmed; incremental-least-squares deferred per OQ). **L2-layer-intro-refresh** now part of cycle-020 dispatch #3.
- ~~**ksp-solve-l2-promotion-non-identity-substantive-gap**~~ — **DISPATCHING cycle-021, dispatch #2** (harvester). Gates per-solver pipeline L2 story (all 5 solvers wrap ksp_solve). Routes `harvester`.
- ~~**l3-vocabulary-inventory-gap**~~ — **PARTIALLY DISPATCHED cycle-020** (ksp_solve → cycle-021 #3 theme). **CONTINUING cycle-021** (eigsolve dispatch #9). L3 backfill beyond BLAS-1 (gemv done-via-apply_linop; trsv blocked; ksp_solve done; eigsolve pending). Routes `harvester`. (Companion to roadmap §Layered-spec L3.)
- ~~**blas1-l1-l0-lowering-theme-gap**~~ — **FULLY DISPATCHED cycle-020** (dot/scal/assemble-diagonal themes firm). **CONTINUING cycle-021** (axpby/axpbypcz themes dispatch #4). Completes BLAS-1 lowering coverage. Routes `abstractor`.
- **deflate-gram-combinator-candidate** — nleps.cpp recurrent `X[j]ᴴ·` deflation-subspace projection pattern (:522, :529, :568); pins conjugation convention at combinator boundary. **fan-out:** supports NLEPS lowering + polynomial preconditioner variants. Routes `combinator-miner`. **Appended cycle-021 (integrator-signals "unblocked + suggested").**
- **orthog-hpp-localdot-globalsum-unweighted-inner-product-surface** — Second unweighted inner-product surface bypassing `linalg::Dot` (LocalDot+GlobalSum at `palace/linalg/orthog.hpp:35`); likely Condition-5 coverage-gap extension. **fan-out:** closes Gram-Schmidt coverage gap. Routes `same-layer-cross-cutter` (localize + classify) → harvester (if new primitive) or in-line elevation. **Appended cycle-021 (integrator-signals "unblocked + suggested").**

### Medium fan-out
- **matrix-weighted-norm + bilinear-form firm-promotion** — author `matrix-weighted-norm-mutation-rotation` L1>L0 theme + close the variant-axis/test-coverage firm-promotion gates for both rough-ins. **fan-out:** energy-norm consumers (CG/eigenmode residual tests). Routes `abstractor` + `lowering-verifier`. OQs `matrix-weighted-norm-mutation-rotation-l1-l0-theme`, `matrix-weighted-norm-mixed-element-type-variant`, `bilinear-form-real-vector-coverage-gap`, `bilinear-form-variant-axis-test-coverage`. **Held for cycle-020.**
