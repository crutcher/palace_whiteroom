...
- **Dispatch target: up to 8 sub-agents per cycle** (user directive 2026-05-27 — scaled back from initial 15 after cycle-004 surfaced integrator token-budget concern at 7 wave-mates). Cycle-004 ran **7 wave-1 dispatches** with zero structural conflict at integration (5 wave-mates appending to `book/src/L1/index.md` cleanly). Cycle-005 target: **up to 8 dispatches**. See `.claude/agents/cycle-planner.md` Discipline section. The integrator was split into `integrator-per-report` + `integrator-finalize` (cycle-004 → cycle-005 boundary) so per-dispatch context stays bounded regardless of wave-mate count.
