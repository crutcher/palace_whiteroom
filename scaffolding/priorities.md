...
6. **(combinator-miner / same-layer-cross-cutter — HEADLINE cycle-017; HUMAN-RAISED) BLAS-1 variadic linear-combination fold unification** — the `scal`/`axpy`/`axpby`/`axpbypcz` family are fixed-arity specializations of one variadic fold `linear_combination :: [(Scalar, Tensor[N])] -> Tensor[N]` (in-place forms = output-aliasing; fusion = the single-aligned-pass implementation of the fold). Currently represented 3× at fixed arity (L1 leaves / L1>L0 themes / L3 identity cohort), unified 0×. **Dispatch a `combinator-miner` (or `same-layer-cross-cutter`) at the family** to propose `linear_combination` as an L2 combinator with the four BLAS-1 ops as arity specializations, plus a sibling note distinguishing the inner-product fold (`dot` = reduce-to-scalar, a *different* fold — do not over-unify). Then the **batch-4 meta-phase (after cycle-018) extends the `combinator-miner` spec** with a parametric/variadic-family detection mode (the instance-counting heuristic is arity-blind — proximate cause it was never surfaced). OQ `blas1-variadic-linear-combination-fold-unification`.
