...
## Now (active) — cycle-020 (SECOND primary cycle of meta-batch-5; cycles 019/020/021)

**Batch-5 primary cycles carry forward:**
Cycle-019 landed 5 firm promotions (L2 `orthogonalize` + `inner_product`, L2>L1 `inner-product-fold-specialization`, L1 `assemble-diagonal`, L1>L0 `nrm2-mutation-rotation`) + L0 `fespace-file` anchor. The cycle-019 active head (7 items) all COMPLETED and removed per the active-head focus discipline. Cycle-020 pulls from the backlog High tier, focusing on BLAS-1 L1>L0 lowering-theme closure (dot/scal/assemble-diagonal remaining) + fold-specialization audit + L2 Part-intro refresh + L3 vocabulary backfill + gmres large carry-forward.

**CYCLE-020 DISPATCH STATUS:**
1. **(abstractor) `dot-mutation-rotation` + `scal-mutation-rotation` + `assemble-diagonal-mutation-rotation` L1>L0 themes (BLAS-1 gap closure)** — firms the remaining L1>L0 mutation-rotation themes for the BLAS-1 operators. Firm L1 leaves exist (`dot`, `scal`, `assemble-diagonal`); the L1>L0 stub homes are ready at SUMMARY.md :82/:84. **fan-out:** completes BLAS-1 lowering coverage that every solver lowering reuses. High-priority backlog High-tier. **DISPATCHING cycle-020, wave-1.**
2. **(lowering-verifier) `inner-product-fold-specialization` L2>L1 theme audit (per-line dispatch/re-order/summation-order rules)** — audits the firm L2>L1 theme against the conjunction-pair-reorder caller sites + dispatch-rule table. **fan-out:** validation gate for the fold-cohort machinery. **DISPATCHING cycle-020, wave-1.**
3. **(layer-intro-author) L2 Part-intro refresh (`book/src/L2/index.md` Working-Notes + dep-map overlay)** — L2 now at 5 firm operators (cycle-019 added named-composition + fold-cohort); folds two converging refresh flags into one dispatch. **fan-out:** narrative clarity for the growing L2 vocabulary. **DISPATCHING cycle-020, wave-2 (depends on #1/#2 proposed-changes blocks visible).**
4. **(harvester/lifter — large carry-forward) `gmres.md §L4 v0.6→v0.7` self-rotation** — firms both cycle-008 GMRES + cycle-011 FGMRES sister themes; recurring across batches 2/3/4. **fan-out:** unblocks all Krylov-downstream L2/L3/L4 work. **DISPATCHING cycle-020, wave-1.**
5. **(harvester) `l3-vocabulary-inventory-gap — gemv/trsv L3 cohort growth`** — L3 backfill beyond the closed BLAS-1 cohort. L3 is the iteration-rotation layer; every per-solver L3 lowering reuses these. **fan-out:** shared-vocabulary priority per cycle-009 directive. **DISPATCHING cycle-020, wave-1.**
6. **(abstractor) `conjugate-pair-reorder caller-classification` (real-projected-invisible vs full-complex-observable)** — after lowering-verifier audit confirms #2, scan every `linalg::Dot` call site to classify risk points. **fan-out:** complexity-risk inventory for Dot conjugation asymmetry. **DISPATCHING cycle-020, wave-2 (depends on #2 audit findings).**
...
- **ksp-solve-l2-promotion-non-identity-substantive-gap** — author the L2 `ksp_solve` outer-driver framing (non-identity vs the L1 entry; real coverage gap). **fan-out:** gates the per-solver pipeline L2 story (all 5 solvers wrap a ksp_solve). Routes `harvester`. **Held for cycle-021.**
- **l3-vocabulary-inventory-gap** — L3 backfill beyond the closed BLAS-1 cohort (`gemv`, `trsv`, `ksp_solve`, `eigsolve` identity rows). **fan-out:** every per-solver L3 lowering reuses these; L3 is the iteration-rotation layer. Routes `harvester`. (Companion to roadmap §Layered-spec L3.) **DISPATCHING cycle-020, dispatch #5.**
- ~~**blas1-l1-l0-lowering-theme-gap**~~ — **PARTIALLY DISPATCHED cycle-019 (nrm2 firmed); CONTINUING cycle-020 (dot/scal/assemble-diagonal)** (dispatch #1). **fan-out:** completes BLAS-1 lowering coverage that every solver lowering leans on; pins the dot conjugation-asymmetry + nrm2 abs-guard + Dot/Allreduce/sqrt chain. Routes `abstractor`. OQs mostly resolved by cycle-019 landing; residual: `inner-product-fold-specialization-lowering-verifier-audit` (dispatch #2) + `conjugate-pair-reorder-caller-classification` (dispatch #6).
