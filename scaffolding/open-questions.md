...
**[RESOLUTION — cycle-017, integrator-per-report position 2]** OQ `divfree-l1-entry-apply-close-and-reltol-line-drift` RESOLVED IN FULL by the cycle-017 harvester (`reports/2026-05-28T223336Z-harvester-divfree-l1-citation-fix/`). All flagged drifts in the firm L1 entry `book/src/L1/divfree-projector.md` are corrected and re-verified against `reference/palace/linalg/divfree.{cpp,hpp}` (critic citation-validity = pass; independently re-confirmed via codemap `read_range`). Eleven surgical line-reference corrections applied: (a) the apply `Mult(VecType &y)` close-brace `:155-186`→`:155-187` at ALL FOUR occurrence sites — §Context L14, §Semantics L122, §Status L237, §Evidence L301 (the OQ named the drift but enumerated only one site; the harvester found all four); (b) CG rel-tol `:140`→`:141` at the §Algebraic-laws idempotence caveat (`:140` is `SetInitialGuess(false)`; `SetRelTol(tol)` is `:141`; abs-tol `:142` correctly unchanged); (c) the inherited dangling `(see Variant axes)` pointer at §Context L43 re-pointed to `(see Signature, the y element type)` (the entry has no `## Variant axes` heading; matches the cycle-016 repairer's resolution of the L1>L0 theme's twin pointer); (d) the cycle-015-carry-forward hpp anchor hygiene surfaced three additional off-by-one drifts plus one range-tighten, all corrected — MPI_Comm `:62`→`:63` (§Signature MPI prose), psi/rhs scratch `divfree.hpp:55`→`:54` (§Context; `:55` was a blank line), `class DivFreeSolver` `divfree.hpp:34`→`:33` (§Evidence; `:34` is the `{`), member-fields range `divfree.hpp:40-55`→`:40-54` (§Evidence; tightened to last actual decl), ksp-solver comment `divfree.hpp:51`→`:50` (§Evidence; `:50` is the quoted comment, `:51` is the decl). Pure citation maintenance — NO signature/semantics/law/status/variant-axis change; operator stays `firm`; dep-map row and SUMMARY unchanged. The bare substring `divfree.cpp:155-186` recurs 4×, so each of the four close-brace edits was matched on full-line context (all four distinct anchors matched exactly once on fresh disk re-read). The out-of-OQ-scope `arpack.cpp`/`slepc.cpp` driver call-site line lists (§Evidence L306-308) were correctly NOT touched (a separate audit if drift is suspected there). NOTE: this report's CYCLE.md proposed-changes channel is the CORRECT delivery; the harvester's original dispatch-phase in-place `book/` edit was a write-authority phase-boundary violation REVERTED by the repairer (`git checkout`, book/ clean), and these 11 corrections were applied through the proper integrator channel. Deferred to integrator-finalize: formal `status:` field flip → `resolved` + `last_revisited: cycle-017`. Resolution-note format used because open-questions.md is append-only (per role-spec / write-authority partition). Source: cycle-017 harvester report `reports/2026-05-28T223336Z-harvester-divfree-l1-citation-fix/` + META.md (critic citation-validity pass; repairer revert of the dispatch-phase leak).
...
**[ANSWER] (cycle-017 `cross-layer-cross-cutter`, critic-VERIFIED-SOUND.) The "first … does not recur" premise is REFUTED.** The firm `eigsolve-mutation-rotation` theme (landed cycle-011, commit `8bb16b7`, five cycles before divfree's `b54ea1c`) is the prior — and richer — instance: its closure `E` carries TWO nested constructed-operator gates, `E.linear : Solver[A]` (an inner `ksp_solve` gate) and `E.projector : Maybe DivFreeSolver[ComplexVector]` (the divfree projector itself), and its **core sub-pattern B** (`book/src/L1-L0/eigsolve-mutation-rotation.md:213-258`) is precisely the nested-gate lowering — each of the ten `opInv->Mult` sites "rewrites by the firm `ksp-solve-mutation-rotation` theme", i.e. the inner gate is lowered through its own gate's theme. The L1 `eigsolve` entry already prose-names the pattern: "first L1 operator to compose two layers of constructed-operator absorption" + "structurally the same nesting pattern … composed-not-inherited" (`book/src/L1/eigsolve.md:136,140`). So `divfree-projector` is the **third** (not first) L1>L0 mutation-rotation exhibiting gate-carrying-gate, and the two confirmed instances are **transitively nested** (eigsolve ⊃ divfree ⊃ ksp), since `E.projector` is the divfree projector. `ksp_solve` is a **latent** further site (`K.M⁻¹` typed plain `LinearOperator[N,N]` at `book/src/L1/ksp_solve.md:31`; a gate only when the preconditioner is a `Solver` via `solver-as-operator` — un-verified against L0). With **≥2 FIRM instances cleared** (eigsolve cycle-011 + divfree cycle-016) the pattern warrants a named concept page; the follow-up routing is in the NEW OQ `nested-constructed-operator-gate-concept-and-divfree-correction` below. Source: cycle-017 `cross-layer-cross-cutter` report (`reports/2026-05-28T220000Z-cross-layer-cross-cutter-closure-nesting-gate/CYCLE.md`) §Summary + §Specific-finding + §"Open questions / caveats"; critic META.md (refutation VERIFIED-SOUND, all load-bearing citations independently confirmed); integrator-per-report promotion (position 5). `status:` flip → `answered` + `last_revisited: cycle-017` deferred to finalize per append-only discipline.

**[ANSWER ENACTED — prong (b) — cycle-018]** The artifact correction that gives effect to the cycle-017 ANSWER above is now LANDED. The cycle-018 `lifter` dispatch (`reports/2026-05-28T231500Z-lifter-divfree-first-claim-correction/`, critic 7/8 pass + 1 cross-reference-integrity warning [the same-cycle forward reference to the concept page — resolved by the report-2 prong-a landing], repairer tightened the gates-vs-themes wording → `ready`) applied two surgical edits to `book/src/L1-L0/divfree-projector-mutation-rotation.md`: the sub-pattern A bullet (`:108-113`) and the Open-questions item (`:457-464`) no longer claim divfree is the **first** / **only** gate-carrying L1>L0 mutation-rotation. Both now position divfree accurately as the **second** gate-carrying L1>L0 theme (after the firm `eigsolve-mutation-rotation`, cycle-011, sub-pattern B at `book/src/L1-L0/eigsolve-mutation-rotation.md:213-258`), carrying the **third** nested gate overall (after eigsolve's two, `E.linear` + `E.projector`), with the transitive three-deep nesting (eigsolve ⊃ divfree ⊃ ksp) preserved, and both passages now live-link the new `book/src/concepts/nested-constructed-operator-gate.md` concept page (landed report 2 this cycle) + the eigsolve precedent. The theme's structure, rotation, and sub-patterns A–D are untouched — bounded content-correction only. This OQ remains `answered` (the answer is unchanged; this note records its enactment in the artifact). Source: cycle-018 `lifter` report §Proposed changes 1+2; integrator-per-report promotion (position 5 of 5, LAST per-report of cycle-018).
...
**[RESOLUTION — cycle-017, integrator-per-report position 3]** OQ `l3-l2-body-identity-cg-md-citation-sweep` (opened cycle-016, ledger line 2903, above) RESOLVED IN FULL by the cycle-017 lifter sweep (`reports/2026-05-28T223135Z-lifter-l3-l2-body-identity-cg-sweep/`). All three dangling `cg.md` provenance pointers in `book/src/L3-L2/krylov-step-body-identity.md` re-anchored to the in-document terminal firm home — this theme's OWN §Verified-against bullet 1, which physically carries the verbatim cycle-002 combinator-miner Claim-2 quote (*"The L2→L3 rotation on the step body is therefore the **identity in form**: no unfolding, no global lift, no schema change."*) — with the dead slice ranges demoted to parenthetical "original pre-reduction slice range … lifted here per the cycle-009 corpus reduction" provenance annotations (cycle-013/014/015/016 convention). Mapping: §Applicability-conditions condition 3 (line 97, `cg.md:351-362`) → "preserved verbatim in this theme's §Verified-against bullet 1 … original pre-reduction slice range `cg.md:351-362`"; §Justification-kind empirical-match (line 107, `cg.md:341-362`) → "the CG body-identity claim, now preserved verbatim in this theme's §Verified-against bullet 1 … original pre-reduction slice range `cg.md:341-362` … and the live anchor `arnoldi_step.md:178-213`"; §Verified-against bullet 1 (line 125, `cg.md:341-362`) → declared the terminal firm home with the slice range demoted to a trailing parenthetical and an explicit "the `cg.md` slice is now a 165-line stub and that range no longer resolves" note. **NOT a self-referential dangle** (the critic's load-bearing finding): the bullet holds the verbatim quote (genuine terminal content), and both `L3/krylov-step.md:188` and `L4-L3/krylov-step-typed-wrapper-dissolution.md:204` independently designate this §section as the lifted-in home — exactly the established claim-home semantics (cf. the two-hop-to-dangle structural fact recorded in the cycle-016 position-2 resolution above; this sweep terminates the second of those two hops). The body-identity observation is a terminal *methodological* claim and does not lower further to an L0 Palace source (the distinct CG *primitive-sequence* L0 home is `L1-L0/ksp-solve-mutation-rotation.md` Sub-pattern B, `iterative.cpp:360-486`, named at L2:138). Status stays `firm` (the dangling provenance never gated firm status, only citation hygiene). Co-pointers correctly left untouched: `gmres.md:459-471` (§Verified-against bullet 4 — live 671-line slice, `inner_loop` body) and `arnoldi_step.md:178-213` / `:185-188` (live 302-line slice, MGS sequential-obstruction) — should `gmres.md` later be reduced, a parallel sweep re-points that one (not this dispatch's job). Cross-reference-integrity gate: all NEW intra-document/cross-chain references resolve; all 3 OLD anchor strings matched disk byte-for-byte on fresh re-read (no in-cycle file overlap — reports 1+2 touched `book/src/L2/index.md` + `book/src/L1/divfree-projector.md`). Deferred to integrator-finalize: formal `status:` field flip (→ `resolved`, answered_in this CYCLE.md, last_revisited cycle-017) on the `l3-l2-body-identity-cg-md-citation-sweep` YAML block (ledger line 2903). Resolution-note format used because open-questions.md is append-only (per role-spec / write-authority partition).
...
**[RESOLUTION — cycle-017, integrator-per-report position 4]** OQ `l3-chebyshev-sibling-formm-foldm-prose-sweep` RESOLVED-IN-FULL by the cycle-017 lifter dispatch (`reports/2026-05-28T223110Z-lifter-l3-chebyshev-sibling-refresh/`). All five sibling `forM_`/`foldM` mentions in `book/src/L3/chebyshev.md` were refreshed onto the firm `iterate_while_pure`/`iterate_while_pure_L3` family with step-count predicates, matching the register of the cycle-016 named-sentence refresh (lines 236–241) and the firm L4 §"L4 > L3" anchor (`book/src/L4/chebyshev.md:427-436`). The five sites (line numbers drifted +4 from the OQ's pre-cycle-016 listing, as the OQ itself flagged): §Context (`chebyshev`'s loops → "two nested **step-count-predicate** `iterate_while_pure` folds … the `iterate_while_pure_L3` tail recursions"), §Upward wrapper-dissolution bullet (the two nested `iterate_while_pure` folds → `iterate_while_pure_L3` tail recursions over their step-count predicates), §Non-adjacent-identity caveat (the surrounding two nested `iterate_while_pure` folds), §"L3 vs L4 distinction" L4-bullet (obstructions surface as two nested `iterate_while_pure` folds with step-count predicates), §"L3 vs L4 distinction" L3-bullet (the two nested `iterate_while_pure` folds are the `iterate_while_pure_L3` tail recursions). Plus one in-scope disambiguation: §Context `krylov-step` clause "predicate-driven" → "convergence-predicate-driven" (necessary because the refresh introduces "step-count-predicate" in the same sentence; restores the convergence-gated vs step-count distinction the entry draws elsewhere). Post-apply re-grep of `forM_\|foldM` returned ZERO residual mentions — the L3 entry is now fully consistent with the firm L4 sibling. Pure vocabulary refresh — NO L3 semantics/structure/status change (entry stays `partial-obstruction`); no new citations, no new markdown links. The critic confirmed all five are present-tense own-rendering (none historical-narrative provenance — correctly differing from the cycle-016 L4-side sweep which kept 4 provenance mentions); all five `[old]` anchors matched disk byte-for-byte on fresh re-read (no in-cycle file overlap — cycle-017 reports 1-3 touched `book/src/L2/index.md`, `book/src/L1/divfree-projector.md`, `book/src/L3-L2/krylov-step-body-identity.md`). The pre-existing OQ for a dedicated thin L4>L3 chebyshev identity-theme (referenced at this entry's §Upward) is untouched — out of scope for a pure vocabulary refresh. Deferred to integrator-finalize: formal `status:` field flip (→ `resolved`, answered_in this CYCLE.md, last_revisited cycle-017) on the `l3-chebyshev-sibling-formm-foldm-prose-sweep` YAML block (ledger line 2928). Resolution-note format used because open-questions.md is append-only (per role-spec / write-authority partition).
...
last_revisited: cycle-018
status: open (prong-a only; prong-b ENACTED cycle-017→cycle-018)
...
```yaml
---
slug: linear-combination-harvester-formalization
opened_at: cycle-017
opened_by: combinator-miner
last_revisited: cycle-018
status: resolved
relates_to: book/src/L2/index.md, book/src/L2/linear_combination.md, book/src/L1/scal.md, book/src/L1/axpy.md, book/src/L1/axpby.md, book/src/L1/axpbypcz.md, book/src/concepts/scalar-promotion.md, scaffolding/open-questions.md (blas1-variadic-linear-combination-fold-unification)
---
```

**[OQ] The L2 combinator `linear_combination` is landed as a rough-in dep-map row only; harvester must formalize it (operator chapter + laws + empirical-match + the L2>L1 lowering theme).** Cycle-017 combinator-miner (`reports/2026-05-28T223022Z-combinator-miner-linear-combination-fold/`) landed the rough-in dep-map row at `book/src/L2/index.md` for `linear_combination :: [(Scalar, Tensor[N])] -> Tensor[N]` (≡ `foldl (\acc (a,t) -> acc + a·t) zeros pairs`), unifying the BLAS-1 scalar-weighted-sum cohort (`scal`/`axpy`/`axpby`/`axpbypcz`) along the **arity** axis — the constructive half (prong b) of the human-raised OQ `blas1-variadic-linear-combination-fold-unification` (above). It did NOT create the operator file `book/src/L2/linear_combination.md` (forward-link is intentional, matching the krylov-step/chebyshev-iteration rough-in pattern). **Harvester follow-up (cycle-018+ candidate):** (1) author `book/src/L2/linear_combination.md` with the firm signature, the shape precondition (`all tᵢ : Tensor[N]` one shared `N`; element type one shared `T ∈ {real,complex}` with the `real ⊑ complex` `scalar-promotion` lattice inherited), and the algebraic laws sketched in the report (empty-list identity = `zeros[N]`; concatenation homomorphism `lc(a ++ b) = lc a + lc b` — the defining fold law; multilinearity in the scalar list; scalar absorption; zero-coefficient term-drop = the exact algebraic content of the L0 `γ==0` branch at `vector.cpp:749-751`); register the SUMMARY chapter entry. (2) **Permutation-invariance must be stated as an EXACT-ARITHMETIC law with the IEEE-754 non-associative-reduction caveat as an explicit paired non-law** (matching axpby.md "Floating-point associativity" + axpbypcz.md's two-branch bit-mismatch note) — do NOT assert unqualified permutation-invariance; the `foldl` left-to-right order is the canonical L2 naming order, with bit-identical L0-site reproduction deferred to the L2>L1 theme. (3) **Pull the `test/unit/test-vector.cpp` per-arity value assertions** to anchor the concatenation law as an `empirical_match` rather than a purely algebraic claim (the report cited them transitively via the L1 entries, not re-read). (4) **The `L2-L1/linear-combination-fold-specialization` lowering theme is abstractor work** (after harvester formalizes): it narrates how the variadic L2 fold lowers into the fixed-arity L1 leaves (list-length 1 → `scal`, 2 → `axpy`/`axpby`, 3 → `axpbypcz`; longer → left-fold of `axpby`/`axpbypcz` chains) and which L0 pinned summation order a given call site fixes (single fused pass vs split-call) — the load-bearing numerical content. **Upside note (per CLAUDE.md "literature-anchored L1 form may inform higher abstractions"):** Palace's L0 surface stops at arity 3 (`AXPBYPCZ`); arity > 3 combinations are open-coded as accumulation loops (`nleps.cpp:343-344`, `romoperator.cpp:188-189`, both γ=1 fold-into-output). The variadic `linear_combination` is genuinely more general than any single Palace symbol and may later let an L4 combinator express n-term basis-reconstruction (ROM / eigenvector synthesis) directly — upside, not scope creep. Not blocking. Source: cycle-017 combinator-miner report §Open questions 2/3/4/5 + §Proposed combinator.

**[RESOLUTION — cycle-018]** RESOLVED by the cycle-018 harvester (`reports/2026-05-28T231026Z-harvester-linear-combination-L2/`, overall_status `ready`): the firm operator chapter `book/src/L2/linear_combination.md` is authored (signature + shape contract + 7 holding laws + 3 explicit non-laws), the `book/src/L2/index.md` dep-map row is flipped rough-in→firm, and the SUMMARY chapter is registered. Items (1)/(2) of the harvester follow-up are DONE: the firm signature/laws are authored, and permutation-invariance is stated as an **exact-arithmetic law (law 7)** paired with the explicit IEEE-754 non-associative-reduction **non-law** per the load-bearing-numerical-trick discipline. Item (3) (`test-vector.cpp` per-arity assertions for an `empirical_match`) was investigated and recorded as an honest **empirical-match caveat (NOT a status reduction)**: there is NO dedicated unit test exercising the BLAS-1 linear-combination free functions (`search_text` returned zero hits; the `test-vector.cpp` "Vector Sum" tests exercise `linalg::Sum`, a *different* reduce-to-scalar fold) — the entry lands `firm` on direct source-transcription per the `chebyshev-iteration` firm-without-dedicated-test precedent, with the upgrade condition stated (a future dedicated test → clean `empirical_match`). Item (4), the **`L2-L1/linear-combination-fold-specialization` lowering theme, is LANDING THIS SAME CYCLE as cycle-018 report 4** (the abstractor follow-up; the operator chapter's `## Dependencies` forward-references it as plain text). The arity>3 upside note is recorded in the chapter's §Semantics. (`status:` YAML flip → `resolved` + `last_revisited: cycle-018` deferred to integrator-finalize per the append-only write-authority partition.)

**[RESOLUTION cont'd — cycle-018, item (4) LANDED]** Item (4) of the harvester follow-up is now ON DISK: cycle-018 report 4 (`reports/2026-05-28T231508Z-abstractor-l2-l1-linear-combination-theme/`, overall_status `ready`) authored the firm L2>L1 lowering theme `book/src/L2-L1/linear-combination-fold-specialization.md`, appended its dep-map row to `book/src/L2-L1/index.md` (after `chebyshev-iteration-fusion`), and registered the chapter under the `# L2 > L1 — Lowering` SUMMARY Part. The theme narrates the fusion-selection rewrite forward (high→low): arity 1 → `scal`, arity 2 → `axpy` (unit 2nd coeff) / `axpby` (general), arity 3 → `axpbypcz`, arity ≥4 → iterated left-fold of `γ=1` `axpbypcz`-into-output chains (Palace's fused-kernel ceiling is `AXPBYPCZ`, `vector.hpp:305-316`), with the arity-3→arity-2 `γ==0` fall-through read off the in-source branch (`vector.cpp:749-751`). It carries the **pinned summation-order table** that the L2 entry's permutation IEEE-754 non-law deferred here (the load-bearing-numerical content), verified for the real-real path; complex overloads delegate to MFEM member ops with operand-order parity marked MFEM-internal (not Palace-confirmed). LHS link `../L2/linear_combination.md` resolves cleanly (report 1 landed it earlier this same cycle); the four L1 RHS leaf links + concept + decision links all resolve on disk. `status: firm` (the selection rule IS the L2 entry's already-firm laws 6+2 read as a lowering; no speculative operator). With items (1)/(2)/(4) DONE and item (3) recorded as an honest empirical-match caveat, the harvester-formalization OQ is fully discharged — the formal `status:` YAML flip → `resolved` + `last_revisited: cycle-018` remains deferred to integrator-finalize per append-only discipline.

```yaml
---
slug: inner-product-fold-sibling-candidate
opened_at: cycle-017
opened_by: combinator-miner
last_revisited: cycle-018
status: answered
relates_to: book/src/L1/dot.md, book/src/L2/index.md, book/src/L2/linear_combination.md, scaffolding/open-questions.md (blas1-variadic-linear-combination-fold-unification)
---
```

**[OQ] `dot` is a DIFFERENT fold from `linear_combination` — recommend a sibling `inner_product` L2 combinator candidate, but do NOT over-unify into one mega-combinator.** The cycle-017 combinator-miner proposal for `linear_combination` (above) explicitly kept `dot :: (x, y) -> Scalar` out of scope: `dot` is `foldl (+) 0 (zipWith (*) x y)` (or `conj`-weighted for the Hermitian complex case) — a **reduce-to-scalar inner-product fold**, NOT a scalar-weighted *tensor* sum. Its result type is `Scalar`, not `Tensor[N]`; its laws are symmetry / Hermitian-symmetry / positive-semi-definiteness (`dot.md:55-75`), which have no analogue in `linear_combination`. The methodology target is a small **algebra of folds** — a tensor-producing linear-combination fold AND a scalar-producing inner-product fold — not one mega-combinator (the "do-not-over-unify" nuance the parent OQ `blas1-variadic-linear-combination-fold-unification` itself requested). **Follow-up candidate (a SEPARATE future combinator-miner invocation, one pattern per dispatch):** a sibling `inner_product :: (Tensor[N], Tensor[N]) -> Scalar` L2 fold capturing `dot`/`tdot` as **conjugation-convention** variants (the axis there is conjugation-convention, not arity). Deliberately NOT proposed in the cycle-017 dispatch (one pattern per invocation). Not blocking. Source: cycle-017 combinator-miner report §Open questions 1 + §Summary.

**[RESOLUTION — cycle-018]** The follow-up combinator-miner invocation this OQ recommended has run: `reports/2026-05-28T231046Z-combinator-miner-inner-product-fold/CYCLE.md` proposes the sibling `inner_product :: (Tensor[N], Tensor[N]) -> Scalar` L2 fold and lands a **rough-in dep-map row** in `book/src/L2/index.md` (`(rough-in, proposed-by: combinator-miner:2026-05-28T231046Z)`, appended after the now-firm `linear_combination` row). The fold subsumes the four genuine kernel/weighting members (real dot, complex-Hermitian dot, complex-unconjugated `tdot`, and the M-weighted `bilinear-form`), with **conjugation-convention** and **weighting** recorded as variant axes (NOT separate operators) and `matrix-weighted-norm` deliberately held OUT as a **consumer** (`√ ∘ inner_product_M` at `y=x`, SPD precondition) — exactly the "algebra of folds, do-not-over-unify" boundary this OQ requested. The "do NOT over-unify into one mega-combinator" guidance is honored: `inner_product` is kept a SEPARATE sibling of `linear_combination` (different result type, no shared concatenation/PSD laws). Two follow-ups remain open beyond this rough-in (the OQ `status:` YAML flip → `resolved` is deferred to integrator-finalize per append-only discipline): **(1) harvester-formalization** — `book/src/L2/inner_product.md` is NOT created by this combinator-miner dispatch (rough-in only); a future harvester authors the firm chapter and the `L2-L1/inner-product-fold-specialization` lowering theme (abstractor work) per the `linear_combination` precedent. **(2) M-weighted conjugation/argument-order pinning** — the report uniformly uses the shorthand `xᴴ M y`, which does NOT exactly reconcile with the Palace-documented convention: `operator.hpp:386-394` documents `Dot(comm,x,A,y)` as `yᴴ A x`, body `(Ax)ᴴ y = xᴴ Aᴴ y` (`operator.cpp:621-639`). The harvester must pin the exact form and inherit the resolution recorded under `bilinear-form.md` Status / OQ `bilinear-form-conjugation-convention-anchor` rather than introduce a fresh framing. The rough-in row carries an inline shorthand caveat at the point of use (dep-map cell + this report's caveat 7) so the harvester sees the conjugation flag.

```yaml
---
slug: nested-constructed-operator-gate-concept-and-divfree-correction
opened_at: cycle-017
opened_by: cross-layer-cross-cutter
last_revisited: cycle-018
status: resolved
relates_to: book/src/concepts/nested-constructed-operator-gate.md, book/src/concepts/constructed-operator-factory.md, book/src/concepts/solver-as-operator.md, book/src/L1/eigsolve.md, book/src/L1/divfree-projector.md, book/src/L1/ksp_solve.md, book/src/L1-L0/eigsolve-mutation-rotation.md, book/src/L1-L0/divfree-projector-mutation-rotation.md, scaffolding/open-questions.md (divfree-closure-nesting-constructed-gate-carrying-constructed-gate)
---
```

**[OQ] The gate-carrying-gate ("nested constructed-operator gate") shape clears the ≥2-firm-instance bar — route two cycle-018 follow-ups: (a) a named concept page; (b) a scoped divfree-theme "first"/"no other op" correction.** The cycle-017 `cross-layer-cross-cutter` audit (`reports/2026-05-28T220000Z-cross-layer-cross-cutter-closure-nesting-gate/`, critic VERIFIED-SOUND) refuted the divfree theme's "first … does not recur" premise (see the ANSWERED OQ `divfree-closure-nesting-constructed-gate-carrying-constructed-gate` above): `eigsolve` (firm, cycle-011 `8bb16b7`) is the prior, richer instance, with two nested gates (`E.linear : Solver[A]`, `E.projector : Maybe DivFreeSolver` — the latter is the divfree projector itself, so eigsolve ⊃ divfree ⊃ ksp transitively), and its core sub-pattern B lowers each nested `opInv->Mult` through the firm `ksp-solve-mutation-rotation` theme. With **≥2 FIRM instances confirmed** (eigsolve cycle-011 + divfree cycle-016) plus the prose-named precedent at `book/src/L1/eigsolve.md:136,140` and the latent `ksp_solve` `K.M⁻¹`-as-`Solver` site:

- **(a) `nested-constructed-operator-gate` concept page (route to cycle-018 `layer-intro-author`).** Author `book/src/concepts/nested-constructed-operator-gate.md`, sibling to `book/src/concepts/constructed-operator-factory.md`, replacing the in-line note in the divfree theme. The page should: (i) name the shape — a constructed-operator closure whose **field is itself a constructed-operator gate** (`Solver[A]`, `DivFreeSolver`, …), distinct from a closure carrying a *raw* operator (`chebyshev-smoother`'s `op.A`, `apply_linop`'s operand — negative cases); (ii) state the cross-layer fidelity rule — the inner gate's iteration **stays interior to the inner gate's own theme** and does not leak into the outer theme (at the outer theme's resolution the inner gate is an opaque action: `ksp->Mult(rhs,psi)` = opaque `K⁻¹`; `opInv->Mult(b,x)` = opaque inner solve), i.e. the "composed-not-inherited" remark at `book/src/L1/eigsolve.md:140`; (iii) index the instances — `eigsolve` (`E.linear`, `E.projector` — two gates), `divfree-projector` (`P.ksp` — one gate), latent `ksp_solve` (`K.M⁻¹` when the preconditioner is a `Solver`), and note the three-deep transitivity (eigsolve ⊃ divfree ⊃ ksp); (iv) relate to siblings `constructed-operator-factory`, `solver-as-operator`, `variant-absorption`. Two caveats the author must confirm before citing: `eigsolve` is a clean FIRM instance for the count (its `LinearSolveFailed` partly-constructive sub-part is a *separate* status concern, orthogonal to the firm source-anchored nesting in sub-pattern B); and the latent `ksp_solve` site is NOT yet verified against an L0 Palace site where a `BaseKspSolver`'s preconditioner is itself a `BaseKspSolver` — list it as *latent* until confirmed. Concept-over-combinator is the recommended first home (the shape is a documentation/fidelity convention, not a new evaluation primitive); re-route to `combinator-miner` only if a later cycle finds the nesting needs calculus-level support (e.g. an L4 `withInnerSolver` bracket).
- **(b) Divfree-theme "first" / "no other op" correction (route to cycle-018 `lifter`/`harvester`, scoped append-only dispatch).** Three claims are inaccurate against firm artifact authored five cycles earlier and must cite `eigsolve-mutation-rotation` sub-pattern B (`book/src/L1-L0/eigsolve-mutation-rotation.md:213-258`) as the prior, richer instance, and re-point the in-line note at the new concept page once it lands: `book/src/L1-L0/divfree-projector-mutation-rotation.md:108-113` ("the first L1>L0 mutation-rotation whose closure carries another constructed-operator gate"), `:457-464` ("the first such case … shared with no other current L1 op"), and the ANSWERED OQ-ledger prose above (now corrected by the ANSWER note; the divfree-theme file edits remain to be applied). The divfree theme is append-only after `integrated_at:`, so this is a scoped correction dispatch, not a free edit.

Not blocking; informational + documentation-completeness. Source: cycle-017 `cross-layer-cross-cutter` report §Recommendation + §"Open questions / caveats" "Follow-up dispatches to route" (repairer-added preamble); integrator-per-report promotion (position 5).

**[RESOLUTION — prong (a) — cycle-018]** Prong (a) the `nested-constructed-operator-gate` concept page is now LANDED. The cycle-018 `layer-intro-author` dispatch (`reports/2026-05-28T231017Z-layer-intro-author-nested-gate-concept/`, critic 7/8 pass + 1 informational `skill-uptake-survey` warning, repairer REPAIRED 2 citation/quote-fidelity findings → `ready`) authored `book/src/concepts/nested-constructed-operator-gate.md` and registered it as the final Concepts-Part entry in `book/src/SUMMARY.md` (after `scalar-promotion`). The page delivers all four prong-(a) requirements: (i) names the gate-carrying-gate shape with the raw-operator-field vs gate-field distinguishing test (`chebyshev-smoother`'s `op.A` + `apply_linop`'s operand = negative cases); (ii) states the cross-layer fidelity rule (inner gate's iteration stays interior to its own theme; the lowering of the whole = composition of adjacent-edge themes, never a flattened rewrite; the "composed-not-inherited" remark at `book/src/L1/eigsolve.md:140`); (iii) indexes the two FIRM instances (`eigsolve` E.linear+E.projector = two gates; `divfree-projector` P.ksp = one gate), the three-deep transitivity (eigsolve ⊃ divfree-projector ⊃ ksp_solve), and the LATENT `ksp_solve` `K.M⁻¹`-as-`Solver` site (un-verified against L0 — listed latent, NOT firm, with a stated promotion condition); (iv) relates to siblings `constructed-operator-factory` (materialisation site), `solver-as-operator`, `constructed-operators`, `variant-absorption`. Both author-confirm caveats are honoured: `eigsolve` is counted as a clean FIRM instance (the `LinearSolveFailed` partly-constructive sub-part is explicitly distinguished as a separate status concern orthogonal to the firm source-anchored nesting in sub-pattern B); the `ksp_solve` preconditioner-as-`Solver` site is listed latent pending L0 confirmation. Concept-over-combinator first home preserved (re-route to `combinator-miner` only if calculus-level support is later needed — page does not foreclose). **Prong (b) REMAINS OPEN — lands THIS SAME cycle as report 5 (the cycle-018 divfree-correction lifter/harvester dispatch)**: the divfree theme's three inaccurate "first" / "no other op" claims (`book/src/L1-L0/divfree-projector-mutation-rotation.md:108-113`, `:457-464`, and the OQ-ledger prose) still need the append-only correction citing `eigsolve-mutation-rotation` sub-pattern B as the prior, richer two-gate instance and re-pointing the in-line note at this new concept page. The `status:` flip (→ `partially-addressed` or `resolved` once prong (b) lands) + `last_revisited: cycle-018` are deferred to integrator-finalize per append-only discipline.

**[RESOLUTION — prong (b) — cycle-018]** Prong (b) the divfree-theme "first" / "no other op" correction is now LANDED, so **both prongs of this OQ are complete**. The cycle-018 `lifter` dispatch (`reports/2026-05-28T231500Z-lifter-divfree-first-claim-correction/`, critic 7/8 pass + 1 cross-reference-integrity warning on the same-cycle forward reference to the prong-(a) concept page — resolved by report-2 landing the page first; repairer tightened the gates-vs-themes wording → `ready`) applied two surgical edits to `book/src/L1-L0/divfree-projector-mutation-rotation.md`: the sub-pattern A bullet (current `:108-113`) and the Open-questions item (current `:457-464`). The corrections (i) remove the false "**first** L1>L0 mutation-rotation whose closure carries another constructed-operator gate" and "**first** such case … shared with no other current L1 op" claims; (ii) re-position divfree as the **second** gate-carrying L1>L0 theme (after the firm `eigsolve-mutation-rotation`, cycle-011), carrying the **third** nested gate overall (after eigsolve's two, `E.linear` + `E.projector`), preserving the three-deep transitivity (eigsolve ⊃ divfree ⊃ ksp); (iii) cite `eigsolve-mutation-rotation` sub-pattern B (`book/src/L1-L0/eigsolve-mutation-rotation.md:213-258`) as the prior, richer two-gate instance + the prose-named precedent at `book/src/L1/eigsolve.md:60,136,140`; (iv) re-point the in-line notes at the new `book/src/concepts/nested-constructed-operator-gate.md` concept page (landed prong (a), report 2). The third inaccurate location — the OQ-ledger prose — was already corrected by the cycle-017 ANSWER note on the sibling OQ `divfree-closure-nesting-constructed-gate-carrying-constructed-gate` (its prong-(b) enactment is also recorded there this cycle). The theme's structure, rotation, and sub-patterns A–D are untouched (bounded content-correction within the lifter boundary). The `status:` flip (→ `resolved`, both prongs landed) + `last_revisited: cycle-018` are deferred to integrator-finalize per append-only discipline. Source: cycle-018 `lifter` report §Proposed changes 1+2 + §Discipline notes; integrator-per-report promotion (position 5 of 5, LAST per-report of cycle-018).

```yaml
---
slug: inner-product-harvester-formalization-and-conjugation-pinning
opened_at: cycle-018
opened_by: integrator-finalize
last_revisited: cycle-018
status: open
relates_to: book/src/L2/index.md (inner_product rough-in row), book/src/L2/inner_product.md (to be authored), book/src/L1/dot.md, book/src/L1/bilinear-form.md, scaffolding/open-questions.md (inner-product-fold-sibling-candidate, bilinear-form-conjugation-convention-anchor)
---
```

**[OQ] Follow-up to the answered `inner-product-fold-sibling-candidate`: harvester-formalize the `inner_product` L2 chapter + its L2>L1 lowering theme, pinning the exact conjugation/arg-order convention.** Cycle-018 combinator-miner landed the `inner_product` rough-in dep-map row at `book/src/L2/index.md` (`(Tensor[N], Tensor[N]) -> Scalar` ≡ `foldl (+) zero (zipWith kernel x y)`; M-weighted member `inner_product_M(x,M,y) = xᴴ M y`; fold-sibling of `linear_combination` along the conjugation-convention axis). **Remaining work (cycle-019+ harvester + abstractor, mirroring the `linear_combination` operator+theme pairing):** (1) author the firm `book/src/L2/inner_product.md` chapter (signature + the real-dot / complex-Hermitian-dot / complex-unconjugated-`tdot` / M-weighted-`bilinear-form` members as conjugation-convention + weighting variant axes, NOT separate operators; `matrix-weighted-norm` held OUT as a consumer `√ ∘ inner_product_M` at `y=x`); (2) **pin the exact conjugation/argument-order convention** — the rough-in row uses the shorthand `xᴴ M y`, which does NOT exactly reconcile with the Palace-documented `Dot(comm,x,A,y) = yᴴ A x` (`operator.hpp:386-394`), body `(Ax)ᴴ y = xᴴ Aᴴ y` (`operator.cpp:621-639`); inherit the resolution recorded under `bilinear-form.md` Status / OQ `bilinear-form-conjugation-convention-anchor` rather than introduce a fresh framing; (3) the `L2-L1/inner-product-fold-specialization` lowering theme (abstractor work after the harvester). Not blocking. Source: cycle-018 finalize, carried from the answered `inner-product-fold-sibling-candidate`.

```yaml
---
slug: linear-combination-fold-specialization-theme-followups
opened_at: cycle-018
opened_by: integrator-finalize
last_revisited: cycle-018
status: open
relates_to: book/src/L2-L1/linear-combination-fold-specialization.md, book/src/L2/linear_combination.md, book/src/L1/scal.md, book/src/L1/axpy.md, book/src/L1/axpby.md, book/src/L1/axpbypcz.md
---
```

**[OQ] Standard follow-ups on the cycle-018 firm L2>L1 theme `linear-combination-fold-specialization`.** The theme landed firm cycle-018 (the arity-dispatch fusion-selection lowering of L2 `linear_combination` into the L1 fixed-arity leaves, carrying the pinned summation-order table the L2 entry's IEEE-754 permutation non-law deferred). Two non-blocking residuals carried for a future lowering-verifier / lifter: (1) the **complex-overload operand-order parity** is currently marked MFEM-internal (not Palace-confirmed) — the real-real summation-order path is verified, but the complex path delegates to MFEM member ops; a lowering-verifier pass should confirm or document the obstruction; (2) the **arity≥4 iterated-left-fold-of-`γ=1`-`axpbypcz`-chains** claim (Palace's fused-kernel ceiling is `AXPBYPCZ`, `vector.hpp:305-316`) is read off two open-coded accumulation sites (`nleps.cpp:343-344`, `romoperator.cpp:188-189`); an exhaustiveness audit of arity>3 call sites would firm the ≥4 case. Not blocking. Source: cycle-018 abstractor report (`reports/2026-05-28T231508Z-abstractor-l2-l1-linear-combination-theme/`) §Open questions; integrator-finalize routing.
