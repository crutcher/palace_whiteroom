...
Producer role specs (harvester / abstractor / lifter / layer-intro-author §Discipline) carry a bullet pointing at this section (cycle-015 meta-phase). The role-spec bullet is necessary but — as the cycle-014 auditor-drift showed (the lowering-verifier had a citation bullet since cycle-012 and still drifted) — **not sufficient on its own**.

**Mechanical check (`tools/citecheck/`) — the deterministic half (added cycle-024 meta-phase, batch-6).** The mechanical codemap-backed checker filed as an ASK across batches 3–5 was BUILT (`tools/citecheck/citecheck.py`, user go, commit 88b7893) and WIRED into the producer/critic/lowering-verifier/integrator procedures this meta-phase (friction-ledger `producer-citation-drift-verify-not-self-invoked` — role-spec wiring enacted). Run it as the mechanical companion to steps 2–5 above:

- **Per pinpoint citation:** `python3 tools/citecheck/citecheck.py <path:lo-hi> --anchor '<token the citation points at>'` — `OK` proves the anchor is in-range; `[DRIFT ±N]` emits the suggested corrected line; `[NOANC]` means the token isn't in the file (wrong file, or the anchor text itself drifted). `--regex` treats the anchor as a regex; `--show` prints the cited lines for the meaning-read.
- **Whole-report pre-emit / pre-apply pass:** `python3 tools/citecheck/citecheck.py --scan <CYCLE.md> --quiet` — bounds + path-hygiene over every `path:lo-hi` token (non-zero exit on any failure; `AMBIG` = a bare basename matching several files, write the full path).
- The tool is the **single authoritative line-map** all roles share, which is what ends the critic↔repairer↔verifier line-number disagreement (it caught every batch-5/6 drift in validation). It is a **lint, not a semantic checker** — it confirms WHERE the citation resolves and whether the anchor sits there, NOT that the cited lines *mean* what the prose claims; steps 2–3's meaning-read still has to happen. See `tools/citecheck/README.md` §"Suggested integration points".
