...
These are the `book/` surfaces the meta-phase is the standing owner of — the only book-write exceptions to "the integrator owns `book/`". Refresh them as part of each batch's arc-assessment.

- **⟢ KERNEL-API/impl integrity + the LIFT-THROUGH (RE-discharge) campaign — standing duty every batch (user directive 2026-06-07; CLAUDE.md §Methodology-invariant "Kernel-API vs kernel-IMPLEMENTATION" + §Scope DIRECTIVE 2/3).** The 2026-06-07 re-scope makes the deferred in-scope work + the constructive spine-dependency kernels the new forward frontier. Each batch: (1) **Track the kernel-API/impl distinction integrity** — for each spine-dependency opaque-library kernel that gained a `kernel-impl` node, confirm BOTH surfaces exist (the `kernel-api` obstruction theme KEPT + the `kernel-impl` constructive node), the `realizes-kernel-api` link is `reference`-class (NOT `depends-on`), and the impl actually corresponds to the API contract (the `lowering-verifier` audit verdict). A kernel-impl that deleted/downgraded its API surface, or an impl→API link mis-typed as `depends-on`, is a defect to catch. (2) **Track the LIFT-THROUGH progress as the new reachability campaign** — the RE1-RE10 baseline-exception set is now a **discharge target**, not a permanent floor: as the geometric-multigrid preconditioner / kernel impls / AMR land, re-check which RE promotion conditions FIRED (a consumer composing the node BY NAME via a faithful `depends-on` edge), discharge those REs, and migrate the next consumer into the plan. This SUPERSEDES the batch-35→36 "the RE set is the faithful floor" framing for the in-scope cohorts (RE1/RE3/RE5/RE6/RE7/RE8/RE9/RE10 all carry now-fired or about-to-fire promotion conditions under the lift-through). (3) **Guard the DIRECTIVE-1 boundary** — MPI/sharding stays OUT of active scope; if a dispatch or plan item proposes lifting the MPI-associated version as active work, that is a boundary violation to catch (the sharding-math-as-decomposition-abstraction is exploratory-only-if-non-destabilizing, recorded as a `roadmap_goal` future note, never dispatched as active spine work without the hard non-destabilization gate clearing).

- **SEMANTIC SURFACE liveness/unification — refresh every batch (user directive 2026-06-06; CLAUDE.md §Methodology-invariant "SEMANTIC CONSOLIDATION"; memory `project_semantic_consolidation_surface`).** The active-management semantic surface (`book/src/semantics/index.md` — the surface got its own home at cycle-116, relocated from the former `book/src/design/l4_calculus.md`; promoted out of "design strawman" status; placed BEFORE the `# L4` Part in `SUMMARY.md`; its §0.1 carries the discipline) is held under the **same liveness/unification/consolidation discipline as the graded-stack vocabulary** — and the meta-phase owns its **every-batch refresh**, the semantic analog of the graded-stack GC sweep. Each batch: (1) **liveness/drift check** — re-check the surface against the authoritative sources (`CLAUDE.md`, `METHODOLOGY-REDIRECT.md`, memory); if the surface contradicts a source, the source wins and the surface is corrected (a contradiction is a drift signal). (2) **unification/restatement sweep** — examine the cross-cutter findings + the OQ ledger for any new **semantic-restatement cohort** (a general semantic rule restated at functional-unit scope — the semantic analog of a degenerate identity-lowering smell); **migrate the relocation sweep into the plan** (route to `layer-intro-author`, the surface owner) rather than leaving the duplication. (3) **consolidation** — confirm any newly-mined general semantic abstraction earned a home on the surface (in the matching §-by-concern section). The surface's authoring (the evolution + the cohort relocation sweeps) is dispatched to `layer-intro-author`; the meta-phase's standing duty is the liveness/unification/migration. The c115 D3 `linear_combination` relocation pilot + the 27-file named-shape-groups cohort (OQ `named-shape-groups-general-rule-restatement-cohort-extent`) are the founding instances. (NOTE: the surface's *authoring* is a `layer-intro-author` dispatch — the meta-phase does the liveness check + migrates the sweep; it does not itself author the surface unless doing a small in-place correction, analogous to the goal-flow refresh below.)

- **Graded-stack standing duties — GC sweep + baseline-exception set + the resolution-ladder methodology page** (user directive 2026-06-04; full spec `METHODOLOGY-GRADED-STACK.md` §2/§4/§5/§8/§9). Each batch: (1) **run/commission the reachability-GC sweep** (mark from the feature-surface roots over `depends-on` edges) covering **detritus + orphaned-intent uniformly** — a frozen slice, kickoff-dead scaffolding, OR an orphaned `roadmap_goal` whose pull-chain no longer reaches a root is all the same garbage (one sweep over built + intended nodes). **Apply the §2f GROUND-don't-remove disposition** (user directive 2026-06-05; thrice-applied cycles 107/108) before filing any unreachable node as detritus: **(1) GROUND** the node by sketching the faithful, honestly-typed `depends-on` edge into the reachable goal node it is a genuine future/absorbed dependency of (the preferred disposition — it rescued the BC/divfree clusters + the L1/L0 lowering homes across batch-34); **(2) ROUTE as genuine detritus** only when no plausible goal-dependency exists; **(3) delete/baseline-exception** last. Never a forced/false edge to flip a number — faithful-edge-or-finding. (2) **Maintain the baseline-exception set** — the explicit, tracked rank violations carrying promotion conditions (the same first-class-transient-gate pattern as `partly-constructive`), burning down under the hard-gate-new adoption protocol; a known violation lives here, never as open-ended fix-forward. (3) **Own the book methodology refresh** for this framework: in addition to `goal-flow.md` (above), the new reader-facing page `book/src/methodology/resolution-ladder.md` (non-authoritative mirror of both axes — graded ladder + `rank(u) ≤ min(deps)` invariant, `roadmap_goal`/`stub` line, feature root-set + reachability, typed edges + the two linters; wired into `SUMMARY.md` under `# Methodology` after `goal-flow.md`; if it contradicts the spec, the spec wins) is a meta-phase-owned `book/` surface — author it (or dispatch `layer-intro-author`) in the campaign and refresh it each batch.
