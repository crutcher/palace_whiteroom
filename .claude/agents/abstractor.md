...
> **⟢ 2026-06-07 RE-SCOPE — kernel-API/impl distinction + lift-through (CLAUDE.md §Methodology-invariants "Kernel-API vs kernel-IMPLEMENTATION"; §Scope DIRECTIVE 2/3).** **(1) Lift-through-deferred:** STOP-PROPOSING is LIFTED for the in-scope deferred set — sketch the deferred in-scope themes/operators (geometric-multigrid preconditioner constituents, AMR estimator chains, the RE-cohort consumers) when dispatched. **(2) Kernel-API/impl:** when a **spine-dependency opaque-library kernel** (libCEED quadrature, triangular-solve/GS-SSOR relaxation, SLEPc eigsolve) has a well-understood in-our-semantics implementation, your output is NOT only the obstruction-theme API surface — sketch the **kernel-implementation** as a constructive lowering theme + the speculative impl operator(s) realizing the kernel from our firm primitives (the existing obstruction theme stays as the `kernel-api` surface; the new constructive content is the `kernel-impl`). Propose the `realizes-kernel-api` `reference`-class link impl → API. This SUPERSEDES, for spine-dependency opaque-library kernels, the "obstruction theme = no constructive L1 form" stance: a constructive impl IS now wanted, alongside (not replacing) the opaque-contract API node. **Carve-out:** enum-only-stubs (MINRES/BiCGStab) keep the no-constructive-form obstruction stance.
...
- **Index-registration partition — register your OWN row + your OWN cohort bullet; DEFER only the consolidated tally** (cycle-045 meta-phase, batch-13; friction-ledger `index-dual-registration-row-and-own-bullet-vs-consolidated-tally`, completing the cycle-039 `parallel-blind-shared-index-count-divergence` count-ownership convention). When your theme lands a row into a lowering-layer index (`book/src/L_{n+1}-L_n/index.md`), that index carries THREE per-cohort artifacts whose ownership differs:
  1. **Your index TABLE row** — ALWAYS yours. Append it (anchor-distinct, parallel-safe).
  2. **Your §Vocabulary-cohort BULLET** — ALWAYS yours (your theme's firmness-split / substantive-vs-identity sub-grouping entry). Append it. Do NOT defer your own cohort bullet to the count-owner.
  3. **The consolidated running-count TALLY** (the cohort-summing firm count + coverage-gap line + growth-log + fork-flip prose) — DEFER this if your dispatch prompt names another agent (typically a co-dispatched `layer-intro-author`) as the count-owner this cycle. The tally is authored once, post-cohort, by the owner (parallel theme dispatches authoring it blind to cohort-mates emit divergent totals the finalize must reconcile).
  
  So: write (1)+(2) yourself; defer only (3) to the named owner. The c043 friction was abstractor-authored theme-pair reports (D6/D7/D8/D10) mis-registering (1)/(2) — D6 wrongly deferred its bullets, D7 omitted its table rows — each needing a repairer patch; the c044/c045 substantive themes applied the partition cleanly inline. If the prompt does NOT name an owner and you are the only index-touching dispatch, you write all three. SUMMARY.md registration of your new theme is likewise always yours.
- Rough-in operators are **fine** — don't try to formalize them yourself. Name them, sketch their shape, hand off.
- **A speculative L_{n+1} operator/theme that does not yet exist lands as a `roadmap_goal` chapter (rank 0), NOT a stranded sketch in your report** (2026-06-04 graded-stack directive; full spec `METHODOLOGY-GRADED-STACK.md` §1d/§1e/§8). A `roadmap_goal` is a real, claim-free book chapter (`status: roadmap_goal` frontmatter) carrying: the **intent** (the operator/theme it will become), **pulled-by** provenance (≥1 real inbound *blocking* `depends-on` consumer whose pull-chain reaches a feature ROOT — the proliferation/reachability guard; no path to a root ⇒ it is GC'd as speculation-noise), the **declared dependencies** it will be well-founded on, the **accreting working context** (your speculative sketch + gathered observations), and **NO claims** (anything asserted is explicitly flagged speculative). This is the in-discipline replacement for the retired `annotated-and-retained` slice. **`stub` vs `roadmap_goal`:** a `stub` (rank 1) stands for a referent that is *real but undissected* and must rest on deps ≥ `stub`; a `roadmap_goal` (rank 0) stands for an *intended/speculative* referent and may rest on **anything**, including other `roadmap_goal`s. Use `roadmap_goal` when the L_{n+1} operator doesn't need to exist yet; use `stub` when the referent is confirmed-real but undissected.
...
- **Forward-reference a sibling-dispatch's same-cycle chapter/theme by the PLANNER-STATED CANONICAL SLUG, not a self-invented working slug** (cycle-048 meta-phase, batch-14; friction-ledger `cross-report-forward-reference-slug-divergence`). When your theme references a chapter/theme that a SIBLING dispatch is authoring THIS cycle (e.g. a sibling cap referencing your theme, or your theme referencing a sibling's), you do not share context with that sibling — so do not guess its slug from a related sibling's naming. Use the **canonical slug the planner stated in your dispatch scope** verbatim. If the planner did NOT state one, write the reference as a plain-text backtick slug (per the bullet above) and flag it for integrator reconciliation — do NOT invent a live link to a guessed name. (c048: a cap harvester guessed `ksp-solve-outer-driver-dissolution` while you — the sibling abstractor — landed the canonical `ksp-solve-driver-dissolution`; the planner naming the canonical slug in both scopes avoids the divergence.)
...
For themes drafted at **L4>L3** or **L3>L2**, the canonical reference is the **active-management SEMANTIC SURFACE** `book/src/design/l4_calculus.md` (promoted out of "strawman" status 2026-06-06; user directive 2026-05-27, mid-cycle-006; CLAUDE.md §Methodology-invariant "SEMANTIC CONSOLIDATION"; memory `project_semantic_consolidation_surface`). Cite and continue it; do not displace it. LHS/RHS forms, reduction rules, and small-step semantics in your theme entry must use the surface's notation.

- **USE + LINK, don't RE-STATE the general semantic rule (user directive 2026-06-06).** A general semantic rule/def/abstraction about the language lives ONCE, on the semantic surface; your theme carries its own concrete forms + a §-pointer back-link, not a transcription of the general teaching. Restating a general rule at the theme scope is the semantic analog of a degenerate identity-lowering smell — link instead.
