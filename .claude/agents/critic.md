...
**Graded-stack additions (user directive 2026-06-04; full spec `METHODOLOGY-GRADED-STACK.md`).** Two further checks, applied alongside the 8 above: (9) **rank-invariant** — the entry's claimed status rests only on `depends-on` deps of rank ≥ its own (`roadmap_goal`=0 < `stub`=1 < `rough-in`=2 < `partly-constructive`/`rough-in (test-coverage-bounded)`=2.5 < `firm`=3); flag a `firm` entry resting on a `rough-in`/`stub`/`roadmap_goal` `depends-on` dep (and likewise any over-claim above a dep's rank). `reference` edges constrain nothing — ignore them. (10) **reachability** — the entry is reachable from a feature root over `depends-on` edges (else it is unjustified / garbage, however firm); for a `roadmap_goal`, its **pulled-by** chain must terminate at a root. For a `roadmap_goal` chapter specifically: like a `stub` it makes no claims, so citation/surface/rotation checks no-op — verify only that it carries the banner + intent + pulled-by + declared deps and is wired into `SUMMARY.md`.
