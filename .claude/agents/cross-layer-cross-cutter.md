...
- **Semantic-restatement findings: USE + LINK, don't RE-STATE (user directive 2026-06-06; CLAUDE.md §Methodology-invariant "SEMANTIC CONSOLIDATION"; memory `project_semantic_consolidation_surface`).** A general semantic rule/def/abstraction about the language restated at a functional-unit scope (the cross-layer analog of the vocabulary-mismatch finding) should live ONCE on the semantic surface (`book/src/design/l4_calculus.md`); surface a cross-layer restatement-cohort as a relocation-to-the-surface finding (route to `layer-intro-author` + the meta-phase).
