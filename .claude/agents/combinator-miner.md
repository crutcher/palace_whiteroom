...
> **⟢ 2026-06-08 FINALIZATION — author chapters as a STATIC-STATE surface (batch-47 directive; skill `finalization-debulk`; memory `project_finalization_debulk_directive`).** The book is a finalized static-state spec, NOT a process log. When your proposed-changes author or edit any `book/src/**` chapter, do **NOT** introduce process/judgment accounting: no `## Status` promotion-history prose; no inline `cycle-NNN`/`cNNN` attributions; no `verified_against:` blocks; no `reports/…` pointers; no "this dispatch"/"self-verified"/mining-narrative; no forward-process speculation. **Firmness lives in frontmatter `rank:`/`firmness:`**; non-firm entries keep a CONCISE static unresolved-state + promotion-condition; no-frontmatter-rank chapters keep the prose `## Status` leading token as the sole rank carrier. Citations stay; couplings become explicit `## Relationship`/`## Structural fact` sections. Exemplar: `book/src/L4/krylov-step.md`.
...
- **USE + LINK, don't RE-STATE a general semantic rule (user directive 2026-06-06; CLAUDE.md §Methodology-invariant "SEMANTIC CONSOLIDATION"; memory `project_semantic_consolidation_surface`).** A combinator proposal that touches a general semantic rule about the language (the named-shape-groups syntax, the closure-returning-signature convention, the notation invariant, a monad/ownership convention) carries its own concrete fact + a §-pointer back-link to the semantic surface `book/src/semantics/index.md` (the surface got its own home at cycle-116, relocated from the former `book/src/design/l4_calculus.md`), not a transcription of the general teaching. **A general SEMANTIC ABSTRACTION you mine** (a new shape-semantic convention, a new ownership/reduction rule, a notation generalization — as opposed to a vocabulary *combinator*) is a candidate for the **semantic surface**, not a per-op chapter: propose it as a relocation/addition to the surface (route to `layer-intro-author` via OQ), the same replace-and-propagate discipline applied to semantics.
- **Closure-returning / high-order combinator signatures — group the closure, use the operator-VALUE spelling, never the opaque type-application form** (user directive 2026-06-07; authoritative def `book/src/semantics/index.md` §1.3.1; memory `project_closure_returning_signature_convention`). The L4 calculus is **high-order**; a mined combinator that *constructs* an operator/closure (the constructed-operator-action family above, partial-application factories) has a function-typed codomain. Spell it: a **bare returned function** groups its closure sub-signature in parens (`-> (τ_in -> τ_out)`); a returned **operator instance** with closed-over `!`-params uses `Op[τ_in → τ_out]` / `LinOp[(N: ...), $N]` (ALREADY COMPLIANT, no outer parens — a constructor or transformer stands as-is). **Never** the opaque `LinearOperator[N,N]` type-application (no in/out arrow → the non-compliant smell); re-spell, do not wrap. Cite + link §1.3.1; do not restate.
- **Destructive replace-and-propagate sweeps THREE de-link surfaces, not two (batch-40 codification; friction `deleted-slug-frontmatter-edge-gap`; skill `deleted-slug-inbound-live-link-sweep` step 7).** When your replace-and-propagate ELIMINATES a standalone node (folding an arity/specialization leaf into its combinator and `delete:`-ing the leaf chapter — the c124 RE6 shape), your proposed-changes must re-point or strike EVERY inbound reference across **(i)** markdown body links `](.../<slug>.md)`, **(ii)** prose code-span mentions, AND **(iii) YAML frontmatter typed `edges:` blocks** (`depends-on` / `reference` / `lifts-from` / `realizes-kernel-api`) that name the deleted slug. Surface (iii) is the silent-dangler trap: `mdbook-linkcheck2` is BLIND to frontmatter edges, so a stale `depends-on` to a deleted slug reaches finalize and only trips the rank linter's `unresolved_depends_on_targets` (a stale `reference` is caught by nothing). Run skill `deleted-slug-inbound-live-link-sweep` step 7's `grep -rnE '(depends-on|reference|lifts-from|realizes-kernel-api)[^]]*\b<slug>\b' book/src` and re-point each hit to the surviving consolidation target (the combinator the leaf folded into) in your proposed-changes — do not leave it for finalize to repair (the c124 RE6 elimination shipped two such edges, `L3/normalize → L3/scal` and `L3/orthogonalize → L3/axpy`, that the finalize had to surgically fix).
