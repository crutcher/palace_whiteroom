...
- **MANDATORY pre-dispatch deliverable-presence check for every named-artifact-slug scope** (cycle-033 meta-phase, batch-9; friction-ledger `cycle-planner-stale-priorities-line-recruitment`; promoted skill `verify-dispatch-scope-not-already-discharged`). The prior bullet (`verify-candidate-is-genuinely-open` via `counts_after`/STAGING/struck-Backlog) checks the *cycle-record trail*; this bullet checks the **artifact itself**, which is the ground truth and catches a deeper class of staleness the c027 bullet did NOT catch. For every dispatch whose `scope` resolves to a named file path under `book/src/` (an L_n operator, an L_{n+1}>L_n theme, a `concepts/<slug>` page, a Phase-1 slice-reduction audit, a `verified_against:` audit), run the **four-step deliverable-presence sequence** BEFORE finalizing the dispatch in the plan:
  1. **File existence:** `ls book/src/<layer>/<slug>.md`. If the file does not exist AND the dispatch is to author it → genuinely-open by construction (proceed).
  2. **Maturity / already-discharged check:** if the file EXISTS, read its `## Status` line: `firm` / `partly-constructive` / `rough-in` / `obstruction` / `stub`. If the proposed dispatch's deliverable is at-or-below the on-disk maturity, the dispatch is a no-op — do NOT recruit. For audit-class dispatches (lowering-verifier on an L1>L0 theme, slice-reduction audit), grep the target file for `verified_against:` blocks (or for the slice case, scan the OQ ledger for a recent RESOLVED disposition); if an audit block already exists at the timestamp class the proposed audit would emit, the deliverable is already present.
  3. **OQ-ledger RESOLVED-grep:** `grep '<slug>.*RESOLVED\|<slug>.*CLOSED' scaffolding/open-questions.md` — if the dispatch's primary OQ is already in the Closed-index AND the plan line is older than the closure, the line is stale. (Closed-index entries are append-only; meta-phase compacts them in unification passes.)
  4. **Structural-block check:** is the candidate dispatch's deliverable structurally blocked by a methodology gate that has not changed? Common blocks: `rough-in (test-coverage-bounded)` (the c021 invariant — promotion gated on out-of-scope dedicated test; if no upstream Palace test change since the priorities line was authored, the gate is still blocking — do NOT re-propose the firm-promotion); `partly-constructive` promotion gate (positive Palace anchor not yet found — same logic); `partial-obstruction` (loop structure does not lift); etc.

  Only recruit when ALL four checks pass: target file genuinely absent (or below proposed maturity) AND OQ not RESOLVED AND no structural block. Record the check outcome **inline per-dispatch in the plan** (the c033 cycle-planner CYCLE.md `## Deliverable-presence verification` section is the canonical working precedent — emit a similar section in every multi-dispatch plan). Cheap (one `ls`/`grep`/`grep`/structural-check per scope; <30 seconds total for a 6-dispatch plan). The c031/c032 stale recruitment incidents (recurrence-2 escalation of the c027 entry, captured as the standalone `cycle-planner-stale-priorities-line-recruitment` friction-ledger entry; the c033 working precedent demonstrates haiku can run the procedure). Skip ONLY for dispatches that are open by construction (a fresh harvest of a new operator with no prior-cycle history; a meta-phase-routed follow-up explicitly justified in the immediately-prior cycle's integrator-signals).
