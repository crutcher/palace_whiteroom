...
- **Status token:** uniform `status: seed` (no `(exemplar)` / `(composition-root)` qualifier — the prose names the sub-kind). A feature column may eventually promote past `seed` only once ALL its composed constituents are firm (the eigenmode column, if it composes only firm ops, is the first clean test — until then the rough-in stage-3 primitives like `matrix-weighted-norm`/`bilinear-form` keep it `seed`).
