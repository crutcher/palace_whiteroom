...
5. **Run per-report safety-net gates:**
   ...
   - **deleted-slug frontmatter-edge sweep (the silent-dangler surface; batch-40 codification; friction `deleted-slug-frontmatter-edge-gap`; skill `deleted-slug-inbound-live-link-sweep` step 7).** When this report's proposed-changes **`delete:` any `book/src/**` chapter** (a destructive replace-and-propagate elimination — e.g. an arity/specialization leaf folded into its combinator), the inbound de-link sweep has THREE surfaces, not two: markdown body links, prose code-spans, AND **YAML frontmatter typed `edges:` blocks**. As a pre-apply check, run `grep -rnE '(depends-on|reference|lifts-from|realizes-kernel-api)[^]]*\b<slug>\b' book/src` for every deleted slug (exclude the deleted files themselves by source-path prefix). Any residual `depends-on` edge to a deleted slug is a `unresolved_depends_on_targets` violation the finalize rank-linter WILL catch (but too late); a residual `reference` edge is a silent navigational dangler NEITHER linkcheck2 NOR the rank-linter flags. **Defensively re-point** each to the surviving consolidation target (the node the deleted slug folded into) per build-repair authority if the report did not, OR mark the report `deferred` if the re-point would alter load-bearing content. Record as `applied-discretionarily` with rationale `deleted-slug-frontmatter-edge-repointed`. This moves the c124 RE6 finalize-repair (2 stale `depends-on` edges: `L3/normalize → L3/scal`, `L3/orthogonalize → L3/axpy`) from finalize-time to per-report-time. (`mdbook-linkcheck2` is BLIND to frontmatter edges — it cannot be relied on for this surface.)

   - **KaTeX `$`-sigil pre-apply fence lint (the indented-`$`-sigil rendering-collision surface; batch-45 meta-phase codification, cycle-141; friction-ledger `katex-dollar-sigil-eaten-in-indented-pseudocode`, recurrence-2; memory `project_katex_dollar_sigil_fence_requirement`).** Before writing each ` ````edit:<path>` payload whose target is a `book/src/**` chapter, **scan the payload body with a FENCE-STATE tracker** and flag the KaTeX collision class BEFORE the write (moving the catch from the `integrator-finalize` step-5c post-build assertion to per-report-apply-time). The mdBook `[preprocessor.katex]` skips *fenced* and *inline* code but NOT *indented* code, so a 4-space-indented pseudocode line carrying a named-shape-group `$`-sigil token (`$S`/`$N`/`$`-letter — `Tensor[$S]`, `LinOp[$S,$S]`) has its `$...$` run eaten as inline-math and renders `<span class="katex">` garbage into the `<pre>`. **The check (fence-aware so it does NOT mis-flag alignment-indented lines inside a real fence):** walk the payload line-by-line, toggle an `in_fence` flag on each ` ``` ` fence line; flag a line ONLY when `in_fence` is false AND the line is a markdown indented-code-block line (≥4 leading spaces, in an indented-code context) AND it contains a `$`-sigil token. (The bare grep `^    .*\$[A-Z]` is unreliable — it false-positives on `$`-sigil lines indented FOR ALIGNMENT *inside* a ` ```text ` fence; the fence-tracker is what makes the check trustworthy.) **On a hit:** wrap the offending indented block in a ` ```text ` fence per the standing convention (build-repair authority — the indented→fenced surgical fix, zero content change) and record `katex-dollar-sigil-pre-apply-fenced` in the staging-row Notes; if the wrap would alter load-bearing structure, mark the row `deferred` for the planner instead. The `integrator-finalize` step-5c post-build assertion STAYS as the backstop (it catches anything this heuristic misses + non-edit-block drift). This moves the c139 D2 `lanczos_step` finalize-time build-repair (an indented `lanczos_step :: LinOp[(S: ...), $S] -> ...` signature line) to per-report-apply-time.
