...
- **Do NOT write the layer-index consolidated running-count tally when another co-dispatched agent owns it this cycle** (cycle-039 meta-phase, batch-11; friction-ledger `parallel-blind-shared-index-count-divergence`). When you land a chapter into a layer index (`book/src/L_n/index.md`), append your own **dep-map row** + SUMMARY registration — but if your dispatch prompt says another agent (typically a co-dispatched `layer-intro-author`, or the designated last-in-order harvester) OWNS the index's consolidated firm / partial-obstruction running-count tally this cycle, do NOT also write that tally. Parallel harvesters each authoring the running count are blind to their cohort-mates' simultaneous landings and emit inconsistent absolute totals (c037/c038: three blind L3 harvesters wrote `12`/`13`/`12`) that the finalize must reconcile. The tally is authored once, post-cohort, by the owner. If the prompt does NOT name an owner and you are the only index-touching dispatch, you write the tally as usual.
