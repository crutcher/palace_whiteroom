...
edges:
  ...
  reference:
    ...
    - feature/waveguide-mode.L4           # SIBLING output-product cross-link — homes this driver's stage-(3) readout reduction; a reference, NOT a blocking constituent (OWN-COMPOSITION rule)
---
...
- The readout (stage 3) is a pure `map` over the converged modes, homed in the [`waveguide-mode`](./waveguide-mode.L4.md) output-product column.

Both composed solve-side combinators ([`fe_assemble`](../L4/fe_assemble.md), [`eigsolve`](../L4/eigsolve.md)) are **firm**, and the stage-(3) readout reduction is homed in the [`waveguide-mode`](./waveguide-mode.L4.md) output-product column. Under the OWN-COMPOSITION promotion rule (a column promotes off `seed` when its OWN composition + directly-owned constituents are firm; cross-linked sibling columns are references, NOT blockers), this column's directly-owned constituents — the [`fe_assemble`](../L4/fe_assemble.md) block-pencil assemble, the [`eigsolve`](../L4/eigsolve.md) black-box eigen-solve, and its per-mode readout (homed in the `waveguide-mode` column) — are all firm. The cross-link to the [`waveguide-mode`](./waveguide-mode.L4.md) output-product column is a **SIBLING reference (the drift-guard), NOT a blocking constituent**.
...
| per-mode readout (kn, n_eff, Et, En, Bz) | [`waveguide-mode`](./waveguide-mode.L4.md) (`waveguide_mode_reduce`) — SIBLING output-product column | (sibling ref) | `boundarymodesolver.cpp:273-334` |

This chapter carries the *compositional* claim (boundary-mode = this composition of these constituent pieces), not the constituents' per-op algebraic claims (those live in the linked chapters). The `BoundaryModeSolver` is a 6th `ProblemType` dispatch branch that routes through the same `switch` as the 5 drivers (`palace/main.cpp:276-278`), so it is a co-equal leaf driver column — the second clean witness of the composition-root pattern over a single opaque-library black-box eigen-iteration (the SAME [`eigsolve`](../L4/eigsolve.md) corner as [`eigenmode`](./eigenmode.L4.md), distinguished by the 2D-submesh extraction preface). (`feature_root: seed` in the frontmatter is the permanent root-set marker, NOT a maturity; the maturity is carried by `rank: firm`.)
