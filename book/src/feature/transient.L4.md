...
## The composition structure (the fold-sibling witness)

Both stages compose firm L4 combinators with no obstruction at the composition level:
...
The whole feature lowers cleanly outward to the L4 backend surface: `transient = fold_solve ∘ fe_assemble` — a two-stage pipeline of firm combinators with a single shared operator capture and a threaded field-state carry. The transient column is the spine's first **fold-pipeline** witness — the structural complement to the electrostatic/magnetostatic map-pipeline columns.
