...
- The reduction is [`eigenfreq_qfactor_reduce`](../L4/eigenfreq_qfactor_reduce.md), a **per-mode scalar-ratio map** — there is no `symmetric_from_upper`, no family-PAIR `xⱼᵀ K xᵢ` bilinear (the load-bearing distinction from `gram_reduce`).
...
| eigenfrequency un-transform (folded) | [`eigenvalue_untransform`](../L1/eigenvalue_untransform.md) (firm L1; folded by [`eigenfreq_qfactor_reduce`](../L4/eigenfreq_qfactor_reduce.md)) | firm | `eigensolver.cpp:430-439` |
