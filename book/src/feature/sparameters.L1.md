...
- **L4** ([`sparameters.L4`](./sparameters.L4.md)): the whole reduction is the [`sparameter_reduce`](../L4/sparameter_reduce.md) combinator (the projection grid + self-reflection + port-kind closing made *structural*). The L4 form is the one the outward backend consumes; the L1 form is the pure-function decomposition the L4 combinator names.
