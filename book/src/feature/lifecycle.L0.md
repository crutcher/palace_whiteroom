...
This is a meta-feature whose constituents include other feature columns (the per-driver specializations) rather than only vocabulary ops: the per-driver dispatch at the `switch (iodata.problem.type)` specialization seam is over **sibling feature columns** = **references, NOT blocking constituents** (the spine-ROOT sub-kind). The per-driver column down-links are the specializations this seam selects; their own `status:` does not gate this ROOT's.
