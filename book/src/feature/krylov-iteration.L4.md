...
2. **The auxiliary orthogonalize stage** — [`orthogonalize`](../L3/orthogonalize.md)
   (**partial-obstruction**). The `op.orthog (V_prefix, w)` auxiliary stage that keeps the
...
fold this iteration spine, not constituents of it. Wiring them
...
`rough-in` — the iteration-rotation infrastructure feature-surface composition-root (DIRECTIVE-2
...
bodies all lift, so the composition is cleanly statable BY NAME — a real rough-in column, not a
roadmap_goal. **Promotion route:** `rough-in → firm` if the two partial-obstruction iteration-views
promote; their obstructions are intrinsic (carry-threading + MGS numerical stability are
non-removable), so `rough-in` with the inherited sequential-obstruction recorded is the honest
