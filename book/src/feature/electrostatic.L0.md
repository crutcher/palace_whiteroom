...
The L0 ground-truth surface for the electrostatic feature: a fixed-operator solve — assemble `K` once, sweep the terminal-source family with the operator captured once, reduce to the capacitance matrix. Every stage is a cited range into `palace/drivers/electrostaticsolver.cpp`. The chapter's evidence IS the driver-source range + the per-stage site map to the constituent ops.
