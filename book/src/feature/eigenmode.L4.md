...
edges:
  depends-on:
    ...
    - target: L4/eigsolve
      kind: composes
    - target: L3/divfree_projector
      kind: constrains-eigvec           # the eigenmode driver wires the divergence-free projector into the eigensolver (`eigen->SetDivFreeProjector(*divfree)` eigensolver.cpp:233; the initial starting vector projected `divfree->Mult(v0)` :262) to keep eigenvectors in the divergence-free subspace — a directly-wired absorbed constituent of this pipeline (highest entry L3). Grounds the firm-but-absorbed divfree_projector cluster (and its set_subvector_zero) from this root.
...
The whole feature therefore lowers cleanly outward to the L4 backend surface: `eigenmode = map readout ∘ eigsolve ∘ eig_pencil ∘ (fe_assemble ×3)`. Both directly-composed combinators ([`fe_assemble`](../L4/fe_assemble.md), [`eigsolve`](../L4/eigsolve.md)) are **firm**, so under the OWN-COMPOSITION rule (a column is firm when its OWN composition + directly-owned constituents are firm; cross-linked sibling columns are references, NOT blockers) this column is **`firm`**. The stage-(3) reduction into the [`eigenfrequency-qfactor`](./eigenfrequency-qfactor.L4.md) output-product column is a **sibling cross-link** (a reference / drift-guard), not a directly-owned constituent — it does NOT gate this driver column. (That column is itself firm independently on its own reduce verb `eigenfreq_qfactor_reduce`.) The directly-owned constituent vocabulary is firm and composes without forcing the spine.
