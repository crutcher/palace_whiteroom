...
4. **The schedule is a fixed precomputed list (Palace transient), NOT state-generated.** The schedule is a uniform `delta_t`/`n_step` march known before the sweep (`palace/drivers/transientsolver.cpp:35-36`). **When the schedule is generated from the carry (SweepAdaptive greedy, `palace/drivers/drivensolver.cpp:389`), this rewrite does not apply** — that is the state-generated superset, outside this theme's scope.
