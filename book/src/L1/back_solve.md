...
rewrites into the in-place `s[0..j]` overwrite at L0 — belongs to the
L2>L1 `incremental-least-squares-composition-lowering` theme, whose landing target
this leaf is.
...
- **L1**: pure-functional `y = back_solve(R, s)`. No destination buffer in the
  ...
  access at L0 — belongs to the L2>L1
  `incremental-least-squares-composition-lowering` theme.)
...
- `palace/linalg/iterative.cpp:652` — GMRES back-solve comment "Reconstruct the
  ...
  restart-cycle terminal back-solve.
...
- `palace/linalg/iterative.cpp:655` — `ScalarType *Hi = H.data() + i * (max_dim + 1);`
  ...
  upper-triangular dense-materialized shape and the L0 flat storage.
...
- `palace/linalg/iterative.cpp:666` — `x.Add(s[k], V[k]);` — the downstream
  ...
  leaf — recorded to ground law 6 (basis-lift independence).
- `palace/linalg/iterative.cpp:831` — FGMRES back-solve comment "Reconstruct the
  solution" — the FGMRES restart-cycle terminal back-solve.
...
- `palace/linalg/iterative.cpp:838` — FGMRES `s[k] -= Hi[k] * s[i];` — the
  super-diagonal subtraction, identical to GMRES `:659`.
...
- `palace/linalg/iterative.hpp:193` — `mutable std::vector<ScalarType> s, sn;` — the
  ...
  the complex case). Grounds the `s`/`R` element-type axis.
...
- `book/src/L1/lu_solve.md` — the firm small-dense-general direct-solve sibling;
  the structural template and the firm-on-positive-structure /
  no-dedicated-test precedent.

## Status

`firm` — the operator structure is read directly from positive Palace source.
