...
`firm` — **firm-on-positive-structure**. The operator's structure is read directly from a positive Palace source site (`QuasiNewtonSolver::GetResidualNorm`, `palace/linalg/nleps.cpp:807-821`) and corroborated at four further sites (`:496-499`, `:556-559`, `:655`, `:729`); the signature's pencil shape matches the nonlinear `SetOperators` virtual (`palace/linalg/eps.hpp:69-74`) and the real-argument `A2`-closure contract matches the `SetExtraSystemMatrix` closure type (`palace/linalg/nleps.cpp:177-181`). The nonlinearity is fully localised in the opaque `A2` closure, read (not reconstructed) from source, so the pencil apply needs no `partly-constructive` caveat. NLEPS has no dedicated unit test, but every law is a syntactic identity on fully-specified source (the `apply_linop` situation, not the `eigsolve`-convergence-semantics situation): the laws do not depend on convergence behaviour, so the missing test does not gate them.
...
- No dedicated unit test exists for the NLEPS path (`QuasiNewton` / `nleps` / `GetResidualNorm` / `funcA2` have no hits under `test/unit/**`); the firm decision rests on positive structural citation, not on a test.
