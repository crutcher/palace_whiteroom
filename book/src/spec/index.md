...
| [GMRES](./slices/gmres.md) | L4 v0.2 | 2026-05-26 | L4 form lands: state stratification (SimState/OpParams/Krylov), Solve monad coordination, sequential-obstruction typing for ls_update_column/back_solve, and constructed-operator absorption of convergence + termination (v0.2 tightening adds build_convergence, should_stop_inner, classify_outcome). |
...
| [arnoldi step](./slices/arnoldi_step.md) | L4 | 2026-05-26 | L0–L4 present. Cycle 75 (tightening): citation re-anchoring per Explorer ExplorationFinding — `Sources` table corrected (621-628 kernel; 288-305 ApplyBA; 308-325 OrthogonalizeIteration; 519-541 Update; orthog.hpp:38-89 inner impls), L0 pseudocode aligned to source (`VecType &w = V[j+1]` alias + lazy `Update(j)` made explicit), FGMRES citation tightened to 794-822 with `PreconditionerSide::RIGHT` hard-wiring noted, breakdown-via-CheckDot surfaced, open-question on unit-test coverage refined (orthog primitive IS covered; only the four-line kernel is not). |
