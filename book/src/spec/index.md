...
| [GMRES](./slices/gmres.md) | L2 | 2026-05-24 (cycle 24) | L1 + L2 on disk. L1 absorbs four variant axes (`pc_side`, `gs_orthog`, `flexible`, restart-vs-not) via constructed operators (`apply_BA`, `apply_correction`). L2 unfolds into BLAS-style primitives (`apply_linop`, `axpy`, `dot`, `nrm2`, `scal`, `givens_generate/apply2`, `trsv`); orthogonalisation deferred to `orthog` slice. L3+L4 pending. |
| [Orthogonalisation](./slices/orthog.md) | L2 | 2026-05-24 (cycle 23) | MGS / CGS / CGS2 variants. L1 absorbs variant at procedural level via constructed-operator dispatch. L2 explicitly does NOT unify primitive sequence across variants — the variants exist precisely because their collective shape differs; documented as legitimate residual axis per `variant-absorption` concept levels. |
| [Divergence-free projection](./slices/divfree.md) | L2 | 2026-05-25 | L1 + L2 on disk. L2 expresses apply as apply_linop → set_subvector_zero → ksp_solve → axpy chain; complex specialization is primitive-level unrolling over {re, im}. Variant absorption (VecType, MG/AMG, empty-bdr) carries through to L2 transparently. |
| [Chebyshev smoother](./slices/chebyshev.md) | L1 | 2026-05-24 (cycle 19) | Polynomial smoother used in multigrid preconditioning. L1 includes both Chebyshev-1st-kind and Chebyshev-4th-kind variants with variant-conditional setup (`lambda_min`, `theta`, `delta` only set for 1st-kind; documented as residual axis). L2 pending. |
