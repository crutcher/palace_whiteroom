...
| [GMRES](./slices/gmres.md) | L4 (v0.4) | 2026-05-26 | L0/L1/L2/L3/L4 v0.1-v0.4 landed. v0.4 (L4→L4 self-rotation) extracts `commit_outcome` as the single SimState write site for `(converged, final_res)`, fusing the v0.3 pre-loop short-circuit and post-correction commit paths. Constructed-operator surface: 8 helpers (two added, none removed); `commit_outcome` reads no OpParams. |
...
| [cg_preconditioning_framework](./slices/cg_preconditioning_framework.md) | L3 | 2026-05-26 | L3 lift: composition surface is L3-trivial — all run-time primitives are global at L2; build-time scaffolding (factories, binds, finest-level-unwrap) is outside the tensor-field lift. Negative-obstruction result recorded; new methodology concept `build-time-vs-run-time-stratification` extracted. |
