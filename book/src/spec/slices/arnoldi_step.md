...
- The post-Arnoldi small-dense Givens-QR triangularisation has been recorded across prior cycles as a distinct sequential obstruction at the small-dense / Hessenberg-column scope (see [incremental-least-squares](../../concepts/incremental-least-squares.md), [sequential-obstruction](../../concepts/sequential-obstruction.md)). Currently this slice excludes it (it belongs to the GMRES outer loop's incremental-least-squares concept). Should it instead be folded in here as a logical third phase? Current call: **keep separate**. Rationale: the two obstructions live on disjoint state — MGS sequential is a field-side `w`-mediated dependency across DoF-space allreduces, while Givens-replay-and-generate is a small-dense column-write dependency on `H[:,j]`. Conflating them would obscure that the field-side obstruction is variant-dependent (MGS only) while the small-dense one is variant-uniform (always present, every Krylov flavour). The Arnoldi step is the field-side boundary kernel; the small-dense update is consumed in [gmres](./gmres.md) at L2.
