...
Per [`first-iteration-unrolling.md:34-37`](../concepts/first-iteration-unrolling.md), the rotation's natural driver is exactly this combinator. [`krylov-step`](./krylov-step.md) Form B (`first_step` / `steady_step`) consumes it as its driver.
...
As with [`iterate-while`](./iterate-while.md), the dedicated L4>L3 theme for this combinator is the standalone chapter [`iterate-while-with-prev-dissolution`](../L4-L3/iterate-while-with-prev-dissolution.md), the carry-bootstrapped sister of [`iterate-while-dissolution`](../L4-L3/iterate-while-dissolution.md). It captures **both** L3 forms: the **trajectory-keeping unpruned form** `iterate_while_with_prev_L3` — the bootstrap-then-tail-recursive ground form that materialises the `[e₀] ++ trajectory` accumulator this firm L4 form keeps (per Law 2) — and the **§3.8-pruned form** `iterate_while_with_prev_L3_pruned`, the collapse-rule image (Law 2's L3-side demand-pruning rewrite applied to both bodies under a `final_state`-only consumer). See [`iterate-while-with-prev-dissolution`](../L4-L3/iterate-while-with-prev-dissolution.md) §"L3 form (RHS)" for the two forms, the `prev`-positional delta, and the collapse rule.

The L3>L2 lowering for the combinator itself is identity-in-form, same as [`iterate-while`](./iterate-while.md).
...
Same effect-threading-and-demand-pruning-placement difference as [`iterate-while`](./iterate-while.md). The L4>L3 lowering is the dedicated standalone theme [`iterate-while-with-prev-dissolution`](../L4-L3/iterate-while-with-prev-dissolution.md) (the carry-bootstrapped sister of [`iterate-while-dissolution`](../L4-L3/iterate-while-dissolution.md)) — it erases the monadic packaging, positionalises the `prev` closure parameter, renders the bootstrap as a non-recursive prefix, and resolves the demand-pruning per consumer; it does *not* re-introduce the iteration-zero branch (the rotation is preserved across the lowering).
...
- `book/src/L4/krylov-step.md` (firm) — Form B signature consumes this combinator (`first_step` is the bootstrap, `steady_step` is the steady-step body).
- `book/src/L4/iterate-while.md` (companion) — Law 1 of this entry states the degeneracy to [`iterate-while`](./iterate-while.md) when `β = ()`.
