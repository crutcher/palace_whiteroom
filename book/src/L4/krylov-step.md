...
- `SimState` — externally-visible state that persists across the entire solve call. Per `state-stratification`, contains `x: Tensor[(S: ...)]`, `it: Int`, `converged: Bool`, `final_res: Scalar`, `initial_res: Scalar`. Threaded by the `Solve a = StateT SimState Identity a` monad (see [`solve-monad`](../concepts/solve-monad.md)); the monadic effect of `krylov-step` is the `SimState` transition.
