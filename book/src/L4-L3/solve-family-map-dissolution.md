...
- **Reduction-chain** (secondary): the higher-order `map (\inp -> ksp_solve op inp) rhss` desugars to the explicit positional `for` with the accumulator threaded (the standard `map`-to-fold-to-imperative-loop reduction); the pure-map degenerate of the strawman §3.7 `iterate_while` family (the cap's alternate rendering) desugars to the same accumulating loop with the carry `{ remaining, solutions }` becoming the loop index + the mutable collection (`book/src/semantics/index.md` §3.7).
