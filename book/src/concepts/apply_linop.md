...
When a slice's L1 names a *constructed operator* (e.g., `apply_BA` in GMRES, see [concept: constructed-operators](constructed-operators.md)), that operator's L2 unfolding is typically a small chain of `apply_linop` calls — the constructed operator hides the chain at L1 while preserving it as the L2 primitive sequence.
