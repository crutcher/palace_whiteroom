...
- **`Mult2(x, y, r)`** and **`MultTranspose2(x, y, r)`** (lines 52-64) — virtual entry points with a pre-allocated temporary storage vector `r`. Both default to `MFEM_ABORT`; the temporary-storage form is intended for hot-path consumers that want to avoid the per-call workspace allocation in the iterative-solver subclasses. Not currently used outside the immediate hierarchy.
...
- [`L1/ksp_solve`](../L1/ksp_solve.md) — the pure-functional solve operator over the `Solver<OperType>` type axis.
