...
- **`linalg::` free functions** (`palace/linalg/operator.hpp:369-403`) — a small namespace at the file's bottom for matrix-weighted operations that take an `Operator &B` as the inner-product weight:
  ...
  - `Dot(comm, x, A, y)` — the bilinear-form inner product `yᴴ A x` for either a real `A` (line 388) or complex `A` (line 393); implementations at `operator.cpp:621-639` use the [`mutable-workspace-pattern`](./mutable-workspace-pattern.md) "allocates workspace internally" form (Category 4 — synthetic workspace).
...
- [`mutable-workspace-pattern`](./mutable-workspace-pattern.md) — `SumOperator::z`, `BaseProductOperator::z`, and the free-function `Dot`'s synthetic workspace are Categories 2 and 4.
...
- [`L0/mutable-workspace-pattern`](./mutable-workspace-pattern.md) — Category 2 (composition-class workspaces) cites `SumOperator::z` and `BaseProductOperator::z`.
