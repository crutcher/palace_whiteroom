...
- **L2 firm** ([`L2/ksp_solve`](../L2/ksp_solve.md)) — the **outer-driver composition**: `(K, b) -> SolveResult` with body = the convergence-test fold of [`krylov-step`](../L2/krylov-step.md), the kernel and the fold visible (but the iteration view named-by-role, not rendered as explicit recursion). The LHS of this theme.
...
- `book/src/L4/iterate-while.md` (firm) — the firm `iterate_while` combinator the L2 fold references (and the L1 opacity collapses).
