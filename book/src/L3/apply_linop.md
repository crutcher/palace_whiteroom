...
2. **Zero-vector annihilation**: `apply_linop A 0_D = 0_R` (where `0_D` is the zero vector of the domain group `D` and `0_R` is the zero vector of the range group `R`). Follows from law 1 with `α = β = 0`.
...
7. **Zero operator**: `apply_linop 0_op x = 0_R` for the zero operator `0_op : V → W`. Special case of law 6 with `α = 0`.
