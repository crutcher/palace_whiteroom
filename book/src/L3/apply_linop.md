...
4. **Composition (operator product)**: `apply_linop (A · B) x = apply_linop A (apply_linop B x)` where `A · B` is the operator-composition construct (an operator with domain matching `B`'s domain and range matching `A`'s range, requiring `B`'s range group to equal `A`'s domain group). This is the L3 algebraic identity that enables constructed-operator chains like `apply_BA = A · M⁻¹` to be unfolded into a sequence of single-operator applies — directly consumed by `krylov-step` at L3 in the form `apply_linop op.T K.<input_field>` (where `op.T` may be a composed operator). Witnessed at L0 transitively via the L1 entry's `palace/linalg/operator.hpp:202-206` citation.
...
**Strawman reference**: `book/src/semantics/index.md` is the L4 conventions source; this L3 entry follows the strawman's Haskell `::` signature notation. The L4 layer does not surface `apply_linop` as a standalone entry; the L4 form is the implicit use inside the L4 `krylov-step` body's let-chain.
...
- `book/src/L4-L3/krylov-step-typed-wrapper-dissolution.md` (firm) §"L3 form (RHS)" line 64 — `let w = apply_linop op.T K.<input_field>` is the first line of the L3 `krylov-step` body's let-chain. This is one of the two firm cross-layer themes that render the L3 form of `apply_linop`.
...
- `book/src/L3/krylov-step.md` (firm) §Semantics — the precedent L3 entry consumes `apply_linop` as a per-step matvec at the very first line of the `krylov-step` body (`let w = apply_linop op.T K.<input_field>`). The L3 form named by this entry is the form that L3 `krylov-step` reads.
...
- CG L4 v0.5 step bodies (`cg_first_step` and `cg_steady_step`; firm-homed at `book/src/L4/krylov-step.md` Form B); each has `let Ap = apply opA p'` as the per-step matvec call. The L0 matvec home is `book/src/L1-L0/ksp-solve-mutation-rotation.md` Sub-pattern B (`iterative.cpp:360-486`).
