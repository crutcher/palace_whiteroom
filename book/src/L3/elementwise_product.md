...
- `book/src/L3/jacobi-smoother.md` (firm) — the consumer whose apply body is *one* `elementwise_product` (`y = op.dinv ⊙ x`); the thinnest constructed-operator gate. The freshest L3 identity-row template precedent (along with `assemble-diagonal`).
- `book/src/L3/assemble-diagonal.md` (firm) — the freshest L3 identity-in-form template precedent; the operator-to-data sibling that opens the `assemble_diagonal → reciprocal → elementwise_product` diagonal-preconditioner chain `elementwise_product` closes.
