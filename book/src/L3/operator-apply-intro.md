...
- [`assemble_diagonal`](./assemble_diagonal.md) — the operator-to-data introspection primitive `(A: LinOp[(S: ...), $S]) -> Tensor[$S]` (`A -> diag(A)`, square operator — one shape group `S`); the operator-to-data sibling of `apply_linop`. Carries the load-bearing exact-vs-approximate (matrix-free) non-law as a representation-aware non-law.
