...
- **result** — `Tensor[$S]` — congruent to `x` (same shape group `S`); same element type as `x`. Every output element equals the field-multiplicative-inverse of the corresponding input element. The result element type **tracks** the input element type (real `x` → real result; complex `x` → complex result) — unlike [`nrm2`](./inner_product.md#consumer-not-an-instance-nrm2--matrix-weighted-norm), which collapses both to a real-valued result. `reciprocal` is a self-map on the vector type's element field.
