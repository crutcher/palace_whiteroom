...
The `assemble-diagonal` and `jacobi-smoother` L3 backfills (`book/src/L3/assemble-diagonal.md`, `book/src/L3/jacobi-smoother.md`) are the structural precedents on the same diagonal-preconditioner-apply chain: identity-in-form rotation on the primitive's signature, layer-coherence backfill, methodology invariant enacted. `reciprocal` is the elementwise `D⁻¹`-forming step between them.
