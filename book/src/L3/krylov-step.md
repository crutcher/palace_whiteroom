...
- `book/src/L4/krylov-step.md` (firm) — the L4 typed-wrapper form this L3 entry value-thread-rewrites. Body shape, semantics (effect placement, three placements made structural by the typing), variant axes (six), algebraic laws (three that hold; non-laws catalogued).
...
- `book/src/L2/krylov-step.md` (firm) — the L2 primitive-composition form this L3 entry lowers to. The body's let-chain maps line-for-line per the L3>L2 theme.
...
- CG step bodies + `cg_solve` outer driver — firm-homed at `book/src/L2/krylov-step.md` §Evidence and `book/src/L4/krylov-step.md` §Semantics Form A / Form B; L0 ground Sub-pattern B (`iterative.cpp:360-486`; `BaseKspSolver::Mult` `ksp.cpp:296-310` wrapping the inner CG for-loop `iterative.cpp:427-464`).
