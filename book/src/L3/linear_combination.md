...
**Same-layer (L3)**: this combinator unifies the four L3 BLAS-1 linear-update leaves `scal` (arity 1), `axpy` (arity 2, second coeff 1), `axpby` (arity 2), `axpbypcz` (arity 3) as list-length specializations (law 6). These four leaf chapters were re-expressed through this combinator and eliminated (RE6) — their unique L0 anchors folded into the [§"Arity specializations"](#arity-specializations) notes here. The composition surfaces that *consume* `linear_combination` at L3 are the iterate-stratum update inside [`krylov-step`](./krylov-step.md)'s `krylov_update` (the GMRES basis-correction sum is a `linear_combination` over scalar-weighted basis terms).
