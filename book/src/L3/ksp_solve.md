...
The L3 fold structure is directly read from the Palace per-method `Mult` bodies; the four-field result and convergence predicate are read from the `IterativeSolver` base; the obstruction is inherited from the firm `krylov-step` entry.
