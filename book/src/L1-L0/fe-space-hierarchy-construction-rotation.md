...
# endpoint (fe_space_hierarchy) is firm (rank 3); the L0 endpoint is rank-terminal
...
The per-level dof-resolution tail (`GetEssentialTrueDofs`, `multigrid.hpp:99`/`:109`/`:120`) is
**MFEM-owned-read-as-given** and is the subject of the sibling theme
...
would be the identity-in-named-terms smell the vocabulary-shift redirect warns against). The lazy
prolongation `P[l]` / discrete-interpolator machinery is read-as-given (a property of the result record,
sibling-pull-gated), NOT a constructed sub-part. MPI / `Par*` and mesh partitioning are flagged once and
read single-rank (out of scope per CLAUDE.md §Scope).
...
- **DELEGATED to siblings (not re-narrated here).** The per-level `fe_space(·, ·)` construction at each
  ...
  the `[FECollection]` schedule the p-loop ranges over is produced by
  [`fe-collection-construction-rotation`](./fe-collection-construction-rotation.md); the per-level
  dbc single-space block is [`essential-dofs-construction-rotation`](./essential-dofs-construction-rotation.md).
  This theme **composes** those three — it is the fold whose body is a `fe_space` call and whose
...
- `palace/fem/fespace.hpp:200-286` — the `FiniteElementSpaceHierarchy` class: the level vector `fespaces`
  ...
  (`:217-221`), `GetNumLevels` (`:215`), `GetFinestFESpace` (`:236-247`).
- [`L1/fe_space_hierarchy`](../L1/fe_space_hierarchy.md) — the prime L1 entry this theme lowers; its
  laws 1 (coarse-seed base case), 2 (AddLevel-fold structure), 3 (level-monotonicity), 4 (determinism),
  5 (per-level essential-dof coherence) are the LHS this RHS realizes.
- Sibling delegated rewrites: [`fe-space-construction-rotation`](./fe-space-construction-rotation.md)
  (per-level `fe_space` construction), [`fe-collection-construction-rotation`](./fe-collection-construction-rotation.md)
  (the `[FECollection]` schedule), [`essential-dofs-construction-rotation`](./essential-dofs-construction-rotation.md)
  (the single-space dbc block whose per-level fan-out this fold sequences).

## Caveats

- **`fe_space_hierarchy` has no faithful inbound consumer yet** (RE9 baseline-exception). This theme
  grounds the *home* (the theme file), not the op's reachability; no inbound consumer edge is forced.
- The multigrid-transfer `BuildProlongationAtLevel` algebra (the lazy prolongation slot's content) is
  read-as-given here, sibling-pull-gated.
