...
with **no fold-parent** — unlike [`dot`](./inner_product.md#specializations) (a leaf-of [`inner_product`](./inner_product.md))
...
structural distinction from its BLAS-1-floor cohort siblings [`dot`](./inner_product.md#specializations) and
[`scal`](./linear_combination.md#arity-specializations):

- [`dot`](./inner_product.md#specializations) is the conjugation-axis **leaf-of** the reduce-to-`Scalar` fold
...
- **result** — `Tensor[$S]` — congruent to `x` (same shape group `S`); **same element type** as `x`. Every
  ...
  `x` → complex result) — unlike [`nrm2`](./inner_product.md#consumer-nrm2-and-matrix-weighted-norm), which collapses both to a real-valued
